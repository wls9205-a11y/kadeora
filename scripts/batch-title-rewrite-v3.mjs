#!/usr/bin/env node
/**
 * 세션 153 — Title v3. title length<25 + batch 결과 미반영 35편 재제출.
 */
...
타이틀 30~50자 한국어 재작성. 지역·연도·키워드·수식어 포함. 카더라 스타일.
응답 JSON: {"title": "..."}`;
...
async function main() {
  ...
  for (let offset = 0; offset < 12000; offset += 1000) {
    const { data: page } = await sb.from('blog_posts')
      ...
      .eq('is_published', true).order('id', { ascending: true })
  ...
  const targets = collected.filter(p =>
    (p.title || '').length < 25 &&
    !(p.metadata && typeof p.metadata === 'object' && p.metadata.title_batch_v3_submitted === true)
  );
  console.log(`대상: ${targets.length}`);
  if (targets.length === 0) return;

  const requests = targets.map(p => ({
    custom_id: `titlev3_${p.id}`,
    params: {
  ...
  for (let i = 0; i < ids.length; i += 200) {
    ...
    await Promise.all((existing || []).map(r => {
      const newMeta = { ...(r.metadata && typeof r.metadata === 'object' ? r.metadata : {}), title_batch_v3_submitted: true, title_batch_v3_id: body.id };
