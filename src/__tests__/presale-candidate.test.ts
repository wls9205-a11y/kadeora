...
/**
 * ⚠️ 영구 등재 — 2026-09-05 실측. 「희망더함」은 부산도시공사 공공주택 브랜드다.
 *    인허가 「범천동 858-6번지 일원 희망더함아파트」가 같은 동·같은 시기의 «민간»
 *    분양예정 시드와 동일 사업으로 오인될 뻔했다. 공급유형이 다르면 다른 사업이다.
 *    ⛔ 이 케이스를 지우지 말 것 — 브랜드로 오는 공공을 다시 민영으로 읽는다.
 */
describe('공급유형 — 브랜드로 오는 공공', () => {
  it('희망더함은 공공이다', () => {
    expect(judgeSupplyType('범천동 858-6번지 일원 희망더함아파트')).toBe('공공');
    expect(adBlockedFor(judgeSupplyType('범천동 858-6번지 일원 희망더함아파트'))).toBe(true);
  });

  it('⛔ 정비사업은 여전히 민영이다 — 공공 표지를 넓혀도 이쪽이 흔들리면 안 된다', () => {
    expect(judgeSupplyType('상남1구역 재건축')).toBe('민영');
    expect(judgeSupplyType('반월구역 재개발 공동주택')).toBe('민영');
  });
});
