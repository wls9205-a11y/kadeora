...
describe('pickHeroCandidates — 전용 홈페이지(경로 강제 해제)', () => {
  ...
  it('내용 이미지는 남는다', () => {
    expect(got).toContain('https://prugio-riverfront.com/resources/img/main/landscape/ls_intro_feature.webp');
    expect(got).toContain('https://prugio-riverfront.com/resources/img/main/detail/detail_panel_unit.jpg');
  });

  it('후보 수에 상한이 있다 — 전용 홈페이지는 이미지가 수십 장이다', () => {
    const many = Array.from({ length: 40 }, (_, i) => `<img src="/resources/img/p/x${i}.jpg" />`).join('');
