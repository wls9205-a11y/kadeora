...
export default function BlogEndCTA({ slug, isLoggedIn }: Props) {
  ...
  return (
    <div ref={ref} style={{
      ...
      <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginBottom: 6 }}>이 글이 도움 되셨나요?</div>
      <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', marginBottom: 14, color: 'var(--text-primary)' }}>매주 실시간 분양·시세 알림 받기</div>
      <button onClick={handle} style={{
        background: '#FEE500', color: '#000', fontWeight: 'var(--fw-body)', fontSize: 'var(--fs-sm)',
        padding: '12px 28px', borderRadius: 10, border: 'none', cursor: 'pointer',
        minWidth: 220, boxShadow: '0 6px 14px rgba(0,0,0,0.2)',
      }}>카카오로 1초 가입 · 100P 지급</button>
      <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 10 }}>언제든 알림 해지 가능 · 무료</div>
    </div>
  );
}
