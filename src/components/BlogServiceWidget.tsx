...
export default function BlogServiceWidget({ category, tags, sourceRef }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-secondary)', marginBottom: 12 }}>
        📌 관련 서비스
      </div>
        ...
        {widgets.map((w) => (
          ...
          >
            <div style={{ fontSize: 'var(--fs-sm)', marginBottom: 4 }}>
              <span>{w.icon}</span>{' '}
              <span style={{ fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{w.title}</span>
            </div>
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginBottom: 8, lineHeight: 1.3 }}>{w.desc}</div>
            <div style={{
              fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--brand)',
            }}>{w.cta} →</div>
          </Link>
        ))}
      </div>
