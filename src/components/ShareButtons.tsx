...
export default function ShareButtons({title,url,postId}:ShareButtonsProps){
  ...
  const btn:React.CSSProperties={display:'flex',alignItems:'center',gap:'6px',padding:'7px 14px',borderRadius:'8px',border:'1px solid var(--kd-border)',background:'var(--kd-surface)',color:'var(--kd-text-muted)',fontSize:'13px',fontWeight:500,cursor:'pointer',transition:'all 0.15s'}
  return(
    <div style={{display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap',marginTop:'16px',paddingTop:'16px',borderTop:'1px solid var(--kd-border)'}}>
      <span style={{fontSize:'12px',color:'var(--kd-text-dim)',marginRight:'4px'}}>공유하기</span>
      ...
      <button onClick={handleCopy} style={{...btn,...(copied?{background:'var(--kd-success-dim)',color:'var(--kd-success)',borderColor:'var(--kd-success)'}:{})}} aria-label="링크 복사">{copied?'✓ 복사됨':'링크 복사'}</button>
    </div>
  )
}