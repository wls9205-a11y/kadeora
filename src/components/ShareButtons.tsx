    ...
    return (
      <>
        <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setOpen(true); }} aria-label="공유" className="kd-action-btn" style={{ textDecoration: 'none' }}>
          ...
          {' '}공유 <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', opacity: shareCount > 0 ? 1 : 0.5 }}>{shareCount}</span>
        </button>
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} aria-label="공유" style={{
        ...
        fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-body)',
        ...
        공유 <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', opacity: shareCount > 0 ? 1 : 0.5, marginLeft: 2 }}>{shareCount}</span>
      </button>
      <BottomSheet open={open} onClose={() => setOpen(false)} title="공유하기" maxWidth={480}>
        <ShareGrid platforms={platforms} share={share} copied={copied} />
        <UrlPreview />
    ...
    <div style={{ display: 'flex', gap: 'var(--sp-lg)', justifyContent: 'center', flexWrap: 'wrap', marginBottom: 'var(--sp-lg)' }}>
      {platforms.map(p => (
        <button key={p.id} onClick={() => share(p.id)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--sp-sm)', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, minWidth: 56, position: 'relative' }}>
          <div style={{ width: 52, height: 52, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: p.id === 'copy' && copied ? 'var(--accent-green)' : p.bg, color: p.color, fontSize: p.id === 'naver-blog' || p.id === 'naver-cafe' || p.id === 'daum-cafe' ? 'var(--fs-base)' : 'var(--fs-lg)', fontWeight: 'var(--fw-body)', transition: 'transform 0.12s' }}>
            {p.id === 'copy' && copied ? '✓' : p.emoji}
          </div>
          <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>{p.id === 'copy' && copied ? '복사됨!' : p.label}</span>
          {p.isNew && <span style={{ position: 'absolute', top: -4, right: -4, fontSize: 'var(--fs-xs)', background: 'var(--accent-red)', color: '#fff', padding: '1px 4px', borderRadius: 'var(--radius-sm)', fontWeight: 'var(--fw-body)' }}>N</span>}
        </button>
      ))}
    </div>
  );
