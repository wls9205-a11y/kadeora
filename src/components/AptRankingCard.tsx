  ...
  return (
    <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-card)', padding: 12, marginBottom: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <span style={{ fontSize: 'var(--fs-md)', fontWeight: 'var(--fw-num)' }}>🏆 아파트 랭킹</span>
        <Link href="/apt/complex" style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', textDecoration: 'none' }}>전체보기 →</Link>
      </div>
      <div className="apt-pill-scroll kd-scroll-row" style={{ display: 'flex', gap: 4, overflowX: 'auto', marginBottom: 12, scrollbarWidth: 'none', paddingBottom: 2 }}>
        {TYPES.map(t => (
          <button key={t.key} onClick={() => setType(t.key)} style={{
            padding: '5px 10px', borderRadius: 'var(--radius-lg)', fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0, fontFamily: 'inherit',
            background: type === t.key ? t.color + '18' : 'var(--bg-hover)', color: type === t.key ? t.color : 'var(--text-secondary)',
          }}>{t.label}</button>
        ))}
      </div>
      {loading ? (
        <div style={{ textAlign: 'center', padding: 20, color: 'var(--text-tertiary)', fontSize: 'var(--fs-xs)' }}>로딩 중...</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {data.map((a, i) => (
            <Link href={`/apt/complex/${encodeURIComponent(a.apt_name)}`} key={i} style={{
              ...
              <span style={{ width: 20, fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-num)', color: i < 3 ? cur?.color : 'var(--text-tertiary)', textAlign: 'center' }}>{i + 1}</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.apt_name}</div>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>{a.region_nm} {a.sigungu}</div>
              </div>
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                {type === 'price_up' || type === 'price_down' ? (
                  <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-num)', color: a.price_change_1y > 0 ? '#E24B4A' : '#3B7BF6' }}>
                    {a.price_change_1y > 0 ? '▲' : '▼'}{Math.abs(a.price_change_1y).toFixed(1)}%
                  </span>
                ) : type === 'trade_volume' ? (
                  <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-secondary)' }}>{a.sale_count_1y}건</span>
                ) : type === 'jeonse_ratio' ? (
                  <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: '#FBB724' }}>{a.jeonse_ratio}%</span>
                ) : (
                  <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-secondary)' }}>{fmtPrice(a.latest_sale_price)}</span>
                )}
              </div>
            </Link>
          ))}
