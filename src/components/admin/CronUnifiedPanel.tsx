...
export default async function CronUnifiedPanel() {
  ...
  return (
    ...
    >
      <summary style={{ cursor: 'pointer', fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-primary)', wordBreak: 'keep-all' }}>
        ⏱️ Cron 통합 뷰 — vercel {totals.vercel} + pg_cron {totals.pgcron} = {totals.total}
        <span style={{ marginLeft: 10, fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)' }}>
      ...
      <div style={{ marginTop: 10, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
        <div>
          <h3 style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)', margin: '0 0 6px' }}>
            VERCEL.JSON ({totals.vercel})
          </h3>
          <div style={{ maxHeight: 300, overflowY: 'auto', fontSize: 'var(--fs-xs)' }}>
            {vercelCrons.map(c => (
              <div key={c.path} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', borderBottom: '1px solid var(--border)', gap: 8 }}>
                ...
                <span style={{ color: 'var(--text-tertiary)', fontSize: 'var(--fs-2xs)', whiteSpace: 'nowrap' }}>{c.schedule}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ marginTop: 8, fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>
