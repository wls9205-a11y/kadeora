    ...
    <>
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', color: 'var(--text-inverse)', marginBottom: 2 }}>앱처럼 사용하기</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-xs)', flexShrink: 0 }}>
          <button onClick={() => { hap('light'); handleInstall(); }} style={{
            ...
            border: 'none', borderRadius: 'var(--radius-sm)', fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-body)', cursor: 'pointer',
      ...
      {showIOSGuide && (
        <>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 'var(--fs-lg)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 'var(--sp-xs)' }}>📲 홈화면에 추가하기</div>
            ...
            {[
              ...
              <div key={s.n} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--brand)', color: 'var(--text-inverse)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-body)', flexShrink: 0 }}>{s.n}</div>
            ...
            <button onClick={() => { hap('light'); setShowIOSGuide(false); handleDismiss(); logInstall('ios'); setTimeout(requestPush, 1000); }}
              style={{ marginTop: 'var(--sp-xl)', width: '100%', padding: '14px 0', background: 'var(--brand)', color: 'var(--text-inverse)', border: 'none', borderRadius: 'var(--radius-card)', fontSize: 'var(--fs-md)', fontWeight: 'var(--fw-body)', cursor: 'pointer' }}>
              알겠어요!
            </button>
          </div>
        </>
