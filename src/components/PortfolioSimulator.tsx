  ...
  return (
    <div style={{ marginBottom: 'var(--sp-lg)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, marginBottom: 'var(--sp-md)' }}>
        {[
          ...
          <div key={kpi.label} style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '8px 10px' }}>
            <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)', marginBottom: 3 }}>{kpi.label}</div>
            <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: kpi.color, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.2px' }}>{kpi.val}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-sm)', marginBottom: 10 }}>
        <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)', flexShrink: 0 }}>원금</span>
        ...
        <span style={{ fontSize: 'var(--fs-xs)', fontFamily: 'var(--font-mono)', color: 'var(--text-primary)', minWidth: 52, textAlign: 'right' }}>{amount.toLocaleString()}만</span>
      </div>
          ...
          <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', zIndex: 50, marginTop: 'var(--sp-xs)', overflow: 'hidden' }}>
            {searchResults.map(s => (
              <button aria-label="닫기" key={s.symbol} onClick={() => addHolding(s)} style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-sm)', width: '100%', padding: '8px 12px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', borderBottom: '1px solid var(--border)' }}>
                <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-num)', color: 'var(--text-primary)', flex: 1 }}>{s.name}</span>
                <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>{s.symbol}</span>
                <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: (s.change_pct ?? 0) > 0 ? (s.currency === 'USD' ? 'var(--accent-green)' : 'var(--accent-red)') : 'var(--accent-blue)' }}>{(s.change_pct ?? 0) > 0 ? '+' : ''}{(s.change_pct ?? 0).toFixed(2)}%</span>
              </button>
            ))}
          </div>
        )}
      ...
      {!holdings.length && (
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)', marginBottom: 5 }}>시총 상위 빠른 추가</div>
          <div style={{ display: 'flex', gap: 'var(--sp-xs)', flexWrap: 'wrap' }}>
            {quickStocks.map(s => (
              <button key={s.symbol} onClick={() => addHolding(s)} style={{ fontSize: 'var(--fs-2xs)', padding: '4px 8px', borderRadius: 'var(--radius-xs)', background: 'var(--bg-surface)', border: '1px solid var(--border)', color: 'var(--text-secondary)', cursor: 'pointer', fontWeight: 'var(--fw-title)' }}>
      ...
      {holdings.length > 0 && (
        <>
          ...
          <div style={{ height: 20, borderRadius: 'var(--radius-xs)', overflow: 'hidden', display: 'flex', marginBottom: 6 }}>
            {holdings.map((h, i) => (
              <div key={h.stock.symbol} style={{ width: `${h.alloc}%`, background: SEG_COLORS[i % SEG_COLORS.length], display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'width .3s', overflow: 'hidden' }}>
                {h.alloc > 10 && <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-num)', color: 'rgba(255,255,255,0.9)', whiteSpace: 'nowrap', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis' }}>{h.stock.name.slice(0, 4)}</span>}
              </div>
            ))}
          </div>
          {totalAlloc !== 100 && (
            <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--accent-yellow)', fontFamily: 'var(--font-mono)', marginBottom: 'var(--sp-xs)' }}>비중 합계 {totalAlloc}% (100% 맞춰주세요)</div>
          )}
              ...
              return (
                <div key={h.stock.symbol} style={{ padding: '9px 12px', borderBottom: i < holdings.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                    ...
                    <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-num)', color: 'var(--text-primary)', flex: 1 }}>{h.stock.name}</span>
                    <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-title)', color: c, fontFamily: 'var(--font-mono)' }}>{h.stock.pct > 0 ? '+' : ''}{h.stock.pct.toFixed(2)}%</span>
                    <span style={{ fontSize: 'var(--fs-2xs)', color: c, fontFamily: 'var(--font-mono)' }}>→{itemProfit > 0 ? '+' : ''}{fmtAmt(Math.abs(itemProfit / 10000))}</span>
                    <button onClick={() => removeHolding(h.stock.symbol)} style={{ background: 'none', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: 'var(--fs-sm)', lineHeight: 1, padding: '0 2px' }} aria-label="종목 제거">×</button>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', width: 24, textAlign: 'right', flexShrink: 0 }}>{h.alloc}%</span>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {holdings.map((h, i) => (
              <div key={h.stock.symbol} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>
      ...
      {!holdings.length && (
        <div style={{ textAlign: 'center', padding: '28px 0', color: 'var(--text-tertiary)' }}>
          ...
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-num)', marginBottom: 'var(--sp-xs)' }}>종목을 담아보세요</div>
          <div style={{ fontSize: 'var(--fs-xs)' }}>비중 조절 후 예상 수익을 계산합니다</div>
        </div>
      )}
    </div>
  );
