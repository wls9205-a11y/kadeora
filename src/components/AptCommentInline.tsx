  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 'var(--sp-md)' }}>
        ...
        <span style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: 'var(--text-primary)' }}>한줄평</span>
        ...
        <div style={{ marginBottom: 'var(--sp-md)' }}>
          {commentImage && (
            <div style={{ marginBottom: 6, display: 'inline-flex', position: 'relative' }}>
              ...
              <button onClick={() => setCommentImage(null)} aria-label="이미지 제거"
                style={{ position: 'absolute', top: -6, right: -6, width: 16, height: 16, borderRadius: '50%', background: 'var(--bg-surface)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', padding: 0 }}>✕</button>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 6, alignItems: 'center' }}>
            <button onClick={() => imgRef.current?.click()} disabled={uploadingImage || !!commentImage} type="button" aria-label="사진 첨부"
              style={{ padding: '6px 10px', background: 'var(--bg-hover)', border: 'none', borderRadius: 'var(--radius-sm)', cursor: 'pointer', color: commentImage ? 'var(--brand)' : 'var(--text-tertiary)', display: 'flex', alignItems: 'center', gap: 4, fontSize: 'var(--fs-xs)' }}>
            ...
            <button onClick={submit} disabled={sending || (!text.trim() && !commentImage)} style={{ padding: '6px 16px', background: 'var(--brand)', color: 'var(--text-inverse)', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', cursor: 'pointer', opacity: (!text.trim() && !commentImage) ? 0.5 : 1 }}>
      ...
      <div>
        {comments.length === 0 && <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-tertiary)', fontSize: 'var(--fs-sm)' }}>첫 한줄평을 남겨보세요!</div>}
        {comments.map((c: Record<string, any>) => (
          <div key={c.id} style={{ display: 'flex', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
            <div style={{ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, background: getAvatarColor(c.nickname || '사용자'), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-num)' }}>
              {(c.nickname || '사')[0].toUpperCase()}
            </div>
            <div style={{ flex: 1 }}>
              <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-num)', color: 'var(--text-primary)' }}>{c.nickname || '사용자'}</span>
