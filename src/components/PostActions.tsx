    ...
    <div style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{ position: 'absolute', right: 0, top: 32, background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'hidden', minWidth: 120, zIndex: 50, boxShadow: '0 4px 16px rgba(0,0,0,0.2)' }}>
          ...
          <button onClick={() => { handleDelete(); setOpen(false); }} style={{ display: 'block', width: '100%', padding: '10px 16px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 'var(--fs-base)', color: 'var(--accent-red)', textAlign: 'left', borderTop: '1px solid var(--border)' }}>🗑️ 삭제</button>
        </div>
      )}
    </div>
  );
