...
export default function RelatedContentCard({ type, entityName, showSignup }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 10 }}>
        📊 함께 보면 좋은 콘텐츠
      </div>
        ...
        {links.map((l, i) => (
          <Link key={i} href={l.href}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{l.title}</div>
      ...
      {showSignup && (
        <Link href={`/login?redirect=${encodeURIComponent(pathname)}&source=related_card`}
          style={{
            ...
            fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--brand)', textDecoration: 'none',
          }}>
          🔔 무료 가입하고 알림 받기
        </Link>
      )}
