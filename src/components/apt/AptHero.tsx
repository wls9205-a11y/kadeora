  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
        {lifecycle && (
          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: '#FFD688', padding: '4px 10px', borderRadius: 999, background: 'rgba(255,214,136,0.14)', border: '1px solid rgba(255,214,136,0.36)' }}>
            {lifecycle}
          </span>
        )}
        {region && (
          <span style={{ fontSize: 'var(--fs-xs)', color: '#B4B2A9', fontWeight: 'var(--fw-title)' }}>{region}</span>
        )}
      </div>
      {/* s2: 상세 페이지의 h1 은 본문 헤더 하나뿐이다. 여기는 시각적 제목이라 div. */}
      <div style={{ margin: 0, fontSize: 'var(--fs-lg)', fontWeight: 'var(--fw-body)', color: '#FFFFFF', lineHeight: 1.3, letterSpacing: '-0.4px', wordBreak: 'keep-all' }}>
        {site.name}
      </div>
      <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
        {meta.length > 0 && (
          <div style={{ fontSize: 'var(--fs-xs)', color: '#B4B2A9', fontWeight: 'var(--fw-body)', lineHeight: 1.5 }}>
            {meta.join(' · ')}
          </div>
        )}
        {typeof interestCount === 'number' && interestCount > 0 && (
          <div style={{ fontSize: 'var(--fs-xs)', color: '#FFD688', fontWeight: 'var(--fw-title)', whiteSpace: 'nowrap' }}>
