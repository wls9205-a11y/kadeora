...
export async function AptSitePageView({ data }: { data: AptSitePageData }) {
  ...
  return (
    <article className="min-h-screen bg-white pb-24 sm:pb-8">
      <header className="relative">
        {site.cover_image_url && (
          <div className="relative aspect-[16/9] sm:aspect-[3/1] bg-slate-200">
            <Image
      ...
      <div className="max-w-3xl mx-auto px-4 mt-6 sm:mt-8 space-y-8">
        ...
        {keyFeatures && keyFeatures.length > 0 && (
          <section aria-label="단지 특징">
            ...
            <div className="flex flex-wrap gap-2">
              {keyFeatures.map((f, i) => (
                <span key={i} className="text-xs px-3 py-1.5 bg-slate-100 rounded-full">
                  {f}
                </span>
              ))}
            </div>
        ...
        {nearbyFacilities && (
          <section aria-label="주변 편의시설" className="bg-slate-50 rounded-xl p-4">
        ...
        {analysisText && analysisText.length > 200 && (
          <details className="bg-slate-50 rounded-xl p-4 group">
            ...
            <div className="mt-4 prose prose-sm max-w-none whitespace-pre-line text-slate-700">
              {analysisText}
            </div>
          </details>
        )}
        ...
        <section id="reviews" aria-label="후기" className="scroll-mt-16">
          ...
          {data.reviews_count === 0 ? (
            ...
            <Link href={`/apt/${encodeURIComponent(site.slug)}/후기`} className="block text-center text-sm font-medium py-3 border border-slate-200 rounded-xl hover:bg-slate-50">
          ...
          <section aria-label="관련 블로그">
            ...
            <ul className="space-y-2">
              {data.related_blogs.map((b) => (
                <li key={b.post_id}>
                  <Link href={`/blog/${b.post_id}`} className="block px-4 py-3 bg-slate-50 hover:bg-slate-100 rounded-xl text-sm">
                    {b.anchor_text} ▸
                  </Link>
                </li>
              ))}
        ...
        <footer className="mt-12 pt-6 border-t border-slate-200 text-xs text-slate-500 space-y-1">
          {sub?.house_manage_no && (
            <p>청약홈 공고번호: <code className="text-slate-700">{sub.house_manage_no}</code></p>
          )}
          {sub?.pblanc_url && (
            <p>
...
function ScheduleStep({ label, date, highlight }: { label: string; date: string; highlight?: boolean }) {
  return (
    <li className={`flex justify-between items-center py-2.5 px-4 rounded-lg ${highlight ? 'bg-red-50 border border-red-200' : 'bg-slate-50'}`}>
      <span className="text-sm font-medium">{label}</span>
      <time className="text-sm text-slate-700">{date}</time>
    </li>
  );
}
