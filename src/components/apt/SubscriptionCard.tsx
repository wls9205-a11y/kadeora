    ...
    <Link href={href} className="kd-lrow kd-lrow--thumb" style={{ textDecoration: 'none', color: 'inherit' }}>
      ...
      <span className="kd-lrow-r">
        {rate != null ? (
          <>
            {rate.toFixed(1)}
            <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)' }}>:1</span>
          </>
        ) : item.households ? (
          <>
            {item.households.toLocaleString('ko-KR')}
            <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)' }}>세대</span>
          </>
        ) : (
          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)' }}>미공개</span>
        )}
      </span>
    </Link>
  );
