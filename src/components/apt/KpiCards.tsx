    ...
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 6, marginBottom: 14 }}>
      {cards.map(s => (
        ...
        >
          ...
          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: s.c, lineHeight: 1.3, whiteSpace: 'pre-line' }}>{s.v}</div>
          {s.sub && <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', marginTop: 2, lineHeight: 1.3, whiteSpace: 'pre-line' }}>{s.sub}</div>}
          {s.bar > 0 && (
