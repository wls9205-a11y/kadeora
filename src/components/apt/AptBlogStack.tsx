  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', letterSpacing: 0 }}>관련 블로그</span>
        <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-tertiary)', padding: '2px 8px', borderRadius: 'var(--radius-pill)', background: 'var(--bg-hover)', border: '1px solid var(--border)' }}>
          7,961 자산
        </span>
      </div>
      <div
      ...
      >
        {blogs.map(b => {
          ...
          return (
            ...
            >
              <div style={{ width: 110, height: 56, borderRadius: 'var(--radius-sm)', background: c.bg, color: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', letterSpacing: 0 }}>
                {c.label}
              </div>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', lineHeight: 1.3, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                {b.title}
              </div>
              {(b.view_count ?? 0) > 0 && (
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>👀 {b.view_count?.toLocaleString()}</div>
              )}
            </Link>
          );
        })}
