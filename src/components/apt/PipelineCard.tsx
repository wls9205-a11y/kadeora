  ...
  const body = (
    <>
      ...
      <span className="kd-lrow-r">
        {units.value === '미확인' ? (
          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)' }}>미정</span>
        ) : (
          <>
            {units.value}
            <span style={{ display: 'block', fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)', lineHeight: 1.3 }}>
              {units.note ?? '세대'}
            </span>
          </>
        )}
