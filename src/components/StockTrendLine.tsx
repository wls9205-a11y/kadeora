  ...
  return (
    <div style={{ marginTop: 'var(--sp-sm)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--sp-xs)' }}>
        <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>{sorted.length}일 차트 + 트렌드라인</span>
        <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: lineColor, fontFamily: 'var(--font-mono)' }}>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible', display: 'block' }}>
        ...
        {yLabels.map((l, i) => (
          <g key={i}>
            ...
            <text x={PADDING.left - 5} y={l.y} textAnchor="end" dominantBaseline="middle" fontSize="10" fill="var(--text-tertiary)" fontFamily="var(--font-mono)">{currency === 'USD' ? `$${l.val.toFixed(0)}` : `${(l.val / 1000).toFixed(0)}K`}</text>
          </g>
        ))}
        {/* X축 날짜 */}
        {[0, Math.floor(sorted.length / 2), sorted.length - 1].map(i => (
          <text key={i} x={toX(i)} y={H - 4} textAnchor="middle" fontSize="10" fill="var(--text-tertiary)" fontFamily="var(--font-mono)">{sorted[i].date.slice(5)}</text>
        ))}
        {/* 트렌드 방향 라벨 */}
        <text x={W - PADDING.right - 2} y={PADDING.top + 6} textAnchor="end" fontSize="10" fill={lineColor} opacity="0.8" fontFamily="var(--font-mono)">추세↗</text>
      </svg>
    </div>
  );
}
