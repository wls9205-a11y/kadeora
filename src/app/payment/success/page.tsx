...
export default function PaymentSuccessPage() {
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center px-6 bg-[#0F0F0F]">
      {status === 'loading' && (
        <>
          <Loader size={48} className="text-brand animate-spin mb-4" />
          <p className="text-white/60">결제 확인 중...</p>
        </>
      )}
      {status === 'success' && (
        <>
          ...
          <p className="text-white/40 text-sm">잠시 후 상점으로 이동합니다</p>
        </>
      )}
      {status === 'error' && (
        <>
          ...
          <button onClick={() => router.push('/shop')} className="btn-brand px-6 py-3">
            상점으로 돌아가기
          </button>
        </>
      )}
