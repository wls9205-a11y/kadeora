...
export default function PaymentClient() {
  ...
  if (step === 'fail' || searchParams.get('fail')) return (
    <div className="min-h-screen flex items-center justify-center p-4"><div className="bg-[var(--kd-surface)] border border-[var(--kd-border)] rounded-2xl p-8 max-w-md w-full text-center">
      <div className="text-5xl mb-4">{'\uD83D\uDE22'}</div><h2 className="text-xl font-bold text-[var(--kd-text)] mb-2">Payment Failed</h2>
      <p className="text-[var(--kd-text)]/60 mb-6">{error || 'Payment was cancelled or an error occurred.'}</p>
      <button onClick={() => { setStep('select'); setError(''); router.push('/payment'); }} className="px-6 py-3 bg-[var(--kd-primary)] text-white rounded-xl hover:opacity-90 transition-opacity">Try Again</button>
    </div></div>
  );
  ...
  if (step === 'processing') return (
    <div className="min-h-screen flex items-center justify-center p-4"><div className="bg-[var(--kd-surface)] border border-[var(--kd-border)] rounded-2xl p-8 max-w-md w-full text-center">
      ...
      <h2 className="text-xl font-bold text-[var(--kd-text)] mb-2">Processing...</h2>
    </div></div>
  );

  if (step === 'success' && paymentResult) return (
    <div className="min-h-screen flex items-center justify-center p-4"><div className="bg-[var(--kd-surface)] border border-[var(--kd-border)] rounded-2xl p-8 max-w-md w-full text-center">
      <div className="text-5xl mb-4">{'\uD83C\uDF89'}</div><h2 className="text-xl font-bold text-[var(--kd-success)] mb-2">Payment Complete!</h2>
      <p className="text-[var(--kd-text)]/60 mb-4">{product?.name || 'Item'} purchased</p>
      <div className="bg-[var(--kd-bg)] rounded-xl p-4 mb-6 text-left text-sm space-y-2">
        <div className="flex justify-between"><span className="text-[var(--kd-text)]/50">Order ID</span><span className="text-[var(--kd-text)] font-mono text-xs">{paymentResult.orderId}</span></div>
        <div className="flex justify-between"><span className="text-[var(--kd-text)]/50">Amount</span><span className="text-[var(--kd-text)] font-bold">{Number(paymentResult.amount).toLocaleString()} KRW</span></div>
        <div className="flex justify-between"><span className="text-[var(--kd-text)]/50">Method</span><span className="text-[var(--kd-text)]">{paymentResult.method}</span></div>
      </div>
      <button onClick={() => router.push('/feed')} className="px-6 py-3 bg-[var(--kd-primary)] text-white rounded-xl hover:opacity-90 transition-opacity">Back to Feed</button>
    </div></div>
  );

  return (
    <div className="max-w-2xl mx-auto p-4 py-8">
      <h1 className="text-2xl font-bold text-[var(--kd-text)] mb-2">Shop</h1>
      <p className="text-[var(--kd-text)]/60 mb-6">Browse and purchase items</p>
      {step === 'confirm' && product && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"><div className="bg-[var(--kd-surface)] border border-[var(--kd-border)] rounded-2xl p-6 max-w-sm w-full">
          ...
          <div className="bg-[var(--kd-bg)] rounded-xl p-4 mb-4 text-center"><span className="text-2xl font-bold text-[var(--kd-primary)]">{product.price_krw.toLocaleString()} KRW</span></div>
          <div className="flex gap-3">
            <button onClick={() => { setStep('select'); setProduct(null); }} className="flex-1 py-3 border border-[var(--kd-border)] text-[var(--kd-text)] rounded-xl hover:bg-[var(--kd-bg)] transition-colors">Cancel</button>
            <button onClick={() => startPayment(product)} className="flex-1 py-3 bg-[var(--kd-primary)] text-white rounded-xl hover:opacity-90 transition-opacity font-bold">Pay</button>
          </div>
        </div></div>
      )}
        ...
        {products.map((p) => (
          <button key={p.id} onClick={() => { setProduct(p); setStep('confirm'); }} className="bg-[var(--kd-surface)] border border-[var(--kd-border)] rounded-2xl p-5 text-left hover:border-[var(--kd-primary)]/50 transition-colors group">
            <div className="flex items-start gap-3"><span className="text-3xl">{p.icon}</span><div className="flex-1 min-w-0">
              ...
              <p className="text-lg font-bold text-[var(--kd-primary)] mt-2">{p.price_krw.toLocaleString()} KRW</p>
            </div></div>
          </button>
        ))}
      </div>
      {products.length === 0 && <div className="text-center py-12 text-[var(--kd-text)]/40">No products available</div>}
    </div>
  );
}