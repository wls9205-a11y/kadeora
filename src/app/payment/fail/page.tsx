...
export default function PaymentFail() {
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'60vh', gap:16, padding:24, textAlign:'center' }}>
      ...
      <h1 style={{ color:'var(--text-primary)', fontWeight:'var(--fw-num)', margin:0 }}>결제 실패</h1>
      <p style={{ color:'var(--text-secondary)', margin:0 }}>결제 중 문제가 발생했습니다. 다시 시도해주세요.</p>
      <Link href="/shop" style={{ backgroundColor:'var(--brand)', color:'var(--text-inverse)', borderRadius:9999, padding:'12px 32px', fontWeight:'var(--fw-num)', textDecoration:'none', display:'inline-block' }}>다시 시도</Link>
    </div>
  );
}
