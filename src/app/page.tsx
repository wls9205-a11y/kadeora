...
const SECTIONS = [
  ...
  },
  {
    ...
    emoji: '📰',
    title: '투자 정보 블로그',
    desc: '매일 업데이트되는 시황 분석, 청약 가이드, 재테크 정보 20,000편+',
    tags: ['시황분석', '청약가이드', '재테크'],
...
export default async function HomePage() {
  ...
  let stats = { blogs: 20857, stocks: 1733, apts: 5525, posts: 4506, profiles: 125, redev: 206 };
  let indices: any[] = [];
  let openApts: any[] = [];
    ...
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        mainEntity: [
          ...
          { '@type': 'Question', name: '블로그에는 어떤 글이 있나요?', acceptedAnswer: { '@type': 'Answer', text: '매일 자동 업데이트되는 20,000편+ 투자 정보 블로그를 운영합니다. 코스피·코스닥 시황 분석, 청약 가이드, 미분양 리포트 등을 매일 발행합니다.' } },
        ...
        <section style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px 64px' }}>
          ...
          <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 'var(--sp-md)' }}>
            {[
              ...
              { q: '블로그에는 어떤 글이 있나요?', a: '매일 자동 업데이트되는 20,000편+ 투자 정보 블로그를 운영하고 있습니다. 코스피·코스닥 시황 분석, 청약 가이드, 미분양 리포트, ETF 비교 등 주식과 부동산 관련 정보를 매일 발행합니다.' },
