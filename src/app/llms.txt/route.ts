...
- 공식 URL: ${SITE_URL}
- 운영자: 카더라 (소개 ${SITE_URL}/about · 작성 주체 ${SITE_URL}/about/authors)
...
- 수치는 출처가 있는 값만 싣습니다. 분양가·세대수·일정이 공고 전이면 「미공개(입주자모집공고 후 확정)」로 표기하고 추정치를 쓰지 않습니다.
- 분양예정 시기는 보도·공고의 정밀도 그대로(예: 「2026년 3분기」) 기준일과 함께 표기합니다.
- 제도 수치(청약 순위·가점·LTV·DSR·전매제한·취득세)는 법령·정부 발표 원문과 확인일을 둔 상수표에서만 인용합니다. 청약 순위는 1·2순위 체계입니다(3순위 없음).
- 세대수는 「공급(이번 분양)」과 「총(단지 전체)」을 구분합니다.

## 인용하기 좋은 핵심 페이지

- [지역별 분양예정 단지 · 질문형 FAQ](${SITE_URL}/apt/region/부산): 시도별 분양예정 현장 목록(시기·기준일)과 「하반기 분양 예정 단지는 어디인가요?」 형 FAQ — 서울·경기·부산·울산·경남 등 17개 시도
- [현장 상세](${SITE_URL}/apt): 현장별 「단지 개요」 정의문(위치·시공사·규모·국면) + FAQ
- [청약 가점 진단](${SITE_URL}/apt/diagnose): 가점 계산(무주택 32 + 부양가족 35 + 통장 17 = 84점)
- [재개발 현황](${SITE_URL}/apt/redev): 정비사업 단계별 현장

## 갱신 주기 (요약)

- 현장 상세·분양예정 목록: 청약·보도 수집 크론이 매일 갱신
- 블로그: 발행 시 수치 출처 검증기를 통과한 글만 공개(검증 실패 글은 비공개)
- 실거래: 국토교통부 공개 주기(매월)
- 이 파일: 24시간 캐시
