...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (section === 'conversion') {
      const sbAny2 = sb as any;
      const since7d = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
      const { data: events } = await sbAny2.from('conversion_events')
      ...
      const { count: emailSubs } = await sbAny2.from('email_subscribers').select('id', { count: 'exact', head: true }).eq('is_active', true);
      const { count: pushSubs } = await sbAny2.from('push_subscriptions').select('id', { count: 'exact', head: true });
      const { count: newUsers } = await sbAny2.from('profiles').select('id', { count: 'exact', head: true }).gte('created_at', since7d);
