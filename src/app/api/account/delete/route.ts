...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const { error: pErr } = await supabase.from('profiles').update({ nickname: `deleted_${user.id.slice(0,8)}`, avatar_url: null, bio: null, is_deleted: true, deleted_at: new Date().toISOString() }).eq('id', user.id);
