...
> 청약홈(applyhome.co.kr) 공공데이터 기반. 정확한 정보는 청약홈에서 확인하세요.`;
      ...
      await admin.from('blog_posts').insert({ slug, title: regTitle, content: ensureMinLength(content, 'apt'), excerpt: `${month} ${region} 지역 청약 ${apts.length}건 · ${totalUnits.toLocaleString()}세대.`, category: 'apt', tags: [`${region} 청약`, `${region} 분양`, '아파트 청약', '청약일정'], source_type: 'auto', cron_type: 'monthly', cover_image: `https://kadeora.app/api/og?title=${encodeURIComponent(regTitle)}&type=blog` });
      created++;
    }
...
> 국토교통부 미분양주택현황 기반. 투자 권유가 아니며, 정확한 정보는 해당 시행사에 직접 확인하세요.`;
      ...
      await admin.from('blog_posts').insert({ slug: slugUnsold, title: unsoldTitle, content: ensureMinLength(content, 'unsold'), excerpt: `${month} 전국 미분양 ${total.toLocaleString()}세대. ${regionCount}개 지역 분석.`, category: 'unsold', tags: ['미분양', '미분양아파트', '부동산', '전국미분양'], source_type: 'auto', cron_type: 'monthly', cover_image: `https://kadeora.app/api/og?title=${encodeURIComponent(unsoldTitle)}&type=blog` });
      created++;
    }
...
> 국토교통부 미분양주택현황 기반. 투자 권유가 아닙니다.`;
        await admin.from('blog_posts').insert({ slug: bSlug, title: bTitle, content: ensureMinLength(bContent, 'unsold'), excerpt: `${b.label} 미분양 ${budgetApts.length}건 · ${bTotalUnsold.toLocaleString()}세대. ${bRegions.slice(0, 3).join(', ')}.`, category: 'unsold', tags: [`${b.label} 미분양`, '미분양 아파트', '가격대별', '부동산'], source_type: 'auto', cron_type: 'monthly', cover_image: `https://kadeora.app/api/og?title=${encodeURIComponent(bTitle)}&type=blog` });
        created++;
      }
    }
    ...
    if (!calE) {
      ...
      if (calApts && calApts.length > 0) {
        ...
        await admin.from('blog_posts').insert({ slug: calSlug, title: calTitle, content: ensureMinLength(calContent, 'apt'), excerpt: `${calMonth} 접수 예정 청약 ${calApts.length}건 캘린더.`, category: 'apt', tags: ['청약캘린더', `${calMonth} 청약`, '청약일정'], source_type: 'auto', cron_type: 'monthly', cover_image: `https://kadeora.app/api/og?title=${encodeURIComponent(calTitle)}&type=blog` });
        created++;
      }
    }
