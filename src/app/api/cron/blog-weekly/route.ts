...
> 청약홈 공공데이터 기반. 투자 권유가 아닙니다.`;
      ...
      const _r = await safeBlogInsert(admin, { slug: slug1, title: weekTitle, content: ensureMinLength(content, 'apt'), excerpt: `이번 주 청약 ${aptCount}건 · ${totalUnits.toLocaleString()}세대. ${regions.join(', ')}.`, category: 'apt', tags: ['청약일정', '이번주청약', '아파트분양'], cron_type: 'weekly', cover_image: `${SITE_URL}/api/og?title=${encodeURIComponent(weekTitle)}&design=2&type=blog`, image_alt: generateImageAlt('apt', weekTitle), meta_description: generateMetaDesc(content), meta_keywords: generateMetaKeywords('apt', ['청약일정', '이번주청약', '아파트분양']) });
...
> 청약홈(applyhome.co.kr) 공공데이터 기반. 투자 권유가 아닙니다.`;
        const _r = await safeBlogInsert(admin, { slug: slug3, title: cmpTitle, content: ensureMinLength(cmpContent, 'apt'), excerpt: `${region} 두 청약 단지 비교.`, category: 'apt', tags: [a.house_nm, b.house_nm, `${region} 청약`, '청약비교'], cron_type: 'weekly', cover_image: `${SITE_URL}/api/og?title=${encodeURIComponent(cmpTitle)}&design=2&type=blog`, image_alt: generateImageAlt('apt', cmpTitle), meta_description: generateMetaDesc(cmpContent), meta_keywords: generateMetaKeywords('apt', [a.house_nm, b.house_nm, `${region} 청약`, '청약비교']) });
      if (_r.success) created++;
      }
    }
...
> 커뮤니티 인기 질문 기반 자동 생성. 답변은 개인 의견이며 투자 권유가 아닙니다.`;
        const _r = await safeBlogInsert(admin, { slug: slug4, title: `자주 묻는 질문 — 청약·주식·부동산 (${weekStr})`, content: ensureMinLength(faqContent, 'general'), excerpt: '커뮤니티에서 가장 많이 물어본 질문 모음.', category: 'general', tags: ['FAQ', '자주묻는질문', '청약', '주식'], cron_type: 'weekly', cover_image: `${SITE_URL}/api/og?title=${encodeURIComponent('자주 묻는 질문')}&design=2&type=blog`, image_alt: generateImageAlt('general', `자주 묻는 질문 — 청약·주식·부동산 (${weekStr})`), meta_description: generateMetaDesc(faqContent), meta_keywords: generateMetaKeywords('general', ['FAQ', '자주묻는질문', '청약', '주식']) });
      if (_r.success) created++;
      }
    }
