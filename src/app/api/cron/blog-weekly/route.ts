...
> 카더라 커뮤니티 좋아요 기준 자동 집계. 게시글 내용은 개인 의견이며 투자 권유가 아닙니다.`;
      ...
      await admin.from('blog_posts').insert({ slug: slug2, title: `이번 주 카더라 HOT 게시글 TOP 5 (${weekStr})`, content: ensureMinLength(content, 'general'), excerpt: '이번 주 커뮤니티에서 가장 인기 있던 게시글 TOP 5.', category: 'general', tags: ['HOT', '인기글', '커뮤니티'], source_type: 'auto' });
      created++;
    }
...
> 청약홈(applyhome.co.kr) 공공데이터 기반. 투자 권유가 아닙니다.`;
        await admin.from('blog_posts').insert({ slug: slug3, title: cmpTitle, content: ensureMinLength(cmpContent, 'apt'), excerpt: `${region} 두 청약 단지 비교.`, category: 'apt', tags: [a.house_nm, b.house_nm, `${region} 청약`, '청약비교'], source_type: 'auto', cron_type: 'weekly', cover_image: `https://kadeora.app/api/og?title=${encodeURIComponent(cmpTitle)}&type=blog` });
        created++;
      }
    }
