...
> 커뮤니티 인기 질문 기반 자동 생성. 답변은 개인 의견이며 투자 권유가 아닙니다.`;
        const _r = await safeBlogInsert(admin, { slug: slug4, title: `자주 묻는 질문 — 청약·주식·부동산 (${weekStr})`, content: ensureMinLength(faqContent, 'general'), excerpt: '커뮤니티에서 가장 많이 물어본 질문 모음.', category: 'general', tags: ['FAQ', '자주묻는질문', '청약', '주식'], cron_type: 'weekly', cover_image: `https://kadeora.app/api/og?title=${encodeURIComponent('자주 묻는 질문')}&type=blog`, image_alt: generateImageAlt('general', `자주 묻는 질문 — 청약·주식·부동산 (${weekStr})`), meta_description: generateMetaDesc(faqContent), meta_keywords: generateMetaKeywords('general', ['FAQ', '자주묻는질문', '청약', '주식']) });
      if (_r.success) created++;
      }
    }
