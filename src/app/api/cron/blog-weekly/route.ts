...
> 청약홈 공공데이터 기반. 투자 권유가 아닙니다.`;
      ...
      await admin.from('blog_posts').insert({ slug: slug1, title: weekTitle, content: ensureMinLength(content, 'apt'), excerpt: `이번 주 청약 ${aptCount}건 · ${totalUnits.toLocaleString()}세대. ${regions.join(', ')}.`, category: 'apt', tags: ['청약일정', '이번주청약', '아파트분양'], source_type: 'auto', cron_type: 'weekly', cover_image: `https://kadeora.app/api/og?title=${encodeURIComponent(weekTitle)}&type=blog`, image_alt: generateImageAlt('apt', weekTitle), meta_description: generateMetaDesc(content), meta_keywords: generateMetaKeywords('apt', ['청약일정', '이번주청약', '아파트분양']) });
      created++;
    }
...
> 카더라 커뮤니티 좋아요 기준 자동 집계. 게시글 내용은 개인 의견이며 투자 권유가 아닙니다.`;
      ...
      await admin.from('blog_posts').insert({ slug: slug2, title: `이번 주 카더라 HOT 게시글 TOP 5 (${weekStr})`, content: ensureMinLength(content, 'general'), excerpt: '이번 주 커뮤니티에서 가장 인기 있던 게시글 TOP 5.', category: 'general', tags: ['HOT', '인기글', '커뮤니티'], source_type: 'auto', image_alt: generateImageAlt('general', `이번 주 카더라 HOT 게시글 TOP 5 (${weekStr})`), meta_description: generateMetaDesc(content), meta_keywords: generateMetaKeywords('general', ['HOT', '인기글', '커뮤니티']) });
      created++;
    }
...
> 청약홈(applyhome.co.kr) 공공데이터 기반. 투자 권유가 아닙니다.`;
        await admin.from('blog_posts').insert({ slug: slug3, title: cmpTitle, content: ensureMinLength(cmpContent, 'apt'), excerpt: `${region} 두 청약 단지 비교.`, category: 'apt', tags: [a.house_nm, b.house_nm, `${region} 청약`, '청약비교'], source_type: 'auto', cron_type: 'weekly', cover_image: `https://kadeora.app/api/og?title=${encodeURIComponent(cmpTitle)}&type=blog`, image_alt: generateImageAlt('apt', cmpTitle), meta_description: generateMetaDesc(cmpContent), meta_keywords: generateMetaKeywords('apt', [a.house_nm, b.house_nm, `${region} 청약`, '청약비교']) });
        created++;
      }
    }
...
> 커뮤니티 인기 질문 기반 자동 생성. 답변은 개인 의견이며 투자 권유가 아닙니다.`;
        await admin.from('blog_posts').insert({ slug: slug4, title: `자주 묻는 질문 — 청약·주식·부동산 (${weekStr})`, content: ensureMinLength(faqContent, 'general'), excerpt: '커뮤니티에서 가장 많이 물어본 질문 모음.', category: 'general', tags: ['FAQ', '자주묻는질문', '청약', '주식'], source_type: 'auto', cron_type: 'weekly', cover_image: `https://kadeora.app/api/og?title=${encodeURIComponent('자주 묻는 질문')}&type=blog`, image_alt: generateImageAlt('general', `자주 묻는 질문 — 청약·주식·부동산 (${weekStr})`), meta_description: generateMetaDesc(faqContent), meta_keywords: generateMetaKeywords('general', ['FAQ', '자주묻는질문', '청약', '주식']) });
        created++;
      }
    }
