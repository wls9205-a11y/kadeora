...
> 본 콘텐츠는 청약홈(applyhome.co.kr) 공공데이터 기반이며, 정확한 일정과 자격 요건은 청약홈에서 확인하세요. 투자 권유가 아닙니다.`;
      ...
      const _r = await safeBlogInsert(admin, {
        ...
        cron_type: 'apt-new', cover_image: `${SITE_URL}/api/og?title=${encodeURIComponent(aptTitle&design=2)}&type=blog`,
...
> 국토교통부 미분양주택현황 기반. 정확한 정보는 해당 시행사에 직접 확인하세요. 투자 권유가 아닙니다.`;
      ...
      const _r = await safeBlogInsert(admin, {
        ...
        cron_type: 'apt-new', cover_image: `${SITE_URL}/api/og?title=${encodeURIComponent(unsoldTitle&design=2)}&type=blog`,
