  ...
  try {
    ...
    for (let i = 0; i < TEMPLATES.length; i++) {
      ...
      const result = await safeBlogInsert(admin, {
        ...
        cover_image: `${SITE_URL}/api/og?title=${encodeURIComponent(blogTitle)}&design=2&type=blog`,
...
> 투자 권유가 아니며 참고용입니다. 모든 투자 판단과 손익은 투자자 본인에게 귀속됩니다.`;
      ...
      const result = await safeBlogInsert(admin, {
        ...
        cover_image: `${SITE_URL}/api/og?title=${encodeURIComponent(stockTitle)}&design=2&type=blog`,
    ...
    if (created > 0) {
      submitIndexNow([`/blog`, `/sitemap.xml`, `/image-sitemap.xml`]).catch(() => {});
    }
