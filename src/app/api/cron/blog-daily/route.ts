...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const contents = [
      ...
      // 코스닥
      `## 코스닥 주요 종목 시세 (${today})\n\n${today} 기준 **코스닥** 주요 종목의 시세를 정리했습니다. 코스닥 시장은 바이오, IT, 2차전지 관련 종목이 주를 이루며, 코스피 대비 변동성이 큰 편입니다.\n\n---\n\n### 코스닥 주요 종목\n\n| 종목 | 현재가 | 등락률 |\n|---|---|---|\n${mkTable(kosdaq.slice(0, 10))}\n\n---\n\n### 코스닥 시장 특징\n\n코스닥 시장은 **성장주 중심**으로 구성되어 있어 실적 시즌이나 정책 변화에 민감하게 반응합니다. 특히 바이오 섹터의 임상 결과, IT 섹터의 수주 소식 등이 개별 종목 변동에 큰 영향을 미칩니다.\n\n중소형주 투자 시에는 거래량, 재무 건전성, 대주주 지분율 등을 꼼꼼히 확인하는 것이 중요합니다.\n\n---\n\n- [카더라 **주식 시세** →](/stock)\n- [**코스닥 토론** →](/feed?category=stock)\n\n> 투자 권유가 아니며 참고용입니다.`,
      // 급등락
      ...
      // 섹터
      `## 섹터별 대표 종목 동향 (${today})\n\n${today} 기준 **섹터별 대표 종목**의 시세 동향을 정리했습니다. 섹터 흐름은 개별 종목 선정의 기본이 되는 중요한 분석 요소입니다.\n\n---\n\n### 주요 종목 현황\n\n${kospi.slice(0, 8).map(s => `- [**${s.name}**](/stock/${s.symbol}): ${s.price?.toLocaleString()}원 (${(s.change_pct ?? 0) > 0 ? '+' : ''}${(s.change_pct ?? 0).toFixed(2)}%)`).join('\n')}\n\n---\n\n같은 섹터 내에서도 종목별로 실적 차별화가 이뤄지고 있어, 업종 대표주와 중소형 성장주를 구분하여 살펴볼 필요가 있습니다.\n\n- [카더라 **전체 시세** →](/stock)\n- [섹터별 **토론** →](/feed?category=stock)\n\n> 참고용 정보입니다.`,
      // 미국주식
      `## 미국 증시 마감 요약 (${today})\n\n${today} 기준 **미국 증시** 주요 지수 및 종목의 마감 시세를 정리했습니다. 미국 시장의 흐름은 다음 날 국내 증시에 직접적인 영향을 미치므로 꼭 확인하세요.\n\n---\n\n### 미국 주요 종목\n\n| 종목 | 가격 | 등락률 |\n|---|---|---|\n${(stocks ?? []).filter(s => s.market === 'NYSE' || s.market === 'NASDAQ').slice(0, 8).map(s => `| [**${s.name}**](/stock/${s.symbol}) | $${s.price?.toLocaleString()} | ${(s.change_pct ?? 0) > 0 ? '▲' : '▼'} ${Math.abs(s.change_pct ?? 0).toFixed(2)}% |`).join('\n')}\n\n---\n\n### 미국 시장 포인트\n\n미국 연준(Fed)의 금리 정책, 고용 지표, CPI 등 **매크로 데이터**가 시장 방향을 좌우합니다. 빅테크(FAANG+) 실적 시즌에는 나스닥 변동성이 특히 확대되는 경향이 있습니다.\n\n해외 주식 투자 시 **환율 리스크**도 함께 고려하세요.\n\n- [카더라 **해외 종목** →](/stock)\n- [해외주식 **토론** →](/feed?category=stock)\n\n> 투자 권유가 아니며 참고용입니다.`,
    ];
