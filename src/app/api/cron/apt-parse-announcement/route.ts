...
async function handler(_req: NextRequest) {
  ...
  if (!targets?.length) {
    return NextResponse.json({ ok: true, processed: 0, failed: 0, message: '파싱 대상 없음' });
  }
  ...
  console.info(`[apt-parse-announcement] processed=${processed} failed=${failed} batch=${targets.length}`);
  return NextResponse.json({ ok: true, processed, failed, batch: targets.length, errors: errors.slice(0, 5) });
}

export const GET = withCronAuth((req: NextRequest) => withCronLogging('apt-parse-announcement', () => handler(req)));
