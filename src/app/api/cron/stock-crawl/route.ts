...
function guessSector(name: string): string | null {
  if (/반도체|하이닉스|삼성전자|DB하이텍|리노공업|원익IPS|한미반도체|이오테크닉스|테스|주성엔지니어링|피에스케이|솔브레인|동진쎄미켐|티씨케이|ISC|HPSP|파두|심텍|RFHIC|서진시스템|태성|SFA반도체|에이디테크|유진테크|코미코|하나마이크론|실리콘|웨이퍼|패키지/.test(name)) return '반도체';
  if (/바이오|셀트리온|삼성바이오|유한양행|녹십자|한미약품|대웅|종근당|일양|JW|보령|동아에스티|SK바이오|에이비엘|메디톡스|오스코텍|헬릭스미스|제넥신|알테오젠|레고켐|씨젠|에스디바이오|파미셀|차바이오|코오롱생명|HK이노엔|제약|팜|파마|테라|오믹스|올릭스|케어젠|보로노이|코오롱티슈|리브스메드|에스티팜|삼천당|비올|클래시스|셀|젠|메디|진단|이뮨|뉴로|백신|헬스|치료/.test(name)) return '바이오';
  if (/금융|은행|지주|보험|증권|KB|신한|하나|우리|NH|메리츠|미래에셋|키움|삼성생명|삼성화재|한화생명|현대해상|DB손해|한국금융|카드|캐피탈|BNK|DGB|JB|iM|저축|자산운용/.test(name)) return '금융';
  if (/자동차|현대차|기아|만도|한온|현대모비스|에스엘|화신|덴소|HL만도|계양전기|타이어|브레이크|시트/.test(name)) return '자동차';
  if (/배터리|에너지솔루션|SDI|에코프로|포스코퓨처|엘앤에프|코스모신소재|일진머티리얼즈|천보|2차전지|SK이노베이션|SK온|양극|음극|전해/.test(name)) return '2차전지';
  if (/건설|대림|GS건설|현대건설|삼성물산|HDC|DL이앤씨|태영|한신공영|계룡|금호|대우건설|코오롱글로벌|건영|토건/.test(name)) return '건설';
  if (/통신|SKT|SK텔레콤|KT |KT&G|LG유플러스|SK스퀘어|네트워크|5G|광통신/.test(name)) return '통신';
  if (/카카오|네이버|엔씨소프트|크래프톤|넷마블|컴투스|위메이드|펄어비스|데브시스터즈|NHN|더존비즈온|한글과컴퓨터|카페24|두나무|토스|비바리퍼블리카|소프트|솔루션|데이터|클라우드|플랫폼|디지털/.test(name)) return 'IT';
  if (/화학|LG화학|롯데케미칼|한화솔루션|금호석유|OCI|SKC|효성|대한유화|SK케미칼|코오롱인더|케미칼|수지|코팅/.test(name)) return '화학';
  if (/방산|한화에어|LIG넥스원|현대로템|한국항공|풍산|한화시스템|LIG|국방|무기/.test(name)) return '방산';
  if (/미디어|CJ ENM|스튜디오|하이브|SM엔터|JYP|YG|위지윅|덱스터|카카오엔터|콘텐트리|엔터|게임|방송|영화/.test(name)) return '미디어';
  if (/철강|포스코|현대제철|동국제강|세아|고려아연|풍산홀딩스|금속|스틸|알루미늄|합금/.test(name)) return '철강';
  if (/에너지|발전|태양|풍력|신재생|전력|수소|원전|가스|한국전력|한전|지역난방|서울가스|SK가스/.test(name)) return '에너지';
  if (/식품|오리온|농심|CJ|풀무원|삼양|오뚜기|하이트진로|빙그레|매일|남양|음료|양조|제과|유업/.test(name)) return '소비재';
  if (/호텔|신라|롯데관광|하나투어|모두투어|교원|이랜드|신세계|현대백화점|롯데쇼핑|GS리테일|BGF|리테일|면세|편의/.test(name)) return '소비재';
  if (/해운|항공|대한항공|아시아나|HMM|팬오션|흥아해운|한진칼|물류|택배|운송/.test(name)) return '운송';
  if (/조선|한국조선|HD현대|삼성중공업|대우조선|HD한국조선해양|마린/.test(name)) return '조선';
  if (/로보|로봇|AI|자율주행|레인보우로보|인공지능|딥러닝/.test(name)) return 'AI/로봇';
  if (/전선|전기|전력|가온전선|대한전선|LS|일진|전장/.test(name)) return '전기장비';
  if (/두산$|한화$|SK$|LG$|GS$|롯데$|현대$|LX|앤컴퍼니|홀딩스|홀딩|그룹/.test(name)) return '지주';
  if (/화장품|뷰티|코스|실리콘투|아모레|LG생활|코스맥스|한국콜마/.test(name)) return '뷰티/화장품';
  if (/디스플레이|LED|LCD|OLED|센서|PCB|커넥터|모듈|전자부품/.test(name)) return '전자부품';
  if (/기계|엔진|펌프|밸브|보일러|냉동|공조|중공업/.test(name)) return '기계';
  if (/환경|폐기물|재활용|수처리|정수|그린/.test(name)) return '환경';
  if (/섬유|의류|패션|직물|방직/.test(name)) return '소비재';
  if (/교육|학원|에듀|러닝/.test(name)) return '교육';
  if (/KODEX|TIGER|ACE|KINDEX|SOL|ETF/.test(name)) return 'ETF';
  if (/SPAC|스팩/.test(name)) return '기타';
  return null;
}
...
export async function GET(req: NextRequest) {
  ...
  const result = await withCronLogging('stock-crawl', async () => {
    ...
    const stockApiKey = (process.env.STOCK_DATA_API_KEY || process.env.BUSAN_DATA_API_KEY || '').trim();
    let krxDebug = '';
    ...
    return {
      ...
      metadata: {
        ...
        krx_debug: krxDebug.slice(0, 500),
      },
    };
  });
