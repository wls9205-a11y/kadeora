...
function guessSector(name: string): string | null {
  if (/반도체|하이닉스|삼성전자|DB하이텍|리노공업|원익IPS|한미반도체|이오테크닉스|테스|주성엔지니어링|피에스케이|솔브레인|동진쎄미켐|티씨케이|ISC|HPSP|파두|심텍|RFHIC|서진시스템|태성|SFA반도체|에이디테크|유진테크|코미코|하나마이크론/.test(name)) return '반도체';
  if (/바이오|셀트리온|삼성바이오|유한양행|녹십자|한미약품|대웅|종근당|일양|JW|보령|동아에스티|SK바이오|에이비엘|메디톡스|오스코텍|헬릭스미스|제넥신|알테오젠|레고켐|씨젠|에스디바이오|파미셀|차바이오|코오롱생명|HK이노엔|제약|팜|파마|테라|오믹스|올릭스|케어젠|보로노이|코오롱티슈|리브스메드|에스티팜|삼천당|비올|클래시스/.test(name)) return '바이오';
  ...
  if (/식품|오리온|농심|CJ|풀무원|삼양|오뚜기|하이트진로|빙그레|매일|남양/.test(name)) return '소비재';
  ...
  if (/로보|로봇|AI|자율주행|레인보우로보/.test(name)) return 'AI/로봇';
  if (/전선|전기|전력|가온전선|대한전선|LS|일진/.test(name)) return '전기장비';
  if (/두산$|한화$|SK$|LG$|GS$|롯데$|현대$|LX|앤컴퍼니/.test(name)) return '지주/복합';
