...
const LAWD_CODES: Record<string, string> = {
  ...
  // 광주 (5개)
  // ⚠️ **이 5개 코드는 지금 아무것도 돌려주지 않는다.** 실측(2026-08-26):
  //    5개 전부 202606 · 202608 에 대해 resultCode 000 · totalCount 0.
  //    광주는 2026-05-27 까지 41,116 행이 정상 적재되다가 그 뒤로 완전히 끊겼다.
  //    광역시에 두 달 연속 거래 0건은 실재할 수 없다 — 코드 쪽 문제다.
  //    ⚠️ 현행 코드를 «모르는 채로 추측해서 넣지 않는다». 그렇게 넣은 값이 강원·전북을
  //       3년 동안 0건으로 만든 원인이다. 법정동코드 조회로 확인한 뒤에 고친다.
  //       (강원·전북과 달리 「접두 2자리 교체」 규칙이 적용될 근거가 없다.)
  ...
  '울산 울주군':'31710',
  // 세종 (1개)
  // ⚠️ 코드 36110 은 «정상» 이다 (실측 202608 193건 · 202606 372건).
  //    문제는 키에 공백이 없다는 것이다 — `label.split(' ')` 이 ['세종시'] 를 내므로
  //    region_nm='세종시' · sigungu=undefined 로 적재된다. 나머지 230개는 '시도 시군구' 꼴이다.
  //    그래서 DB 에 `세종`(3,898행, ~2026-05-06) 과 `세종시`(1,038행, ~2026-07-30) 두 표기가 있다.
  //    ⚠️ 키를 '세종 세종시' 로 고치는 건 «동작 변경» 이고, 기존 1,038행 백필과 «짝» 이어야 한다.
  //       한쪽만 하면 표기가 다시 갈린다. DB 담당과 같이 처리할 것.
  '세종시':'36110',
  // 강원 (18개) — D5-4: 42xxx → 51xxx (2023 강원특별자치도 전환)
  // ⚠️ 42xxx 로는 «한 번도» 들어온 적이 없다. apt_transactions 의 강원 행이 0건이었다.
  //    32개 신코드 전량을 실제 호출로 확인했다 (2026-08-26):
  //    resultCode 000 · 응답의 sggCd 가 신코드와 정확히 일치 · 전 지역 거래 존재.
  ...
  '충남 연기군':'44830',
  // 전북 (14개) — D5-4: 45xxx → 52xxx (2024 전북특별자치도 전환)
  // ⚠️ `전북 전주시`(52111)는 «완산구만» 이다. 덕진구 52113 은 목록에 없다 —
  //    교체 전 45111 도 완산구뿐이었으므로 여기서는 «있던 만큼만» 옮긴다.
  //    덕진구 추가는 호출 수가 늘어 D5-3(한도) 판단 뒤에 할 일이다.
  '전북 전주시':'52111','전북 군산시':'52130','전북 익산시':'52140',
  '전북 정읍시':'52180','전북 남원시':'52190','전북 김제시':'52210',
  '전북 완주군':'52710','전북 진안군':'52720','전북 무주군':'52730',
  '전북 장수군':'52740','전북 임실군':'52750','전북 순창군':'52770',
  ...
  // 경남 (18개)
  // ⚠️ **창원시는 5개 구인데 의창구(48121) 하나만 있다.** 실측 202608 기준
  //    성산구 48123(197건) · 마산합포 48125(76건) · 마산회원 48127 · 진해 48129 가 통째로 빠졌다.
  //    부울경이 이 서비스의 커버리지인데 창원 거래의 대부분을 못 받고 있다는 뜻이다.
  //    ⚠️ 지금 추가하지 않는다 — 항목이 늘면 호출 수가 늘어 D5-2 관측을 오염시킨다.
  //       D5-3 에서 호출 예산을 정한 «뒤에» 넣는다.
