...
function ChatRoom({room,user,onClose}:{room:DiscussionRoom;user:User|null;onClose:()=>void}) {
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:1000,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      <div style={{background:'#111827',border:'1px solid #1E293B',borderRadius:16,width:'100%',maxWidth:640,height:'80vh',maxHeight:700,display:'flex',flexDirection:'column',boxShadow:'0 24px 64px rgba(0,0,0,0.5)'}} className="animate-modalIn">
        <div style={{padding:'16px 20px',borderBottom:'1px solid #1E293B',display:'flex',alignItems:'center',gap:10}}>
          <div style={{flex:1,minWidth:0}}>
            <h2 style={{margin:0,fontSize:16,fontWeight:700,color:'#F1F5F9',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{room.display_name}</h2>
          ...
          : display.map(msg=>(
            <div key={msg.id} style={{display:'flex',gap:8,flexDirection:user?.id===msg.author_id?'row-reverse':'row'}}>
              ...
              <div style={{maxWidth:'70%'}}>
                ...
                <div style={{background:user?.id===msg.author_id?'#3B82F6':'#1a2234',border:'1px solid '+(user?.id===msg.author_id?'transparent':'#1E293B'),borderRadius:12,padding:'8px 12px',fontSize:14,color:'#F1F5F9',lineHeight:1.5,wordBreak:'break-word'}}>
                  {msg.content}
                </div>
              </div>
            </div>
          ...
          {user ? (
            <>
              <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();}}}
                ...
                style={{flex:1,background:'#0A0E17',border:'1px solid #1E293B',borderRadius:8,color:'#F1F5F9',padding:'10px 12px',fontSize:14,fontFamily:'inherit'}} />
              <button onClick={send} disabled={!input.trim()||sending} style={{padding:'10px 16px',borderRadius:8,background:'#3B82F6',color:'white',border:'none',fontWeight:600,fontSize:14,cursor:'pointer',opacity:(!input.trim()||sending)?0.5:1}}>전송</button>
            </>
          ) : (
            <div style={{flex:1,textAlign:'center',color:'#64748B',fontSize:13,padding:'10px 0'}}><a href="/login" style={{color:'#3B82F6',textDecoration:'none'}}>로그인</a>하면 채팅에 참여할 수 있습니다</div>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12}}>
        {filtered.map(room=>(
          <div key={room.id} onClick={()=>setActive(room)} style={{background:'#111827',border:'1px solid #1E293B',borderRadius:14,padding:'18px 20px',cursor:'pointer',transition:'all 0.15s'}}
            ...
            <h3 style={{margin:'0 0 8px',fontSize:15,fontWeight:700,color:'#F1F5F9',lineHeight:1.4}}>{room.display_name}</h3>
