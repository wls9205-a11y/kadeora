...
export default async function GuidePage({ params }: { params: Promise<Params> }) {
  ...
  return (
    <div style={{ maxWidth: 840, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <h1 style={{ fontSize: 'var(--fs-xl)', fontWeight: 'var(--fw-title)', marginBottom: 16 }}>{title}</h1>
      <p style={{ color: 'var(--text-secondary)', marginBottom: 24 }}>
        {region} 지역의 {keyword} 관련 최신 소식을 한눈에 정리했습니다. 매일 자동 업데이트.
      </p>
      <section style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', marginBottom: 12 }}>핵심 요약</h2>
      ...
      <section style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', marginBottom: 12 }}>자주 묻는 질문</h2>
        <dl>
          {faqs.map((f, i) => (
            <div key={i} style={{ marginBottom: 12 }}>
              <dt style={{ fontWeight: 'var(--fw-title)' }}>Q. {f.q}</dt>
      ...
      {queue?.id && (
        <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 32 }}>
