      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {notifications.length === 0 ? (
          ...
          notifications.map(notif => {
            const Wrapper = notif.post_id ? Link : 'div' as React.ElementType;
            const wrapperProps = notif.post_id ? { href: `/feed/${notif.post_id}`, style: { textDecoration: 'none' } } : {};
            return (
              <Wrapper key={notif.id} {...(wrapperProps as Record<string, unknown>)} onClick={() => !notif.is_read && markRead(notif.id)}>
                <div style={{
                  background: notif.is_read ? 'var(--kd-surface)' : 'rgba(59,130,246,0.05)',
                  border: `1px solid ${notif.is_read ? 'var(--kd-border)' : 'rgba(59,130,246,0.3)'}`,
