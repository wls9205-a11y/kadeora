...
export default async function SitesListPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  let query = sb.from('apt_sites').select('slug, name, site_type, region, sigungu, total_units, status, interest_count, content_score', { count: 'exact' })
  ...
  // 지역별 카운트
  const { data: regionCounts } = await sb.from('apt_sites')
