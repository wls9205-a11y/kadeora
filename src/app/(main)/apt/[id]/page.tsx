  ...
  return (
    <article className="kd-detail" itemScope itemType='https://schema.org/ApartmentComplex'>
      ...
      {/* v3 커밋3 · 섹션 점프 바. 칩이 가리키는 섹션이 실제로 렌더될 때만 칩을 낸다 —
           ...
           가리켜 눌러도 아무 일이 없었다.
           M6 A-5: 칩 10개를 4개로 줄였다. 섹션 하나에 칩 하나씩 붙이면 바가 가로로
           흘러넘쳐 '무엇이 어디 있는지' 를 오히려 감춘다. 이제 칩은 A-1 의 네 묶음을
           가리킨다 — 각 묶음의 선두에 높이 0 앵커(*-group)를 두었다.
      ...
      {/* ── M6 A-1 · ③ 될 것 같은가 ──
           점프바 '될까' 칩의 목적지. 높이 0 앵커라 레이아웃에 끼어들지 않는다. */}
      <div id="odds-group" aria-hidden="true" style={{ scrollMarginTop: SECTION_SCROLL_MARGIN }} />
      ...
      {/* ── M6 A-1 · ④ 더 알아보기 ──
           점프바 '더보기' 칩의 목적지. 높이 0 앵커라 레이아웃에 끼어들지 않는다. */}
      <div id="more-group" aria-hidden="true" style={{ scrollMarginTop: SECTION_SCROLL_MARGIN }} />
      ...
      {/* M6 A-3: 요약줄에 접수기간을 낸다 — 접어 둔 채로도 «언제 것인지» 는 보여야 한다.
      ...
      {sub && (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 'var(--sp-md)', paddingTop: 10, borderTop: '1px solid var(--border)' }}>
            ...
            {sub.pblanc_url && <a href={sub.pblanc_url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 'var(--radius-md)', background: 'var(--accent-green-bg)', border: '1px solid var(--accent-green-border)', color: 'var(--accent-green)', fontSize: 'var(--fs-sm)', fontWeight: 500, textDecoration: 'none' }}>청약홈 원문</a>}
          </div>
          </div>
        </DetailSection>
      )}
