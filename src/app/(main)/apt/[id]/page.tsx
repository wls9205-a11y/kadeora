...
const ct: React.CSSProperties = { fontSize: 'var(--fs-base)', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 };
...
export default async function AptUnifiedPage({ params }: Props) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 16px' }}>
      ...
      {/* Schedule */}
      {sub && (() => { const rows = [['분양유형', sub.mdatrgbn_nm], ['청약접수', sub.rcept_bgnde && sub.rcept_endde ? `${sub.rcept_bgnde} ~ ${sub.rcept_endde}` : null], ['특별공급', sub.spsply_rcept_bgnde ? `${sub.spsply_rcept_bgnde} ~ ${sub.spsply_rcept_endde}` : null], ['당첨자발표', sub.przwner_presnatn_de], ['계약', sub.cntrct_cncls_bgnde ? `${sub.cntrct_cncls_bgnde} ~ ${sub.cntrct_cncls_endde}` : null], ['입주예정', fmtYM(sub.mvn_prearnge_ym)], ['총공급', sub.tot_supply_hshld_co ? `${Number(sub.tot_supply_hshld_co).toLocaleString()}세대` : null]].filter(r => r[1]); return (<div style={crd}><div style={ct}>📅 분양 일정</div>{rows.map(([l, v], i) => <div key={l as string} style={{ ...rw, borderBottom: i < rows.length - 1 ? '1px solid var(--border)' : 'none' }}><span style={rl}>{l}</span><span style={rv}>{v}</span></div>)}</div>); })()}

      {/* Overview */}
      {sub && (() => { const rows = [['시공사', sub.constructor_nm || site?.builder], ['시행사', sub.developer_nm || site?.developer], ['일반분양', sub.tot_supply_hshld_co ? `${Number(sub.tot_supply_hshld_co).toLocaleString()}세대` : null], ['동수', sub.total_dong_co ? `${sub.total_dong_co}개 동` : null], ['최고 층수', sub.max_floor ? `지상 ${sub.max_floor}층` : null], ['주차대수', sub.parking_co ? `${Number(sub.parking_co).toLocaleString()}대` : null], ['난방방식', sub.heating_type]].filter(r => r[1]); if (!rows.length) return null; return (<div style={crd}><div style={ct}>🏗️ 단지 개요</div>{rows.map(([l, v], i) => <div key={l as string} style={{ ...rw, borderBottom: i < rows.length - 1 ? '1px solid var(--border)' : 'none' }}><span style={rl}>{l}</span><span style={rv}>{v}</span></div>)}</div>); })()}

      {/* Conditions */}
      {sub && (sub.is_price_limit || sub.transfer_limit || sub.residence_obligation || sub.model_house_addr) && (
        <div style={crd}><div style={ct}>📋 분양 조건</div>
      ...
      {sub?.competition_rate_1st && Number(sub.competition_rate_1st) > 0 && (
        <div style={{ ...crd, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)' }}>
          <div style={ct}>🏆 청약 경쟁률</div>
      ...
      {/* Unsold section */}
      {unsold && (
        <div style={{ ...crd, borderLeft: '4px solid var(--accent-red)' }}>
          <div style={ct}>🏚️ 미분양 현황</div>
      ...
      {redev && redevStage && (() => { const ci = STAGES.indexOf(redevStage) >= 0 ? STAGES.indexOf(redevStage) : STAGES.findIndex(s => redevStage.includes(s.slice(0, 2))); const pct = ci >= 0 ? Math.round(((ci + 1) / STAGES.length) * 100) : 0; return (
        <div style={crd}><div style={ct}>🏗️ 재개발 진행 현황</div>
          <div style={{ display: 'flex', gap: 3, marginBottom: 8 }}>{STAGES.map((s, i) => <div key={s} style={{ flex: 1, textAlign: 'center', padding: '6px 2px', borderRadius: 6, fontSize: 'var(--fs-xs)', fontWeight: 600, background: i <= ci ? (i === ci ? '#B794FF' : 'rgba(183,148,255,0.2)') : 'var(--bg-hover)', color: i === ci ? 'var(--bg-base)' : i < ci ? '#B794FF' : 'var(--text-tertiary)' }}>{s.replace('사업시행인가', '시행인가').replace('정비구역지정', '구역지정')}</div>)}</div>
      ...
      {/* Location */}
      <div style={crd}><div style={ct}>📍 위치 정보</div>
      ...
      {/* Comments */}
      {sub && <div style={crd}><AptCommentInline houseKey={sub.house_manage_no || String(sub.id)} houseNm={name} houseType="sub" /></div>}

      {/* Related posts */}
      {relatedPosts.length > 0 && <div style={crd}><div style={ct}>💬 커뮤니티 게시글</div>{relatedPosts.map(p => <Link key={p.id} href={`/feed/${p.id}`} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)', textDecoration: 'none', color: 'inherit', fontSize: 'var(--fs-sm)' }}><span style={{ color: 'var(--text-primary)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</span><span style={{ color: 'var(--text-tertiary)', flexShrink: 0, marginLeft: 8, fontSize: 'var(--fs-xs)' }}>댓글 {p.comments_count || 0}</span></Link>)}</div>}

      {/* Related blogs */}
      {relatedBlogs.length > 0 && <div style={crd}><div style={ct}>📰 관련 분석 블로그</div>{relatedBlogs.map(b => <Link key={b.slug} href={`/blog/${b.slug}`} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)', textDecoration: 'none', color: 'inherit', fontSize: 'var(--fs-sm)' }}><span style={{ color: 'var(--text-primary)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.title}</span><span style={{ color: 'var(--text-tertiary)', flexShrink: 0, marginLeft: 8, fontSize: 'var(--fs-xs)' }}>👀 {(b.view_count || 0).toLocaleString()}</span></Link>)}</div>}

      {/* Nearby sites (internal linking SEO) */}
      {nearbySites.length > 0 && <div style={crd}><div style={ct}>🏗️ {region} 다른 현장</div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>{nearbySites.map(ns => <Link key={ns.slug} href={`/apt/${ns.slug}`} className="kd-card" style={{ background: 'var(--bg-hover)', borderRadius: 8, padding: 10, textDecoration: 'none', color: 'inherit' }}><div style={{ fontSize: 'var(--fs-sm)', fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: 2 }}>{ns.name}</div><div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>{ns.sigungu || ns.region} · {ns.total_units ? `${ns.total_units}세대` : ''} · {tLabel[ns.site_type]}</div></Link>)}</div></div>}

      {/* FAQ */}
      {faq.length > 0 && <div style={crd}><div style={ct}>❓ 자주 묻는 질문</div>{faq.map((f, i) => <details key={i} style={{ borderBottom: i < faq.length - 1 ? '1px solid var(--border)' : 'none', padding: '10px 0' }}><summary style={{ fontSize: 'var(--fs-sm)', fontWeight: 600, color: 'var(--text-primary)', cursor: 'pointer', listStyle: 'none', display: 'flex', justifyContent: 'space-between' }}><span>{f.q}</span><span style={{ color: 'var(--text-tertiary)' }}>+</span></summary><p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.7, margin: '6px 0 0' }}>{f.a}</p></details>)}</div>}
