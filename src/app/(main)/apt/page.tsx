...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ section?: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    other: { 'article:section': '부동산', 'article:tag': '부동산,청약,분양,미분양,재개발,실거래가,아파트', 'naver:written_time': '2026-04-12T00:00:00Z', 'naver:updated_time': '2026-04-12T00:00:00Z', 'naver:author': '카더라', 'og:updated_time': '2026-04-12T00:00:00Z', 'dg:plink': SITE_URL + '/apt' },
  };
}
...
async function fetchAptData() {
  ...
  try {
    ...
    const [aptsR, unsoldR, alertsR, redevCountR, tradeByRegionR, redevByRegionR, subCountR, unsoldCountR, ongoingCountR, redevRedevR, redevRebuildR] = await Promise.all([
      ...
      sb.from('unsold_apts').select('id, house_nm, region_nm, sigungu_nm, tot_supply_hshld_co, tot_unsold_hshld_co, supply_addr, completion_ym, sale_price_min, sale_price_max, pblanc_url, contact_tel, source, created_at, is_active, constructor_nm, developer_nm, discount_info, nearest_station, price_per_pyeong').eq('is_active', true).order('tot_unsold_hshld_co', { ascending: false }),
