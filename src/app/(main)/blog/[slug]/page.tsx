...
export default async function BlogDetailPage({ params }: Props) {
  ...
  return (
    <div className="blog-detail-layout">
      ...
      <nav aria-label="breadcrumb" style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginBottom: 20, flexWrap: 'wrap', letterSpacing: 0 }}>
        <Link href="/" style={{ textDecoration: 'none', color: 'var(--text-tertiary)' }}>홈</Link>
        <span style={{ opacity: 0.3 }}>/</span>
        <Link href="/blog" style={{ textDecoration: 'none', color: 'var(--text-tertiary)' }}>블로그</Link>
        {post.category && <><span style={{ opacity: 0.3 }}>/</span><Link href={`/blog?category=${post.category}`} style={{ textDecoration: 'none', color: catStyle.color, fontWeight: 'var(--fw-title)' }}>{({ stock: '주식', apt: '청약', unsold: '미분양', finance: '재테크', general: '생활' } as Record<string, string>)[post.category] || post.category}</Link></>}
      </nav>
      ...
      <article itemScope itemType={`https://schema.org/${isNewsArticle ? 'NewsArticle' : 'BlogPosting'}`} style={{ paddingBottom: 40 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
            <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', padding: '4px 12px', borderRadius: 'var(--radius-xl)', background: catStyle.bg, color: catStyle.color, letterSpacing: 0 }}>
            ...
            {post.rewritten_at && <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', padding: '3px 8px', borderRadius: 'var(--radius-xl)', background: 'var(--success-bg)', color: 'var(--success)' }}>UP</span>}
          </div>
          {/* 제목 */}
          <h1 style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', lineHeight: 1.5, margin: '0 0 10px', wordBreak: 'keep-all', letterSpacing: '-0.4px' }}>{post.title}</h1>
          {/* s213: H1 직후 컴팩트 메타 한 줄 — 큰 저자 카드 / TLDR / key_points / 태그 pills 모두 본문 종료 후 "이 글 정보" 섹션으로 이동.
              독자가 페이지 열자마자 본문 첫 문단까지 스크롤이 짧아짐. */}
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginBottom: 16 }}>
          ...
          return (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <Link href={`/blog/series/${seriesInfo.series.slug}`} style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-body)', color: 'var(--brand)', textDecoration: 'none' }}>
                  📚 {seriesInfo.series.title}
                </Link>
                <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', fontWeight: 'var(--fw-title)' }}>
        ...
        {post.category === 'apt' && (
          <div style={{
            ...
            <a href="/apt" style={{
              ...
              <div>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)' }}>청약 일정 보기</div>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>접수중·예정 전체</div>
              </div>
            </a>
            <a href="/apt/diagnose" style={{
              ...
              <span style={{ fontSize: 'var(--fs-md)' }}>🎯</span>
              <div>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)' }}>청약 가점 계산</div>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>내 당첨 확률은?</div>
              </div>
            </a>
            <a href="/calc/real-estate/brokerage-fee" style={{
              ...
              <span style={{ fontSize: 'var(--fs-md)' }}>🧮</span>
              <div>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)' }}>중개수수료 계산</div>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>매매·전세 복비</div>
              </div>
            </a>
          </div>
        )}
        ...
        {showFaq && (
          <section aria-labelledby="blog-sec-faq" style={{ marginTop: 'var(--sp-xl)' }}>
            <h2
              id="blog-sec-faq"
              style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', margin: '0 0 var(--sp-sm)' }}
            >
              자주 묻는 질문
            </h2>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 'var(--fs-sm)', marginRight: 6 }}>🎉</span>
          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-primary)' }}>약 {readingTimeMin}분 읽기 완료</span>
          {related.length > 0 && (
            <Link href={`/blog/${related[0].slug}`} style={{
              ...
              fontWeight: 'var(--fw-body)', textDecoration: 'none',
            }}>
        ...
        {((post as any).tldr || (Array.isArray((post as any).key_points) && (post as any).key_points.length > 0)) && (
          <details style={{ marginTop: 'var(--sp-md)', padding: '10px 14px', borderRadius: 'var(--radius-md)', background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
            <summary style={{ cursor: 'pointer', fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-secondary)', listStyle: 'none' }}>
              📝 요약 보충 자료 (TLDR · 핵심 요약)
            </summary>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center' }}>
            ...
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
              <span aria-hidden style={{
                ...
                fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', flexShrink: 0,
              }}>{(post.category === 'field_note' ? '분양 담당' : (post.author_name || '카더라')).charAt(0)}</span>
              <span style={{ fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{post.category === 'field_note' ? '분양 담당' : (post.author_name || '카더라 부동산팀')}</span>
              {post.author_role && <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>· {post.author_role}</span>}
            </span>
            {/* 카테고리 / 서브카테고리 */}
            ...
            {post.source_type && (
              <>
                <span aria-hidden style={{ color: 'var(--text-tertiary)' }}>|</span>
                <span style={{ fontSize: 'var(--fs-2xs)', padding: '2px 8px', borderRadius: 'var(--radius-xl)', background: 'var(--bg-hover)', color: 'var(--text-tertiary)' }}>
                  {post.source_type}
                </span>
              </>
            )}
        ...
        {(() => {
          ...
          return (
            <section aria-labelledby="blog-sec-refs" style={{ marginTop: 'var(--sp-xl)', padding: 'var(--sp-md) var(--card-p)', borderRadius: 'var(--radius-md)', background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
              <h2 id="blog-sec-refs" style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', margin: '0 0 var(--sp-sm)' }}>참고자료</h2>
              <ul style={{ margin: 0, paddingLeft: 18, fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
      ...
      <div id="blog-comments" style={{ marginBottom: 'var(--sp-xl)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>{comments.length}</span>
        </div>
        ...
        {isLoggedIn ? (
          ...
          <div style={{ padding: 16, background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', textAlign: 'center', marginBottom: 'var(--sp-lg)', fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)' }}>
            <Link href={`/login?redirect=/blog/${slug}&source=blog_comment`} style={{ color: 'var(--brand)', fontWeight: 'var(--fw-body)', textDecoration: 'none' }}>로그인</Link>하면 의견을 남길 수 있어요
          </div>
        )}
          ...
          {comments.map((c: Record<string, any>) => {
            ...
            return (
              <div key={c.id} style={{ display: 'flex', gap: 10, padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', flexShrink: 0, background: getAvatarColor(nick), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)' }}>
                  {nick[0].toUpperCase()}
                </div>
                <div style={{ flex: 1 }}>
                  <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{nick}</span>
                  <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginLeft: 6 }}>{timeAgo(c.created_at)}</span>
                  <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-primary)', lineHeight: 1.6, marginTop: 3 }}>{c.content}</div>
                  {(c as any).image_url && (
      ...
      {hubSite && (
        ...
        >
          ...
          <span className="kd-lrow-r" style={{ fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)' }}>보기 ›</span>
        </Link>
      )}
      ...
      {(() => {
        ...
        return (
          <details style={{
            ...
            <div style={{ marginTop: 10, fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', lineHeight: 1.6 }}>
              {isYmyl && (
                <YMYLBanner
                  category={post.category}
        ...
        return (
          <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-card)', padding: 16, marginBottom: 'var(--sp-xl)' }}>
            <Link href={`/blog/series/${seriesInfo.series.slug}`} style={{ display: 'flex', alignItems: 'center', gap: 6, textDecoration: 'none', marginBottom: 10 }}>
              <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', padding: '3px 8px', borderRadius: 4, background: 'var(--accent-blue-bg)', color: 'var(--accent-blue)' }}>📚 시리즈</span>
              <span style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{seriesInfo.series.title}</span>
            ...
            <div style={{ display: 'flex', gap: 'var(--sp-sm)' }}>
              {prev && (
                <Link href={`/blog/${prev.slug}`} style={{ flex: 1, padding: '8px 10px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-hover)', textDecoration: 'none', fontSize: 'var(--fs-xs)' }}>
                  ...
                  <div style={{ color: 'var(--text-secondary)', fontWeight: 'var(--fw-body)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{prev.title}</div>
                </Link>
              )}
              {next && (
                <Link href={`/blog/${next.slug}`} style={{ flex: 1, padding: '8px 10px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-hover)', textDecoration: 'none', fontSize: 'var(--fs-xs)', textAlign: 'right' }}>
                  ...
                  <div style={{ color: 'var(--text-secondary)', fontWeight: 'var(--fw-body)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{next.title}</div>
                </Link>
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 'var(--sp-sm)' }}>
            {prevPost && (
              <Link href={`/blog/${prevPost.slug}`} style={{ flex: 1, padding: '8px 10px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-hover)', textDecoration: 'none', fontSize: 'var(--fs-xs)' }}>
                ...
                <div style={{ color: 'var(--text-secondary)', fontWeight: 'var(--fw-body)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{prevPost.title}</div>
              </Link>
            )}
            {nextPost && (
              <Link href={`/blog/${nextPost.slug}`} style={{ flex: 1, padding: '8px 10px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-hover)', textDecoration: 'none', fontSize: 'var(--fs-xs)', textAlign: 'right' }}>
                ...
                <div style={{ color: 'var(--text-secondary)', fontWeight: 'var(--fw-body)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{nextPost.title}</div>
              </Link>
            )}
          </div>
        </div>
      ...
      {related.length > 0 && (
        <section aria-labelledby="blog-sec-popular" style={{ marginBottom: 'var(--sp-xl)' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-xs)', letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--brand)', fontWeight: 'var(--fw-title)', marginBottom: 3 }}>POPULAR — 이번주</div>
          <h2 id="blog-sec-popular" style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', margin: '0 0 var(--sp-sm)' }}>이번주 인기글</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {related.slice(0, 3).map((r: any, i: number) => (
              <Link key={r.slug} href={`/blog/${r.slug}`} style={{
                ...
                <span style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', color: i === 0 ? 'var(--accent-red)' : i === 1 ? 'var(--warning)' : 'var(--text-tertiary)', minWidth: 24 }}>{i + 1}</span>
                <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.title?.slice(0, 40)}</span>
              </Link>
            ))}
          </div>
        </section>
      ...
      {(related ?? []).length > 0 && (
        <section aria-labelledby="blog-sec-related" style={{ marginBottom: 'var(--sp-xl)' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-xs)', letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--brand)', fontWeight: 'var(--fw-title)', marginBottom: 3 }}>RELATED — 함께 보기</div>
          <h2 id="blog-sec-related" style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', margin: '0 0 var(--sp-sm)' }}>관련 글</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 'var(--sp-sm)' }}>
            {related!.slice(0, 4).map((r: any) => (
              <Link key={r.slug} href={`/blog/${r.slug}`} className="kd-feed-card" style={{ display: 'block', padding: '12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', background: 'var(--bg-surface)', textDecoration: 'none', transition: 'border-color var(--transition-fast)' }}>
                <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const, overflow: 'hidden', lineHeight: 1.3 }}>{r.title}</span>
                <div style={{ display: 'flex', gap: 'var(--sp-sm)', marginTop: 6, fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>
      ...
      {relatedSites.length > 0 && (
        <div style={{ marginBottom: 'var(--sp-xl)' }}>
          <div style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 10 }}>🏢 관련 현장 정보</div>
          <div style={{ display: 'flex', gap: 'var(--sp-sm)', flexWrap: 'wrap' }}>
            {relatedSites.map((s: Record<string, any>) => (
              <Link key={s.slug} href={`/apt/${s.slug}`} style={{ flex: '1 1 calc(33.3% - 6px)', minWidth: 140, padding: '10px 12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', background: 'var(--bg-surface)', textDecoration: 'none', transition: 'border-color var(--transition-fast)' }}>
                <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.name}</div>
      ...
      {/* 세션74: 유용한 도구 + 뉴스레터 */}
      <div style={{ marginBottom: 'var(--sp-xl)' }}>
        <div style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 10 }}>🛠️ 유용한 도구</div>
        <div style={{ display: 'flex', gap: 'var(--sp-sm)', flexWrap: 'wrap' }}>
          {(post.category === 'apt' || post.category === 'unsold') && (
            <Link href="/apt/diagnose" style={{ flex: '1 1 calc(50% - 4px)', minWidth: 140, padding: '12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--brand-border)', background: 'var(--bg-surface)', textDecoration: 'none' }}>
              <div style={{ fontSize: 'var(--fs-sm)', marginBottom: 4 }}>🎯</div>
              <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>청약 가점 계산기</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 2 }}>무주택·부양가족·청약통장 가점 자동 계산</div>
            </Link>
          )}
          {post.category === 'stock' && (
            <Link href="/stock/compare" style={{ flex: '1 1 calc(50% - 4px)', minWidth: 140, padding: '12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--brand-border)', background: 'var(--bg-surface)', textDecoration: 'none' }}>
              <div style={{ fontSize: 'var(--fs-sm)', marginBottom: 4 }}>⚖️</div>
              <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>종목 비교</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 2 }}>국내외 종목 핵심 지표 비교</div>
            </Link>
          )}
          <Link href="/apt/complex" style={{ flex: '1 1 calc(50% - 4px)', minWidth: 140, padding: '12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', background: 'var(--bg-surface)', textDecoration: 'none' }}>
            <div style={{ fontSize: 'var(--fs-sm)', marginBottom: 4 }}>🏘️</div>
            <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>단지백과</div>
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 2 }}>전국 34,500+ 아파트 시세 검색</div>
          </Link>
        </div>
      </div>
      ...
      {relatedStocks.length > 0 && (
        <div style={{ marginBottom: 'var(--sp-xl)' }}>
          <div style={{ fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 10 }}>📈 관련 종목</div>
          <div style={{ display: 'flex', gap: 'var(--sp-sm)', flexWrap: 'wrap' }}>
            {relatedStocks.map((s: Record<string, any>) => {
              ...
              return (
                <Link key={s.symbol} href={`/stock/${s.symbol}`} style={{ flex: '1 1 calc(33.3% - 6px)', minWidth: 140, padding: '10px 12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', background: 'var(--bg-surface)', textDecoration: 'none' }}>
                  <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{s.name}</div>
      ...
      {sidebarMetrics.length > 0 && (
        <div style={{ marginBottom: 16, padding: '12px 14px', borderRadius: 'var(--radius-card)', background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 8 }}>📊 핵심 지표</div>
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(sidebarMetrics.length, 4)}, 1fr)`, gap: 8 }}>
            {sidebarMetrics.map((m, i) => (
              <div key={i} style={{ textAlign: 'center', padding: '8px 4px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-hover)' }}>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginBottom: 2 }}>{m.label}</div>
                <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{m.value}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 16, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {sidebarRelatedLinks.map((link, i) => (
            <Link key={i} href={link.href} style={{
              padding: '5px 12px', borderRadius: 'var(--radius-card)', fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)',
              background: 'var(--bg-surface)', border: '1px solid var(--border)',
