...
export default function BlogDetailError({ reset }: { error: Error; reset: () => void }) {
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '60px 16px', textAlign: 'center' }}>
      ...
      <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 8px' }}>글을 불러오지 못했어요</h2>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
        ...
        <Link href="/blog" style={{ padding: '10px 20px', borderRadius: 10, background: 'var(--brand)', color: '#fff', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
          블로그 목록
        </Link>
      </div>
    </div>
