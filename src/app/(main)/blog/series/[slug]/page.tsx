...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    other: {
      'naver:written_time': s.created_at || new Date().toISOString(),
      'naver:updated_time': s.created_at || new Date().toISOString(),
      'naver:author': '카더라',
      'article:section': '블로그',
      'article:tag': `${s.title},시리즈,카더라`,
      'article:published_time': s.created_at || new Date().toISOString(),
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 var(--sp-lg)' }}>
      ...
      <div style={{ marginBottom: 'var(--sp-xl)' }}>
        ...
        <h1 style={{ margin: '8px 0 0', fontSize: 'var(--fs-xl)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>📚 {series.title}</h1>
        {series.description && (
        ...
        {(posts || []).map((post, idx) => (
          <Link key={post.id} href={`/blog/${post.slug}`} style={{
            ...
            <div style={{
              ...
              fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', zIndex: 1,
