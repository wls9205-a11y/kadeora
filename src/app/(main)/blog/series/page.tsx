  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 var(--sp-lg)' }}>
      ...
      <div style={{ marginBottom: 'var(--sp-xl)' }}>
        ...
        <h1 style={{ margin: '8px 0 0', fontSize: 'var(--fs-xl)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>📚 시리즈</h1>
            ...
            <Link key={s.id} href={`/blog/series/${s.slug}`} className="kd-card-hover" style={{
              ...
              <div style={{ height: 80, background: `linear-gradient(135deg, ${catColor}18 0%, ${catColor}08 100%)`, display: 'flex', alignItems: 'center', padding: '0 20px', gap: 14, position: 'relative' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <h2 style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', margin: 0, lineHeight: 1.3 }}>{s.title}</h2>
                  {s.description && <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', margin: '2px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.description}</p>}
                </div>
                <div style={{ position: 'absolute', top: 10, right: 14, fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', padding: '3px 10px', borderRadius: 'var(--radius-xl)', background: `${catColor}20`, color: catColor, border: `1px solid ${catColor}40` }}>
                  {s.post_count}편
                </div>
              </div>
              {/* 진행률 바 */}
