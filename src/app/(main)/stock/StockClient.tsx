  ...
  function StockRow({ s, rank }: { s: Stock; rank: number }) {
    ...
    return (
      <div onClick={() => setSelectedStock(s)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 0', borderBottom: '1px solid var(--border)', cursor: 'pointer' }}>
        ...
        <button onClick={e => { e.stopPropagation(); toggleWatchlist(s.symbol); }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, fontSize: 'var(--fs-lg)', lineHeight: 1, color: isWatched ? '#FBBF24' : 'var(--text-tertiary)', flexShrink: 0 }} title={isWatched ? '관심 해제' : '관심 추가'}>
          {isWatched ? '★' : '☆'}
        </button>
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        <button onClick={() => setMode('domestic')} style={{
          ...
          background: isDomestic ? 'var(--bg-elevated, #162544)' : 'var(--bg-surface)',
          color: isDomestic ? '#E2E8F0' : 'var(--text-tertiary)',
      ...
      {isDomestic && themes.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 6 }}>🔥 오늘의 테마</div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 4 }}>
            {themes.map(t => (
              <div key={t.id} onClick={() => { setDomesticTab('themes'); setSelectedTheme(t.theme_name); }} style={{ minWidth: 110, padding: '10px 12px', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 10, flexShrink: 0, cursor: 'pointer' }}>
                <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 700, color: 'var(--text-primary)' }}>{t.is_hot && '🔥'}{t.theme_name}</div>
                <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 700, marginTop: 3, color: stockColor(t.change_pct??0, true) }}>
      ...
      {isDomestic && domesticTab === 'themes' && (
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 10, overflowX: 'auto', scrollbarWidth: 'none' }}>
            <button onClick={() => setSelectedTheme(null)} style={{ padding: '4px 12px', borderRadius: 999, fontSize: 'var(--fs-xs)', fontWeight: 600, border: 'none', cursor: 'pointer', flexShrink: 0, background: !selectedTheme ? 'rgba(96,165,250,0.15)' : 'var(--bg-hover)', color: !selectedTheme ? '#93C5FD' : 'var(--text-tertiary)' }}>전체</button>
            {themes.map(t => (
              <button key={t.id} onClick={() => setSelectedTheme(selectedTheme === t.theme_name ? null : t.theme_name)} style={{ padding: '4px 12px', borderRadius: 999, fontSize: 'var(--fs-xs)', fontWeight: 600, border: 'none', cursor: 'pointer', flexShrink: 0, background: selectedTheme === t.theme_name ? 'rgba(96,165,250,0.15)' : 'var(--bg-hover)', color: selectedTheme === t.theme_name ? '#93C5FD' : 'var(--text-tertiary)' }}>{t.is_hot ? '🔥' : ''}{t.theme_name}</button>
            ))}
          </div>
      ...
      {currentTab === 'movers' && (
        <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
          {([['up','📈 상승'],['down','📉 하락'],['volume','🔥 거래량']] as const).map(([k,l]) => (
            <button key={k} onClick={() => setMoversTab(k)} style={{ padding: '6px 14px', borderRadius: 999, fontSize: 'var(--fs-sm)', fontWeight: 600, border: 'none', cursor: 'pointer', background: moversTab===k?'rgba(96,165,250,0.15)':'var(--bg-hover)', color: moversTab===k?'#93C5FD':'var(--text-secondary)' }}>{l}</button>
          ))}
        </div>
      )}
          ...
          <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 24, maxWidth: 480, width: '100%' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                ...
                <span style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-tertiary)' }}>{selectedStock.symbol} · {selectedStock.market}</span>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div style={{ background: 'var(--bg-hover)', borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginBottom: 4 }}>현재가</div>
              ...
              <div style={{ background: 'var(--bg-hover)', borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginBottom: 4 }}>등락률</div>
                <div style={{ fontSize: 'var(--fs-lg)', fontWeight: 800, color: stockColor(selectedStock.change_pct??0, isDomestic) }}>
