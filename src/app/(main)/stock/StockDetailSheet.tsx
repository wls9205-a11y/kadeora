  ...
  return (
    <BottomSheet open={true} onClose={onClose} title={stock.name}>
      ...
      <div style={{ background: 'var(--bg-hover)', borderRadius: 'var(--radius-card)', padding: 16, marginBottom: 'var(--sp-md)' }}>
        <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-num)', color: 'var(--text-primary)' }}>
          {stock.currency === 'USD' ? `$${stock.price?.toFixed(2)}` : `₩${fmt(stock.price)}`}
        </div>
        <div style={{ fontSize: 'var(--fs-lg)', fontWeight: 'var(--fw-title)', color: stockColor(stock.change_pct ?? 0, isDomestic), marginTop: 'var(--sp-xs)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--sp-sm)', marginBottom: 'var(--sp-lg)' }}>
        <div style={{ background: 'var(--bg-hover)', borderRadius: 'var(--radius-sm)', padding: '8px 10px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: 'var(--text-primary)', marginTop: 2 }}>{fmtCap(stock.market_cap, stock.currency)}</div>
        </div>
        <div style={{ background: 'var(--bg-hover)', borderRadius: 'var(--radius-sm)', padding: '8px 10px', textAlign: 'center' }}>
          <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>거래량</div>
          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginTop: 2 }}>{stock.volume ? fmt(stock.volume) : '-'}</div>
        </div>
        <div style={{ background: 'var(--bg-hover)', borderRadius: 'var(--radius-sm)', padding: '8px 10px', textAlign: 'center' }}>
          <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>전일대비</div>
          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: stockColor(stock.change_pct ?? 0, isDomestic), marginTop: 2 }}>{stock.change_amt ? `${stock.change_amt > 0 ? '+' : ''}${fmt(stock.change_amt)}` : '-'}</div>
        </div>
      </div>

      {/* 관심종목 + 상세 버튼 */}
      <div style={{ display: 'flex', gap: 'var(--sp-sm)' }}>
        <button onClick={() => onToggleWatchlist(stock.symbol)} style={{ flex: 1, padding: 12, borderRadius: 'var(--radius-sm)', border: `1px solid ${isWatched ? 'var(--accent-yellow)' : 'var(--border)'}`, background: isWatched ? 'rgba(251,191,36,0.08)' : 'var(--bg-hover)', color: isWatched ? '#D97706' : 'var(--text-secondary)', fontWeight: 'var(--fw-body)', fontSize: 'var(--fs-sm)', cursor: 'pointer', transition: 'all var(--transition-fast)' }}>
          {isWatched ? '★ 관심 해제' : '☆ 관심 추가'}
        </button>
        <Link href={`/stock/${encodeURIComponent(stock.symbol)}`} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isDomestic ? 'var(--brand)' : 'var(--accent-blue)', color: 'var(--text-inverse)', padding: 12, borderRadius: 'var(--radius-sm)', textDecoration: 'none', fontWeight: 'var(--fw-body)', fontSize: 'var(--fs-sm)' }}>
          종목 상세 →
        </Link>
      </div>
    </BottomSheet>
