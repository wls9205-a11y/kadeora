      ...
      {prices.length > 5 && (() => {
        ...
        return (
          <div style={{ marginTop: 12 }}>
            <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 4 }}>가격 범위</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 10, color: 'var(--text-tertiary)' }}>{low.toLocaleString()}</span>
              ...
              <span style={{ fontSize: 10, color: 'var(--text-tertiary)' }}>{high.toLocaleString()}</span>
            </div>
          </div>
        );
      })()}
      ...
      <div style={{ display: 'flex', gap: 2, marginBottom: 16, overflowX: 'auto', scrollbarWidth: 'none', borderBottom: '1px solid var(--border)', paddingBottom: 0 }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            padding: '8px 14px', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, background: 'transparent',
      ...
      {tab === 'overview' && (
        <div>
          ...
          <div style={{ background: 'var(--bg-hover)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
            ...
            <p style={{ margin: 0, fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.75 }}>{description}</p>
          </div>
        </div>
      )}
          ...
          {investorFlow.length === 0 ? (
            ...
            return (
              <div key={d.date || d.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                <span style={{ fontSize: 11, color: 'var(--text-tertiary)', minWidth: 50 }}>{(d.date || '').slice(5)}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                    <span style={{ fontSize: 10, minWidth: 36, color: 'var(--text-tertiary)' }}>외국인</span>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 600, minWidth: 50, textAlign: 'right', color: foreignNet >= 0 ? '#3b82f6' : '#ef4444' }}>
                      {foreignNet >= 0 ? '+' : ''}{(foreignNet / 10000).toFixed(1)}만
                    </span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 10, minWidth: 36, color: 'var(--text-tertiary)' }}>기관</span>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 600, minWidth: 50, textAlign: 'right', color: instNet >= 0 ? '#f59e0b' : '#ef4444' }}>
          ...
          {disclosures.length === 0 ? (
            ...
            return (
              <div key={d.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: 'var(--bg-hover)', color: 'var(--text-secondary)', fontWeight: 600 }}>
                    {typeMap[d.disclosure_type] || '📋공시'}
                  </span>
                  <span style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 500 }}>{d.title}</span>
                </div>
