...
export default async function StockComparePage({ params }: Props) {
  ...
  return (
    <article style={{ maxWidth: 720, margin: '0 auto', padding: '16px var(--sp-lg)' }}>
      ...
      <h1 style={{ fontSize: 'clamp(20px, 5vw, 28px)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 8 }}>{a.name} vs {b.name}</h1>
      ...
      <div style={{ borderRadius: 'var(--radius-card)', overflow: 'hidden', border: '1px solid var(--border)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', background: 'var(--bg-surface)', fontWeight: 'var(--fw-body)', fontSize: 'var(--fs-sm)' }}>
        ...
        {rows.map(([label, va, vb], i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', background: i % 2 === 0 ? 'var(--bg-base)' : 'var(--bg-surface)' }}>
            <div style={{ ...cs, color: 'var(--text-secondary)', fontWeight: 'var(--fw-title)' }}>{label}</div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 24 }}>
        <Link href={`/stock/${a.symbol}`} style={{ flex: 1, textAlign: 'center', padding: '12px 0', borderRadius: 'var(--radius-sm)', background: 'var(--brand)', color: '#fff', textDecoration: 'none', fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)' }}>{a.name} 상세</Link>
        <Link href={`/stock/${b.symbol}`} style={{ flex: 1, textAlign: 'center', padding: '12px 0', borderRadius: 'var(--radius-sm)', background: 'var(--bg-hover)', border: '1px solid var(--border)', color: 'var(--text-primary)', textDecoration: 'none', fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)' }}>{b.name} 상세</Link>
      </div>
