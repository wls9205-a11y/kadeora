...
function DailyFortune() {
  ...
  if (!year) return (
    <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '14px 16px', marginBottom: 10 }}>
      <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', marginBottom: 8 }}>출생연도를 선택하면 오늘의 띠별 운세를 확인할 수 있어요</div>
      <select onChange={e => handleSelect(parseInt(e.target.value))}
        style={{ width: '100%', padding: '8px 10px', fontSize: 'max(16px, var(--fs-xs))', background: 'var(--bg-base)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', color: 'var(--text-primary)', cursor: 'pointer' }}>
    ...
    <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '14px 16px', marginBottom: 10 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 'var(--fs-xl)' }}>{ZE[animal]}</span>
          <div>
            <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{animal}띠 · {year}년생</div>
            <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>{now.getMonth() + 1}월 {now.getDate()}일 운세</div>
          </div>
        </div>
        <button onClick={() => { setYear(null); setSaved(false); localStorage.removeItem('kd_birth_year'); }} style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', background: 'none', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '3px 8px', cursor: 'pointer' }}>변경</button>
      </div>

      {/* 종합 운세 */}
      <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-primary)', lineHeight: 1.6, marginBottom: 12, padding: '10px 12px', background: 'rgba(59,123,246,0.03)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(59,123,246,0.08)' }}>
        {f.summary}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 6 }}>
        {[
          ...
          <div key={item.label} style={{ padding: '8px 10px', borderRadius: 'var(--radius-md)', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)' }}>
            <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', marginBottom: 3, display: 'flex', alignItems: 'center', gap: 4 }}>
              <span>{item.icon}</span> {item.label}
            </div>
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.6 }}>{item.text}</div>
          </div>
        ))}
      </div>
    </div>
...
const SH = ({ icon, title }: { icon: string; title: string }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-sm)', margin: '20px 0 10px' }}>
    ...
    <span style={{ fontSize: 'var(--fs-md)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', letterSpacing: '-0.2px' }}>{title}</span>
  </div>
);
    ...
    return (
      <div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 'var(--fs-lg)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 8 }}>
            카더라 데일리 리포트
          </div>

          <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 20, maxWidth: 320, margin: '0 auto 20px' }}>
          ...
          <div style={{ padding: '14px 16px', borderRadius: 'var(--radius-md)', background: G.goldBg, border: `1px solid ${G.goldBorder}`, marginBottom: 16, textAlign: 'left' }}>
            <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: G.goldText, marginBottom: 8, textAlign: 'center' }}>🔓 리포트 무료 열람 방법</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                <span style={{ fontSize: 'var(--fs-sm)', flexShrink: 0, marginTop: 1 }}>1️⃣</span>
                <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.5 }}>아래 <b style={{ color: 'var(--text-primary)' }}>공유하기</b> 버튼을 눌러 카카오톡, 밴드 등 아무 곳에나 <b style={{ color: G.goldText }}>1회</b> 공유해 주세요</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                <span style={{ fontSize: 'var(--fs-sm)', flexShrink: 0, marginTop: 1 }}>2️⃣</span>
                <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.5 }}>공유 완료 즉시 리포트가 열리며, <b style={{ color: 'var(--accent-green)' }}>앞으로 영구 무료</b>로 매일 읽을 수 있어요</span>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 'var(--fs-base)', fontWeight: 'var(--fw-title)', cursor: shareLoading ? 'wait' : 'pointer',
          ...
          <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 12, marginBottom: 0, lineHeight: 1.5 }}>
          ...
          <div style={{ marginTop: 16, padding: '10px 14px', borderRadius: 'var(--radius-sm)', background: 'rgba(255,255,255,0.02)', border: `1px solid ${G.goldBorder}` }}>
            <div style={{ fontSize: 'var(--fs-xs)', color: G.goldText, fontWeight: 'var(--fw-title)', marginBottom: 4 }}>✦ 오늘 리포트 미리보기</div>
    ...
    <div>
      ...
      {isArchive && (
        <div style={{ padding: '6px 12px', borderRadius: 'var(--radius-sm)', background: G.goldBg, border: `1px solid ${G.goldBorder}`, marginBottom: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: G.goldText }}>📂 {viewDate} 아카이브</span>
          <button onClick={goToToday} style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: G.goldText, background: 'none', border: `1px solid ${G.goldBorder}`, borderRadius: 'var(--radius-xs)', padding: '3px 10px', cursor: 'pointer' }}>오늘 보기 →</button>
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
              <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: G.goldText, letterSpacing: '0.14em', textTransform: 'uppercase' }}>KADEORA DAILY REPORT</span>
              <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)', background: G.goldBg, padding: '3px 8px', borderRadius: 4, border: `1px solid ${G.goldBorder}` }}>#{d.issueNo}</span>
            </div>
            <div style={{ fontSize: 'var(--fs-lg)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', letterSpacing: '-0.4px', marginBottom: 4 }}>
              {(() => {
            ...
            <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-secondary)', marginBottom: 6 }}>
          ...
          <select
            ...
            style={{ fontSize: 'max(16px, var(--fs-xs))', fontWeight: 'var(--fw-body)', color: G.goldText, background: G.goldBg, border: `1px solid ${G.goldBorder}`, borderRadius: 'var(--radius-xs)', padding: '4px 10px', cursor: 'pointer' }}
          >
        ...
        <div style={{ padding: '10px 12px', borderRadius: 'var(--radius-sm)', background: 'rgba(255,255,255,0.02)', border: `1px solid ${G.goldBorder}`, marginBottom: 10 }}>
          <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', margin: 0, lineHeight: 1.6 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderTop: `1px solid ${G.goldBorder}`, marginBottom: 6 }}>
          <button
            ...
            disabled={!prevDate}
            style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: prevDate ? G.gold : 'var(--text-tertiary)', background: 'none', border: 'none', cursor: prevDate ? 'pointer' : 'default', padding: '4px 8px' }}
          >◀ 이전</button>

          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{dateLabel}</span>
          ...
          <button
            ...
            style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: (nextDate || isArchive) ? G.gold : 'var(--text-tertiary)', background: 'none', border: 'none', cursor: (nextDate || isArchive) ? 'pointer' : 'default', padding: '4px 8px' }}
        ...
        <div style={{ marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
            <span style={{ fontSize: 'var(--fs-sm)' }}>🔮</span>
            <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>오늘의 운세</span>
          </div>
          <DailyFortune />
        </div>
        ...
        <div style={{ padding: '8px 10px', borderRadius: 'var(--radius-sm)', background: G.goldBg, border: `1px solid ${G.goldBorder}`, marginBottom: 'var(--sp-sm)' }}>
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: G.goldText, marginBottom: 'var(--sp-xs)' }}>✦ 어젯밤 달라진 것</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--sp-xs)' }}>
            {d.stockTop10.slice(0, 4).filter(s => s.week_pct != null && s.week_pct !== 0).map(s => (
              <span key={s.symbol} style={{
                fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', padding: '3px 8px', borderRadius: 'var(--radius-xs)',
            ...
            {d.subscriptions.filter(s => s.status === '예정' && s.rcept_bgnde === d.date).map(s => (
              <span key={s.house_nm} style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', padding: '3px 8px', borderRadius: 'var(--radius-xs)', background: 'rgba(251,146,60,0.08)', color: 'var(--accent-yellow)' }}>
        ...
        <div className="report-summary" style={{ padding: '10px 12px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-surface)', border: '1px solid var(--border)', marginBottom: 8 }}>
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 4 }}>📋 오늘의 핵심 요약</div>
          <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', margin: 0, lineHeight: 1.6 }}>
        ...
        <div className="kd-kpi-5">
          {[
            ...
            <div key={i} style={{ background: G.goldBg, borderRadius: 'var(--radius-sm)', padding: '10px 6px', textAlign: 'center', border: `1px solid ${G.goldBorder}` }}>
              <div style={{ fontSize: 'var(--fs-md)', fontWeight: 'var(--fw-num)', color: G.goldText }}>{k.v}</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 2 }}>{k.l}</div>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: k.sc, marginTop: 1 }}>{k.s}</div>
            </div>
          ))}
        </div>
      </div>
            ...
            {d.aiBriefing && (
              <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>🇰🇷 국내</span>
                  <span style={{
                    fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', padding: '3px 8px', borderRadius: 'var(--radius-xs)',
                ...
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 6 }}>{d.aiBriefing.title}</div>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.6 }}>{d.aiBriefing.summary.slice(0, 200)}</div>
              </div>
            )}
            {d.aiBriefingUS && (
              <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>🇺🇸 해외</span>
                  <span style={{
                    fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', padding: '3px 8px', borderRadius: 'var(--radius-xs)',
                ...
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 6 }}>{d.aiBriefingUS.title}</div>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.6 }}>{d.aiBriefingUS.summary.slice(0, 200)}</div>
              </div>
            )}
          </div>
        </>
      ...
      {d.marketSentiment && (d.marketSentiment.positive + d.marketSentiment.negative + d.marketSentiment.neutral) > 0 && (
        <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '16px', marginBottom: 8 }}>
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 10 }}>🌡️ 시장 심리 지수</div>
          <div style={{ textAlign: 'center', marginBottom: 10 }}>
            <span style={{ fontSize: 'var(--fs-2xl)', fontWeight: 'var(--fw-num)', color: d.marketSentiment.score >= 60 ? 'var(--accent-green)' : d.marketSentiment.score <= 40 ? 'var(--accent-red)' : 'var(--text-secondary)' }}>
              {d.marketSentiment.score}
            </span>
            <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', marginLeft: 4 }}>/ 100</span>
            <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: d.marketSentiment.score >= 70 ? 'var(--accent-green)' : d.marketSentiment.score >= 55 ? '#86EFAC' : d.marketSentiment.score >= 45 ? 'var(--text-tertiary)' : d.marketSentiment.score >= 30 ? '#FCA5A5' : 'var(--accent-red)', marginTop: 4 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>
            <span>극도의 공포</span><span>중립</span><span>극도의 탐욕</span>
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 10, justifyContent: 'center' }}>
            <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--accent-green)' }}>긍정 {d.marketSentiment.positive}건</span>
            <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>중립 {d.marketSentiment.neutral}건</span>
            <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--accent-red)' }}>부정 {d.marketSentiment.negative}건</span>
          </div>
        </div>
      )}
      ...
      {(d.priceChanges.stockUp.length > 0 || d.priceChanges.aptUp.length > 0) && (
        <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '16px', marginBottom: 8 }}>
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 10 }}>📊 오늘의 가격 변동 TOP</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {/* 주식 급등 */}
            <div>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--accent-red)', marginBottom: 6 }}>🔴 급등 종목</div>
              {d.priceChanges.stockUp.map((s, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)', padding: '3px 0', borderBottom: '1px solid var(--border)' }}>
                  <span style={{ color: 'var(--text-primary)', fontWeight: 'var(--fw-title)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '60%' }}>{s.name}</span>
                  <span style={{ color: 'var(--accent-red)', fontWeight: 'var(--fw-num)' }}>+{s.change_pct.toFixed(1)}%</span>
                </div>
              ))}
            </div>
            {/* 주식 급락 */}
            <div>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--brand)', marginBottom: 6 }}>🔵 급락 종목</div>
              {d.priceChanges.stockDown.map((s, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)', padding: '3px 0', borderBottom: '1px solid var(--border)' }}>
                  <span style={{ color: 'var(--text-primary)', fontWeight: 'var(--fw-title)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '60%' }}>{s.name}</span>
                  <span style={{ color: 'var(--brand)', fontWeight: 'var(--fw-num)' }}>{s.change_pct.toFixed(1)}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {d.priceChanges.aptUp.length > 0 && (
            <div style={{ marginTop: 10 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <div>
                  <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--accent-red)', marginBottom: 6 }}>🏠 시세 상승 단지</div>
                  {d.priceChanges.aptUp.map((a, i) => (
                    <div key={i} style={{ fontSize: 'var(--fs-xs)', padding: '3px 0', borderBottom: '1px solid var(--border)' }}>
                      <div style={{ fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.name}</div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ color: 'var(--text-tertiary)' }}>{a.sigungu}</span>
                        <span style={{ color: 'var(--accent-red)', fontWeight: 'var(--fw-num)' }}>+{a.change_pct.toFixed(1)}%</span>
                      </div>
                    </div>
                  ))}
                </div>
                <div>
                  <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--brand)', marginBottom: 6 }}>🏠 시세 하락 단지</div>
                  {d.priceChanges.aptDown.map((a, i) => (
                    <div key={i} style={{ fontSize: 'var(--fs-xs)', padding: '3px 0', borderBottom: '1px solid var(--border)' }}>
                      <div style={{ fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.name}</div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ color: 'var(--text-tertiary)' }}>{a.sigungu}</span>
                        <span style={{ color: 'var(--brand)', fontWeight: 'var(--fw-num)' }}>{a.change_pct.toFixed(1)}%</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {(d.hotTopics.polls.length > 0 || d.hotTopics.vsBattles.length > 0 || d.hotTopics.hotPosts.length > 0) && (
        <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '16px', marginBottom: 8 }}>
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 10 }}>🔥 커뮤니티 핫토픽</div>
          {/* VS 배틀 */}
          {d.hotTopics.vsBattles.map((v, i) => (
            <div key={`vs-${i}`} style={{ marginBottom: 8, padding: '8px 10px', borderRadius: 8, background: 'rgba(139,92,246,0.06)', border: '1px solid rgba(139,92,246,0.12)' }}>
              <div style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: 'var(--accent-purple)', marginBottom: 4 }}>⚔️ VS 배틀 ({v.total}명 참여)</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <div style={{ flex: 1, textAlign: 'center' }}>
                  <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{v.option_a}</div>
                  <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: v.a_pct >= v.b_pct ? 'var(--accent-red)' : 'var(--text-tertiary)' }}>{v.a_pct}%</div>
                </div>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-tertiary)' }}>VS</div>
                <div style={{ flex: 1, textAlign: 'center' }}>
                  <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{v.option_b}</div>
                  <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: v.b_pct >= v.a_pct ? 'var(--brand)' : 'var(--text-tertiary)' }}>{v.b_pct}%</div>
                </div>
              </div>
            </div>
          ))}
          ...
          {d.hotTopics.predictions.map((p, i) => (
            <div key={`pred-${i}`} style={{ marginBottom: 8, padding: '8px 10px', borderRadius: 8, background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.12)' }}>
              <div style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: 'var(--warning)', marginBottom: 4 }}>🔮 예측 ({p.total}명 참여)</div>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{p.title}</div>
          ...
          {d.hotTopics.hotPosts.length > 0 && (
            <div style={{ marginTop: 6 }}>
              <div style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-tertiary)', marginBottom: 4 }}>🔥 인기글</div>
              {d.hotTopics.hotPosts.slice(0, 3).map((p, i) => (
                <a key={p.id} href={`/feed/${p.id}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 'var(--fs-xs)', padding: '4px 0', borderBottom: '1px solid var(--border)', textDecoration: 'none', color: 'inherit' }}>
                  <span style={{ fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, marginRight: 8 }}>{p.title || '글'}</span>
                  <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', flexShrink: 0 }}>♥{p.like_count} 💬{p.comment_count}</span>
                </a>
              ))}
            </div>
          )}
      ...
      {d.activityRanking.length > 0 && (
        <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '16px', marginBottom: 8 }}>
          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 10 }}>🏆 오늘의 활발한 카더라인</div>
          {d.activityRanking.slice(0, 3).map((u, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: i < 2 ? '1px solid var(--border)' : 'none' }}>
              <span style={{ fontSize: 'var(--fs-sm)', width: 24, textAlign: 'center' }}>{i === 0 ? '🥇' : i === 1 ? '🥈' : '🥉'}</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{u.nickname}</div>
                <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>글 {u.posts}개 · {u.points.toLocaleString()}P</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <table style={{ width: '100%', fontSize: 'var(--fs-xs)', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              <th style={{ textAlign: 'left', padding: '5px 4px', color: 'var(--text-tertiary)', fontWeight: 'var(--fw-title)' }}>#</th>
              <th style={{ textAlign: 'left', padding: '5px 4px', color: 'var(--text-tertiary)', fontWeight: 'var(--fw-title)' }}>종목</th>
              <th style={{ textAlign: 'right', padding: '5px 4px', color: 'var(--text-tertiary)', fontWeight: 'var(--fw-title)' }}>현재가</th>
              <th style={{ textAlign: 'right', padding: '5px 4px', color: 'var(--text-tertiary)', fontWeight: 'var(--fw-title)' }}>전주比</th>
              <th style={{ textAlign: 'right', padding: '5px 4px', color: 'var(--text-tertiary)', fontWeight: 'var(--fw-title)' }}>시총</th>
            </tr>
          </thead>
          <tbody>
            {d.stockTop10.map((s, i) => (
              <tr key={s.symbol} style={{ borderBottom: i < 9 ? '1px solid var(--border)' : 'none' }}>
                ...
                <td style={{ padding: '6px 4px' }}>
                  <span style={{ fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{s.name}</span>
                  <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginLeft: 3 }}>{s.sector}</span>
                </td>
                <td style={{ textAlign: 'right', padding: '6px 4px', color: 'var(--text-primary)' }}>{Number(s.price).toLocaleString()}</td>
                <td style={{ textAlign: 'right', padding: '6px 4px' }}>
                  {s.week_ago ? (
                    <span style={{ color: pctColor(s.week_pct), fontWeight: 'var(--fw-title)' }}>
                      {pctStr(s.week_pct)}
                    </span>
                  ) : <span style={{ color: 'var(--text-tertiary)' }}>-</span>}
                </td>
                <td style={{ textAlign: 'right', padding: '6px 4px', color: 'var(--text-tertiary)', fontSize: 'var(--fs-xs)' }}>
        ...
        {d.sectors.slice(0, 14).map(s => {
          ...
          return (
            <div key={s.sector} style={{
              ...
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{s.sector}</div>
              <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: pctColor(s.avg_pct) }}>{pctStr(s.avg_pct)}</div>
              <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>{s.cap_t}조</div>
            </div>
          );
        })}
      </div>
            ...
            {(d.indices || []).map(idx => {
              ...
              return (
                <div key={idx.label} style={{ background: G.goldBg, borderRadius: 'var(--radius-sm)', padding: '6px 4px', textAlign: 'center', border: `1px solid ${G.goldBorder}`, borderLeft: `3px solid ${color}` }}>
                  <div style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)' }}>{idx.label}</div>
                  <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>{idx.label.includes('S&P') || idx.label === 'NASDAQ' ? Number(idx.value).toLocaleString('en', { maximumFractionDigits: 0 }) : fmt(idx.value)}</div>
                  {idx.change_pct !== 0 && <div style={{ fontSize: 'var(--fs-2xs)', color, fontWeight: 'var(--fw-num)' }}>{idx.change_pct > 0 ? '▲' : '▼'}{Math.abs(idx.change_pct).toFixed(2)}%</div>}
                </div>
              );
            })}
            {d.exchangeRate > 0 && (
              <div style={{ background: G.goldBg, borderRadius: 'var(--radius-sm)', padding: '6px 4px', textAlign: 'center', border: `1px solid ${G.goldBorder}` }}>
                <div style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-tertiary)' }}>USD/KRW</div>
                <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>₩{d.exchangeRate.toLocaleString('ko-KR', { maximumFractionDigits: 0 })}</div>
              </div>
            )}
          </div>
        </>
        ...
        {d.globalStocks.slice(0, 6).map(s => {
          ...
          return (
            <div key={s.symbol} style={{ background: G.goldBg, borderRadius: 'var(--radius-sm)', padding: '6px 4px', textAlign: 'center', border: `1px solid ${G.goldBorder}` }}>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: G.goldText }}>{s.symbol}</div>
              <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: 'var(--text-primary)' }}>${Number(s.price).toFixed(0)}</div>
              {pct !== 0 && <div style={{ fontSize: 'var(--fs-2xs)', color, fontWeight: 'var(--fw-num)' }}>{pct > 0 ? '+' : ''}{pct.toFixed(2)}%</div>}
              <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>${fmtB(s.market_cap)}</div>
            </div>
          );
        })}
      </div>
          ...
          d.subscriptions.filter(s => s.status !== '마감').map((s, i) => (
            <div key={i} style={{ display: 'flex', gap: 'var(--sp-sm)', padding: '8px 0', borderBottom: i < d.subscriptions.filter(s => s.status !== '마감').length - 1 ? '1px solid var(--border)' : 'none' }}>
              <div style={{ width: 42, textAlign: 'center', flexShrink: 0, borderRight: '1px solid var(--border)', paddingRight: 6 }}>
                <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{new Date(s.rcept_bgnde).getDate()}</div>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>{(new Date(s.rcept_bgnde).getMonth() + 1)}월</div>
                {s.rcept_bgnde === d.date && <div style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: 'var(--accent-red)' }}>TODAY</div>}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-xs)', flexWrap: 'wrap' }}>
                  <span style={{ fontWeight: 'var(--fw-title)', fontSize: 'var(--fs-sm)', color: 'var(--text-primary)' }}>{s.house_nm}</span>
                  <span style={{
                    fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', padding: '3px 8px', borderRadius: 'var(--radius-xs)',
                    background: s.status === '접수중' ? 'rgba(16,185,129,0.08)' : 'rgba(59,130,246,0.08)',
                    color: s.status === '접수중' ? 'var(--accent-green)' : 'var(--brand)',
                  }}>{s.status}</span>
                  {s.region_nm === d.region && <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: 'var(--accent-yellow)' }}>내 지역</span>}
                </div>
        ...
        <>
          ...
          <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '14px 16px', marginBottom: 8 }}>
              ...
              return (
                <div key={g.sigungu} style={{ marginBottom: i < Math.min(d.guPrices.length, 12) - 1 ? 6 : 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)', marginBottom: 3 }}>
                    <span style={{ fontWeight: i < 3 ? 'var(--fw-title)' : 'var(--fw-body)', color: i < 3 ? 'var(--text-primary)' : 'var(--text-secondary)' }}>{g.sigungu}</span>
                    <span>
                      <span style={{ color: 'var(--accent-red)', fontWeight: 'var(--fw-title)' }}>{fmt(g.sale)}</span>
                      <span style={{ color: 'var(--text-tertiary)', margin: '0 3px' }}>·</span>
                      <span style={{ color: 'var(--brand)' }}>{fmt(g.jeonse)}</span>
                      <span style={{ color: g.jeonse_ratio >= 68 ? 'var(--accent-green)' : 'var(--text-tertiary)', marginLeft: 4, fontSize: 'var(--fs-xs)' }}>{g.jeonse_ratio}%</span>
                    </span>
                  </div>
            ...
            <div style={{ display: 'flex', gap: 'var(--sp-sm)', fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 'var(--sp-xs)' }}>
              ...
              <span>전세율 <span style={{ color: 'var(--accent-green)', fontWeight: 'var(--fw-title)' }}>68%+</span> = 갭투자 유리</span>
            </div>
          </div>
        </>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 6 }}>
        <div>
          ...
          <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '14px 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <div><span style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)' }}>{d.unsoldUnits.toLocaleString()}</span><span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>세대</span></div>
              <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-title)', color: localUnsoldPct < 5 ? 'var(--accent-green)' : 'var(--accent-red)' }}>
              ...
              return (
                <div key={r.region_nm} style={{ marginBottom: 3 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)' }}>
                    <span style={{ color: r.region_nm === d.region ? 'var(--brand)' : 'var(--text-secondary)', fontWeight: r.region_nm === d.region ? 'var(--fw-title)' : 'var(--fw-quiet)' }}>{r.region_nm}</span>
                    <span style={{ fontWeight: 'var(--fw-title)', color: i === 0 ? 'var(--accent-red)' : 'var(--text-primary)' }}>{r.units.toLocaleString()}</span>
                  </div>
            ...
            {d.unsoldLocal.length > 0 && (
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', marginTop: 'var(--sp-xs)' }}>
                <span style={{ fontWeight: 'var(--fw-title)', color: 'var(--text-secondary)' }}>{d.region} TOP</span>: {d.unsoldLocal.slice(0, 3).map(r => `${r.sigungu} ${r.units}`).join(' · ')}
              </div>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '14px 16px' }}>
            <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', marginBottom: 'var(--sp-xs)' }}>
              {d.redevTotal}건 <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-secondary)' }}>재개발 {d.redevTotal - d.redevRebuild} · 재건축 {d.redevRebuild}</span>
            </div>
                ...
                {d.redevStages.map((st, i) => {
                  ...
                  return (
                    <div key={st.stage} style={{
                      ...
                      fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: '#fff',
                    }}>{st.cnt}</div>
                  );
                })}
              </div>
            ...
            {d.redevMajor.length > 0 && (
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', marginTop: 'var(--sp-xs)', lineHeight: 1.5 }}>
                <span style={{ fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>재건축</span>: {d.redevMajor.slice(0, 5).join(' · ')}
              </div>
            )}
          </div>
        </div>
          ...
          <div style={{ background: 'var(--bg-surface)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border)', padding: '14px 16px', marginBottom: 8 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
              <div style={{ textAlign: 'center', padding: '8px 4px', borderRadius: 'var(--radius-sm)', background: G.goldBg, border: `1px solid ${G.goldBorder}` }}>
                <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: G.goldText }}>{d.tradeTrend.thisMonth.deals}</div>
                <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>이달 거래</div>
                {d.tradeTrend.lastMonth.deals > 0 && (
                  <div style={{ fontSize: 'var(--fs-2xs)', color: d.tradeTrend.thisMonth.deals > d.tradeTrend.lastMonth.deals ? 'var(--accent-red)' : 'var(--accent-blue)', fontWeight: 'var(--fw-title)' }}>
                    전월 {d.tradeTrend.lastMonth.deals}건
                  </div>
                )}
              </div>
              <div style={{ textAlign: 'center', padding: '8px 4px', borderRadius: 'var(--radius-sm)', background: G.goldBg, border: `1px solid ${G.goldBorder}` }}>
                <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-num)', color: G.goldText }}>{fmt(d.tradeTrend.thisMonth.avgPrice)}</div>
                <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>평균 매매가</div>
                {d.tradeTrend.lastMonth.avgPrice > 0 && (() => {
                  ...
                  return <div style={{ fontSize: 'var(--fs-2xs)', color: diff >= 0 ? 'var(--accent-red)' : 'var(--accent-blue)', fontWeight: 'var(--fw-title)' }}>{diff >= 0 ? '▲' : '▼'}{Math.abs(diff)}%</div>;
                })()}
              </div>
              <div style={{ textAlign: 'center', padding: '8px 4px', borderRadius: 'var(--radius-sm)', background: G.goldBg, border: `1px solid ${G.goldBorder}` }}>
                <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--accent-red)' }}>{fmt(d.tradeTrend.thisMonth.maxPrice)}</div>
                <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>최고가</div>
                {d.tradeTrend.thisMonth.maxAptName && <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.tradeTrend.thisMonth.maxAptName}</div>}
              </div>
            </div>
            {/* 최근 고가 거래 */}
            {d.tradeTrend.hotDeals.length > 0 && (
              <>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: G.goldText, marginBottom: 6 }}>🔥 최근 2주 고가 거래</div>
                {d.tradeTrend.hotDeals.map((deal, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: i < d.tradeTrend!.hotDeals.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    <div>
                      <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{deal.apt_name}</span>
                      <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', marginLeft: 4 }}>{deal.sigungu} · {Math.round(deal.exclusive_area)}㎡</span>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--accent-red)' }}>{fmt(deal.deal_amount)}</span>
                      <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', marginLeft: 4 }}>{deal.deal_date.slice(5)}</span>
                    </div>
                  </div>
                ))}
              </>
            ...
            {d.recommendBlogs.map(blog => {
              ...
              return (
                <Link key={blog.slug} href={`/blog/${blog.slug}`} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderRadius: 'var(--radius-card)', background: 'var(--bg-surface)', border: '1px solid var(--border)', textDecoration: 'none', transition: 'border-color 0.15s' }}>
                  <span style={{ fontSize: 'var(--fs-lg)', flexShrink: 0 }}>{catIcon}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{blog.title}</div>
                    <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{blog.excerpt || '카더라에서 읽기'}</div>
                  </div>
                  <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-title)', padding: '3px 8px', borderRadius: 4, background: 'rgba(59,123,246,0.08)', color: 'var(--brand)', flexShrink: 0 }}>{catLabel}</span>
                </Link>
              );
            })}
          </div>
      ...
      <div className="report-summary" style={{
        ...
        {/* 상단 골드 라인 */}
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${G.goldDark}, ${G.gold}, ${G.goldLight}, ${G.gold}, ${G.goldDark})` }} />
        <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 'var(--sp-md)' }}>

          {/* 주식 시장 요약 */}
          <div style={{ marginBottom: 'var(--sp-md)' }}>
            <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: G.goldText, marginBottom: 4 }}>✦ 주식 시장</div>
            국내 <Link href="/stock" style={{ color: 'var(--brand)', textDecoration: 'none', fontWeight: 'var(--fw-title)' }}>시총 TOP 10</Link> 종목 중 <b style={{ color: 'var(--accent-red)' }}>{weekUp}개 종목이 상승</b>, <b style={{ color: 'var(--accent-blue)' }}>{weekDn}개 종목이 하락</b>했습니다.
            {d.sectors[0] && <> 섹터별로는 <Link href={`/stock/sector/${encodeURIComponent(d.sectors[0].sector)}`} style={{ color: pctColor(d.sectors[0].avg_pct), textDecoration: 'none', fontWeight: 'var(--fw-body)' }}>{d.sectors[0].sector}</Link> 섹터가 <span style={{ color: pctColor(d.sectors[0].avg_pct), fontWeight: 'var(--fw-body)' }}>{pctStr(d.sectors[0].avg_pct)}</span>로 가장 강한 흐름을 보였습니다.</>}
            {' '}전체 {d.sectors.length}개 섹터 가운데 <span style={{ color: 'var(--accent-red)', fontWeight: 'var(--fw-title)' }}>{sectorUp}개 상승</span>, <span style={{ color: 'var(--accent-blue)', fontWeight: 'var(--fw-title)' }}>{sectorDn}개 하락</span>하며 {sectorUp > sectorDn ? '시장 전반에 매수 심리가 우세한' : sectorUp === sectorDn ? '관망세가 짙은' : '매도 압력이 강한'} 장세를 보이고 있습니다.
            {d.stockTop10[0] && <> 시총 1위 <Link href={`/stock/${d.stockTop10[0].symbol}`} style={{ color: 'var(--text-primary)', textDecoration: 'none', fontWeight: 'var(--fw-num)' }}>{d.stockTop10[0].name}</Link>은 현재 <b style={{ color: 'var(--text-primary)' }}>{Number(d.stockTop10[0].price).toLocaleString()}원</b>{d.stockTop10[0].week_pct != null && d.stockTop10[0].week_pct !== 0 ? <>, 주간 <span style={{ color: pctColor(d.stockTop10[0].week_pct), fontWeight: 'var(--fw-num)' }}>{pctStr(d.stockTop10[0].week_pct)}</span>의 변동을 기록</> : ''}하고 있습니다.</>}
            {d.globalStocks.length > 0 && <> 해외 시장에서는 {d.globalStocks.slice(0, 3).map((s, i) => <span key={s.symbol}>{i > 0 && ', '}<Link href={`/stock/${s.symbol}`} style={{ color: 'var(--text-primary)', textDecoration: 'none', fontWeight: 'var(--fw-title)' }}>{s.symbol}</Link> ${Number(s.price).toFixed(0)}{s.change_pct ? <span style={{ color: s.change_pct > 0 ? 'var(--accent-green)' : 'var(--accent-red)', fontSize: 'var(--fs-xs)' }}>({s.change_pct > 0 ? '+' : ''}{s.change_pct.toFixed(1)}%)</span> : ''}</span>)} 수준에서 거래되고 있습니다.</>}
          ...
          {/* 청약 시장 요약 */}
          <div style={{ marginBottom: 'var(--sp-md)' }}>
            <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: G.goldText, marginBottom: 4 }}>✦ 청약 시장</div>
            이번 주 전국 <Link href="/apt" style={{ color: 'var(--brand)', textDecoration: 'none', fontWeight: 'var(--fw-num)' }}>{d.subCountThisWeek}건</Link>의 아파트 청약이 예정되어 있으며, 총 <b style={{ color: 'var(--text-primary)' }}>{d.subUnitsThisWeek.toLocaleString()}세대</b> 규모입니다.
          ...
          {/* 재개발 동향 */}
          <div>
            <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: G.goldText, marginBottom: 4 }}>✦ 재개발·재건축</div>
            <Link href={`/apt/region/${encodeURIComponent(d.region)}`} style={{ color: 'var(--brand)', textDecoration: 'none', fontWeight: 'var(--fw-title)' }}>{d.region}</Link> 지역에서는 현재 총 <Link href="/apt?tab=redev" style={{ color: 'var(--brand)', textDecoration: 'none', fontWeight: 'var(--fw-num)' }}>{d.redevTotal}건</Link>의 정비사업이 진행 중입니다.
        ...
        <div style={{ padding: '8px 10px', borderRadius: 'var(--radius-sm)', background: G.goldBg, border: `1px solid ${G.goldBorder}` }}>
          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: G.goldText, marginBottom: 4 }}>✦ 내일 체크포인트</div>
          <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
      ...
      <div style={{ marginTop: 'var(--sp-md)', padding: '16px', borderRadius: 'var(--radius-card)', background: 'linear-gradient(135deg, rgba(59,123,246,0.06), rgba(46,232,165,0.04))', textAlign: 'center' }}>
        <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', marginBottom: 4 }}>오늘 리포트가 유익했다면?</div>
        <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', marginBottom: 12 }}>카카오톡·밴드로 공유하면 +5P 적립!</div>
      ...
      <div style={{ marginTop: 'var(--sp-md)', padding: '14px 16px', borderRadius: 'var(--radius-card)', background: G.goldBg, border: `1px solid ${G.goldBorder}` }}>
        <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
          <span style={{ color: G.goldText, fontWeight: 'var(--fw-title)' }}>발행</span> 매일 오전 7시 (평일) · 주말판 토요일 오전 발행<br/>
          <span style={{ color: G.goldText, fontWeight: 'var(--fw-body)' }}>내용</span> 국내외 <Link href="/stock" style={{ color: 'var(--brand)', textDecoration: 'none', fontWeight: 'var(--fw-title)' }}>주식 시황</Link> · <Link href="/apt" style={{ color: 'var(--brand)', textDecoration: 'none', fontWeight: 'var(--fw-title)' }}>청약 캘린더</Link> · <Link href="/apt?tab=unsold" style={{ color: 'var(--brand)', textDecoration: 'none', fontWeight: 'var(--fw-title)' }}>미분양 현황</Link> · <Link href="/apt?tab=redev" style={{ color: 'var(--brand)', textDecoration: 'none', fontWeight: 'var(--fw-title)' }}>재개발 동향</Link> · 시군구별 시세<br/>
          <span style={{ color: G.goldText, fontWeight: 'var(--fw-title)' }}>대상</span> 카더라 회원 (거주지 등록 필수)
        </div>
      </div>

      <div style={{ textAlign: 'center', padding: '10px 0', marginTop: 'var(--sp-sm)', fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', lineHeight: 1.5 }}>
        <span style={{ color: G.goldText, fontWeight: 'var(--fw-body)', letterSpacing: 0 }}>KADEORA DAILY REPORT</span> #{d.issueNo}<br/>
