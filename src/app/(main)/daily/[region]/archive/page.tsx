  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 16px 40px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--sp-lg)' }}>
        <div>
          <h1 style={{ fontSize: 'var(--fs-md)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', margin: 0 }}>📂 카더라 데일리 리포트 아카이브</h1>
          <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', margin: '4px 0 0' }}>{region} · {reports.length}개 리포트</p>
        </div>
        <Link href={`/daily/${encodeURIComponent(region)}`} style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--brand)', textDecoration: 'none' }}>오늘 보기 →</Link>
      </div>
      ...
      {reports.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-tertiary)' }}>
          ...
          <p style={{ fontSize: 'var(--fs-xs)' }}>아직 저장된 리포트가 없습니다.</p>
          <p style={{ fontSize: 'var(--fs-xs)', marginTop: 'var(--sp-xs)' }}>매일 오전 7시에 자동 저장됩니다.</p>
        </div>
      ) : (
        Array.from(grouped.entries()).map(([month, items]) => {
          ...
          return (
            <div key={month} style={{ marginBottom: 'var(--sp-lg)' }}>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-primary)', marginBottom: 6, padding: '4px 0', borderBottom: '1px solid var(--border)' }}>
                {y}년 {parseInt(m)}월 <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-tertiary)' }}>({items.length}일)</span>
              </div>
                ...
                {items.map(r => {
                  ...
                  return (
                    <Link key={r.report_date} href={`/daily/${encodeURIComponent(region)}/${r.report_date}`} style={{
                      ...
                      <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)' }}>{d.getDate()}일 ({dayNames[d.getDay()]})</div>
                      <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>#{r.issue_no}</div>
                    </Link>
                  );
                })}
              </div>
