    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <button onClick={load} disabled={loading}
          style={{ fontSize: 12, padding: '4px 12px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-hover)', color: 'var(--text-secondary)', cursor: 'pointer' }}>
          {loading ? '...' : '새로고침'}
        </button>
      </div>
      ...
      {data && missingCount > 0 && (
        <div style={{ padding: '10px 14px', borderRadius: 8, background: 'var(--error-bg, #fef2f2)', border: '1px solid var(--error, #ef4444)', marginBottom: 12, fontSize: 13, color: 'var(--error, #ef4444)', fontWeight: 600 }}>
            ...
            <tbody>
              {allVars.map((v, i) => (
                <tr key={v.key} style={{ background: i % 2 === 0 ? 'var(--bg-surface)' : 'var(--bg-hover)' }}>
                  <td style={{ padding: '8px 12px', color: 'var(--text-primary)', fontFamily: 'monospace', fontSize: 12 }}>
