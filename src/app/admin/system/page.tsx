...
function SeedSection() {
  ...
  if (!stats) return <div style={{ ...card }}><div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-tertiary)', padding: 16 }}>로딩 중...</div></div>;
  ...
  return (
    <div style={card}>
      ...
      {!confirmAll ? (
        ...
        <button onClick={() => handleDelete('all')} disabled={!!deleting} style={{ width: '100%', padding: '10px 0', borderRadius: 8, border: 'none', background: 'var(--error)', color: '#fff', fontSize: 'var(--fs-sm)', fontWeight: 700, cursor: 'pointer' }}>
          {deleting === 'all' ? '삭제 중...' : '정말 전체 삭제합니다 (되돌릴 수 없음)'}
        </button>
      )}
      {result && <div style={{ fontSize: 'var(--fs-sm)', color: result.includes('실패') ? 'var(--error)' : 'var(--success)', marginTop: 6 }}>{result}</div>}
      ...
      <div style={{ marginTop: 16, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
        ...
        {showHistory && (
          <div style={{ marginTop: 8 }}>
            ...
            <div style={{ marginTop: 8, padding: 16, background: 'var(--bg-hover)', borderRadius: 8, fontSize: 'var(--fs-sm)', color: 'var(--text-secondary)', lineHeight: 1.8 }}>
...
function CronSection() {
  ...
  return (
    <div style={card}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {crons.map(c => (
          <div key={c.endpoint} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: 'var(--bg-hover)', borderRadius: 8 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 600, color: 'var(--text-primary)' }}>{c.label}</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>{c.desc}</div>
            </div>
      ...
      {result && <div style={{ fontSize: 'var(--fs-sm)', marginTop: 8, color: result.includes('실패') ? 'var(--error)' : 'var(--success)' }}>{result}</div>}
    </div>
  );
}
