  ...
  return (
    <section style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <h2 style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', margin: 0 }}>⚙️ 운영</h2>
        <button
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 220, overflowY: 'auto' }}>
            {failed.slice(0, 12).map(f => (
              <div key={f.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '5px 8px', borderRadius: 6, background: 'rgba(0,0,0,0.18)' }}>
                <code style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)' }}>{f.name}</code>
                <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--accent-orange)', fontWeight: 'var(--fw-num)' }}>×{f.fail_count}</span>
                <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {f.last_error?.trim() || '—'}
                </span>
              </div>
            ))}
        ...
        >
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <strong style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-primary)' }}>🛠️ 일괄 최신화</strong>
              <button onClick={() => setGodOpen(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: 'var(--fs-sm)' }}>✕</button>
            </div>
            <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', margin: 0 }}>
              긴급 운영 액션 — 다음 단계에서 채워질 예정. 현재는 파이프라인 즉시 실행만 노출.
            </p>
            <a href="/api/admin/issues/run-pipeline" style={{ fontSize: 'var(--fs-xs)', color: 'var(--brand)', textDecoration: 'none' }}>
