...
export default function WatchlistWidget() {
  ...
  const healthBadge = useMemo<{ bg: string; fg: string; label: string }>(() => {
    ...
    if (orgToday === 0) return { bg: 'rgba(251,191,36,0.16)', fg: 'var(--accent-yellow)', label: '오늘 자연 등록 0' };
    if (Number.isFinite(orgD) && orgD < -30) return { bg: 'rgba(248,113,113,0.16)', fg: 'var(--accent-red)', label: `자연 등록 ${orgD.toFixed(0)}%` };
    if (Number.isFinite(orgD) && orgD < 0) return { bg: 'rgba(251,191,36,0.16)', fg: 'var(--accent-yellow)', label: `자연 등록 ${orgD.toFixed(0)}%` };
    return { bg: 'rgba(52,211,153,0.14)', fg: 'var(--accent-green)', label: 'OK' };
  ...
  return (
    ...
    >
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <h2 style={{ margin: 0, fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-primary)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
          <span aria-hidden style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)' }}>▸</span>
          🏠 관심 등록 현황
        </h2>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <button
            ...
            style={{
              fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-body)', padding: '6px 12px', borderRadius: 6,
      ...
      {data && (
        <>
          ...
          <div>
            <div style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-tertiary)', letterSpacing: 0, marginBottom: 6 }}>
              14D DAILY <span style={{ marginLeft: 6, color: 'var(--text-tertiary)', fontWeight: 'var(--fw-body)' }}>■ total</span>
              <span style={{ marginLeft: 4, color: 'var(--accent-green)', fontWeight: 'var(--fw-body)' }}>■ organic</span>
            </div>
...
function KpiTile({ label, value, delta, sub, highlight, muted }: { label: string; value: any; delta: any; sub?: string; highlight?: boolean; muted?: boolean }) {
  return (
    <div style={{
      ...
      {sub && (
        <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', marginTop: 3, lineHeight: 1.3 }}>
          {sub}
        </div>
      )}
    </div>
...
function DailyBars({ rows }: { rows: DailyRow[] }) {
  if (!rows || rows.length === 0) {
    return <div style={{ padding: 16, textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 'var(--fs-xs)' }}>14일 데이터 없음</div>;
  }
  // 날짜 ascending
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} preserveAspectRatio="none" style={{ display: 'block', minWidth: 320 }}>
            ...
            <g key={r.date ?? i}>
              ...
              {organic > 0 && (
                <rect x={x} y={yO} width={barW} height={Math.max(0, H - PADY - yO)}
                  style={{ fill: 'var(--accent-green)' }} opacity={0.92} />
              )}
            </g>
          );
        })}
        ...
        {sorted.length > 0 && (
          <>
            <text x={PADX} y={H - 1} fontSize={10} style={{ fill: 'var(--text-tertiary)' }}>{(sorted[0].date ?? '').slice(5)}</text>
            {sorted.length > 1 && (
              <text x={W - PADX} y={H - 1} fontSize={10} textAnchor="end" style={{ fill: 'var(--text-tertiary)' }}>
...
function TopColumn({ title, rows, emptyMsg }: { title: string; rows: TopRow[]; emptyMsg: string }) {
  return (
    <div>
      <div style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-tertiary)', letterSpacing: 0, marginBottom: 6 }}>{title}</div>
      {!rows || rows.length === 0 ? (
        <div style={{ padding: 12, fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', textAlign: 'center', background: 'var(--bg-base)', borderRadius: 6, border: '1px solid var(--border)' }}>{emptyMsg}</div>
      ) : (
          ...
          {rows.slice(0, 10).map((r, i) => {
            ...
            return (
              <div key={(r.item_id ?? r.name ?? '') + i} style={{
                ...
                <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: i < 3 ? 'var(--brand)' : 'var(--text-tertiary)', textAlign: 'right' }}>
                  {i + 1}
                </span>
                <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 'var(--fw-title)' }}>
                  {r.name ?? '—'}
                </span>
                <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-title)', color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>
                  {showSplit
                    ? <>총 {fmtNum(users)} <span style={{ color: 'var(--accent-green)', fontWeight: 'var(--fw-num)' }}>(자연 {fmtNum(organicUsers)})</span></>
...
function RecentList({ rows }: { rows: RecentRow[] }) {
  if (!rows || rows.length === 0) {
    return <div style={{ padding: 16, textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 'var(--fs-xs)' }}>최근 자연 등록 없음</div>;
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      {rows.slice(0, 10).map((r, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)',
              padding: '1px 6px', borderRadius: 999,
              background: isApt ? 'rgba(96,165,250,0.18)' : 'rgba(251,191,36,0.16)',
              color: isApt ? 'var(--brand)' : 'var(--accent-yellow)',
            }}>{isApt ? '단지' : '종목'}</span>
            <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 180 }}>
              {r.item_name ?? '—'}
            </span>
          </div>
        );
...
function SiteList({ rows }: { rows: SiteRow[] }) {
  if (!rows || rows.length === 0) {
    return <div style={{ padding: 12, textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 'var(--fs-xs)' }}>분양현장 등록 없음</div>;
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      {rows.slice(0, 5).map((r, i) => (
        <div key={(r.site_id ?? '') + i} style={{
          ...
          <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-tertiary)', textAlign: 'right' }}>{i + 1}</span>
          <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 'var(--fw-title)' }}>
            {r.site_name ?? '—'}
          </span>
          <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 'var(--fw-body)', color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>
            👤 {fmtNum(r.members)} {n(r.guests) > 0 && <span style={{ color: 'var(--accent-yellow)' }}>· 게스트 {fmtNum(r.guests)}</span>}
          </span>
          <span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-tertiary)', whiteSpace: 'nowrap' }}>
