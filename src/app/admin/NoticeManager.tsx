...
export default function NoticeManager() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 'var(--fs-md)', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 12 }}>📡 공지 전광판 관리</h2>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        ...
        <button onClick={handleSave} disabled={saving || !content.trim()}
          style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: saving ? 'var(--bg-hover)' : 'var(--brand)', color: saving ? 'var(--text-tertiary)' : 'var(--text-inverse)', fontSize: 'var(--fs-sm)', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>
          {saving ? '저장 중...' : '전광판 등록'}
        </button>
      </div>
      {preview && content.trim() && (
        <div style={{ marginTop: 12, borderRadius: 8, overflow: 'hidden', border: '1px solid #152240' }}>
          <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', padding: '4px 10px', background: 'var(--bg-hover)' }}>미리보기</div>
          <div style={{ background: '#050A18', height: 32, display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
      ...
      {result && <p style={{ marginTop: 8, fontSize: 'var(--fs-sm)', color: result.startsWith('✅') ? 'var(--success)' : 'var(--error)' }}>{result}</p>}

      {/* 공지 발송 내역 */}
      {history.length > 0 && (
        <div style={{ marginTop: 16, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 8 }}>📋 공지 내역</div>
          {history.map((n: any) => (
            <div key={n.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: 'var(--fs-sm)' }}>
              ...
              <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', flexShrink: 0 }}>{new Date(n.created_at).toLocaleDateString('ko-KR')}</span>
              <span style={{ fontSize: 'var(--fs-xs)', padding: '1px 6px', borderRadius: 8, flexShrink: 0, background: n.is_active ? 'var(--success-bg)' : 'transparent', color: n.is_active ? 'var(--success)' : 'var(--text-tertiary)', border: `1px solid ${n.is_active ? 'var(--success)' : 'var(--border)'}` }}>
                {n.is_active ? '활성' : '종료'}
              </span>
              {!n.is_active && (
                <button onClick={() => reactivate(n.id)} style={{ fontSize: 'var(--fs-xs)', padding: '2px 6px', borderRadius: 6, background: 'var(--bg-hover)', border: '1px solid var(--border)', color: 'var(--text-secondary)', cursor: 'pointer' }}>재활성</button>
              )}
            </div>
          ))}
        </div>
