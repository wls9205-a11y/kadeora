          ...
          <tbody>
            {loading ? (
              ...
              <tr key={p.id} style={{ background: i % 2 === 0 ? 'var(--bg-surface)' : 'var(--bg-hover)' }}>
                ...
                <td style={{ padding: '10px 12px', color: (p.content_length ?? 0) >= 1500 ? 'var(--accent-green)' : 'var(--accent-red)', fontSize: 'var(--fs-xs)', fontWeight: 700 }}>{(p.content_length ?? 0).toLocaleString()}</td>
                ...
                <td style={{ padding: '10px 12px' }}>
                  <button onClick={() => deletePost(p.id)} style={{ fontSize: 'var(--fs-xs)', color: 'var(--accent-red)', background: 'none', border: 'none', cursor: 'pointer' }}>삭제</button>
                </td>
              </tr>
            ))}
          </tbody>
