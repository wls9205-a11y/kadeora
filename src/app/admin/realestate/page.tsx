    ...
    <div style={card}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
        <div style={{ flex: 1, background: 'var(--bg-hover)', borderRadius: 10, padding: 14, textAlign: 'center' }}>
          <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)' }}>활성 미분양 데이터</div>
          <div style={{ fontSize: 'var(--fs-2xl)', fontWeight: 800, color: 'var(--brand)', margin: '4px 0' }}>{unsoldCount ?? '--'}건</div>
        </div>
      </div>
      <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-tertiary)', marginBottom: 12, lineHeight: 1.6 }}>
  ...
  return (
    <div style={card}>
      ...
      <button onClick={handleRefresh} disabled={refreshing} style={{
        ...
        fontSize: 'var(--fs-sm)', fontWeight: 700, cursor: refreshing ? 'not-allowed' : 'pointer',
      }}>
...
export default function AdminRealEstatePage() {
  return (
    <div>
      <h1 style={{ fontSize: 'var(--fs-xl)', fontWeight: 800, color: 'var(--text-primary)', marginBottom: 20 }}>🏠 부동산</h1>
