...
export default function PushBroadcast() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 'var(--fs-md)', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 12 }}>📣 푸시 알림 발송</h2>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 4 }}>
            ...
            <div style={{ background: '#1a1a2e', borderRadius: 12, padding: 12 }}>
              ...
              <div style={{ background: '#2a2a3e', borderRadius: 8, padding: '8px 10px', display: 'flex', gap: 8 }}>
                <div style={{ width: 28, height: 28, borderRadius: 6, background: 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><LogoSvg size={16} /></div>
            ...
            <div style={{ background: '#1c1c1e', borderRadius: 12, padding: 12 }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 10, padding: '8px 10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                  <div style={{ width: 22, height: 22, borderRadius: 5, background: 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><LogoSvg size={12} /></div>
            ...
            <div style={{ background: '#292a2d', borderRadius: 12, padding: 12 }}>
              ...
              <div style={{ width: 24, height: 24, borderRadius: 5, background: 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><LogoSvg size={14} /></div>
              <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ background: '#1e1e1e', borderRadius: 12, padding: 12 }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: 10, padding: '8px 10px', border: '1px solid rgba(255,255,255,0.1)' }}>
                ...
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 600, color: '#fff' }}>{title || '제목'}</div>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'rgba(255,255,255,0.5)', lineHeight: 1.3 }}>{body || '내용'}</div>
              </div>
            </div>
          </div>
        )}
        ...
        <button onClick={handleSend} disabled={sending || !title.trim() || !body.trim()}
          style={{ padding: '12px 0', background: 'var(--brand)', color: 'var(--text-inverse)', border: 'none', borderRadius: 8, fontSize: 'var(--fs-base)', fontWeight: 700, cursor: sending ? 'not-allowed' : 'pointer', opacity: sending ? 0.7 : 1 }}>
          {sending ? '발송 중...' : `🚀 ${sendTarget === 'all' ? '전체' : sendTarget === 'web' ? '웹' : '앱'} 발송`}
        </button>
        {result && <div style={{ fontSize: 'var(--fs-sm)', color: result.startsWith('✅') ? 'var(--success)' : 'var(--error)', padding: '4px 0' }}>{result}</div>}

        {/* 발송 내역 */}
        {logs.length > 0 && (
          <div style={{ marginTop: 16, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
            <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 8 }}>📊 발송 내역</div>
            {logs.map((l: any) => {
              const ctr = l.sent_count > 0 ? Math.round((l.click_count / l.sent_count) * 100) : 0;
              return (
                <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: 'var(--fs-sm)' }}>
                  <span style={{ flex: 1, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.title}</span>
                  <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', flexShrink: 0 }}>{new Date(l.created_at).toLocaleDateString('ko-KR')}</span>
                  <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', flexShrink: 0 }}>발송 {l.sent_count}</span>
                  <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-tertiary)', flexShrink: 0 }}>클릭 {l.click_count}</span>
                  <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, flexShrink: 0, color: ctr >= 10 ? 'var(--success)' : ctr >= 5 ? 'var(--warning)' : 'var(--text-tertiary)' }}>{ctr}%</span>
                </div>
              );
            })}
          </div>
