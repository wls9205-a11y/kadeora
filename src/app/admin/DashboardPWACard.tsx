  ...
  return (
    <div style={{ position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-primary)' }}>{total}건</div>
        <button
          onClick={() => setShowTooltip(p => !p)}
          style={{
            background: 'none', border: 'none', cursor: 'pointer',
            fontSize: 12, color: 'var(--text-tertiary)', padding: 0,
          }}
          title="측정 방법"
        >ⓘ</button>
      </div>
      ...
      {showTooltip && (
        <div style={{
          ...
          <button
            ...
            style={{ marginTop: 4, fontSize: 10, color: 'var(--text-tertiary)', background: 'none', border: 'none', cursor: 'pointer' }}
          >닫기</button>
        </div>
      )}
    </div>
