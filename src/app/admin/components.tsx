...
export function AdminBadge({ status }: { status: string }) {
  ...
  return <span style={{ fontSize: 'var(--fs-xs)', padding: '2px 8px', borderRadius: 10, fontWeight: 600, background: c.bg, color: c.color }}>{status}</span>;
}
...
export function ActionButton({ label, onClick, variant = 'default' }: { label: string; onClick: () => Promise<any>; variant?: 'default' | 'danger' | 'success' }) {
  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
      ...
      {result && <span style={{ fontSize: 'var(--fs-xs)', color: result.ok ? 'var(--success)' : 'var(--error)' }}>{result.msg}</span>}
    </div>
  );
}
