...
export default function GodModeSection() {
  ...
  const modes = [
    { key: 'full', label: '⚡ 전체 실행', desc: 'Phase 순차 — 58개 활성 크론', color: C.brand },
    { key: 'data', label: '📊 데이터 수집', desc: '청약/실거래/종목발굴/재개발', color: C.green },
    { key: 'process', label: '⚙️ 데이터 가공', desc: '집계/싱크/총세대수검증', color: C.cyan },
    { key: 'ai', label: '🤖 AI 생성', desc: 'AI요약/리라이트/Batch API', color: C.purple },
    { key: 'content', label: '📝 콘텐츠', desc: '블로그 AI생성/SEO/발행큐', color: C.yellow },
    { key: 'system', label: '🔧 시스템', desc: '정리/통계/알림/내부링크', color: C.textSec },
    ...
    <div style={{ animation: 'fadeIn .4s ease' }}>
      ...
      <div style={{ marginTop: 32, borderTop: `1px solid ${C.border}`, paddingTop: 24 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
          ...
          <button onClick={() => runCronSingle('/api/cron/apt-parse-pdf-pricing', '⭐ PDF 가격 파싱 200건 (최저/평균/최고/규제/시설)')}
            disabled={specialRunning}
