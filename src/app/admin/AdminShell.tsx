...
export default function AdminShell() {
  ...
  return (
    <div style={{maxWidth:640,margin:'0 auto',padding:'0 12px 80px',minHeight:'100vh'}}>
      <style>{`
        @keyframes spin{to{transform:rotate(360deg)}}
        .at{flex:1;padding:10px 0;text-align:center;font-size:11px;color:rgba(255,255,255,0.35);cursor:pointer;border:none;background:none;transition:all .15s}
        .at.on{color:#3B7BF6;font-weight:800}
        .at:hover{color:rgba(255,255,255,0.7)}
      `}</style>

      {/* 헤더 — top:0, 전역 네비 없음 */}
      <div style={{position:'sticky',top:0,zIndex:50,background:'rgba(5,10,24,0.97)',backdropFilter:'blur(16px)',padding:'10px 0 6px',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:30,height:30,borderRadius:'50%',border:`2px solid ${sc}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:900,color:sc,flexShrink:0}}>{hp?.s??'—'}</div>
          {hp && <>
            <span style={{fontSize:11,color:hp.cr>=95?'#10B981':'#F59E0B',fontWeight:600}}>크론 {hp.cr}%</span>
            <span style={{fontSize:10,color:'rgba(255,255,255,0.15)'}}>·</span>
            <span style={{fontSize:11,color:'rgba(255,255,255,0.4)'}}>PV {hp.pv}</span>
          </>}
          <div style={{flex:1}}/>
          <span style={{fontSize:9,color:'rgba(255,255,255,0.15)',letterSpacing:1}}>MISSION CONTROL</span>
        </div>
      </div>

      {/* 탭 바 — 헤더 바로 아래 고정 */}
      <div style={{display:'flex',position:'sticky',top:48,zIndex:49,background:'rgba(5,10,24,0.98)',padding:'2px 0 4px',marginBottom:10,borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
        {tabs.map(t=>(
          <button key={t} className={`at${tab===t?' on':''}`} onClick={()=>sw(t)}>
            <div style={{fontSize:15,marginBottom:2,opacity:tab===t?1:0.4}}>{icons[t]}</div>
            {labels[t]}
          </button>
        ))}
      </div>
