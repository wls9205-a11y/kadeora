...
// ─────────────────────────────────────────────────────────────────────────────
// 1-5. 자동차 보험료 — 시장값 공개형 (A1, 2026-09-17)
//
// ⛔ 개인 보험료는 보험사 언더라이팅 값이다. 법정 요율도, 긁을 공개 소스도 없다.
//    그래서 계산기는 «추정·예시» 만 낸다. 계수(차량가 3.5% · 연령·연식·무사고 배수)는 카더라의 가정이지
//    어느 보험사의 요율도 아니다 — 화면에 그렇게 쓴다.
//    대표값으로 전국 평균 하나를 병기한다(개인차가 크다는 사실을 가리지 않기 위해 «범위» 는 지어내지 않는다).
// ─────────────────────────────────────────────────────────────────────────────
export const CAR_INSURANCE_SOURCE = {
  /** 1대당 평균 보험료(원) — 개인용, 2025년 가입분. */
  avgPremium: 680_000,
  basis: '보험개발원 「2025년 개인용 자동차보험 가입현황 분석」',
  /** ⚠️ 원문 PDF 대조 전 — 언론 보도(2026-04-21) 인용. 「그렇게 말합니다」 단계. */
  verification: 'secondary' as 'secondary' | 'primary',
  reportedAt: '2026-04-21',
  quoteUrl: 'https://e-insmarket.or.kr',
  quoteName: '보험다모아(손해보험협회·생명보험협회 온라인 보험슈퍼마켓)',
} as const;
