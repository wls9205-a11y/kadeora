...
export const CALC_REGISTRY: CalcMeta[] = [
  ...
  },
  {
    ...
    description: '전세보증금을 월세로, 월세를 전세로 환산. 전환율을 비우면 법정 상한(min(10%, 기준금리+2%p))을 적용.',
    ...
    inputs: [
      ...
      { id: 'jeonse', label: '전세보증금', type: 'currency', default: 300000000, unit: '원', condition: 'direction=toWolse' },
      { id: 'deposit', label: '월세 보증금', type: 'currency', default: 50000000, unit: '원' },
      { id: 'monthlyRent', label: '월세', type: 'currency', default: 1000000, unit: '원', condition: 'direction=toJeonse' },
      { id: 'rate', label: '전월세전환율 (%)', type: 'percent', default: 0, min: 0, max: 10, step: 0.1, hint: '0 이면 법정 상한 전환율(연 10%와 한국은행 기준금리+2%p 중 낮은 쪽)을 쓴다' },
    ],
    faqs: [
    ...
    seoContent: '<h2>전월세 전환 계산기 완벽 가이드</h2><p>전세보증금을 월세로, 월세를 전세로 전환 시 적정 금액을 계산. 전월세전환율 자동 적용.</p><p>본 계산기는 <strong>주택임대차보호법 제7조의2</strong>를 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>전월세 전환 계산기 계산 방식</h2><p>전세 → 월세 는 «(전세보증금 − 월세 보증금) × 전월세전환율 ÷ 12» 로 월세를 구합니다. 월세 → 전세 는 «보증금 + 월세 × 12 ÷ 전환율» 로 전세보증금을 구합니다. 전환율을 0 으로 두면 법정 상한 전환율 — 주택임대차보호법 시행령 제9조의 연 10% 와 «한국은행 기준금리 + 연 2%» 중 낮은 비율 — 을 쓰고, 결과 화면에 기준금리 변경일과 함께 표시합니다. 직접 입력한 전환율이 상한을 넘으면 경고합니다. 이 상한은 기존 계약의 보증금을 월세로 바꿀 때 적용되며, 새 계약이나 월세 → 전세 전환에는 적용되지 않습니다.</p><h2>이런 분들에게 추천</h2><p>전세를 월세로(또는 반대로) 바꾸는 재계약을 앞두고 적정 금액을 가늠하려는 분에게 유용합니다.</p>', relatedCalcs: ['brokerage-fee', 'rental-yield', 'jeonse-vs-wolse'],
  ...
  },
  {
    ...
    description: '투자금·배당수익률로 세후 연간·월간 배당금을 계산 — 국내 배당은 소득세·지방소득세 성분 표시.',
    keywords: ['배당금 계산기', '배당수익률', '세후 배당금', '배당소득세', '배당 원천징수'],
    legalBasis: '소득세법 제129조제1항제2호나목 · 지방세법 제103조의13', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'dividendCalc', resultLabel: '연간 세후 배당금', resultUnit: '원',
    inputs: [{ id: 'investment', label: '투자금', type: 'currency', default: 50000000 }, { id: 'yieldRate', label: '배당수익률 (%)', type: 'percent', default: 4, min: 0, max: 20, step: 0.1 }, { id: 'market', label: '시장', type: 'radio', default: 'kr', options: [{ value: 'kr', label: '국내' }, { value: 'us', label: '미국 (조약 원천세 — 미확정 공개)' }] }],
    faqs: [
      { q: '배당금 계산기에서 세금은 어떻게 반영되나요?', a: '국내 배당은 배당소득 원천징수세율(소득세)과 그 10%인 지방소득세를 뺀다. 미국 배당은 미국에서 떼는 조약 제한세율 15%로 계산하고 국내 추가 원천징수는 없다 — 조약 원문 대조 전이라 화면에 «미확정» 으로 적는다.' },
      { q: '매매 수수료나 증권거래세도 빠지나요?', a: '아니다. 이 계산기는 배당에 붙는 세금만 뺀다. 매매 비용·주가 변동은 넣지 않는다.' },
      { q: '배당을 재투자하면요?', a: '재투자 복리는 배당 재투자 시뮬레이터에서 계산한다.' },
      { q: '배당이 많으면?', a: '이자·배당 합계가 종합과세기준금액을 넘으면 종합과세 대상이다 — 금융소득종합과세 계산기.' },
    ], seoContent: '<h2>세후 배당금 계산</h2><p>투자금 × 배당수익률 = 세전 배당금. 국내는 소득세와 지방소득세를 성분별로 빼고 12로 나눠 월 배당금을 보여 준다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['drip-sim', 'dividend-income-tax', 'financial-income-tax'],
  ...
  },
  {
    ...
    description: '프리랜서(인적용역 사업소득) 원천징수 — 세전↔세후 역산, 소득세·지방소득세 성분 표시.',
    keywords: ['3.3% 계산기', '원천징수 역산', '프리랜서 세금', '세전 세후', '사업소득 원천징수'],
    legalBasis: '소득세법 제129조제1항제3호 · 지방세법 제103조의13 · 시행령 제149조의3', version: '2026.09', lastUpdated: '2026-09-17',
    ...
    faqs: [
      { q: '3.3%는 무엇인가요?', a: '인적용역 사업소득 원천징수세율(소득세)에 그 10%인 지방소득세를 더한 합계다(소득세법 제129조제1항제3호 · 지방세법 제103조의13). 계산기는 두 성분을 따로 보여 준다.' },
      { q: '세후 금액으로 세전을 구할 수 있나요?', a: '«세후 → 세전» 을 고르면 세후 금액 ÷ (1 − 합계율)로 역산한다.' },
      { q: '작은 금액도 떼나요?', a: '뗀다. 원천징수세액 1천원 미만 소액부징수는 계속·반복 인적용역 사업소득에는 적용되지 않는다(시행령 제149조의3).' },
      { q: '종합소득세 환급액도 나오나요?', a: '아니다. 3.3%는 미리 떼는 세금이고 이듬해 5월 종합소득세 신고에서 정산한다. 정산 세액은 종합소득세 계산기에서 계산한다.' },
    ], seoContent: '<h2>3.3% 원천징수 계산</h2><p>세전 지급액 × 사업소득 원천징수세율 = 소득세, 소득세 × 10% = 지방소득세. 역산은 합계율로 나눈다. 원 단위로 반올림해 몇 원 차이가 날 수 있다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['comprehensive-income-tax', 'net-salary'],
  ...
  },
  {
    ...
    inputs: [
      ...
      { id: 'relationship', label: '수증자 관계', type: 'select', default: 'adultChild', options: [
        { value: 'spouse', label: '배우자로부터' },
        { value: 'adultChild', label: '부모 등 직계존속으로부터 (성년)' },
        { value: 'minorChild', label: '부모 등 직계존속으로부터 (미성년)' },
        { value: 'grandchild', label: '조부모로부터 — 손자녀 (세대생략 할증)' },
        { value: 'descendant', label: '자녀 등 직계비속으로부터' },
        { value: 'otherRelative', label: '4촌 이내 혈족·3촌 이내 인척' },
        { value: 'other', label: '그 밖의 사람 (친족 아님)' },
      ]},
      { id: 'grandchildMinor', label: '손자녀가 미성년자인가요?', type: 'radio', default: 'no', options: [{ value: 'no', label: '성년' }, { value: 'yes', label: '미성년' }], condition: 'relationship=grandchild' },
      { id: 'parentDeceased', label: '손자녀의 부모(증여자의 자녀)가 사망했나요?', type: 'radio', default: 'no', options: [{ value: 'no', label: '아니오' }, { value: 'yes', label: '예 (할증 없음)' }], condition: 'relationship=grandchild' },
      { id: 'priorGifts', label: '10년 내 같은 사람에게 받은 증여액', type: 'currency', default: 0, hint: '합산해 공제 한도·세율을 다시 계산한다. 그때 낸 증여세 공제(§58)는 반영하지 않는다' },
    ],
    faqs: [
      { q: '증여재산공제 한도는?', a: '상속세 및 증여세법 제53조 — 배우자 6억원, 직계존속(부모·조부모)으로부터 성년 5천만원·미성년 2천만원, 직계비속으로부터 5천만원, 4촌 이내 혈족·3촌 이내 인척 1천만원. 수증자 기준 10년 합산 한도입니다. 계산에 쓰는 값은 결과 화면의 근거와 함께 표시됩니다.' },
      { q: '손자녀에게 증여하면 공제가 없나요?', a: '아니요. 조부모도 직계존속이라 성년 5천만원·미성년 2천만원 공제가 있습니다. 대신 산출세액에 30%(미성년이면서 증여재산가액 20억원 초과면 40%)를 더합니다(§57). 손자녀의 부모가 사망한 경우는 할증하지 않습니다.' },
      { q: '결과 금액은 신고세액공제 후인가요?', a: '네. 기한 내 신고 시 (산출세액 + 할증)의 3%를 빼는 신고세액공제(§69)를 반영한 납부 예상액입니다.' },
    ],
    seoContent: '<h2>증여세란?</h2>\n<p>증여세는 타인으로부터 재산을 무상으로 받을 때 수증자(받는 사람)에게 부과되는 세금입니다. 증여받은 재산가액에서 증여재산공제를 차감한 과세표준에 10~50%의 누진세율을 적용합니다.</p>\n<h2>2026년 증여세율</h2>\n<p>1억원 이하 10%, 1~5억원 20%(누진공제 1,000만원), 5~10억원 30%(누진공제 6,000만원), 10~30억원 40%(누진공제 1억6,000만원), 30억원 초과 50%(누진공제 4억6,000만원).</p>\n<h3>증여재산공제 (수증자 기준 10년 합산, 상증법 제53조)</h3>\n<p>배우자로부터: 6억원. 직계존속(부모·조부모)으로부터: 성년 5,000만원, 미성년 2,000만원. 직계비속으로부터: 5,000만원. 4촌 이내 혈족·3촌 이내 인척으로부터: 1,000만원. 그 밖의 사람에게 받으면 공제가 없습니다. 증여 전 10년 안에 받은 공제와 합산한 한도입니다.</p>\n<h3>세대생략 할증 (제57조)</h3>\n<p>조부모가 손자녀에게 직접 증여하면 손자녀도 직계존속 공제를 받되, 산출세액에 30%(미성년이면서 증여재산가액 20억원 초과면 40%)를 더합니다. 손자녀의 부모가 사망한 경우는 할증하지 않습니다.</p>\n<h3>계산기 결과</h3>\n<p>과세표준에 세율을 적용한 산출세액에 할증을 더하고, 기한 내 신고 시 3% 신고세액공제(제69조)를 뺀 금액을 보여줍니다. 10년 내 합산 증여에 이미 낸 세금 공제(제58조)·혼인·출산 공제(제53조의2)는 반영하지 않습니다.</p>\n<h2>증여세 절세 전략</h2>\n<p>10년 주기 분할 증여가 가장 효과적입니다. 자녀에게 10년마다 5,000만원씩 증여하면 비과세입니다. 미성년 시기부터 2,000만원씩 시작하고, 성년 후 5,000만원으로 확대하면 수십 년간 수억원을 비과세로 이전할 수 있습니다. 부동산 증여 시 시가가 아닌 기준시가(공시가격)로 평가하면 증여세를 줄일 수 있습니다.</p>', relatedCalcs: ['inheritance-tax', 'generation-skip', 'acquisition-tax'],
  },
  {
  ...
  },
  {
    ...
    description: '이자·배당 합계가 종합과세기준금액을 넘을 때 더 낼 소득세·지방소득세를 계산. Gross-up·배당세액공제·비교과세 반영.',
    keywords: ['금융소득종합과세','이자소득','배당소득','2000만원','종합과세'],
    legalBasis: '소득세법 제14조·제17조·제56조·제62조 · 시행령 제116조의2', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'tax-bracket', formula: 'financialIncomeTax', resultLabel: '추가 납부세액', resultUnit: '원',
    inputs: [{ id: 'interest', label: '연간 이자소득 (총수입금액)', type: 'currency', default: 15000000 }, { id: 'dividend', label: '연간 배당 — Gross-up 대상', type: 'currency', default: 10000000, hint: '법인세가 과세된 이익의 배당 — 국내 법인 현금배당 대부분' }, { id: 'otherDividend', label: '연간 배당 — Gross-up 비대상', type: 'currency', default: 0, hint: '해외주식 배당·ETF 분배금·리츠 배당 등' }, { id: 'otherIncome', label: '다른 종합소득 과세표준', type: 'currency', default: 50000000, hint: '근로·사업소득에서 소득공제까지 뺀 금액. 없으면 0' }],
    faqs: [
      { q: '종합과세 대상인지 어떻게 판정하나요?', a: '이자와 배당의 «총수입금액» 합계가 종합과세기준금액을 넘는지로 본다. 배당 가산액(Gross-up)은 판정에 넣지 않는다(소득세법 제14조제4항). 기준금액은 결과 화면 근거 줄에 적힌 현행 값이다.' },
      { q: '추가 납부세액은 어떻게 계산하나요?', a: '소득세법 제62조대로 ① 기준금액 초과분과 다른 종합소득을 합쳐 기본세율 + 기준금액×원천징수세율, ② 금융소득×원천징수세율 + 다른 소득 산출세액 중 큰 쪽을 산출세액으로 한다. ①이면 배당세액공제를 빼고, 이미 원천징수된 세금과 다른 소득 산출세액을 뺀 차액을 보여 준다.' },
      { q: 'Gross-up 가산율은 얼마인가요?', a: '2026-09-17 원문 대조 기준 현행 10%다. 2027-01-01 이후 받는 배당부터 11%로 바뀌는 개정이 공포돼 있어 화면에 «시행예정» 으로 따로 적는다.' },
      { q: '계산하지 않는 것은?', a: '세액공제(표준·자녀 등), 비영업대금 이익, 외국납부세액공제는 넣지 않았다. 지방소득세는 추가 소득세의 10%로 근사했다.' },
    ], seoContent: '<h2>금융소득종합과세 계산 방식</h2><p>이자·배당 총수입금액 합계가 기준금액을 넘으면 다른 종합소득과 합산해 누진세율로 다시 계산하되, 원천징수만으로 끝냈을 때보다 적게 내지 않도록 두 산식 중 큰 금액을 적용한다(소득세법 제62조).</p><h2>배당 가산액과 배당세액공제</h2><p>법인세가 과세된 이익의 배당은 가산액만큼 과세표준을 늘리고, 기준금액을 넘는 배당분의 가산액은 세액에서 돌려준다. 이자 → Gross-up 비대상 배당 → 대상 배당 순으로 기준금액을 채운다(시행령 제116조의2).</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['dividend-calc', 'deposit-interest', 'overseas-cgt'],
  ...
  },
  {
    ...
    description: '마지막 생리 시작일과 주기로 출산 예정일(Naegele 법칙)과 현재 임신 주수를 계산.',
    keywords: ['출산 예정일 계산기', '임신 주수', '분만 예정일', '마지막 생리일'],
    legalBasis: 'Naegele 법칙', version: '2026.09', lastUpdated: '2026-09-17',
    ...
    faqs: [
      { q: '어떻게 계산하나요?', a: '마지막 생리 시작일에 280일을 더하고, 평균 주기가 28일과 다르면 그 차이만큼 보정합니다(Naegele 법칙).' },
      { q: '병원 예정일과 다른데요?', a: '초음파로 확정한 예정일이 우선입니다. 이 계산은 관례적 추정입니다.' },
    ],
    seoContent: '<h2>출산 예정일 계산기</h2><p>마지막 생리 시작일 + 280일(주기 보정)로 출산 예정일을 추정합니다. 초음파 검사로 확정한 날짜와 다를 수 있습니다.</p>', relatedCalcs: ['bmi', 'calorie'],
  },
  {
    slug: 'electricity', emoji: '⚡', category: 'life', categoryLabel: '생활/건강',
    title: '주택용 전기요금 계산기', titleShort: '전기요금 계산기',
    description: '월 사용량(kWh)과 계절로 주택용(저압) 누진 전기요금을 계산. 기본요금 구간·하계 누진구간·전력기금(전기사업법 시행령 §36) 반영.',
    keywords: ['전기요금 계산기', '전기세', '누진세', 'kWh', '한전 요금'],
    legalBasis: '한국전력 전기요금표(주택용 저압), 전기사업법 시행령 §36, 부가가치세법 §30', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'tax-bracket', formula: 'electricityBill', resultLabel: '전기요금(부가세·전력기금 포함)', resultUnit: '원',
    inputs: [
      { id: 'usage', label: '월 사용량(kWh)', type: 'number', default: 350, min: 0, max: 2000 },
      { id: 'season', label: '사용 월', type: 'radio', default: 'other', options: [{ value: 'other', label: '봄·가을(3~6월, 9~11월)' }, { value: 'summer', label: '하계(7~8월)' }, { value: 'winter', label: '동계(12~2월)' }] },
      { id: 'climateRate', label: '기후환경요금 단가(원/kWh)', type: 'number', default: 0, min: 0, step: 0.1, hint: '한전 공시값 확인 후 입력. 미입력 시 제외' },
      { id: 'fuelRate', label: '연료비조정요금 단가(원/kWh)', type: 'number', default: 0, min: -10, step: 0.1, hint: '분기별 공시. 미입력 시 제외' },
    ],
    faqs: [
      { q: '어떤 요금표를 쓰나요?', a: '한국전력 주택용(저압) 요금표의 기본요금·전력량요금을 씁니다. 요금표 적용일은 결과 화면에 표시되며, 한전이 요금을 바꾸면 달라집니다.' },
      { q: '여름에는 왜 다른가요?', a: '하계(7~8월)에는 누진 구간 경계가 넓어져 같은 사용량이라도 요금이 낮아질 수 있습니다. 하계·동계에는 1,000kWh 초과분에 별도 단가가 붙습니다.' },
      { q: '기후환경요금·연료비조정요금은 왜 직접 넣나요?', a: '분기마다 한전이 공시하는 값이라 확인 전 숫자를 넣지 않았습니다. 공시 단가를 입력하면 반영됩니다.' },
      { q: '전력기금은 얼마인가요?', a: '전기요금의 일정 비율로 전기사업법 시행령 §36에서 정합니다. 요율과 시행일은 결과 화면에 표시됩니다.' },
    ],
    seoContent: '<h2>주택용 전기요금 계산기</h2><p>월 사용량이 속한 구간의 기본요금과 구간별 전력량요금을 더하고, 입력한 기후환경·연료비조정 단가를 반영한 뒤 부가가치세와 전력산업기반기금을 더합니다. 하계에는 누진 구간 경계가 달라집니다.</p><p>일반용 등 다른 계약종별, 복지할인, 청구 절사 규칙은 반영하지 않았습니다.</p>', relatedCalcs: [],
  },
  {
    slug: 'discharge-date', emoji: '🪖', category: 'military', categoryLabel: '군대/교육',
    title: '전역일 계산기', titleShort: '전역일 계산기',
    description: '입대일과 군종으로 전역 예정일·남은 일수를 계산. 월말 입대일도 민법 §160 기간 계산으로 처리.',
    keywords: ['전역일 계산기', '군대 전역일', '복무 기간', '입대일', '군인 전역'],
    legalBasis: '병역법 §18·§19·§30', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'dischargeDate', resultLabel: '전역 예정일', resultUnit: '',
    inputs: [
      { id: 'enlistDate', label: '입대일', type: 'date', default: '' },
      { id: 'branch', label: '군종', type: 'select', default: 'army', options: [{ value: 'army', label: '육군(18개월)' }, { value: 'marine', label: '해병대(18개월)' }, { value: 'navy', label: '해군(20개월)' }, { value: 'airforce', label: '공군(21개월)' }, { value: 'social', label: '사회복무요원(21개월)' }] },
    ],
    faqs: [
      { q: '복무기간은 법에 몇 개월인가요?', a: '병역법 §18②은 육군 2년, 해군 2년 2개월(해병 2년), 공군 2년 3개월, §30①은 사회복무요원 2년 2개월로 정합니다. 선택지의 개월 수는 §19에 따른 단축 조치 후 실무에 알려진 기간이며, 단축 조치 문서 원문은 대조 전입니다.' },
      { q: '8월 31일처럼 월말에 입대하면?', a: '입영일을 첫날로 세고, 마지막 달에 같은 날짜가 없으면 그 달 말일을 전역 예정일로 계산합니다(민법 §160 준용).' },
    ],
    seoContent: '<h2>전역일 계산기</h2><p>입영일부터 군종별 복무개월 수를 세어 마지막 달의 해당일 전날을 전역 예정일로 계산합니다. 해당일이 없는 달이면 그 달 말일입니다.</p><p>복무기간 불산입, 전역 보류는 반영하지 않았습니다.</p>', relatedCalcs: ['military-pay'],
  },
  ...
  // ════════════════════════════════════════
  {
    ...
    description: '월 소득·가입기간으로 국민연금 노령연금을 국민연금법 §51(1천분의 1,290)·§63(가입기간별 비율) 산식으로 근사 계산. 2026년 보험료율 표시.',
    keywords: ['국민연금 계산기', '국민연금 수령액', '연금 수령 나이', '국민연금 납부액'],
    legalBasis: '국민연금법 §51·§63, 부칙 제4조', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'nationalPension', resultLabel: '예상 월 수령액(근사)', resultUnit: '원',
    inputs: [
      { id: 'monthlySalary', label: '월 소득(기준소득월액)', type: 'currency', default: 3500000, hint: '상·하한 적용' },
      { id: 'years', label: '가입기간(년)', type: 'range', default: 25, min: 1, max: 40 },
      { id: 'aValue', label: 'A값(전체 가입자 평균소득월액)', type: 'currency', default: 0, hint: '0이면 가정값 사용(2026년 값 미확정) — 국민연금공단 공시값을 넣으세요' },
    ],
    faqs: [
      { q: '계산식은?', a: '기본연금액 = (A값 + 본인 기준소득월액) × 1천분의 1,290이고, 20년 초과 1년마다 가산합니다(§51①). 가입 10~20년은 기본연금액의 일정 비율, 10년 미만은 노령연금이 없습니다(§63①).' },
      { q: 'A값은 무엇을 쓰나요?', a: '2026년 적용 A값을 원문으로 확인하지 못해, 입력이 없으면 이전 연도 값으로 알려진 가정값을 쓰고 화면에 그렇게 표시합니다.' },
      { q: '실제 수령액과 왜 다른가요?', a: '2025년 이전 가입기간의 종전 급여율, 재평가율, 물가 연동, 부양가족연금, 크레딧 등을 반영하지 않은 근사치입니다. 정확한 예상액은 국민연금공단에서 확인하세요.' },
    ],
    seoContent: '<h2>국민연금 예상 수령액 계산기</h2><p>국민연금법 §51의 기본연금액 산식과 §63의 가입기간별 노령연금 비율로 월 수령액을 근사합니다. 기준소득월액 상·하한과 2026년 보험료율(부칙 제4조)은 원문과 대조한 상수표에서 읽습니다.</p><p>2025년 이전 가입기간의 종전 급여율과 재평가율은 반영하지 않으므로 참고용 근사치입니다.</p>', relatedCalcs: ['fire-calc', 'irp-deduction', 'retirement-pay'],
  },
  {
  ...
  },
  {
    ...
    description: '연금저축·IRP 납입액으로 연금계좌세액공제(소득세법 §59의3) 소득세 공제액을 계산. 납입 한도·총급여별 공제율 반영.',
    keywords: ['연금저축 세액공제', 'IRP 세액공제', '연말정산', '세액공제 한도', '연금저축 환급'],
    legalBasis: '소득세법 §59의3', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'irpDeduction', resultLabel: '연금계좌세액공제(소득세)', resultUnit: '원',
    inputs: [
      { id: 'annualSalary', label: '총급여', type: 'currency', default: 50000000, hint: '근로소득만 있는 경우 기준' },
      { id: 'pensionSavings', label: '연금저축 납입액', type: 'currency', default: 6000000 },
      { id: 'irp', label: 'IRP(퇴직연금계좌) 납입액', type: 'currency', default: 3000000 },
    ],
    faqs: [
      { q: '공제율이 15%·12%인데 16.5%·13.2%라는 말은 뭔가요?', a: '15%·12%는 소득세 공제율(§59의3①)입니다. 16.5%·13.2%는 지방소득세 감소분까지 더한 실효율로 흔히 쓰는 표현이며, 이 계산기의 결과는 소득세 공제액만 보여 줍니다.' },
      { q: '납입 한도는?', a: '연금저축 한도와 연금저축+IRP 합산 한도가 따로 있습니다. 한도를 넘는 납입액은 공제 대상에서 빠집니다. 금액은 결과 화면 근거와 함께 표시됩니다.' },
      { q: '결과만큼 무조건 돌려받나요?', a: '아니요. 공제액이 산출세액보다 크면 넘는 부분은 환급되지 않습니다. ISA 만기 전환 추가 한도도 반영하지 않았습니다.' },
    ],
    seoContent: '<h2>연금저축·IRP 세액공제 계산기</h2><p>연금저축(한도 적용)과 IRP 납입액을 합산 한도 안에서 더한 뒤, 총급여 기준 공제율을 곱해 소득세 공제액을 계산합니다.</p><p>공제율·한도는 법령 원문과 대조한 상수표(policy_constants)에서 읽어 계산하며, 결과 화면에 근거 조문과 시행일을 함께 표시합니다.</p>', relatedCalcs: ['national-pension', 'year-end-refund', 'net-salary'],
  },
  ...
  },
  {
    ...
    description: '성별·나이·키·몸무게로 BMI 기반 체지방률을 추정(Deurenberg 1991 식). 체성분 측정이 아닌 추정치.',
    keywords: ['체지방률 계산기', '체지방 측정', '비만도', '체지방 비율'],
    legalBasis: 'Deurenberg(1991) BMI 기반 추정식', version: '2026.09', lastUpdated: '2026-09-17',
    ...
    faqs: [
      { q: '어떤 식을 쓰나요?', a: 'Deurenberg(1991) 식(1.2×BMI + 0.23×나이 − 성별 상수)입니다. 허리·목 둘레를 쓰는 미 해군 식은 지원하지 않습니다.' },
      { q: '정확한가요?', a: 'BMI로 추정한 값이라 근육량이 많거나 적은 사람은 실제와 크게 다를 수 있습니다. 등급 구간도 공인 기준표가 아닌 참고용입니다.' },
    ],
    seoContent: '<h2>체지방률 추정 계산기</h2><p>키와 몸무게로 BMI를 구한 뒤 Deurenberg 식으로 체지방률을 추정합니다. 체성분 분석기 측정값이 아니며 참고용입니다.</p>', relatedCalcs: ['bmi', 'calorie', 'bmr'],
  },
  {
  ...
  },
  {
    ...
    description: '생년월일로 만 나이와 다음 생일까지 남은 일수를 계산(민법 §158, 행정기본법 §16).',
    keywords: ['만 나이 계산기', '나이 계산', '한국 나이', '만 나이 통일'],
    legalBasis: '민법 §158, 행정기본법 §16', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'ageCalc', resultLabel: '만 나이', resultUnit: '세',
    inputs: [
      { id: 'birthDate', label: '생년월일', type: 'date', default: '1990-01-01' },
    ],
    faqs: [
      { q: '만 나이는 어떻게 세나요?', a: '출생일을 산입해 생일이 지날 때마다 한 살씩 더합니다(민법 §158).' },
      { q: '2월 29일생은?', a: '평년에는 3월 1일에 나이가 늘어나는 것으로 계산합니다.' },
    ],
    seoContent: '<h2>만 나이 계산기</h2><p>오늘 날짜와 생년월일을 비교해 생일이 지났는지에 따라 만 나이를 계산합니다. 법령·계약·공문서의 나이는 특별한 규정이 없으면 만 나이로 셉니다(행정기본법 §16).</p>', relatedCalcs: ['due-date', 'discharge-date'],
  },
  {
    slug: 'd-day', emoji: '📆', category: 'life', categoryLabel: '생활/건강',
    title: 'D-Day 날짜 계산기', titleShort: 'D-Day 계산기',
    description: '두 날짜 사이의 일수와 D-Day를 계산.',
    keywords: ['D-Day 계산기', '날짜 계산기', '디데이', '일수 계산'],
    legalBasis: '', version: '2026.09', lastUpdated: '2026-09-17',
    ...
    faqs: [
      { q: '시작일도 세나요?', a: '두 날짜의 차이(시작일 제외)로 셉니다. 시작일을 1일째로 세는 경우에는 1일을 더하세요.' },
    ],
    seoContent: '<h2>D-Day 날짜 계산기</h2><p>시작일과 종료일의 날짜 차이를 계산해 D-Day로 보여 줍니다. 시작일을 1일째로 세는 「사귄 날」 계산은 결과에 1일을 더하면 됩니다.</p>', relatedCalcs: ['age-calc', 'discharge-date'],
  },
  {
  ...
  },
  {
    ...
    description: '물품가격(달러)으로 소액물품 면세(관세법 시행규칙 §45②, 운임 제외 미화 150달러 이하)를 판정하고, 과세 시 관세·부가세를 계산.',
    keywords: ['해외직구 관세', '직구 세금', '관세 면제', '면세 한도', '해외구매 관세'],
    legalBasis: '관세법 §94, 관세법 시행규칙 §45, 부가가치세법 §29·§30', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'customsDuty', resultLabel: '관세+부가세 합계', resultUnit: '원',
    inputs: [
      { id: 'priceUsd', label: '물품가격(미화 달러)', type: 'number', default: 200, min: 0, step: 1, hint: '배송비 제외' },
      { id: 'shippingUsd', label: '배송비(미화 달러)', type: 'number', default: 20, min: 0, step: 1 },
      { id: 'exchangeRate', label: '과세환율(원/달러)', type: 'number', default: 0, min: 0, step: 0.01, hint: '관세청 고시 과세환율을 넣으세요. 면세 판정에는 필요 없음' },
      { id: 'category', label: '물품 종류(예시 세율)', type: 'select', default: 'general', options: [{ value: 'general', label: '일반(예시 8%)' }, { value: 'clothing', label: '의류(예시 13%)' }, { value: 'electronics', label: '전자제품(예시 4%)' }, { value: 'food', label: '식품(예시 8%)' }, { value: 'cosmetics', label: '화장품(예시 6.5%)' }] },
    ],
    faqs: [
      { q: '면세 기준에 배송비도 포함되나요?', a: '아니요. 면세 판정은 운임·보험료를 뺀 물품가격이 미화 150달러 이하인지로 합니다(관세법 시행규칙 §45②1호).' },
      { q: '미국은 200달러 아닌가요?', a: '미국발 목록통관 특례는 관세청 고시 사항이라 원문 대조 전이어서 계산에 반영하지 않았습니다.' },
      { q: '관세율이 정확한가요?', a: '품목별 관세율은 HS 코드로 정해집니다. 선택지의 율은 예시이며, FTA 원산지 세율이나 0% 품목일 수 있습니다.' },
    ],
    seoContent: '<h2>해외직구 관세 계산기</h2><p>배송비를 뺀 물품가격이 미화 150달러 이하인 자가사용물품은 관세가 면제됩니다. 넘으면 물품가격과 운임을 더한 과세가격에 관세를, 과세가격과 관세를 더한 금액에 부가세를 매깁니다.</p><p>같은 날 입항 물품 합산과세, 개별소비세 대상 품목, 미국발 특례는 반영하지 않았습니다.</p>', relatedCalcs: ['currency-convert'],
  },
  {
  ...
  // ════════════════════════════════════════
  {
    ...
    legalBasis: '공무원보수규정 별표 13', version: '2026.04', lastUpdated: '2026-04-05',
    ...
    faqs: [
      { q: '군인 월급 계산기 결과는 정확한가요?', a: '계산기에 설정된 계급별 봉급표 값을 보여주는 참고용 결과입니다. 봉급액은 공무원보수규정 별표 13(군인의 봉급표) 비고의 병 봉급이며, 결과 화면에 개정 기준일이 함께 나옵니다.' },
    ...
    seoContent: '<h2>군인 월급 계산기 완벽 가이드</h2><p>2026년 병사 계급별 월급을 확인. 이병~병장.</p><p>본 계산기는 <strong>공무원보수규정 별표 13(군인의 봉급표)</strong> 비고의 병 봉급을 보여줍니다. 규정은 매년 개정됩니다.</p><h2>군인 월급 계산기 계산 방식</h2><p>계급을 고르면 해당 계급의 월 봉급액(대통령령 원문 값)을 보여주며, 개정 기준일은 결과 화면에 나옵니다. 적금 매칭 지원금 등 봉급 외 지원은 들어 있지 않습니다.</p><h2>이런 분들에게 추천</h2><p>입대 예정자와 가족, 복무 중 월급으로 저축 계획을 세우려는 분에게 유용합니다. 본 계산기는 참고용입니다.</p>', relatedCalcs: ['discharge-date', 'net-salary'],
  },
  {
  ...
  },
  {
    ...
    description: '총급여·소득공제·세액공제를 넣으면 근로소득공제(소득세법 §47)·근로소득세액공제(§59)를 자동 반영해 예상 환급(추가납부) 소득세를 계산.',
    keywords: ['연말정산 계산기', '연말정산 환급', '13월의 월급', '세액공제', '소득공제'],
    legalBasis: '소득세법 §47·§55·§59', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'conditional', formula: 'yearEndRefund', resultLabel: '예상 환급액(소득세)', resultUnit: '원',
    inputs: [
      { id: 'annualSalary', label: '총급여', type: 'currency', default: 50000000 },
      { id: 'incomeDeduction', label: '소득공제 합계(근로소득공제 제외)', type: 'currency', default: 8000000, hint: '인적공제·국민연금·건강보험·신용카드·주택자금 등. 근로소득공제는 자동 계산' },
      { id: 'taxCredit', label: '그 밖의 세액공제 합계', type: 'currency', default: 2000000, hint: '자녀·연금계좌·특별세액공제(의료비·교육비·기부금·보험료)·월세 등. 근로소득세액공제는 자동 계산. 특별공제를 신청하지 않으면 표준세액공제' },
      { id: 'alreadyPaid', label: '기납부세액(원천징수 소득세 합계)', type: 'currency', default: 3000000, hint: '지방소득세 제외' },
    ],
    faqs: [
      { q: '근로소득공제는 어떻게 계산되나요?', a: '소득세법 §47①의 총급여 구간별 표(5구간, 공제 한도 2천만원)를 그대로 적용합니다. 직접 넣을 필요가 없습니다.' },
      { q: '근로소득세액공제도 자동인가요?', a: '네. 산출세액에 §59①의 산식을 적용하고, 총급여 구간별 한도(§59②)를 넘지 않게 자동 계산합니다.' },
      { q: '지방소득세도 포함되나요?', a: '아니요. 결과는 소득세 기준이며 개인지방소득세 환급·추가납부는 별도로 정산됩니다.' },
      { q: '결과가 실제와 다를 수 있나요?', a: '세액감면, 공제액의 산출세액 초과분 처리, 입력한 공제 합계의 정확도에 따라 달라집니다. 화면의 「미반영」 항목을 확인하세요.' },
    ],
    seoContent: '<h2>연말정산 예상 환급액 계산기</h2><p>총급여에서 근로소득공제(소득세법 §47)를 빼 근로소득금액을 구하고, 입력한 소득공제를 뺀 과세표준에 기본세율(§55)을 적용해 산출세액을 냅니다. 여기서 근로소득세액공제(§59)와 입력한 그 밖의 세액공제를 빼 결정세액을 구하고, 기납부세액과 비교해 환급 또는 추가납부액을 보여 줍니다.</p><h2>입력할 때 주의</h2><p>근로소득공제와 근로소득세액공제는 자동 계산되므로 입력 합계에 넣지 마세요. 결과는 소득세 기준이며 지방소득세는 별도입니다.</p>', relatedCalcs: ['net-salary', 'irp-deduction', 'credit-card-deduction'],
  },
  {
    slug: 'credit-card-deduction', emoji: '💳', category: 'year-end', categoryLabel: '연말정산',
    title: '신용카드 소득공제 계산기', titleShort: '신용카드 공제 계산기',
    description: '신용·체크·현금영수증·전통시장·대중교통·문화체육 사용액과 자녀 수로 신용카드 등 소득공제(조특법 §126의2)를 계산. 최저사용금액 차감 순서와 추가 한도 반영.',
    keywords: ['신용카드 소득공제', '체크카드 공제', '현금영수증 공제', '연말정산 카드'],
    legalBasis: '조세특례제한법 §126의2', version: '2026.09', lastUpdated: '2026-09-17',
    ...
    inputs: [
      ...
      { id: 'transit', label: '대중교통 이용액', type: 'currency', default: 0 },
      { id: 'culture', label: '도서·공연·박물관·영화 등 문화체육 사용액', type: 'currency', default: 0, hint: '총급여 7천만원 초과면 신용카드 사용분과 같은 율' },
      { id: 'childCount', label: '자녀 등(한도 가산 대상)', type: 'stepper', default: 0, min: 0, max: 2, hint: '2 = 2명 이상' },
    ],
    faqs: [
      { q: '최저사용금액(총급여 25%)은 어느 사용분에서 먼저 빠지나요?', a: '신용카드 사용분(낮은 공제율)부터 빠지고, 모자라면 체크카드·현금영수증, 그다음 전통시장·대중교통 사용분 순서입니다(§126의2②6호).' },
      { q: '공제 한도는 얼마인가요?', a: '총급여 7천만원 이하와 초과로 기본 한도가 갈리고, 자녀 등이 있으면 한도가 늘어납니다. 전통시장·대중교통(7천만원 이하는 문화체육 포함) 공제분은 기본 한도를 넘어도 별도 한도 안에서 추가로 공제됩니다. 금액은 결과 화면에 근거와 함께 표시됩니다.' },
      { q: '총급여 1억2천만원 초과 한도가 따로 있나요?', a: '현행 조문에는 없습니다. 7천만원 초과면 모두 같은 기본 한도를 적용합니다.' },
    ],
    seoContent: '<h2>신용카드 등 소득공제 계산기</h2><p>사용처별 공제율을 곱한 금액에서 최저사용금액(총급여의 25%)에 해당하는 공제분을 빼고, 기본 한도와 추가 한도를 적용합니다. 최저사용금액은 공제율이 낮은 신용카드 사용분부터 채웁니다.</p><p>공제율·한도는 법령 원문과 대조한 상수표(policy_constants)에서 읽어 계산하며, 결과 화면에 근거 조문과 시행일을 함께 표시합니다.</p>', relatedCalcs: ['year-end-refund', 'net-salary'],
  },
  {
    slug: 'monthly-rent-deduction', emoji: '🏠', category: 'year-end', categoryLabel: '연말정산',
    title: '월세 세액공제 계산기', titleShort: '월세 세액공제 계산기',
    description: '무주택 세대주 등의 월세 세액공제(조특법 §95의2)를 총급여 기준 공제율·연 한도로 계산.',
    keywords: ['월세 세액공제', '월세 공제', '연말정산 월세', '무주택 월세'],
    legalBasis: '조세특례제한법 §95의2', version: '2026.09', lastUpdated: '2026-09-17',
    ...
    faqs: [
      { q: '누가 받을 수 있나요?', a: '과세기간 종료일 현재 무주택 세대의 세대주 등으로서 총급여·종합소득금액 요건을 충족하고, 대통령령으로 정하는 주택을 임차한 근로자입니다. 계산기는 총급여만 묻습니다.' },
      { q: '공제율은 어떻게 갈리나요?', a: '총급여 기준(종합소득금액 요건 포함)으로 높은 율과 낮은 율이 갈립니다. 적용 율과 근거는 결과 화면에 표시됩니다.' },
      { q: '배우자도 추가로 받을 수 있나요?', a: '§95의2②에 요건을 갖춘 배우자 추가공제가 있으나 이 계산기에는 반영하지 않았습니다.' },
    ],
    seoContent: '<h2>월세 세액공제 계산기</h2><p>연간 월세액(연 한도 적용)에 총급여 구간별 공제율을 곱해 세액공제액을 계산합니다. 근거는 소득세법이 아니라 조세특례제한법 §95의2입니다.</p><p>공제율·한도는 법령 원문과 대조한 상수표(policy_constants)에서 읽어 계산하며, 결과 화면에 근거 조문과 시행일을 함께 표시합니다.</p>', relatedCalcs: ['year-end-refund', 'jeonse-vs-wolse'],
  },
  {
  ...
  { slug: 'retirement-income-tax', emoji: '🎫', category: 'income-tax', categoryLabel: '소득세', title: '퇴직소득세 계산기', titleShort: '퇴직소득세 계산기', description: '퇴직금에 대한 퇴직소득세·지방소득세를 근속연수공제·환산급여공제(현행 방식)로 계산.', keywords: ['퇴직소득세','퇴직금 세금','근속연수공제','퇴직소득 과세'], legalBasis: '소득세법 제48조·제55조제2항 · 지방세법 제92조제4항', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'tax-bracket', formula: 'retirementIncomeTax', resultLabel: '퇴직소득세 + 지방소득세', resultUnit: '원', inputs: [{ id: 'retirementPay', label: '퇴직금 총액 (퇴직소득금액)', type: 'currency', default: 100000000 }, { id: 'years', label: '근속연수', type: 'number', default: 10, min: 1, max: 45, hint: '1년 미만 기간은 1년으로 올린다' }], faqs: [
      { q: '퇴직소득세는 어떻게 계산하나요?', a: '퇴직소득금액에서 근속연수공제를 빼고 근속연수로 나눠 12를 곱한 «환산급여» 를 구한 뒤, 환산급여공제를 뺀 과세표준에 기본세율을 적용하고 12로 나눠 근속연수를 곱한다(소득세법 제48조·제55조제2항).' },
      { q: '지방소득세도 포함되나요?', a: '포함한다. 퇴직소득 지방소득세는 같은 순서로 국세의 1/10 세율을 적용한다(지방세법 제92조제4항).' },
      { q: '예전 계산식과 무엇이 다른가요?', a: '2015년 이전 방식(40% 정률공제 후 연분)은 현행법에 없다. 현행은 환산급여공제 단계가 있고 근속연수공제 금액도 다르다.' },
      { q: '계산하지 않는 것은?', a: '비과세 퇴직소득, 임원 퇴직금 한도, 중간정산 합산, IRP 이체에 따른 과세이연은 넣지 않았다.' },
    ], seoContent: '<h2>퇴직소득세 계산 순서</h2><p>근속연수공제 → 환산급여(÷근속연수×12) → 환산급여공제 → 기본세율 → ÷12×근속연수. 각 단계 금액을 결과 화면에 모두 적는다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['retirement-pay', 'irp-deduction'] },
  ...
  // 사업자 세금 추가
  {
    slug: 'simplified-vat', emoji: '🏪', category: 'biz-tax', categoryLabel: '사업자 세금',
    title: '간이과세자 부가세 계산기', titleShort: '간이과세 부가세',
    description: '간이과세자 부가세를 공급대가 × 업종별 부가가치율(시행령 §111②) × 10%에서 매입 공급대가 0.5% 공제를 빼 계산. 납부의무 면제 기준 반영.',
    keywords: ['간이과세자', '부가세', '업종별 부가가치율', '간이과세'],
    legalBasis: '부가가치세법 §63·§69, 같은 법 시행령 §111', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'simplifiedVat', resultLabel: '납부 부가세', resultUnit: '원',
    inputs: [
      { id: 'revenue', label: '과세기간 공급대가 합계(부가세 포함 매출)', type: 'currency', default: 30000000 },
      { id: 'industry', label: '업종', type: 'select', default: 'retail_food', options: [{ value: 'retail_food', label: '소매·음식점업 등' }, { value: 'mfg', label: '제조·농림어업·소화물 운송' }, { value: 'lodging', label: '숙박업' }, { value: 'construct_it', label: '건설·운수창고·정보통신' }, { value: 'finance_realty', label: '금융보험·전문과학기술·부동산 관련' }, { value: 'other_service', label: '그 밖의 서비스업' }] },
      { id: 'purchaseAmount', label: '세금계산서 등을 받은 매입액(공급대가)', type: 'currency', default: 5500000, hint: '매입세액이 아니라 부가세 포함 매입 금액' },
    ],
    faqs: [
      { q: '업종별 부가가치율은 얼마인가요?', a: '부가가치세법 시행령 §111②의 표대로 업종에 따라 다르며, 선택한 업종의 율이 결과 화면에 근거와 함께 표시됩니다.' },
      { q: '매입 공제는 어떻게 계산하나요?', a: '매입세액이 아니라 세금계산서 등을 받은 매입 «공급대가»에 0.5%를 곱합니다(§63③1호). 공제액이 납부세액을 넘는 부분은 없는 것으로 봅니다.' },
      { q: '납부 면제 기준은?', a: '해당 과세기간 공급대가 합계가 기준 금액 «미만»이면 납부의무가 면제됩니다(§69①). 신고 의무는 남습니다.' },
    ],
    seoContent: '<h2>간이과세자 부가세 계산기</h2><p>간이과세자의 납부세액은 공급대가 × 업종별 부가가치율 × 10%입니다. 세금계산서 등을 받은 매입 공급대가의 0.5%를 공제하고, 공급대가 합계가 면제 기준 미만이면 납부의무가 면제됩니다.</p><p>율과 기준 금액은 법령 원문과 대조한 상수표에서 읽어 계산하며 결과 화면에 근거를 표시합니다.</p>', relatedCalcs: ['vat-calc', 'comprehensive-income-tax'],
  },
  ...
  { slug: 'penalty-tax', emoji: '⚠️', category: 'biz-tax', categoryLabel: '사업자 세금', title: '가산세 계산기', titleShort: '가산세 계산기', description: '무신고·과소신고(부정행위 포함)·납부지연 가산세와 기한 후 신고·수정신고 감면을 계산.', keywords: ['가산세 계산기','무신고 가산세','과소신고','납부불성실'], legalBasis: '국세기본법 제47조의2·제47조의3·제47조의4·제48조 · 시행령 제27조의4', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'penaltyTax', resultLabel: '가산세', resultUnit: '원', inputs: [{ id: 'type', label: '가산세 종류', type: 'radio', default: 'noFiling', options: [{ value: 'noFiling', label: '무신고' }, { value: 'underReport', label: '과소신고' }, { value: 'latePay', label: '납부지연' }] }, { id: 'taxAmount', label: '해당 세액', type: 'currency', default: 5000000, hint: '무신고·과소신고한 납부세액 또는 미납 세액' }, { id: 'fraud', label: '부정행위 여부 (무신고·과소신고)', type: 'radio', default: 'no', options: [{ value: 'no', label: '일반' }, { value: 'yes', label: '부정행위' }] }, { id: 'monthsLate', label: '기한 후 신고·수정신고까지 개월 수 (무신고·과소신고)', type: 'number', default: 0, min: 0, max: 36, hint: '0 = 감면 없음. 법정신고기한 후 몇 개월 안에 신고했나' }, { id: 'days', label: '법정납부기한 다음 날부터 납부(고지)일 전날까지 일수', type: 'number', default: 30, min: 0, max: 3650, condition: 'type=latePay' }, { id: 'noticed', label: '고지 후 지정납부기한까지도 안 냈나', type: 'radio', default: 'no', condition: 'type=latePay', options: [{ value: 'no', label: '아니오' }, { value: 'yes', label: '예' }] }, { id: 'monthsAfterDue', label: '지정납부기한 후 납부까지 개월 수', type: 'number', default: 1, min: 0, max: 60, condition: 'noticed=yes' }], faqs: [
      { q: '무신고·과소신고 가산세율은?', a: '일반과 부정행위로 나뉜다(국세기본법 제47조의2·제47조의3). 현행 세율은 결과 화면 근거 줄에 적는다. 역외거래 부정행위와 복식부기의무자 수입금액 기준 비교는 계산하지 않는다.' },
      { q: '늦게라도 신고하면 줄어드나요?', a: '기한 후 신고는 법정신고기한 후 6개월 안, 수정신고는 2년 안이면 경과 기간에 따라 가산세를 감면한다(제48조제2항). 세무서가 경정할 것을 미리 알고 한 신고는 제외다.' },
      { q: '납부지연가산세는?', a: '법정납부기한 다음 날부터 납부(고지)일 전날까지 1일 이자율, 고지 후 지정납부기한까지 안 내면 3%, 2026-07-01 이후 지정납부기한이 지난 분은 그 뒤 경과 개월마다 월 이자율이 붙는다(제47조의4 · 시행령 제27조의4). 고지서별·세목별 150만원 미만이면 월 가산은 없다.' },
    ], seoContent: '<h2>가산세 계산</h2><p>신고 의무 위반(무신고·과소신고)과 납부 의무 위반(납부지연)은 따로 계산해 더한다. 기한 후 신고·수정신고 감면은 신고 관련 가산세에만 적용된다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['comprehensive-income-tax', 'vat-calc'] },
  ...
  // 연말정산 추가
  {
    slug: 'medical-deduction', emoji: '🏥', category: 'year-end', categoryLabel: '연말정산',
    title: '의료비 세액공제 계산기', titleShort: '의료비 공제 계산기',
    description: '일반·본인/65세 이상 등·미숙아·난임 의료비를 나눠 넣으면 총급여 3% 문턱 미달분 차감까지 반영해 의료비 세액공제(소득세법 §59의4②)를 계산.',
    keywords: ['의료비 세액공제', '의료비 공제', '연말정산 의료비'],
    legalBasis: '소득세법 §59의4②', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'medicalDeduction', resultLabel: '의료비 세액공제', resultUnit: '원',
    inputs: [
      { id: 'annualSalary', label: '총급여', type: 'currency', default: 50000000 },
      { id: 'generalMedical', label: '일반 의료비(부양가족)', type: 'currency', default: 3000000, hint: '아래 항목에 해당하지 않는 기본공제대상자 의료비' },
      { id: 'specificMedical', label: '본인·6세 이하·65세 이상·장애인·중증질환자 등 의료비', type: 'currency', default: 0 },
      { id: 'pretermMedical', label: '미숙아·선천성이상아 의료비', type: 'currency', default: 0 },
      { id: 'ivfMedical', label: '난임시술비', type: 'currency', default: 0 },
    ],
    faqs: [
      { q: '65세 이상 의료비는 문턱 없이 전액 공제되나요?', a: '한도는 없지만 문턱은 남습니다. 일반 의료비가 총급여 3%에 못 미치면 그 부족분을 본인·65세 이상 등 의료비에서 먼저 뺍니다(§59의4②2호 단서).' },
      { q: '공제율이 다른 의료비가 있나요?', a: '미숙아·선천성이상아 의료비와 난임시술비는 일반보다 높은 공제율이 적용됩니다. 적용 율은 결과 화면에 표시됩니다.' },
      { q: '실손보험으로 받은 돈은?', a: '공제 대상에서 빼야 하지만 이 계산기는 입력값을 그대로 씁니다. 수령액을 뺀 금액을 넣으세요.' },
    ],
    seoContent: '<h2>의료비 세액공제 계산기</h2><p>일반 의료비는 총급여 3% 초과분(연 한도 적용)만, 본인·6세 이하·65세 이상·장애인 등 의료비는 한도 없이 공제 대상이 됩니다. 문턱에 못 미친 부분은 2호→3호→4호 의료비에서 차례로 뺍니다.</p><p>공제율·한도는 법령 원문과 대조한 상수표(policy_constants)에서 읽어 계산하며, 결과 화면에 근거 조문과 시행일을 함께 표시합니다.</p>', relatedCalcs: ['year-end-refund'],
  },
  {
    slug: 'education-deduction', emoji: '🎓', category: 'year-end', categoryLabel: '연말정산',
    title: '교육비 세액공제 계산기', titleShort: '교육비 공제 계산기',
    description: '본인·취학 전 및 초중고·대학생 교육비를 나눠 1명당 한도를 반영해 교육비 세액공제(소득세법 §59의4③)를 계산.',
    keywords: ['교육비 세액공제', '등록금 공제', '연말정산 교육비'],
    legalBasis: '소득세법 §59의4③', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'educationDeduction', resultLabel: '교육비 세액공제', resultUnit: '원',
    inputs: [
      { id: 'selfEdu', label: '본인 교육비', type: 'currency', default: 0, hint: '대학원 포함, 한도 없음' },
      { id: 'schoolEdu', label: '취학 전·초중고 교육비 합계', type: 'currency', default: 3000000 },
      { id: 'schoolCount', label: '취학 전·초중고 인원', type: 'stepper', default: 1, min: 0, max: 5 },
      { id: 'univEdu', label: '대학생(부양가족) 교육비 합계', type: 'currency', default: 0, hint: '대학원 제외' },
      { id: 'univCount', label: '대학생 인원', type: 'stepper', default: 0, min: 0, max: 5 },
    ],
    faqs: [
      { q: '대학생 자녀 한도가 초중고와 같나요?', a: '다릅니다. 대학생 1명당 한도가 취학 전·초중고보다 큽니다(§59의4③1호). 한도 금액은 결과 화면에 근거와 함께 표시됩니다.' },
      { q: '인원이 여러 명이면?', a: '1명당 한도 × 인원수로 합산 한도를 잡습니다. 지출이 한 명에게 몰려 있으면 실제보다 크게 잡힐 수 있습니다.' },
      { q: '빠진 항목은?', a: '장애인 특수교육비(한도 없음), 비과세 학자금 차감, 공제액의 산출세액 초과분은 반영하지 않았습니다.' },
    ],
    seoContent: '<h2>교육비 세액공제 계산기</h2><p>본인 교육비는 한도 없이, 부양가족 교육비는 대학생과 취학 전·초중고생의 1명당 한도 안에서 공제 대상이 되며, 합계에 공제율을 곱합니다.</p><p>공제율·한도는 법령 원문과 대조한 상수표(policy_constants)에서 읽어 계산하며, 결과 화면에 근거 조문과 시행일을 함께 표시합니다.</p>', relatedCalcs: ['year-end-refund'],
  },

  {
    slug: 'donation-deduction', emoji: '❤️', category: 'year-end', categoryLabel: '연말정산',
    title: '기부금 세액공제 계산기', titleShort: '기부금 공제 계산기',
    description: '특례·일반·종교단체 기부금을 소득금액 기준 한도 안에서 인정해 기부금 세액공제(소득세법 §59의4④)를 계산.',
    keywords: ['기부금 세액공제', '기부금 공제', '종교 기부금', '연말정산 기부금'],
    legalBasis: '소득세법 §59의4④', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'donationDeduction', resultLabel: '기부금 세액공제', resultUnit: '원',
    inputs: [
      { id: 'annualSalary', label: '총급여', type: 'currency', default: 50000000, hint: '근로소득만 있다고 보고 소득금액을 계산' },
      { id: 'specialDonation', label: '특례기부금(옛 법정기부금)', type: 'currency', default: 0 },
      { id: 'generalDonation', label: '일반기부금(종교단체 외, 옛 지정기부금)', type: 'currency', default: 500000 },
      { id: 'religiousDonation', label: '종교단체 기부금', type: 'currency', default: 1000000 },
    ],
    faqs: [
      { q: '기부금은 전액 공제되나요?', a: '아니요. 일반기부금은 소득금액의 일정 비율까지만 인정되고, 종교단체 기부금이 있으면 한도 계산이 달라집니다. 넘는 금액은 이월 대상이며 이 계산기에는 반영하지 않았습니다.' },
      { q: '공제율은?', a: '인정된 기부금 합계에서 일정 금액까지는 기본 율, 넘는 부분은 높은 율입니다. 금액과 율은 결과 화면에 표시됩니다.' },
      { q: '정치자금·고향사랑기부금은?', a: '조세특례제한법의 별도 공제라 이 계산기에는 포함되지 않습니다.' },
    ],
    seoContent: '<h2>기부금 세액공제 계산기</h2><p>총급여에서 근로소득공제를 뺀 근로소득금액을 소득금액으로 보고, 특례기부금을 먼저 인정한 뒤 남은 소득금액 기준으로 일반기부금 한도를 적용합니다.</p><p>공제율·한도는 법령 원문과 대조한 상수표(policy_constants)에서 읽어 계산하며, 결과 화면에 근거 조문과 시행일을 함께 표시합니다.</p>', relatedCalcs: ['year-end-refund'],
  },
  {
    slug: 'insurance-deduction', emoji: '🛡️', category: 'year-end', categoryLabel: '연말정산',
    title: '보험료 세액공제 계산기', titleShort: '보험료 공제 계산기',
    description: '보장성보험료와 장애인전용보장성보험료를 각각 한도·공제율로 계산하는 보험료 세액공제(소득세법 §59의4①) 계산기.',
    keywords: ['보험료 세액공제', '보장성보험 공제', '연말정산 보험'],
    legalBasis: '소득세법 §59의4①', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'insuranceDeduction', resultLabel: '보험료 세액공제', resultUnit: '원',
    inputs: [
      { id: 'premium', label: '보장성보험료 합계', type: 'currency', default: 1200000 },
      { id: 'disabilityPremium', label: '장애인전용보장성보험료', type: 'currency', default: 0, hint: '일반 보장성과 한도 별도' },
    ],
    faqs: [
      { q: '장애인전용보험도 같은 공제율인가요?', a: '아니요. 장애인전용보장성보험료는 일반 보장성보다 높은 공제율이 적용됩니다(§59의4①).' },
      { q: '한도는?', a: '두 보험료에 각각 연 한도가 있습니다. 금액은 결과 화면에 근거와 함께 표시됩니다.' },
    ],
    seoContent: '<h2>보험료 세액공제 계산기</h2><p>일반 보장성보험료와 장애인전용보장성보험료를 각각 한도까지 인정하고 서로 다른 공제율을 곱해 더합니다.</p><p>공제율·한도는 법령 원문과 대조한 상수표(policy_constants)에서 읽어 계산하며, 결과 화면에 근거 조문과 시행일을 함께 표시합니다.</p>', relatedCalcs: ['year-end-refund'],
  },

  { slug: 'child-credit', emoji: '👶', category: 'year-end', categoryLabel: '연말정산', title: '자녀 세액공제 계산기', titleShort: '자녀 공제 계산기', description: '자녀수별 세액공제 + 출산/입양 추가공제를 계산.', keywords: ['자녀 세액공제','자녀 공제','출산 공제','연말정산 자녀'], legalBasis: '소득세법 제59조의2', version: '2026.04', lastUpdated: '2026-04-05', pattern: 'simple', formula: 'childCredit', resultLabel: '자녀 세액공제', resultUnit: '원', inputs: [{ id: 'childCount', label: '공제대상 자녀수 (2026 귀속 9세 이상)', type: 'stepper', default: 2, min: 0, max: 7, hint: '기본공제대상 자녀·손자녀. 2027년 10세·2028년 11세·2029년 12세·이후 13세 이상으로 올라간다' }, { id: 'newborn', label: '올해 출산·입양한 자녀수', type: 'stepper', default: 0, min: 0, max: 3 }, { id: 'newbornOrder', label: '그 중 첫 아이의 출생 순위', type: 'select', default: '1', options: [{ value: '1', label: '첫째' }, { value: '2', label: '둘째' }, { value: '3', label: '셋째 이상' }] }], faqs: [
    ...
    ], seoContent: '<h2>자녀 세액공제 계산기 완벽 가이드</h2><p>자녀수별 세액공제 + 출산/입양 추가공제를 계산.</p><p>본 계산기는 <strong>소득세법 제59조의2</strong>를 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>자녀 공제 계산기 계산 방식</h2><p>소득세법 제59조의2 금액표를 씁니다 — 공제대상 자녀 1명 연 25만원, 2명 연 55만원, 3명 이상은 55만원에 2명 초과 1명당 40만원을 더합니다. 올해 출산·입양한 자녀는 그 아이가 첫째면 30만원, 둘째면 50만원, 셋째 이상이면 70만원을 더합니다. 공제대상 나이는 2026년 귀속 9세 이상입니다(2026-04-21 개정 부칙, 해마다 1세씩 올라 2030년부터 13세 이상). 2017년생은 부칙의 경과규정에서 빠져 있어 따로 확인이 필요합니다. 기본공제대상자(소득 요건) 여부와 결정세액 한도는 판단하지 않습니다.</p><h2>이런 분들에게 추천</h2><p>근로소득자, 연말정산 환급을 극대화하고 싶은 직장인에게 필수입니다. 체크카드 사용 비율 높이기, 연금저축/IRP 최대 납입, 월세 세액공제 신청이 핵심 전략입니다.</p>', relatedCalcs: ['year-end-refund'] },
  ...
  { slug: 'rental-income-tax', emoji: '🏠', category: 'property-tax', categoryLabel: '부동산 세금', title: '주택임대소득세 계산기', titleShort: '임대소득세 계산기', description: '주택임대 수입이 분리과세 대상일 때 분리과세 세액을 계산 — 추가공제 요건·등록 경비율·소형주택 감면·지방소득세 반영.', keywords: ['주택임대소득세','임대소득 세금','분리과세','종합과세'], legalBasis: '소득세법 제14조제3항제7호·제64조의2 · 시행령 제122조의2 · 조세특례제한법 제96조', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'compare', formula: 'rentalIncomeTax', resultLabel: '분리과세 세액', resultUnit: '원', inputs: [{ id: 'annualRent', label: '연간 주택임대 수입 (총수입금액)', type: 'currency', default: 15000000 }, { id: 'otherIncome', label: '다른 종합소득금액 (분리과세 임대소득 제외)', type: 'currency', default: 40000000, hint: '근로소득금액·사업소득금액 등 합계 — 추가공제 요건 판정에 쓴다' }, { id: 'registered', label: '등록임대주택 여부', type: 'radio', default: 'yes', options: [{ value: 'yes', label: '등록임대주택' }, { value: 'no', label: '미등록' }] }, { id: 'smallCut', label: '소형주택 임대사업자 감면', type: 'select', default: 'none', options: [{ value: 'none', label: '감면 없음' }, { value: 'one', label: '1호 임대' }, { value: 'oneLong', label: '1호 임대 · 장기일반·공공지원' }, { value: 'multi', label: '2호 이상 임대' }, { value: 'multiLong', label: '2호 이상 · 장기일반·공공지원' }] }], faqs: [
      { q: '누가 분리과세를 고를 수 있나요?', a: '주택임대 총수입금액이 연 2천만원 이하인 사람만이다(소득세법 제14조제3항제7호, 2026-09-17 원문 기준). 넘으면 계산기는 «분리과세 불가» 로 멈춘다.' },
      { q: '추가공제는 누구나 받나요?', a: '아니다. 분리과세 주택임대소득을 뺀 종합소득금액이 2천만원 이하일 때만 뺀다(제64조의2제2항). 계산기는 입력한 다른 종합소득금액으로 판정한다.' },
      { q: '등록임대주택이 되려면?', a: '민간임대주택법 임대사업자 등록, 세무서 사업자등록, 임대료 증액 5% 이하를 모두 충족해야 한다(시행령 제122조의2).' },
      { q: '종합과세와 비교해 주나요?', a: '아니다. 종합과세 세액은 장부·추계 경비율과 인적공제에 달려 이 계산기는 계산하지 않는다.' },
    ], seoContent: '<h2>주택임대소득 분리과세</h2><p>(총수입금액 − 필요경비 − 추가공제) × 분리과세 세율에서 소형주택 감면을 빼고 지방소득세를 더한다. 경비율·공제액·세율은 결과 화면 근거 줄에 적는다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['rental-yield', 'comprehensive-income-tax'] },


  // 금융 세금 추가
  { slug: 'crypto-tax', emoji: '🪙', category: 'finance-tax', categoryLabel: '금융/투자 세금', title: '가상자산(코인) 세금 계산기', titleShort: '코인 세금 계산기', description: '미시행 시뮬레이션 — 2027-01-01 이후 양도·대여분부터 적용 예정인 가상자산소득 분리과세를 미리 계산.', keywords: ['코인 세금', '가상자산 과세', '비트코인 세금', '암호화폐 세금', '가상자산 소득세'], legalBasis: '소득세법 제64조의3제2항 (시행예정 2027-01-01 · 법률 제17757호 부칙)', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'cryptoTax', resultLabel: '가상자산 소득세 (미시행 시뮬레이션)', resultUnit: '원', inputs: [{ id: 'profit', label: '연간 가상자산소득금액 (손익 통산 후)', type: 'currency', default: 10000000 }], faqs: [
      { q: '지금 코인 수익에 세금이 있나요?', a: '아직 없다. 가상자산소득 과세 조문은 현행 소득세법에 들어 있지 않고, 2027-01-01 이후 양도·대여분부터 적용하도록 공포돼 있다(세 차례 연기됨). 계산기 결과는 «미시행 시뮬레이션» 이다.' },
      { q: '어떻게 계산하나요?', a: '시행예정 조문대로 연간 소득금액에서 기본공제를 빼고 분리과세 세율을 곱한다(제64조의3제2항). 공제액·세율은 결과 화면에 적는다.' },
      { q: '지방소득세는요?', a: '분리과세 가상자산소득의 지방소득세 조문을 원문 대조하지 못해 합계에 넣지 않았다(미확정 공개).' },
      { q: '이미 갖고 있는 코인의 취득가액은?', a: '시행일 전부터 보유한 자산은 2026-12-31 시가와 실제 취득가액 중 큰 금액을 취득가액으로 본다(시행예정 조문).' },
    ], seoContent: '<h2>가상자산 과세 — 시행 전</h2><p>가상자산을 양도·대여해 생긴 소득은 기타소득으로 분리과세될 예정이다. 시행일과 세율은 법 개정으로 다시 바뀔 수 있다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['overseas-cgt', 'financial-income-tax'] },
  { slug: 'etf-tax', emoji: '📈', category: 'finance-tax', categoryLabel: '금융/투자 세금', title: 'ETF 과세 계산기', titleShort: 'ETF 과세 계산기', description: '국내 주식형·국내 기타·해외 상장 ETF 매도 차익의 세금을 유형별로 계산.', keywords: ['ETF 세금','국내 ETF 과세','해외 ETF 세금','배당소득세'], legalBasis: '소득세법 시행령 제26조의2제4항 · 소득세법 제94조·제103조·제104조 · 지방세법 제103조의3', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'etfTax', resultLabel: 'ETF 세금', resultUnit: '원', inputs: [{ id: 'type', label: 'ETF 유형', type: 'radio', default: 'domesticStock', options: [{ value: 'domesticStock', label: '국내 상장 주식형' }, { value: 'domesticOther', label: '국내 상장 기타 (해외지수·채권·원자재 등)' }, { value: 'overseas', label: '해외 상장' }] }, { id: 'profit', label: '매도 차익', type: 'currency', default: 5000000 }], faqs: [
      { q: '국내 주식형 ETF 매매차익에도 세금이 있나요?', a: '없다. 집합투자기구가 국내 상장주식을 거래해 낸 손익은 과세 대상 이익에서 뺀다(소득세법 시행령 제26조의2제4항). 분배금은 따로 배당소득으로 원천징수된다.' },
      { q: '해외지수·채권형 국내 ETF는요?', a: '매도 이익이 배당소득으로 원천징수된다. 실제 과세는 매매차익과 과세표준기준가격 증가분 중 작은 쪽인데, 그 규칙은 원문 대조 전이라 계산기는 매매차익 전액으로 계산하고 화면에 그렇게 적는다.' },
      { q: '해외 상장 ETF는요?', a: '해외주식 양도소득이라 연 기본공제를 뺀 차익에 양도소득세율과 지방소득세를 매긴다. 기본공제는 해외주식 전체에 연 1번이다.' },
    ], seoContent: '<h2>ETF 유형별 과세</h2><p>국내 주식형 매매차익은 비과세, 국내 기타 ETF 매도 이익은 배당소득, 해외 상장 ETF 차익은 양도소득이다. 세율·공제액은 결과 화면 근거 줄에 적는다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['overseas-cgt', 'financial-income-tax', 'dividend-calc'] },

  { slug: 'isa-tax-free', emoji: '💎', category: 'finance-tax', categoryLabel: '금융/투자 세금', title: 'ISA 비과세 한도 계산기', titleShort: 'ISA 비과세 계산기', description: 'ISA 비과세 한도와 초과분 분리과세를 반영해 일반 계좌 대비 절세액을 계산.', keywords: ['ISA 비과세','ISA 한도','개인종합자산관리계좌','ISA 절세'], legalBasis: '조세특례제한법 제91조의18 · 지방세법 제103조의13', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'isaTaxFree', resultLabel: '절세 금액', resultUnit: '원', inputs: [{ id: 'profit', label: 'ISA 순이익 (손익통산 후)', type: 'currency', default: 3000000 }, { id: 'type', label: 'ISA 유형', type: 'radio', default: 'general', options: [{ value: 'general', label: '일반형' }, { value: 'lowIncome', label: '서민형·농어민형' }] }], faqs: [
      { q: '절세액은 어떻게 계산하나요?', a: '일반 계좌였다면 낼 이자·배당 원천징수세(소득세+지방소득세)에서, ISA 에서 내는 초과분 분리과세 세금을 뺀다. 비과세분 전액과 초과분의 세율 차이가 모두 들어간다.' },
      { q: '비과세 한도는?', a: '일반형과 서민형·농어민형으로 나뉜다(조세특례제한법 제91조의18제2항). 현행 한도는 결과 화면에 적는다. 서민형은 직전 연도 총급여 5천만원 이하 또는 종합소득금액 3,800만원 이하 등이다(2026-09-17 원문 기준).' },
      { q: '한계는?', a: '일반 계좌에서도 이익 전부가 이자·배당으로 과세된다고 가정했다. 원래 비과세인 국내 상장주식 매매차익이 섞이면 실제 절세액은 줄어든다.' },
    ], seoContent: '<h2>ISA 과세 구조</h2><p>계좌 안 이자·배당 순이익은 한도까지 비과세, 초과분은 낮은 세율로 분리과세하고 종합소득에 합산하지 않는다. 농어촌특별세도 붙지 않는다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['irp-deduction', 'deposit-interest'] },
  ...
  {
    slug: 'accident-compensation', emoji: '🚑', category: 'law', categoryLabel: '법률/가정',
    title: '교통사고 합의금 추정기(예시)', titleShort: '교통사고 합의금',
    description: '치료비·치료기간·일소득·장해 등급으로 교통사고 합의금을 «가정 계수»로 추정한 예시값. 보험 약관·법원 기준이 아님.',
    keywords: ['교통사고 합의금', '합의금 계산', '위자료', '휴업손해'],
    legalBasis: '', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'accidentCompensation', resultLabel: '추정 합의금', resultUnit: '원',
    inputs: [
      { id: 'treatmentCost', label: '치료비', type: 'currency', default: 3000000 },
      { id: 'treatmentDays', label: '치료기간 (일)', type: 'number', default: 30, min: 1, max: 365 },
      { id: 'dailyWage', label: '일 소득', type: 'currency', default: 150000 },
      { id: 'disability', label: '장해 등급', type: 'select', default: '0', options: [{ value: '0', label: '장해 없음' }, { value: '14', label: '14급 (경미)' }, { value: '12', label: '12급' }, { value: '10', label: '10급' }, { value: '8', label: '8급' }, { value: '5', label: '5급' }, { value: '1', label: '1급 (중증)' }] },
    ],
    faqs: [
      { q: '이 금액을 받을 수 있나요?', a: '아니요. 위자료 구간, 휴업손해 100%, 장해 계산식은 카더라의 가정이며 자동차보험 표준약관이나 법원 산정기준이 아닙니다. 과실상계도 반영하지 않았습니다.' },
      { q: '실제 합의금은 어떻게 정해지나요?', a: '보험사 약관 기준과 소송 기준이 다르고, 과실비율·노동능력상실률·향후치료비 등에 따라 크게 달라집니다. 전문가 상담을 권합니다.' },
    ],
    seoContent: '<h2>교통사고 합의금 추정기</h2><p>치료비에 가정한 위자료·휴업손해·장해 보상액을 더한 예시값입니다. 각 항목의 계산 방식은 결과 화면에 공개되어 있으며, 보험 약관이나 판례의 기준값이 아닙니다.</p>', relatedCalcs: [],
  },
  ...
  { slug: 'retirement-pension-sim', emoji: '📊', category: 'pension', categoryLabel: '연금/은퇴', title: '퇴직연금 수령 시뮬레이터', titleShort: '퇴직연금 시뮬', description: '퇴직금을 일시금으로 받을 때와 연금으로 나눠 받을 때의 퇴직소득세를 비교 — 연금 수령연차별 감면율 반영.', keywords: ['퇴직연금 수령','연금 vs 일시금','퇴직소득세','연금소득세'], legalBasis: '소득세법 제48조·제55조제2항·제129조제1항제5호의3 · 시행령 제202조의2', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'compare', formula: 'retirementPensionSim', resultLabel: '세금 차이', resultUnit: '', inputs: [{ id: 'totalAmount', label: '퇴직금 총액', type: 'currency', default: 200000000 }, { id: 'years', label: '근속연수', type: 'number', default: 20, min: 1, max: 45 }, { id: 'pensionYears', label: '연금 수령 기간 (년)', type: 'range', default: 10, min: 5, max: 30, hint: '연차별 연금수령한도를 넘겨 찾으면 그 부분은 일시금 세율이다 — 계산기는 한도를 반영하지 않는다' }], faqs: [
      { q: '연금으로 받으면 세금이 얼마나 줄어드나요?', a: '퇴직소득을 연금으로 받으면 «연금외수령 원천징수세율»(일시금으로 받았을 때의 퇴직소득세 ÷ 퇴직금)에 수령연차별 비율(10년 이하·20년 이하·20년 초과)을 곱해 뗀다(소득세법 제129조제1항제5호의3). 비율은 결과 화면에 적는다.' },
      { q: '일시금 세금은 어떻게 계산하나요?', a: '퇴직소득세 계산기와 «같은 함수» 로 근속연수공제 → 환산급여 → 환산급여공제 → 기본세율 ÷12 × 근속연수 순으로 계산하고 지방소득세를 더한다.' },
      { q: '계산하지 않는 것은?', a: '연금수령한도, 운용수익(연금계좌 세율 대상), 중도 인출, 물가는 넣지 않았다. 균등 수령을 가정했다.' },
    ], seoContent: '<h2>퇴직금 일시금 vs 연금</h2><p>같은 퇴직금이라도 연금으로 나눠 받으면 연금외수령 원천징수세율의 일정 비율만 원천징수된다. 수령연차가 길어질수록 비율이 낮아진다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['retirement-pay', 'irp-deduction', 'national-pension'] },


  // 생활 추가
  {
    slug: 'alcohol-calc', emoji: '🍺', category: 'life', categoryLabel: '생활/건강',
    title: '혈중알코올 농도 계산기', titleShort: '혈중알코올 계산기',
    description: '체중·음주량·경과시간으로 혈중알코올농도를 Widmark 식으로 추정하고, 도로교통법 기준(§44④·§148의2) 구간을 표시. 운전 가능 판단용이 아님.',
    keywords: ['혈중알코올농도', '음주운전', 'BAC', '위드마크 공식'],
    legalBasis: 'Widmark 추정식, 도로교통법 §44·§148의2', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'alcoholCalc', resultLabel: '추정 혈중알코올', resultUnit: '%',
    inputs: [
      { id: 'gender', label: '성별', type: 'radio', default: 'male', options: [{ value: 'male', label: '남성' }, { value: 'female', label: '여성' }] },
      { id: 'weight', label: '체중 (kg)', type: 'number', default: 70, min: 30, max: 150 },
      { id: 'drinks', label: '음주량 (잔)', type: 'number', default: 3, min: 1, max: 20, hint: '소주잔 기준' },
      { id: 'drinkType', label: '주종', type: 'select', default: 'soju', options: [{ value: 'soju', label: '소주 (17%)' }, { value: 'beer', label: '맥주 (5%)' }, { value: 'wine', label: '와인 (13%)' }, { value: 'whiskey', label: '위스키 (40%)' }] },
      { id: 'hours', label: '음주 후 경과시간', type: 'number', default: 2, min: 0, max: 24, step: 0.5 },
    ],
    faqs: [
      { q: '이 수치로 운전해도 되는지 판단할 수 있나요?', a: '안 됩니다. 흡수율·체질·음식 섭취에 따라 실제 농도는 크게 다릅니다. 술을 마셨다면 운전하지 마세요.' },
      { q: '법적 기준은?', a: '도로교통법 §44④은 혈중알코올농도 0.03% 이상을 운전 금지 기준으로 정하고, §148의2③은 0.03%·0.08%·0.2%를 경계로 벌칙 구간을 나눕니다. 면허 정지·취소 행정처분 기준은 시행규칙 원문 대조 전입니다.' },
    ],
    seoContent: '<h2>혈중알코올농도 추정 계산기</h2><p>섭취한 알코올 양을 체중과 체내분포계수로 나눠 농도를 추정하고, 경과시간만큼 시간당 감소량을 뺍니다(Widmark). 결과는 도로교통법 벌칙 구간과 함께 표시되지만 추정치일 뿐입니다.</p>', relatedCalcs: [],
  },
  ...
  // ════ 3차 배치: 부동산 추가 ════
  { slug: 'capital-gains-land', emoji: '🌾', category: 'property-tax', categoryLabel: '부동산 세금', title: '양도소득세 계산기 (토지)', titleShort: '토지 양도세 계산기', description: '토지 매도 시 양도소득세를 계산. 비사업용 토지는 기본세율 + 10%p, 2년 미만 보유는 단기세율과 비교.', keywords: ['토지 양도소득세','비사업용 토지','토지 세금'], legalBasis: '소득세법 제104조', version: '2026.04', lastUpdated: '2026-04-05', pattern: 'tax-bracket', formula: 'capitalGainsLand', resultLabel: '양도소득세', resultUnit: '원', inputs: [{ id: 'sellPrice', label: '양도가액', type: 'currency', default: 300000000 }, { id: 'buyPrice', label: '취득가액', type: 'currency', default: 200000000 }, { id: 'expenses', label: '필요경비', type: 'currency', default: 5000000 }, { id: 'holdYears', label: '보유기간 (년)', type: 'number', default: 5 }, { id: 'nonBusiness', label: '비사업용 토지', type: 'radio', default: 'no', options: [{ value: 'yes', label: '예 (기본세율 + 10%p)' }, { value: 'no', label: '아니오' }] }], faqs: [
      { q: '토지 양도세 계산기 결과는 실제 세금과 같나요?', a: '참고용이며, 감면·중과·특례 등에 따라 차이가 있습니다. 소득세법 제104조를 기준으로 계산합니다. 정확한 세액은 세무사 상담을 권장합니다.' },
      { q: '토지 양도세 계산기에서 조정대상지역은 반영되나요?', a: '아니요. 조정대상지역·주택 수 입력이 없습니다. 토지 양도차익에 장기보유특별공제·기본공제·기본세율(비사업용 토지는 +10%p, 보유 2년 미만은 단기세율과 큰 쪽)만 적용합니다.' },
      { q: '토지 양도세 계산기는 모바일에서도 되나요?', a: '네, 모바일·태블릿·PC 모든 기기에서 최적화되어 있으며 앱 설치 없이 사용 가능합니다.' },
      { q: '결과를 공유할 수 있나요?', a: '계산 완료 후 공유 버튼으로 카카오톡, URL 복사 등으로 공유 가능합니다.' },
    ], seoContent: '<h2>양도소득세 계산기 (토지) 완벽 가이드</h2><p>토지 매도 시 양도소득세를 계산합니다.</p><p>본 계산기는 <strong>소득세법 제104조</strong>를 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>토지 양도세 계산기 계산 방식</h2><p>양도차익 = 양도가액 − 취득가액 − 필요경비. 보유기간별 장기보유특별공제율(3년 이상부터)을 적용하고 기본공제 250만원을 뺀 과세표준에 기본세율(8단계 누진)을 적용합니다. 비사업용 토지는 소득세법 제104조제1항제8호에 따라 각 구간 세율에 10%p를 더한 16~55% 세율을 적용합니다(산출세액 × 1.1 이 아닙니다). 보유 1년 미만은 50%, 1년 이상 2년 미만은 40% 단기세율로 계산한 세액과 비교해 큰 쪽을 씁니다(§104① 후단). 지방소득세는 소득세의 10% 구조로 더합니다. 비사업용 토지 해당 여부 판정·지정지역 추가 10%p(§104④)·미등기·감면은 반영하지 않습니다.</p><h2>이런 분들에게 추천</h2><p>토지 매도를 앞두고 양도소득세를 대략 가늠하려는 분에게 유용합니다. 카더라는 여러 분야의 무료 계산기를 제공합니다.</p>', relatedCalcs: ['capital-gains-housing', 'acquisition-tax'] },
  ...
  // ════ 급여 추가 ════
  { slug: 'daily-worker-tax', emoji: '👷', category: 'salary', categoryLabel: '급여/노동', title: '일용직 소득세 계산기', titleShort: '일용직 세금 계산기', description: '일용근로자 일당의 원천징수세액 — 1일 근로소득공제·근로소득세액공제·소액부징수 반영.', keywords: ['일용직 세금','일용근로소득세','일당 세금','6%'], legalBasis: '소득세법 제47조제2항·제59조제3항·제86조·제129조제1항제4호', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'dailyWorkerTax', resultLabel: '원천징수세액', resultUnit: '원', inputs: [{ id: 'dailyWage', label: '일당', type: 'currency', default: 200000 }, { id: 'days', label: '한 번에 지급받는 일수', type: 'number', default: 1, min: 1, max: 31, hint: '소액부징수는 지급할 때의 세액 합계로 판정한다' }], faqs: [
      { q: '일용직 세금은 어떻게 계산하나요?', a: '(일당 − 1일 근로소득공제) × 일용근로 원천징수세율에서 산출세액의 일정 비율(근로소득세액공제)을 뺀 금액이 소득세이고, 그 10%가 지방소득세다. 공제액·세율은 결과 화면 근거 줄의 현행 값이다.' },
      { q: '일당이 조금 넘어도 세금이 없던데요?', a: '원천징수 소득세가 1천원 미만이면 떼지 않는다(소액부징수, 소득세법 제86조). 여러 날치를 한꺼번에 받으면 합계 세액으로 판정한다.' },
    ], seoContent: '<h2>일용근로소득 원천징수</h2><p>일당에서 1일 공제액을 빼고 세율을 곱한 뒤 세액공제를 빼면 소득세가 나온다. 일용근로소득은 원천징수로 과세가 끝나 종합소득세 신고 대상이 아니다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['withholding-3-3', 'net-salary'] },
  { slug: 'freelancer-tax', emoji: '💻', category: 'salary', categoryLabel: '급여/노동', title: '프리랜서 세금 계산기', titleShort: '프리랜서 세금 계산기', description: '프리랜서 연 수입으로 예상 종합소득세와 환급액을 계산.', keywords: ['프리랜서 세금','종합소득세','경비율','5월 신고'], legalBasis: '소득세법 제50조·제55조·제59조의4, 지방세법 제92조', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'tax-bracket', formula: 'freelancerTax', resultLabel: '예상 환급/추가 납부', resultUnit: '원', inputs: [{ id: 'annualRevenue', label: '연 수입', type: 'currency', default: 50000000 }, { id: 'expenseRate', label: '경비율 (%)', type: 'percent', default: 64.1, hint: '단순경비율 또는 실제 경비를 수입 대비 %로 환산해 넣는다. 기본값은 예시값' }, { id: 'persons', label: '기본공제 인원 (본인 포함)', type: 'number', default: 1, min: 1, max: 10 }, { id: 'otherDeductions', label: '그 밖의 소득공제 합계', type: 'currency', default: 0, hint: '연금보험료·노란우산공제 등' }, { id: 'withheld', label: '기납부 3.3% 합계', type: 'currency', default: 1650000 }], faqs: [
      { q: '프리랜서 세금 계산기에서 4대보험은 어떻게 적용되나요?', a: '적용하지 않습니다. 사업소득에 대한 종합소득세·지방소득세와 기납부 3.3%만 비교하며 지역가입자 보험료는 계산에 들어 있지 않습니다.' },
      { q: '프리랜서 세금 계산기 결과가 실제와 다를 수 있나요?', a: '네. 실제 신고에서는 인적공제·각종 소득공제·세액공제, 다른 소득과의 합산에 따라 세액이 달라집니다. 이 계산기에 들어 있는 공제는 기본공제(인원 입력)·입력한 소득공제·표준세액공제뿐입니다.' },
      { q: '프리랜서 세금 계산기는 모바일에서도 되나요?', a: '네, 모든 기기에서 최적화되어 있습니다.' },
      { q: '관련 계산기가 더 있나요?', a: '카더라는 급여/노동 외에도 여러 분야의 무료 계산기를 제공합니다. 페이지 아래 관련 계산기를 함께 확인하세요.' },
    ], seoContent: '<h2>프리랜서 세금 계산기 완벽 가이드</h2><p>프리랜서 연 수입으로 예상 종합소득세와 환급액을 계산.</p><p>본 계산기는 <strong>소득세법</strong>를 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>프리랜서 세금 계산기 계산 방식</h2><p>소득금액 = 연 수입 × (1 − 경비율). 여기서 기본공제(1명당 금액 × 인원, 소득세법 제50조)와 입력한 그 밖의 소득공제를 빼 과세표준을 만들고 소득세법 기본세율(8단계 누진)을 적용한 뒤 표준세액공제(제59조의4제9항)를 뺍니다. 지방소득세는 지방세법 제92조 표준세율(기본세율의 1/10)이라 결정세액의 10%로 근사합니다. 기납부 3.3% 합계에서 두 세금을 빼 양수면 환급, 음수면 추가 납부로 보여줍니다. 국민연금·건강보험 지역가입자 보험료는 들어 있지 않습니다.</p><h2>이런 분들에게 추천</h2><p>연봉 협상 중인 직장인, 첫 취업을 앞둔 구직자, 급여 명세서를 이해하고 싶은 분에게 유용합니다.</p>', relatedCalcs: ['withholding-3-3', 'comprehensive-income-tax'] },
  ...
  { slug: 'deemed-rent', emoji: '🏠', category: 'property-tax', categoryLabel: '부동산 세금', title: '간주임대료 계산기', titleShort: '간주임대료 계산기', description: '주택 보증금의 간주임대료(사업소득 총수입금액 산입액)를 주택 수 요건·산식 비율·정기예금이자율로 계산.', keywords: ['간주임대료','전세 간주임대','보증금 이자'], legalBasis: '소득세법 제25조제1항 · 시행령 제53조 · 시행규칙 제23조제1항', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'deemedRent', resultLabel: '간주임대료 (수입금액 산입액)', resultUnit: '원', inputs: [{ id: 'deposit', label: '보증금 합계', type: 'currency', default: 500000000 }, { id: 'houseCount', label: '보유 주택 수', type: 'number', default: 3, min: 0, max: 50, hint: '전용 40㎡ 이하·기준시가 2억원 이하 주택은 2026년까지 제외. 2주택 판정에서는 기준시가 12억원 이하 주택도 제외' }, { id: 'financialIncome', label: '임대사업부분 금융수익 (이자·배당)', type: 'currency', default: 0 }], faqs: [
      { q: '누구에게 간주임대료가 붙나요?', a: '3주택 이상이면서 보증금 합계가 3억원을 넘거나, 2주택이면서 보증금 합계가 12억원을 넘는 경우다(소득세법 제25조제1항 · 시행령 제53조제1항, 2026-09-17 원문 기준). 1주택은 대상이 아니다.' },
      { q: '어떻게 계산하나요?', a: '(보증금 합계 − 3억원)의 적수 × 60/100 × 1/365 × 정기예금이자율 − 임대사업부분 금융수익(시행령 제53조제3항). 계산기는 보증금을 1년 내내 받았다고 본다.' },
      { q: '정기예금이자율은?', a: '매년 소득세법 시행규칙으로 고친다. 현행 값과 기준일은 결과 화면 근거 줄에 적는다.' },
      { q: '이 금액이 세금인가요?', a: '아니다. 사업소득 총수입금액에 더하는 금액이다. 세액은 임대소득 과세 방식에 따라 따로 계산한다.' },
    ], seoContent: '<h2>간주임대료 계산</h2><p>보증금에서 기준 금액을 뺀 금액에 산식 비율과 정기예금이자율을 곱해 임대수입으로 본다. 주택 수 요건을 먼저 판정한다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['rental-income-tax'] },
  ...
  // 소득세 +3
  { slug: 'business-income-tax', emoji: '🏪', category: 'income-tax', categoryLabel: '소득세', title: '사업소득세 계산기', titleShort: '사업소득세 계산기', description: '총수입금액에서 필요경비를 뺀 사업소득에 기본공제·표준세액공제를 반영해 기본세율로 소득세를 계산.', keywords: ['사업소득세','사업자 소득세','경비율'], legalBasis: '소득세법 제19조·제50조·제55조·제59조의4, 지방세법 제92조', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'tax-bracket', formula: 'businessIncomeTax', resultLabel: '사업소득세', resultUnit: '원', inputs: [{ id: 'revenue', label: '총 수입금액', type: 'currency', default: 100000000 }, { id: 'expenses', label: '필요경비', type: 'currency', default: 60000000 }, { id: 'persons', label: '기본공제 인원 (본인 포함)', type: 'number', default: 1, min: 1, max: 10 }, { id: 'otherDeductions', label: '그 밖의 소득공제 합계', type: 'currency', default: 0, hint: '연금보험료·노란우산공제 등' }], faqs: [
      { q: '사업소득세 계산기에 적용되는 세율은?', a: '소득세법 기본세율(8단계 누진)을 적용합니다. 과세표준은 «총수입 − 필요경비 − 기본공제(1명당 금액 × 인원) − 입력한 소득공제» 이고, 산출세액에서 표준세액공제를 뺀 뒤 지방소득세(10% 근사)를 더해 보여줍니다.' },
      { q: '사업소득세 계산기 신고 기한은?', a: '근로소득은 연말정산(2월), 종합소득세는 다음 해 5월, 양도소득 예정신고는 양도일이 속하는 달의 말일부터 2개월 이내입니다.' },
      { q: '사업소득세 계산기는 모바일에서도 되나요?', a: '네, 모든 기기에서 최적화되어 있으며 앱 설치 없이 사용 가능합니다.' },
      { q: '관련 계산기가 더 있나요?', a: '카더라는 소득세 외에도 여러 분야의 무료 계산기를 제공합니다. 페이지 아래 관련 계산기를 함께 확인하세요.' },
    ], seoContent: '<h2>사업소득세 계산기 완벽 가이드</h2><p>총수입금액에서 필요경비를 뺀 사업소득에 근사 공제 후 기본세율로 소득세를 계산.</p><p>본 계산기는 <strong>소득세법 제19조</strong>를 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>사업소득세 계산기 계산 방식</h2><p>사업소득 = 총 수입금액 − 필요경비. 과세표준 = 사업소득 − 기본공제(1명당 금액 × 인원, 소득세법 제50조) − 입력한 그 밖의 소득공제. 소득세법 기본세율(8단계 누진)을 적용하고 표준세액공제(제59조의4제9항)를 뺀 소득세에 지방소득세(10% 근사)를 더한 금액을 보여줍니다. 성실사업자 공제·기장세액공제·기납부세액은 반영하지 않습니다.</p><h2>이런 분들에게 추천</h2><p>근로소득자, 프리랜서, 사업자 등 소득세 신고를 준비하는 모든 분에게 유용합니다.</p>', relatedCalcs: ['comprehensive-income-tax'] },
  { slug: 'pension-income-tax', emoji: '🏛️', category: 'income-tax', categoryLabel: '소득세', title: '연금소득세 계산기', titleShort: '연금소득세 계산기', description: '공적연금의 연간 연금소득세와, 연금저축·IRP 사적연금의 원천징수 및 분리과세 기준 초과 시 분리·종합 선택 세액을 계산.', keywords: ['연금소득세','연금 세금','연금수령 세율'], legalBasis: '소득세법 제47조의2·제129조·제14조제3항제9호·제64조의4·제59조의4', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'pensionIncomeTax', resultLabel: '연금 관련 세금 (연간)', resultUnit: '원', inputs: [{ id: 'annualPension', label: '연간 연금 수령액 (과세 대상분)', type: 'currency', default: 12000000, hint: '국민연금은 공단 지급명세의 «과세 대상» 금액을 넣는다' }, { id: 'age', label: '수령 나이', type: 'number', default: 65, min: 55, max: 100 }, { id: 'type', label: '연금 유형', type: 'radio', default: 'private', options: [{ value: 'public', label: '공적연금 (국민연금 등)' }, { value: 'private', label: '사적연금 (연금저축·IRP 납입액·운용수익)' }] }, { id: 'lifeAnnuity', label: '종신계약 여부', type: 'radio', default: 'no', condition: 'type=private', options: [{ value: 'no', label: '아니오' }, { value: 'yes', label: '종신계약' }] }, { id: 'persons', label: '기본공제 인원 (본인 포함)', type: 'number', default: 1, min: 1, max: 10, hint: '종합과세로 계산할 때만 쓴다' }], faqs: [
      { q: '공적연금 세금은 어떻게 계산하나요?', a: '총연금액에서 연금소득공제(4구간·한도)를 빼고, 기본공제를 뺀 과세표준에 기본세율을 곱한 뒤 표준세액공제를 뺀다. 다른 종합소득이 없다고 가정한 연간 결정세액이며, 매달은 연금소득 간이세액표로 떼고 연말정산한다.' },
      { q: '사적연금 세율은?', a: '연금계좌 납입액·운용수익을 연금으로 받으면 나이(70세 미만·80세 미만·80세 이상)와 종신계약 여부에 따라 원천징수세율이 정해지고 지방소득세가 더해진다(소득세법 제129조제1항제5호의2). 현행 세율은 결과 화면에 적는다.' },
      { q: '사적연금이 많으면 어떻게 되나요?', a: '퇴직소득 연금수령분 등을 뺀 사적연금이 분리과세 기준금액을 넘으면 15% 분리과세와 종합과세 중 고를 수 있다(제64조의4). 계산기는 두 값을 모두 보여 준다.' },
      { q: '계산하지 않는 것은?', a: '다른 종합소득과의 합산, 국민연금 과세 대상 비율 계산, 퇴직소득을 연금으로 받는 부분(퇴직연금 시뮬레이터에서 계산)은 넣지 않았다.' },
    ], seoContent: '<h2>연금소득세 계산 구조</h2><p>공적연금은 연금소득공제 → 기본공제 → 기본세율 → 표준세액공제 순으로 계산한다. 사적연금은 받을 때 나이별 세율로 원천징수되고, 연간 합계가 기준금액 이하면 그것으로 끝난다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['national-pension', 'irp-deduction'] },

  { slug: 'dividend-income-tax', emoji: '💰', category: 'income-tax', categoryLabel: '소득세', title: '배당소득세 계산기', titleShort: '배당소득세 계산기', description: '국내·미국 배당 1회 지급분의 원천징수세액 — 소득세·지방소득세 성분과 소액부징수 반영.', keywords: ['배당소득세','배당세금','국내배당','해외배당'], legalBasis: '소득세법 제86조·제129조제1항제2호나목·제129조제4항 · 지방세법 제103조의13', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'dividendIncomeTax', resultLabel: '배당 관련 세금 합계', resultUnit: '원', inputs: [{ id: 'dividend', label: '배당금 (1회 지급분)', type: 'currency', default: 5000000 }, { id: 'market', label: '시장', type: 'radio', default: 'kr', options: [{ value: 'kr', label: '국내' }, { value: 'us', label: '미국 (조약 원천세 — 미확정 공개)' }] }], faqs: [
      { q: '국내 배당 세금은?', a: '배당소득 원천징수세율(소득세)에 그 10%인 지방소득세를 더한다. 세율은 결과 화면 근거 줄의 현행 값이다.' },
      { q: '소액 배당도 세금을 떼나요?', a: '1회 지급분의 원천징수 소득세가 1천원 미만이면 떼지 않는다(소액부징수, 소득세법 제86조).' },
      { q: '미국 배당은?', a: '미국에서 조약 제한세율 15%를 떼고, 그 세액이 국내 원천징수세액보다 크므로 국내에서 더 떼지 않는다(제129조제4항). 15%는 조약 원문 대조 전이라 «미확정» 으로 공개한다.' },
    ], seoContent: '<h2>배당소득세 계산</h2><p>배당금 × 원천징수세율 = 소득세, 소득세 × 10% = 지방소득세. 연간 이자·배당이 종합과세기준금액을 넘으면 금융소득종합과세 계산기로 확인한다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['dividend-calc', 'financial-income-tax'] },
  // 금융세 +3
  ...
  { slug: 'foreign-dividend-credit', emoji: '🌍', category: 'finance-tax', categoryLabel: '금융/투자 세금', title: '해외 배당 이중과세 환급 계산기', titleShort: '외국납부세액공제', description: '해외주식 배당에서 원천징수된 외국세액의 공제(환급)를 계산.', keywords: ['외국납부세액공제','해외배당 이중과세','배당세 환급'], legalBasis: '소득세법 제57조·제129조제4항', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'foreignDividendCredit', resultLabel: '외국납부세액공제', resultUnit: '원', inputs: [{ id: 'foreignTax', label: '외국 원천징수세액', type: 'currency', default: 750000 }, { id: 'foreignIncome', label: '국외원천소득 (해외 배당 등)', type: 'currency', default: 5000000 }, { id: 'totalIncome', label: '종합소득금액', type: 'currency', default: 30000000 }, { id: 'domesticTax', label: '종합소득산출세액', type: 'currency', default: 3015000, hint: '종합소득세 계산기의 산출세액(세액공제 전)' }], faqs: [
    ...
    ], seoContent: '<h2>해외 배당 이중과세 환급 계산기 완벽 가이드</h2><p>해외주식 배당에서 원천징수된 외국세액의 공제(환급)를 계산.</p><p>본 계산기는 <strong>소득세법 제57조</strong>를 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>외국납부세액공제 계산 방식</h2><p>공제한도 = 종합소득산출세액 × 국외원천소득 ÷ 종합소득금액(소득세법 제57조제1항), 외국납부세액공제 = 외국 원천징수세액과 공제한도 중 작은 금액 입니다. 한도를 넘은 외국세는 돌려받는 것이 아니라 다음 과세기간부터 10년간 이월공제됩니다(제57조제2항). 종합과세되지 않는 국외배당은 국내 원천징수 단계에서 외국세를 빼며, 원천징수세액을 넘는 외국세는 없는 것으로 봅니다(제129조제4항).</p><h2>이런 분들에게 추천</h2><p>정확한 금융/투자 세금 계산이 필요한 분에게 유용합니다. 카더라는 여러 분야의 무료 계산기를 제공하며, 계산 결과를 카카오톡으로 공유할 수 있습니다. 본 계산기는 참고용이며 전문가 상담을 권장합니다.</p>', relatedCalcs: ['dividend-calc', 'overseas-cgt'] },
  { slug: 'fis-tax-sim', emoji: '📈', category: 'finance-tax', categoryLabel: '금융/투자 세금', title: '금융투자소득세 — 폐지된 제도 안내', titleShort: '금투세 (폐지)', description: '금융투자소득세 조문(소득세법 제87조의2~제87조의27)은 2024-12-31 전부 삭제됐다 — 계산하지 않고 현행 과세 경로를 안내.', keywords: ['금융투자소득세','금투세','국내주식 과세'], legalBasis: '소득세법 제87조의2~제87조의27 (2024-12-31 삭제)', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'fisTaxSim', resultLabel: '폐지된 제도', resultUnit: '원', inputs: [], faqs: [
      { q: '금투세는 언제 시행되나요?', a: '시행되지 않는다. 근거 장(소득세법 제2장의2)이 2024-12-31 법률 제20615호로 전부 삭제됐다. «유예» 가 아니라 폐지다.' },
      { q: '그럼 국내 주식 차익 세금은?', a: '소액주주가 증권시장에서 판 상장주식 차익은 양도소득세 과세 대상이 아니다. 대주주는 대주주 양도소득세 계산기, 해외주식은 해외주식 양도소득세 계산기를 쓴다.' },
    ], seoContent: '<h2>금융투자소득세 폐지</h2><p>2025년 시행 예정이던 금융투자소득세는 조문이 모두 삭제돼 제도가 없다. 이 페이지는 예전 시뮬레이션을 대신해 현행 과세 경로를 안내한다(2026-09-17 원문 대조).</p>', relatedCalcs: ['major-shareholder-cgt', 'overseas-cgt', 'stock-roi'] },
  ...
  // 사업자 +3
  { slug: 'withholding-calc', emoji: '✂️', category: 'biz-tax', categoryLabel: '사업자 세금', title: '원천징수세액 계산기', titleShort: '원천징수 계산기', description: '소득 유형별 원천징수세액(소득세+지방소득세) 계산 — 기타소득 필요경비 의제·과세최저한·소액부징수 반영.', keywords: ['원천징수','원천징수세액','소득별 원천징수'], legalBasis: '소득세법 제84조·제86조·제127조·제129조 · 시행령 제87조·제149조의3', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'withholdingCalc', resultLabel: '원천징수세액', resultUnit: '원', inputs: [{ id: 'type', label: '소득 유형', type: 'select', default: 'business', options: [{ value: 'salary', label: '근로소득 (간이세액표)' }, { value: 'business', label: '사업소득 (인적용역)' }, { value: 'other', label: '기타소득' }, { value: 'interest', label: '이자소득' }] }, { id: 'otherKind', label: '기타소득 종류', type: 'select', default: 'exp60', condition: 'type=other', options: [{ value: 'exp60', label: '강연료·원고료·인세·자문료 등 (필요경비 60%)' }, { value: 'exp80', label: '공익법인 상금·부상 등 (필요경비 80%)' }, { value: 'none', label: '필요경비 의제 없음' }] }, { id: 'amount', label: '지급액 (1건)', type: 'currency', default: 3000000 }], faqs: [
      { q: '기타소득은 모두 8.8%인가요?', a: '아니다. 8.8%는 필요경비 60%가 의제되는 기타소득(강연료·원고료 등)에서만 나오는 실효율이다. 경비 80% 의제 소득은 4.4%, 의제가 없으면 22%다. 계산기는 경비 → 소득금액 → 세율 순으로 계산한다.' },
      { q: '소액이면 세금을 안 떼나요?', a: '기타소득금액이 건별 5만원 이하면 과세하지 않는다(과세최저한, 제84조). 원천징수세액이 1천원 미만이면 떼지 않지만(소액부징수, 제86조), 이자소득과 계속·반복 인적용역 사업소득은 예외라 작은 금액도 뗀다.' },
      { q: '근로소득은요?', a: '매달 근로소득 간이세액표로 뗀다. 급여 실수령액 계산기를 쓴다.' },
    ], seoContent: '<h2>원천징수 계산</h2><p>지급액에 소득 유형별 원천징수세율을 곱하고, 그 소득세의 10%를 지방소득세로 더한다. 기타소득은 필요경비를 먼저 뺀 소득금액에 세율을 곱한다.</p><h2>기준</h2><p>수치는 국가법령정보센터 현행 원문(2026-09-17 대조)을 담은 정책 상수표에서 읽고, 결과 화면 «근거» 줄에 출처를 적는다.</p>', relatedCalcs: ['withholding-3-3', 'net-salary'] },
  ...
  {
    slug: 'ltv-calc', emoji: '📊', category: 'real-estate', categoryLabel: '부동산',
    title: 'LTV 대출한도 계산기', titleShort: 'LTV 계산기', seoTitle: 'LTV 계산기 — 지역·주택수별 대출한도',
    description: '지역과 주택 보유 상태를 고르면 현행 규제 LTV와 수도권·규제지역 주택가격별 주담대 최대 한도를 함께 적용해 대출 가능액을 계산. 적용 근거와 발표일 표시.',
    keywords: ['LTV 계산기', '주택담보대출', '대출한도', '담보비율', '규제지역 LTV', '생애최초 LTV'],
    legalBasis: '금융위원회 가계부채 관리방안·국토교통부 규제지역 지정', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'conditional', formula: 'ltvCalc', resultLabel: '대출 가능액', resultUnit: '원',
    inputs: [
      { id: 'housePrice', label: '주택 시가', type: 'currency', default: 600000000 },
      { id: 'region', label: '지역', type: 'select', default: 'regulated', options: [{ value: 'regulated', label: '규제지역(투기과열·조정대상)' }, { value: 'capital_nonreg', label: '수도권 비규제' }, { value: 'local_nonreg', label: '수도권 외 비규제' }] },
      { id: 'owner', label: '주택 보유', type: 'select', default: 'none', options: [{ value: 'none', label: '무주택' }, { value: 'first_home', label: '생애최초' }, { value: 'disposal', label: '처분조건부 1주택' }, { value: 'owner', label: '1주택 보유(미처분)' }, { value: 'multi', label: '2주택 이상' }] },
      { id: 'existingLoan', label: '기존 담보대출', type: 'currency', default: 0 },
    ],
    faqs: [
      { q: 'LTV만 보면 되나요?', a: '수도권·규제지역 주택구입목적 주담대는 LTV와 별도로 주택가격 구간별 최대 한도가 있습니다. 계산기는 둘 중 작은 금액을 보여 줍니다.' },
      { q: '규제 수치는 어디서 오나요?', a: '금융당국 발표와 대조한 상수표(policy_constants)에서 읽으며, 결과 화면에 적용 기준과 발표일을 표시합니다. 규제가 바뀌면 표가 갱신됩니다.' },
      { q: '결과만큼 대출이 나오나요?', a: 'LTV·가격별 한도 기준의 상한일 뿐이며 DSR·소득·은행 심사에 따라 더 낮을 수 있습니다.' },
    ],
    seoContent: '<h2>LTV 대출한도 계산기</h2><p>지역(규제지역·수도권 비규제·수도권 외)과 주택 보유 상태로 적용 LTV를 고르고, 수도권·규제지역이면 주택가격 구간별 최대 한도를 함께 적용합니다. 기존 담보대출은 LTV 금액에서 차감합니다.</p><p>규제 수치는 발표 원문과 대조한 상수표에서 읽으며 결과 화면에 근거와 발표일을 표시합니다.</p>', relatedCalcs: ['dsr-calc', 'loan-repayment'],
  },
  ...
  // 급여 +2
  { slug: 'severance-calc', emoji: '📑', category: 'salary', categoryLabel: '급여/노동', title: '해고예고수당 계산기', titleShort: '해고예고수당', description: '해고 시 지급해야 하는 해고예고수당(30일분)을 계산.', keywords: ['해고예고수당','해고 수당','30일분','통상임금'], legalBasis: '근로기준법 제26조 · 근로기준법 시행령 제6조', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'severanceCalc', resultLabel: '해고예고수당', resultUnit: '원', inputs: [{ id: 'monthlySalary', label: '월 통상임금', type: 'currency', default: 3500000 }, { id: 'weeklyHours', label: '1주 소정근로시간', type: 'number', default: 40, hint: '주 15시간 이상이면 주휴시간(최대 8시간)을 더해 기준시간을 잡는다' }, { id: 'dailyHours', label: '1일 소정근로시간', type: 'number', default: 8, step: 0.5 }, { id: 'tenure', label: '계속 근로기간', type: 'radio', default: 'over3m', options: [{ value: 'over3m', label: '3개월 이상' }, { value: 'under3m', label: '3개월 미만' }] }], faqs: [
      { q: '해고예고수당에서 4대보험은 어떻게 적용되나요?', a: '적용하지 않습니다. 입력한 월 통상임금을 30일분 수당으로 보여주는 세전 금액입니다.' },
      { q: '해고예고수당은 월급과 같은 금액인가요?', a: '아닙니다. 1일 통상임금 × 30일입니다. 월급을 월 통상임금 산정 기준시간(주 40시간이면 209시간)으로 나눈 시간급에 1일 소정근로시간을 곱해 1일 통상임금을 구하므로, 보통 월급보다 큽니다. 30일 전에 예고했거나 적용 제외 사유(근속 3개월 미만 등)에 해당하면 수당이 발생하지 않습니다.' },
      { q: '해고예고수당는 모바일에서도 되나요?', a: '네, 모든 기기에서 최적화되어 있습니다.' },
      { q: '관련 계산기가 더 있나요?', a: '카더라는 급여/노동 외에도 여러 분야의 무료 계산기를 제공합니다. 페이지 아래 관련 계산기를 함께 확인하세요.' },
    ], seoContent: '<h2>해고예고수당 계산기 완벽 가이드</h2><p>해고 시 지급해야 하는 해고예고수당(30일분)을 계산.</p><p>본 계산기는 <strong>근로기준법 제26조</strong>를 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>해고예고수당 계산 방식</h2><p>시간급 통상임금 = 월 통상임금 ÷ 월 통상임금 산정 기준시간(주 소정근로시간 + 주휴시간 × 365 ÷ 7 ÷ 12, 반올림), 1일 통상임금 = 시간급 × 1일 소정근로시간(근로기준법 시행령 제6조), 해고예고수당 = 1일 통상임금 × 30일(근로기준법 제26조의 법정 최소액) 입니다. 계속 근로기간 3개월 미만이면 0원입니다. 세금·4대보험은 빼지 않습니다.</p><h2>이런 분들에게 추천</h2><p>연봉 협상 중인 직장인, 첫 취업을 앞둔 구직자, 급여 명세서를 이해하고 싶은 분에게 유용합니다.</p>', relatedCalcs: ['retirement-pay', 'net-salary'] },
  { slug: 'minimum-wage', emoji: '💰', category: 'salary', categoryLabel: '급여/노동', title: '최저임금 계산기', titleShort: '최저임금 계산기', description: '2026년 최저임금 기준 월급·연봉을 계산.', keywords: ['최저임금','2026 최저임금','최저시급','최저월급'], legalBasis: '최저임금법 제10조 · 고용노동부고시 제2025-47호', version: '2026.04', lastUpdated: '2026-04-05', pattern: 'simple', formula: 'minimumWage', resultLabel: '최저월급 (세전)', resultUnit: '원', inputs: [{ id: 'weeklyHours', label: '주당 소정근로시간', type: 'number', default: 40, hint: '주 15시간 미만이면 주휴수당이 없다(근로기준법 §18③)' }, { id: 'includeHoliday', label: '주휴수당 포함', type: 'radio', default: 'yes', options: [{ value: 'yes', label: '포함' }, { value: 'no', label: '미포함' }] }], faqs: [
      { q: '최저임금 계산기에서 4대보험은 어떻게 적용되나요?', a: '적용하지 않습니다. 최저시급 × 월 환산 기준시간으로 세전 최저월급·연봉만 보여줍니다.' },
      { q: '최저임금 계산기 결과가 실제와 다를 수 있나요?', a: '네. 수습기간 감액, 최저임금 산입 범위, 연장근로 가산에 따라 달라집니다. 적용 시급은 해당 연도 고용노동부 고시액(결과 화면에 고시 번호와 함께 표시)이며, 2027년 1월 1일부터는 2027년 고시액으로 바뀝니다.' },
      { q: '최저임금 계산기는 모바일에서도 되나요?', a: '네, 모든 기기에서 최적화되어 있습니다.' },
      { q: '관련 계산기가 더 있나요?', a: '카더라는 급여/노동 외에도 여러 분야의 무료 계산기를 제공합니다. 페이지 아래 관련 계산기를 함께 확인하세요.' },
    ], seoContent: '<h2>최저임금 계산기 완벽 가이드</h2><p>2026년 최저임금 기준 월급·연봉을 계산.</p><p>본 계산기는 <strong>최저임금법</strong>를 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>최저임금 계산기 계산 방식</h2><p>고용노동부 최저임금 고시의 시간급(결과 화면에 고시 번호와 함께 표시)에 월 환산 기준시간을 곱해 최저월급을 구합니다. 주휴수당을 포함하면 주당 유급시간 = 소정근로시간 + 소정근로시간(최대 40) ÷ 5 이고, 주 15시간 미만이면 주휴시간이 없습니다. 월 환산 기준시간 = 주당 유급시간 × 365 ÷ 7 ÷ 12 를 반올림한 값으로, 주 40시간이면 고시와 같은 209시간입니다. 연장근로 가산·수습 감액은 반영하지 않은 세전 금액입니다.</p><h2>이런 분들에게 추천</h2><p>연봉 협상 중인 직장인, 첫 취업을 앞둔 구직자, 급여 명세서를 이해하고 싶은 분에게 유용합니다.</p>', relatedCalcs: ['hourly-annual', 'net-salary'] },
  ...
  { slug: 'credit-loan-est', emoji: '💳', category: 'loan', categoryLabel: '대출/예적금', title: '신용대출 한도 추정기', titleShort: '신용대출 한도', description: '연소득으로 신용대출 한도 상한(차주별 연소득 이내 규제)을 확인.', keywords: ['신용대출 한도','대출 가능 금액','신용등급'], legalBasis: '금융위원회 가계부채 관리 강화 방안(2025-06-27)', version: '2026.09', lastUpdated: '2026-09-17', pattern: 'simple', formula: 'creditLoanEst', resultLabel: '한도 상한', resultUnit: '원', inputs: [{ id: 'annualIncome', label: '연소득', type: 'currency', default: 50000000 }], faqs: [
      { q: '신용대출 한도는 실제 대출과 차이가 있나요?', a: '큰 차이가 날 수 있습니다. 실제 한도는 은행 심사, DSR 규제, 기존 대출, 재직·소득 증빙에 따라 정해지며 2025년 6월 28일부터 전 금융권 신용대출 한도는 차주별 연소득 이내로 제한되어, 이 계산기는 그 상한만 보여줍니다. 신용점수가 높아도 상한을 넘지 못합니다.' },
      { q: '신용대출 한도는 모바일에서도 되나요?', a: '네, 모든 기기에서 최적화되어 있습니다.' },
      { q: '관련 계산기가 더 있나요?', a: '카더라는 대출/예적금 외에도 여러 분야의 무료 계산기를 제공합니다. 페이지 아래 관련 계산기를 함께 확인하세요.' },
    ], seoContent: '<h2>신용대출 한도 추정기 완벽 가이드</h2><p>연소득·신용등급으로 신용대출 가능 한도를 추정.</p><h2>신용대출 한도 계산 방식</h2><p>한도 상한 = 연소득 × 1배 입니다(금융위원회 「가계부채 관리 강화 방안」, 2025-06-28 시행, 전 금융권). 법령이 아닌 금융당국 조치이며 이후 변경 여부는 결과 화면의 옮겨 적은 날을 확인하세요. 실제 한도는 이 상한 이하에서 신용점수·DSR 규제·기존 대출·재직 기간 심사로 정해집니다.</p><h2>이런 분들에게 추천</h2><p>신용대출을 알아보기 전에 소득 대비 한도 규모를 대략 가늠하려는 분에게 유용합니다.</p>', relatedCalcs: ['loan-repayment', 'dsr-calc'] },
  // 연금 +3
  ...
  { slug: 'isa-conversion', emoji: '🔄', category: 'pension', categoryLabel: '연금/은퇴', title: 'ISA 만기 전환 시뮬레이터', titleShort: 'ISA 전환 시뮬', description: 'ISA 만기 자금을 연금계좌로 옮길 때 늘어나는 연금계좌 세액공제(소득세)를 계산.', keywords: ['ISA 전환','ISA IRP','ISA 만기','추가 세액공제'], legalBasis: '소득세법 제59조의3 · 조세특례제한법 제91조의18', version: '2026.04', lastUpdated: '2026-04-05', pattern: 'simple', formula: 'isaConversion', resultLabel: '늘어나는 세액공제', resultUnit: '원', inputs: [{ id: 'isaBalance', label: 'ISA 만기 잔액', type: 'currency', default: 20000000 }, { id: 'transferAmount', label: '연금계좌 전환 금액', type: 'currency', default: 20000000 }, { id: 'transferInto', label: '전환하는 계좌', type: 'radio', default: 'irp', options: [{ value: 'irp', label: 'IRP(퇴직연금계좌)' }, { value: 'savings', label: '연금저축계좌' }] }, { id: 'pensionSavings', label: '올해 연금저축 납입액 (전환분 제외)', type: 'currency', default: 6000000 }, { id: 'irpPaid', label: '올해 IRP 납입액 (전환분 제외)', type: 'currency', default: 3000000 }, { id: 'incomeLow', label: '종합소득 4,500만원 이하 (근로소득만이면 총급여 5,500만원 이하)', type: 'radio', default: 'no', options: [{ value: 'yes', label: '예 (15%)' }, { value: 'no', label: '아니오 (12%)' }] }], faqs: [
      { q: 'ISA 전환 시뮬 결과는 정확한가요?', a: '소득세법 제59조의3에 따라 전환 금액을 연금계좌 납입액에 넣고, 연 900만원(연금저축 600만원) 한도 위에 min(전환액 10%, 300만원)을 더 인정한 뒤, 늘어난 공제대상 납입액에 세액공제율 12% 또는 15%를 곱합니다. 지방소득세분·결정세액 한도는 반영하지 않습니다.' },
      { q: 'ISA 전환 시뮬에서 가장 중요한 입력값은?', a: '올해 이미 넣은 연금저축·IRP 납입액입니다. 기본 한도(연 900만원)를 이미 채웠다면 전환으로 늘어나는 공제는 추가 한도(최대 300만원)분뿐입니다.' },
      ...
      { q: '관련 계산기가 더 있나요?', a: '카더라는 연금/은퇴 외에도 여러 분야의 무료 계산기를 제공합니다. 페이지 아래 관련 계산기를 함께 확인하세요.' },
    ], seoContent: '<h2>ISA 만기 전환 시뮬레이터 완벽 가이드</h2><p>ISA 만기 자금을 연금계좌로 옮길 때 늘어나는 연금계좌 세액공제를 계산합니다.</p><p>본 계산기는 <strong>소득세법 제59조의3</strong>(ISA 는 조세특례제한법 제91조의18)을 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>ISA 전환 시뮬 계산 방식</h2><p>ISA 계약기간 만료일부터 60일 이내에 연금계좌로 옮긴 금액(만기 잔액 이내)은 그해 연금계좌 납입액에 포함됩니다. 공제 한도는 연 900만원(연금저축 600만원)에 «전환 금액의 10%와 300만원 중 적은 금액» 을 더한 금액입니다. 계산기는 전환이 없을 때와 있을 때의 공제대상 납입액을 비교해 늘어난 금액에 세액공제율(12%, 종합소득 4,500만원·총급여 5,500만원 이하면 15%)을 곱한 소득세 공제액을 보여줍니다. 지방소득세분·결정세액 한도·직전 연도에 나눠 옮긴 경우의 한도 차감은 반영하지 않습니다.</p><h2>이런 분들에게 추천</h2><p>정확한 연금/은퇴 계산이 필요한 분에게 유용합니다. 카더라는 여러 분야의 무료 계산기를 제공하며, 계산 결과를 카카오톡으로 공유할 수 있습니다. 본 계산기는 참고용이며 전문가 상담을 권장합니다.</p>', relatedCalcs: ['irp-deduction', 'isa-tax-free'] },
  ...
  // 법률 +4
  {
    slug: 'consolation-money', emoji: '💔', category: 'law', categoryLabel: '법률/가정',
    title: '이혼 위자료 추정기(예시)', titleShort: '위자료 계산기',
    description: '혼인 기간·유책 정도로 이혼 위자료를 «가정 계수»로 추정한 예시값. 법원 산정기준이 아님.',
    keywords: ['위자료 계산기', '이혼 위자료', '위자료 금액'],
    legalBasis: '민법 §843(§806 준용)', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'consolationMoney', resultLabel: '위자료(예시)', resultUnit: '원',
    inputs: [
      { id: 'marriageYears', label: '혼인 기간(년)', type: 'number', default: 10, min: 0 },
      { id: 'faultDegree', label: '유책 정도', type: 'select', default: 'medium', options: [{ value: 'low', label: '경미' }, { value: 'medium', label: '보통' }, { value: 'high', label: '중대(외도 등)' }] },
    ],
    faqs: [
      { q: '소득이 많으면 위자료가 커지나요?', a: '이 계산기는 소득을 계수로 쓰지 않습니다. 이전 버전의 소득 계수는 근거가 없어 제거했습니다(소득이 0이면 위자료가 0원이 되는 문제가 있었습니다).' },
      { q: '금액의 근거는?', a: '유책 정도별 기준액과 혼인 연수 가산은 카더라의 가정입니다. 법원은 유책행위의 내용, 혼인 기간, 파탄 경위, 당사자 사정을 종합해 정합니다.' },
    ],
    seoContent: '<h2>이혼 위자료 추정기</h2><p>유책 정도별 가정 기준액에 혼인 기간 가산을 곱한 예시값입니다. 법원의 산정기준이 아니며 실제 금액은 사안마다 크게 다릅니다.</p>', relatedCalcs: ['child-support'],
  },
  ...
  {
    slug: 'statute-of-limitations', emoji: '⏳', category: 'law', categoryLabel: '법률/가정',
    title: '소멸시효 계산기', titleShort: '소멸시효 계산기',
    description: '채권 종류별 소멸시효 기간(민법·상법·근로기준법)과 권리를 행사할 수 있는 날로 만료일을 계산.',
    keywords: ['소멸시효', '채권 시효', '시효 만료', '소멸시효 기간'],
    legalBasis: '민법 §157·§160·§162~§166·§766, 상법 §64, 근로기준법 §49', version: '2026.09', lastUpdated: '2026-09-17',
    pattern: 'simple', formula: 'statuteOfLimitations', resultLabel: '소멸시효 만료일', resultUnit: '',
    inputs: [
      { id: 'startDate', label: '권리를 행사할 수 있는 날', type: 'date', default: '2020-01-01', hint: '불법행위(안 날)는 손해와 가해자를 안 날' },
      { id: 'type', label: '채권 유형', type: 'select', default: 'general', options: [{ value: 'general', label: '일반 민사채권(10년)' }, { value: 'commercial', label: '상행위 채권(5년)' }, { value: 'wage', label: '임금(일용근로자 포함, 3년)' }, { value: 'severance', label: '퇴직금(3년)' }, { value: 'short3', label: '3년 단기 — 이자·사용료, 의사·공사대금 등' }, { value: 'short1', label: '1년 단기 — 숙박·음식료, 노역인·연예인 임금 등' }, { value: 'tortKnown', label: '불법행위 — 안 날부터(3년)' }, { value: 'tortOccurred', label: '불법행위 — 한 날부터(10년)' }, { value: 'judgment', label: '판결 등으로 확정된 채권(10년)' }] },
    ],
    faqs: [
      { q: '일용직 임금은 1년인가요?', a: '아닙니다. 근로기준법상 근로자면 일용근로자도 임금채권 시효는 3년입니다(§49). 민법 §164의 1년은 노역인·연예인의 임금 등에 해당합니다.' },
      { q: '만료일은 어떻게 세나요?', a: '첫날은 빼고(민법 §157) 다음 날부터 햇수를 세어 마지막 해 해당일의 전날에 끝납니다(§160). 말일이 공휴일이면 다음 날 만료되지만(§161) 이 계산에는 반영하지 않았습니다.' },
      { q: '시효가 멈추거나 새로 시작되는 경우는?', a: '청구·압류·승인 등으로 시효가 중단될 수 있습니다(§168). 이 계산기는 중단·정지를 반영하지 않습니다.' },
    ],
    seoContent: '<h2>소멸시효 계산기</h2><p>채권 유형별 시효 기간을 골라 권리를 행사할 수 있는 날부터 만료일을 계산합니다. 초일 불산입과 역에 의한 기간 계산(민법 §157·§160)을 따릅니다.</p><p>시효중단·정지, 공휴일 만료 연장, 특별법상 다른 기간은 반영하지 않았습니다.</p>', relatedCalcs: [],
  },

  { slug: 'industrial-accident', emoji: '🏗️', category: 'law', categoryLabel: '법률/가정', title: '산재 보상금 계산기', titleShort: '산재 보상금', description: '산업재해 시 휴업급여·장해급여를 추정.', keywords: ['산재 보상','산업재해','휴업급여','장해급여'], legalBasis: '산업재해보상보험법', version: '2026.04', lastUpdated: '2026-04-05', pattern: 'simple', formula: 'industrialAccident', resultLabel: '보상금 추정', resultUnit: '원', inputs: [{ id: 'dailyWage', label: '평균임금 (일당)', type: 'currency', default: 150000 }, { id: 'restDays', label: '휴업일수', type: 'number', default: 60 }, { id: 'disabilityGrade', label: '장해 등급', type: 'select', default: '0', options: [{ value: '0', label: '장해 없음' }, { value: '1', label: '1급' }, { value: '2', label: '2급' }, { value: '3', label: '3급' }, { value: '4', label: '4급' }, { value: '5', label: '5급' }, { value: '6', label: '6급' }, { value: '7', label: '7급' }, { value: '8', label: '8급' }, { value: '9', label: '9급' }, { value: '10', label: '10급' }, { value: '11', label: '11급' }, { value: '12', label: '12급' }, { value: '13', label: '13급' }, { value: '14', label: '14급' }] }, { id: 'payout', label: '4~7급 수령 방식', type: 'radio', default: 'lump', options: [{ value: 'lump', label: '일시금' }, { value: 'pension', label: '연금' }], hint: '1~3급은 연금만, 8~14급은 일시금만 있다(§57③·시행령 §53⑤)' }], faqs: [
      ...
      { q: '관련 계산기가 더 있나요?', a: '카더라는 법률/가정 외에도 여러 분야의 무료 계산기를 제공합니다. 페이지 아래 관련 계산기를 함께 확인하세요.' },
    ], seoContent: '<h2>산재 보상금 계산기 완벽 가이드</h2><p>산업재해 시 휴업급여·장해급여를 추정.</p><p>본 계산기는 <strong>산업재해보상보험법</strong>를 기준으로 계산합니다. 규정은 매년 개정될 수 있으므로 전문가 확인을 권장합니다.</p><h2>산재 보상금 계산 방식</h2><p>휴업급여 = 평균임금(일당) × 70% × 휴업일수(요양 3일 이내면 지급 없음, 산재보험법 제52조), 장해급여 = 평균임금 × 산재보험법 별표 2 장해급여표의 등급별 일수입니다. 일시금은 1급 1,474일분부터 14급 55일분까지이고, 1~3급은 장해보상연금(연 329·291·257일분)으로만, 4~7급은 연금과 일시금 중 선택해 받습니다. 연금을 고르면 결과에 연금 연액을 따로 보여줍니다. 저소득 근로자 휴업급여·최저보상기준·고령자 감액·요양급여는 반영하지 않습니다.</p><h2>이런 분들에게 추천</h2><p>정확한 법률/가정 계산이 필요한 분에게 유용합니다. 카더라는 여러 분야의 무료 계산기를 제공하며, 계산 결과를 카카오톡으로 공유할 수 있습니다. 본 계산기는 참고용이며 전문가 상담을 권장합니다.</p>', relatedCalcs: ['accident-compensation'] },
  // 군대/교육 +2
  ...
  { slug: 'csat-grade', emoji: '📝', category: 'military', categoryLabel: '군대/교육', title: '수능 등급컷 계산기', titleShort: '수능 등급 계산기', description: '원점수로 수능 예상 등급을 확인.', keywords: ['수능 등급컷','수능 등급','원점수','등급 환산'], legalBasis: '', version: '2026.04', lastUpdated: '2026-04-05', pattern: 'simple', formula: 'csatGrade', resultLabel: '예상 등급', resultUnit: '등급', inputs: [{ id: 'score', label: '원점수', type: 'number', default: 85, min: 0, max: 100 }, { id: 'subject', label: '과목', type: 'select', default: 'korean', options: [{ value: 'korean', label: '국어' }, { value: 'math', label: '수학' }, { value: 'english', label: '영어' }] }], faqs: [
      ...
      { q: '관련 계산기가 더 있나요?', a: '카더라는 군대/교육 외에도 여러 분야의 무료 계산기를 제공합니다. 페이지 아래 관련 계산기를 함께 확인하세요.' },
    ], seoContent: '<h2>수능 등급컷 계산기 완벽 가이드</h2><p>원점수로 수능 예상 등급을 확인.</p><h2>수능 등급 계산기 계산 방식</h2><p>영어는 90점 이상 1등급부터 10점 간격 절대평가 구간을 적용합니다. 국어·수학은 카더라가 정한 가정 원점수 구간(특정 시험의 등급컷이 아님)으로 참고 등급을 보여줍니다. 실제 국어·수학 등급은 매 시험의 표준점수 분포와 선택과목으로 정해지므로 크게 다를 수 있습니다.</p><h2>이런 분들에게 추천</h2><p>가채점 원점수로 대략적인 등급을 가늠하려는 수험생에게 유용합니다. 본 계산기는 참고용입니다.</p>', relatedCalcs: ['gpa-convert'] },
  // 쇼핑 +1
