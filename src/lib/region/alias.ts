...
/**
 * 광주·전남 — **별칭을 «걸지 않는다». 걸 것이 없기 때문이다.** (2026-08-30 판정)
 *
 * 15시도 전환은 끝났다. 다만 바꾼 것은 «코드» 뿐이고(29·46 → 12, 실측 27개),
 * 라벨은 「광주 동구」·「전남 목포시」 그대로 두기로 판정됐다 —
 * 라벨은 DB 의 region/sigungu 어휘가 되고 apt_sites 에 그 어휘로 480행이 있다.
 *
 * → 그래서 MOVED 에 넣을 줄이 «없다». 별칭은 「옛 이름 → 현재 라벨」인데
 *   현재 라벨이 옛 이름과 «같다». 여기에 '광주 동구' → '광주 동구' 를 적으면
 *   아무 일도 하지 않는 줄이 27개 늘 뿐이고, 다음 사람은 그걸 보고
 *   「무언가 옮겨졌구나」로 잘못 읽는다.
 *
 * ⚠️ 되살아날 조건은 하나다 — 나중에 라벨을 「전남광주 …」로 옮기기로 하면
 *    그때 27줄이 «한꺼번에» 필요해진다. 그 판단은 DB 마이그레이션과 «같은 배치» 여야 한다.
 */
export const MERGE_LABEL_NOTE =
  '광주·전남은 코드만 12로 전환됐다(라벨 유지) — 별칭 대상이 아니다';
