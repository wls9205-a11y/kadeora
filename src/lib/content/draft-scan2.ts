...
/** 인가 이름 ↔ STAGE_ORDER 위치. 현재 단계가 그 위치 이상이면 «이미 받은» 인가. */
const DONE_APPROVALS: Array<[number, RegExp]> = [
  [1, /조합\s*설립\s*(?:(?:및|과)\s*)?인가/],
  [3, /사업\s*시행(?:\s*계획)?\s*인가/],
  [4, /관리\s*처분(?:\s*계획)?\s*인가/],
];
/** 인가 바로 뒤(8자 안, 쉼표·완료 표지에서 끊음)의 진행형·미래형 — 「단계」 이름표는 제외. */
const APPROVAL_PENDING = /(?!\s*단계)(?:(?!받았|받은|완료|이후|뒤)[^.\n,]){0,8}?(?:절차가?\s*진행|진행\s*(?:중|되고|하고)|추진\s*중|예정|앞두|받을|받아야|신청할|준비\s*중|거쳐야|이\s*필요|가\s*필요)/;
...
export function scanDraft2({ title, content, siteContext, constantsBlock, compact }: Scan2Input): Scan2Defect[] {
  ...
  for (const { s: sentence, paraHasAmount } of sentences) {
    ...
    // 8차 판독 기준(2026-09-19) — 인가 완료 단계에서 «그 인가» 를 진행형·미래형으로 쓰면 결함(112572 「조합 설립 및 인가 절차가 진행되고 있습니다」).
    //   「…인가 단계(로·에서·입니다)」는 단계 이름표라 제외(현재 위치 서술).
    if (DONE_APPROVALS.some(([idx, re]) => stageIdx >= idx && new RegExp(re.source + APPROVAL_PENDING.source).test(s))) { hit('완료 인가 진행형'); continue; }
    ...
    //   출처 문장(「데이터 출처: … 공개 시스템 … 참고했습니다」)은 시세 서술이 아니다 — 112575 오탐.
    if (!paraHasAmount && !/출처|공개\s*시스템/.test(s) && /중위|시세|거래가|거래액/.test(s) && /수준|달라|참고|다양|상당/.test(s)
