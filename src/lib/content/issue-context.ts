    ...
    const lines = [
      ...
      isRedev
        ? '- 사업 성격: 정비사업(재개발·재건축) — 「구역」 표현 가능\n'
          // BN-B §3 E — 상수 블록의 주택 취득세는 «유상 매매» 기준이다. 조합원이 신축 주택을 받는 취득은 원시취득 등
          //   별도 규정이라 그 세율을 조합원 분양에 붙이면 틀린다(112520 「조합원분양: 표준세율 1%」). 원시취득 상수가 생기기 전까지 수치 금지.
          + '- 조합원 취득세: 신축 주택을 받는 조합원 취득은 원시취득 등 별도 규정 — 세율 숫자를 쓰지 않는다(「세무 전문가 확인」 문형). 제도 상수의 주택 취득세는 유상 매매(일반분양·매수) 기준'
...
export interface IssueContext { siteContext: string; sourceText: string; constantsBlock: string; bigEventContext: string; compact?: boolean; stageText?: string | null; dataText?: string | null; siteSlug?: string | null }
