 ...
 *
 * ⚠️ RULES#143. 이 파일은 DB 를 모른다. fetch·AI·upsert 는 라우트가 한다.
 *
 * ── 이 파일이 지키는 두 문장 ────────────────────────────────────────────────
...
const PUBLIC_MARKERS = [
  ...
  // ⚠️ 브랜드로 오는 공공도 있다. 「희망더함」은 부산도시공사 공공주택 브랜드다 —
  //    2026-09-05 실측: 인허가 「범천동 858-6번지 일원 희망더함아파트」가 같은 동의
  //    «민간» 분양예정 시드와 같은 시기라 동일 사업으로 오인될 뻔했다. 공급유형이
  //    다르면 같은 동·같은 시기여도 다른 사업이다.
  '희망더함',
];
