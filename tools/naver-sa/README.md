...
## `export_off.py` — 광고 중단 대상 CSV (R3-1)

```bash
export SUPABASE_DB_URL='postgresql://...'
python tools/naver-sa/export_off.py            # docs/m6/R3_광고_중단대상.csv
```

광고 다운로드와 **같은 컬럼·같은 UTF-8 BOM**이고 `키워드 상태`만 `off` 다.
그대로 대량 수정에 올릴 수 있다. 맨 뒤 `사유` 열은 네이버가 무시한다.

| 사유 | 건 | 근거 |
|---|---|---|
| `slug키워드` | 132 | `keyword ~ '[가-힣]-[가-힣]'` — 하이픈을 넣어 검색하는 사람은 없다 |
| `중복40` | 40 | 두 그룹에 동일 키워드 → **입찰가 낮은 쪽(80원, 기존 A~D)** 을 끈다 |
| `비활성현장` | 5 | `양산-물금-재건축` (`apt_sites.is_active = false`) |

⚠️ **「이중 등록 174곳」을 전부 정리하지 않는다.** 기존에만 있는 키워드 1,169 ·
신규에만 있는 키워드 1,074 로, 같은 현장에 **서로 다른 검색어**를 나눠 넣은 것이다.
각자 값을 한다. 실제 완전 중복은 **40건**뿐이다.

⚠️ 중복 40건에서 **높은 쪽(110원)을 남기는 이유는 비용이 아니라 계측**이다.
같은 키워드에 두 입찰가가 걸리면 어느 쪽이 쓰일지 통제하지 못하고
성과가 두 그룹으로 쪼개져 `n_keyword` 분석이 갈린다.

⚠️ 하이픈 판정은 **한글-한글 경계만**이다. 전체로 걸면 429건이 잡히고
`금곡2-1구역재개발` · `에코델타시티16블록중흥S-클래스` 같은 정상 검색어 297개가 죽는다.
같은 규칙이 `sa.py` 의 `reject_slug_keywords()` 로 **생성 단계에도** 박혀 있다.

## `verify.sql` — 배포 후 검증 (R3-5)

```bash
psql "$SUPABASE_DB_URL" -f tools/naver-sa/verify.sql
```

여섯 가지를 한 번에 찍는다. **0이 나오면 안 되는 자리**가 어디인지 각 쿼리 주석에 있다.
특히 ①이 0이면 지오코딩 필터가 다시 깨진 것이다(`.not(col,'eq',v)` 의 NULL 문제).

### `off` — 중단 대상을 API 로 끈다 (R3-1)

⚠️ **파일 업로드는 반려된다.** 대량 «수정» 은 다운로드 양식이 아니라 «작업 유형별 템플릿» 을
요구해서, `R3_광고_중단대상.csv` 를 그대로 올리면 「불필요한 항목」 에러가 난다(2026-08-26 실측).
양식을 맞추느니 API 로 상태만 바꾸는 편이 안전하다 — 컬럼을 잘못 채워 **다른 값까지
덮어쓸 위험이 없다.**

```bash
export NAVER_SA_API_KEY=... NAVER_SA_SECRET_KEY=... NAVER_SA_CUSTOMER_ID=...
python tools/naver-sa/sa.py off          # 대상만 출력. 아무것도 바꾸지 않는다
python tools/naver-sa/sa.py off --live   # 실제로 OFF
```

- 대상은 **CSV 에 있는 키워드 ID 뿐**이다. 그룹 단위로 끄거나 조건으로 다시 고르지 않는다
- 삭제가 아니라 **OFF(`userLock`)** 다. 되돌릴 수 있어야 한다 — 노출이 붙은 뒤
  「중복 40 중 어느 쪽을 남길지」를 다시 판단할 수 있다
- `PUT /ncc/keywords` 에 **`fields=userLock` 을 반드시 붙인다.** 부분 갱신 지정이 없으면
  입찰가·연결URL 까지 통째로 덮인다
- 실행 후 **재조회해서 「실제로 꺼졌나」를 본다.** 「요청이 200 이었다」는 확인이 아니다

2026-08-26 dry-run: 177건 전부 계정에 실존 · 이미 OFF 0 · 끌 대상 177.
