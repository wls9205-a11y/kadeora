# 카더라 Architecture Rules — **단일 원장** (#11~#145)
...
### RULES#144 — 「이모지 0」은 **사용자 표면 한정**이다. `/admin` 은 예외다 (AD-5 신설)

M4(2026-09-03)가 건 「이모지 0」은 /apt 바로가기·/blog·홈 같은 **사용자 표면**을
겨냥한 규칙이었다(satori 썸네일 예외 유지). 어드민은 그 게이트를 통과해 왔고,
2026-09-07 실측으로도 어드민 38개 파일에 이모지 102건이 그대로 살아 있다
(`admin/layout.tsx` 의 `🎛️ Mission Control`, AdminShellV4 네비 라벨
`📊 이슈 파이프라인`·`📄 PDF 파싱`·`🎯 SEO 측정` 등).

**실물이 이미 그렇게 굴러 왔으므로 그것을 규칙으로 승격한다.**

- 「이모지 0」의 적용 범위는 **사용자에게 보이는 표면**이다.
- `/admin` 라우트와 **어드민 전용 컴포넌트**(`src/components/admin/**`)는 **예외**다.
  내부 도구이고, 이모지가 조밀한 표에서 «항목을 눈으로 찍는» 실용적 표지 노릇을 한다.
- ⛔ 예외를 «어드민 밖으로» 넓히지 않는다. 어드민 컴포넌트를 사용자 표면에서
  재사용하는 순간 그 화면은 사용자 표면 규칙을 따른다.

### 뒤집는 법

이 조항은 **실물 추인**이지 «이모지가 옳다»는 판정이 아니다.
어드민도 이모지 0으로 수렴시키기로 하면, 이 조항을 지우고 어드민 이모지를
전량 제거하면 된다 — Node 한 줄로 방향이 바뀐다.

---

### RULES#145 — 크론·잡 퇴역 검증은 **4면**이다. `.github/workflows` 를 빠뜨리지 않는다 (2026-09-07 신설)

RULES#19 는 cron route 삭제 전 검증을 «3면» 으로 정했다 — `cron_logs` · `cron.job`(pg_cron) · `src/`.
그 셋에 **`.github/workflows/`** 를 더해 **4면**으로 한다.

```bash
grep -rn "<route>" .github/workflows/
```

### 왜 — 유령을 이틀에 여덟 번 쳤다

`.github/workflows/external-cron.yml` 의 세 잡 중 둘(`/api/cron/pr-monitor` ·
`/api/cron/alert-time-based`)이 **라우트가 이미 없어진 뒤에도 계속 호출되고 있었다**.
2026-09-07 실측: 두 경로 모두 프로덕션에서 **404**(GET·POST), 라우트 폴더 부재, `src` 참조 0.
그런데도 워크플로는 6시간마다·매일 그 주소를 쳤고, 최근 스케줄 런 **11건 중 9건이 failure**였다.
성공 2건은 유일한 생존자 `builder-watch` 의 이틀치였다 — 산술이 정확히 맞는다.

⛔ **빨강이 상시화되면 그 빨강은 신호가 아니다.** 두 유령이 매번 실패를 만들어
   진짜 실패가 그 안에 섞여도 아무도 못 본다. 퇴역을 안 한 대가는 「아무 일도 안 일어남」이
   아니라 «관측이 죽는 것» 이다.

⚠️ 404 와 401 을 가른 것이 판정의 열쇠였다 — `builder-watch` 는 401(인증 벽, 살아 있음),
   나머지 둘은 404(없음). 「응답이 200 이 아니다」로 뭉치면 산 것과 죽은 것이 한 덩어리가 된다.

### 곁가지 — 「정상」을 한 건으로 일반화하지 않는다

AD-0(같은 날 오전) 조사는 「external-cron 은 같은 커밋에서 정상(run #792)」이라고 적었다.
하필 #792 가 2건뿐인 success 중 하나였다. **런 하나를 보고 워크플로 전체를 판정하면 안 된다** —
최근 N건의 conclusion 분포를 본다.

---
### RULES#146 — `apt_sites` 에 쓸 때는 **트리거 컬럼 먼저, 별칭 마지막** (2026-09-08 신설)

`name_variants` 를 주입하는 모든 경로는 이 순서를 지킨다.

```
① builder · dong · name · sigungu  (트리거 컬럼)
② display_name                      (트리거와 무관)
③ name_variants                     (마지막)
```

### 왜 — 별칭이 «소리 없이» 사라지는 통로가 실재한다

DB 실측(2026-09-08):

```sql
CREATE TRIGGER trg_apt_sites_auto_variants
  BEFORE INSERT OR UPDATE OF name, sigungu, dong, builder ON public.apt_sites
  FOR EACH ROW EXECUTE FUNCTION apt_sites_auto_variants()
-- 본문: IF NEW.name IS NOT NULL
--         AND COALESCE(jsonb_array_length(NEW.name_variants), 0) < 3
--       THEN NEW.name_variants := generate_apt_name_variants_jsonb(...)   ← 통째 «교체»
```

여기서 두 가지가 따라 나온다.

- **별칭«만» 쓰는 UPDATE 는 트리거를 아예 깨우지 않는다.** `name_variants` 가 감시 컬럼에
  없기 때문이다. 그래서 ③ 은 언제나 안전하다.
- **`builder` 를 쓰는 UPDATE 는 깨운다.** 그 시점의 별칭이 3 미만이면 방금 넣은 별칭이
  생성기 산출물로 «덮인다». 순서를 뒤집으면 ③ 이 ① 에게 지워진다.

⚠️ `COALESCE(..., 0)` 이라 **`NULL` 배열도 0 으로 읽혀 교체 대상**이다. 「NULL 이니 안 건드리겠지」가
   아니다 — 오히려 가장 확실하게 덮인다.

⚠️ 사정권은 작지 않다. 활성 6,285건 중 별칭 3 미만이 **467건(7.4%)** 이다.
   그중에는 «정확히 3» 이라 한 칸 차이로 비켜간 행도 있다(`명서1 재개발`). 한 칸은 규칙이 아니다.

⛔ **생산자를 고쳐서 풀지 않는다.** CV-B 「이중 생산자 금지」이고, `sa.py` 가 이미
   「⛔ 그러니 생산자를 또 고치지 않는다」를 적어 두었다. 순서로 푼다.

### 순서만으로 부족한 자리 — 원장이 되살린다

순서는 «내가 쓰는 경로» 만 지킨다. 다른 크론이 `builder` 를 건드리며 지나가는 것까지 막지 못한다.
그래서 `site_name_applies`(쓰기 대장)와 야간 대사가 따라온다: 「원장 `resolution='applied'`
↔ 별칭 실재」를 대조해 없으면 재주입한다(`op='heal_realias'`). 갭워치 지표 `cvn_alias_heal`
이 그 건수를 들고 있고 **0 이 정상**이다. 1 이상이면 어딘가가 여전히 덮고 있다는 뜻이다.

---

### RULES#147 — 배포 게이트는 **두 층**이다. AI 가 필요한 검사로 push 를 막지 않는다 (2026-09-08 신설)

| 층 | 무엇을 재는가 | 실패하면 |
|---|---|---|
| **L1** 프리디플로이 | 픽스처 기반 «결정» 로직 — AI 무호출 | **push 를 막는다** |
| **L2** 포스트디플로이 | 배포 환경에서 AI 실왕복 1회 | **기능을 끈다. 배포는 성공한다** |

### 왜 — 「골든셋 red = 정지」는 어디서 도는지를 안 적으면 함정이 된다

로컬·CI 에는 `ANTHROPIC_API_KEY` 가 없다(CV-A 실증). 라이브 페이지는 코드와 무관하게 바뀐다.
그 둘에 기대는 검사를 push 조건으로 걸면 **「코드가 멀쩡한데 빨간」 red** 가 나고,
그 red 는 며칠 안에 「무시하는 빨강」이 된다 — RULES#145 가 적은 관측 사망과 같은 병이다.

그래서 갈랐다.

- L1 은 순수 함수만 본다(`src/lib/cvn/decide.ts`). 키도 네트워크도 필요 없다.
- L2 는 배포된 라우트에서 실제 AI 를 한 번 태운다. green 이면 스위치를 «켜고»,
  red 면 **섀도** 로 남긴다 — 크론은 돌되 원장만 쌓이고 적용은 0 이다.

⛔ **자동 적용 스위치를 사람이 켜지 않는다.** 기본값이 OFF 이고 L2 만 켠다.
   사람이 켜는 스위치는 「끄기」 쪽만 있다(`cvn.watcher_enabled`).

⚠️ L2 도 «라이브 외부 페이지» 로 판정하지 않는다. 브랜드관 HTML 이 개편되면 코드와 무관하게
   red 가 되기 때문이다. 대신 골든 기사 «문면» 픽스처를 실제 AI 에 태운다 —
...
### RULES#149 — 발행물 재스캔은 «게이트 통과분» 을 제외하지 않는다. 방어층은 겹쳐 둔다 (2026-09-15 신설 · ABG 증분 5)

**Rule** — 기발행 글 감사·시그니처 재스캔의 대상에서 「수치 게이트를 통과해 발행된 글」을 빼지 않는다.
게이트(생성 경로)와 재스캔(발행 상태)은 서로 다른 층이고, 앞층의 구멍으로 샌 글을 뒷층이 잡는 것이 설계 의도다.

⛔ 「이미 검증된 글」이라는 이유로 재스캔 쿼리에 `gate_result is null`·`created_at < 게이트 배포일` 같은 제외 조건을 넣지 않는다.
⚠️ 뒷층이 앞층 통과분을 잡으면 그건 오탐이 아니라 **앞층 구멍의 표본**이다 — 처분과 함께 앞층 수리 안건으로 올린다.

**실례 (2026-09-15 · 112444)**: 「김해 부원동 지역주택조합」 글이 수치 게이트를 편집 후 통과해 발행됐다.
같은 날 시그니처 재스캔(「예상 분양가」+금액 같은 문단)이 이 글을 잡았다 — 본문 「2억원대 예상 분양가 기준」.
원인은 정수 억 허용폭(±5,000만)이 현장 블록의 실거래 최저값(2억 1,000만)과 «의미 무시» 로 맞은 것.
