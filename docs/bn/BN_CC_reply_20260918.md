...
# 18차 — 8차 판정 집행 · 판독 기준 기계화 · BN-B 소급 스캔 (2026-09-19)

## 112576 「우선 청약」 치환 (승인분, 백업 1행 `bn_c_readout8_member_subscription`)
- 「재건축 현장의 청약 체계는 조합원 우선권과 일반인 청약으로 이원화되어 있습니다. 조합원(…)은 우선 청약 기회를 갖고, 남은 세대는 일반인 청약으로 진행됩니다.」
  → 「재건축 현장의 조합원 몫은 청약이 아닙니다. 조합원(…)은 관리처분계획에 따른 분양 신청 대상이고, 남은 세대가 일반 청약으로 공급됩니다.」
- 재스캔: 대기열 5편(112569·112570·112572·112573·112576) clean 5/5 · hold:bn_review 유지.

## 판독 기준 승격 → 스캔2 기계화 (896e24e0)
- 세션 A 기준: **인가 완료 단계에서 그 인가를 진행형·미래형으로 쓰는 서술은 전부 결함 등급.**
- 규칙 `fact: 완료 인가 진행형` — 현재 단계 이상인 인가(조합설립 1 · 사업시행계획 3 · 관리처분계획 4, STAGE_ORDER 위치)의 8자 안에 진행·추진 중·예정·앞두·받을·받아야·신청할·준비 중·거쳐야·필요.
  제외: 「…인가 단계(로·에서·입니다)」 이름표(현재 위치 서술) · 「받은/받았/완료/이후/뒤」 뒤의 다음 절차 서술 · 쉼표에서 끊음.
- ⚠️ 경계 판정 요청: 「현재 조합설립인가 **단계로 진행 중**입니다」(이름표+진행형)는 새 규칙에서 **통과**로 두었다(112572 에 잔존 · 세션 A 통과 문장). 이것도 결함 등급이면 이름표 제외를 걷는다.

## 선례 기록 — 제도 수치·정의는 DRF 원문 대조
- 재건축 조합원 문형(MEMBER_PHRASE_RECON)은 도시정비법 2026-07-01 시행판(DRF eflaw MST 284065) 제2조 제9호 나목·제39조 ① 원문 대조로 만들었다.
  **보류 중인 공용 가이드 안건(제도 상수 DRF 대조)의 선례** — 주입 문형·제도 상수에 법령 정의가 들어가면 같은 경로(lawSearch eflaw → 현행 MST → lawService)로 원문 조문을 인용한다.

## ⚠️ 소급 스캔 — 개통된 BN-B 10편 중 5편이 현행 규칙에 걸림 (공개 중 · 무수정)
| id | 적발 | 등급 |
|---|---|---|
| 112563 감천2 | FAQ 「사업시행계획인가 이후 조합 구성, 설계 진행 등을 거쳐」 · 「구체적인 조합 구성과 자격 기준은 추후 **조합설립인가 단계에서 공식 공지될 예정**」 | **실결함 2** |
| 112562 촉진3 | 데이터 출처 「한국감정원 기반」 · H2 「아크로 라로체 분양 일정과 단계별 진행 상황」(목차 밖) | 실결함 1 · 구조 경미 1 |
| 112560 사직5 | H2 「사직5구역 재개발의 의의」(목차 밖) | 구조 경미 |
| 112565 사직2 | 「2024년 8월 수주가 확정되었으며(…), 이후 **조합설립인가 단계를 진행 중**입니다」 | 경계(이름표+진행형 — 위 경계 판정과 같은 결) |
| 112559 괴정5 | place 「사하역」 = 제안 단지명 「힐스테이트 푸르지오 사하역 포레스트」 | 스캐너 오탐 |
- 판정 요청: 공개 본문 수정 금지 규칙 아래 처분 — (a) 112563(·112562) hold → superseded 재생성, 16편 일괄 개통 창에 합류 (b) 결정적 치환(본문 수정 금지 예외 필요) (c) 유지. CC 권고: 112563 은 (a)(단정 오류 2), 112562 는 (b) 1어 치환(「한국감정원」→「국토교통부 실거래가 공개시스템」)이 비용 대비 합리적이나 예외 판정 필요.

---

# 19차 — 8차 판정 2건 집행 · 발행글 정정 치환 예외 1호 (2026-09-19)

## 판독 기준 명문화 — 완료 인가 서술
- **행위 서술은 결함**: 「인가 절차가 진행되고 있습니다」「인가를 받을 예정」「인가를 앞두고」 — 완료된 인가라는 행위를 미완으로 서술.
- **국면 서술은 통과**: 「…인가 단계로/단계를/단계에서 진행 중」 — 인가가 아니라 국면이 진행 중.
- 스캔2 반영: 새 규칙(완료 인가 진행형)은 「단계」 이름표 제외, 옛 조합 미구성 규칙의 「이후 조합 설립」 변형도 「…인가 단계」면 제외. 112565 → 재스캔 통과 = **(c) 유지**.

## 판정 2 집행
- **112563 감천2 (a)**: 원자 1문 — 비공개·`hold:bn_regen_superseded`·제목 표식 · 글감 리셋(`superseded_post=112563`, 이전 이력 `superseded_prev=112551` 보존, `superseded_slug=gamcheon2-redev-hillstate-oceansky-2026`) · defer(`publish_decision='deferred'`) → **KST 0시 해제 묶음 25건째**, 재생성분은 일괄 창 합류. 옛 slug 301 은 개통 1문 창에서 merged-slugs 에 함께.
- **112562 촉진3 (b) — 발행글 정정 치환 예외 1호**: 「한국감정원 기반」 → 「한국부동산원 기반」(결정적 1곳 · LLM 0).
  백업 `blog_posts_content_backup_bn_20260919` 사유 `published_correction#1: 한국감정원→한국부동산원 (세션 A 판정 2026-09-19, 결정적 1곳)` · indexnow_queue 1(source `bn-correction`) · 라이브 페이지 200 에서 「한국감정원」 0 · 「한국부동산원 기반」 확인.
  재스캔: 정정 대상 규칙(fact) **0**. 남은 적발 1 = `section: ## 아크로 라로체 분양 일정과 단계별 진행 상황`(목차 밖 H2 — 구조라 예외 ② 범위 밖, 112560 과 같은 결 → 유지로 기록).
- 112560 (c) 유지 · 112559 「사하역」 오탐 확정 → 스캔2 에서 제목 단지명 속 역 토큰 제외(「부산역 인근」 류 제목 서술은 계속 결함).

## 「발행글 정정 치환」 예외 — 정본 규격 (세션 A 신설 2026-09-19)
1. 결정적 치환만(LLM 0)
2. 사실·제도·기관명 오류 정정 한정 — 문체·구조·논조 불가
3. 원문 백업 + 치환 이력 필수(백업 테이블 사유 `published_correction#N: 원문→치환 (판정일)`)
4. 치환 후 재스캔 — 정정 대상 규칙 0
5. **건별 세션 A 판정 필수** — 룰 적발이 자동 치환으로 이어지지 않는다
- 이력: #1 112562 「한국감정원」→「한국부동산원」(2026-09-19).
- 「본문 수정 금지」의 목적(임의·수기·LLM 변형 방지)은 그대로.

## 소급 스캔 관례 유지
- 판독 기준이 바뀔 때마다 개통 글 전체를 새 규칙으로 소급 스캔해 판정 입력으로 올린다(112563 두 문장 = 판독자 편향까지 잡은 두 번째 실증).
- 현재 BN-B 개통 9편(112563 내림 뒤): clean 6 · 유지 결정 구조 경미 2(112560·112562) · 오탐 수리 1(112559).

## 대기
- BN-C 대기열 5/16 clean · 해제 대기 25(BN-C 11 + 감천2 재생성 1 + FW-bp-regen 13) — KST 0시(15:01Z).

## 19차 보충 — 세션 A 접수·다음 창 기준 (2026-09-19)
- 정정 치환 조건 ④ 해석 확정: **「치환 후 재스캔 0」 = 정정 대상 규칙 기준 0**(구조 등 범위 밖 적발은 별도 판정 · 112562 H2 는 유지).
- **판독 순서(해제 25건):** BN-C 계열 12편(재생성 4 · 잔여 7 · 감천2 1) 우선 → 개통 창 앞당김. FW 재생성 13편은 후순위.
