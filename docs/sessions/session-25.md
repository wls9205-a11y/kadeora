...
### 관심종목 미니 스파크라인
- /api/stock/sparkline — 최근 10일 가격 히스토리 API
- MiniSparkline 컴포넌트 (48x20 SVG 인라인 차트)
- StockClient 관심종목에 자동 표시

### 미분양 급증 감지 + 경고 배너
- detect_unsold_surge() RPC (전월 대비 20%+ 증가 지역 자동 감지)
- UnsoldTab 상단에 빨간 경고 배너 + 클릭 시 해당 지역 필터

### 블로그 댓글 시드 1,020건
- 인기글 500건 추가 (총 1,020건)
- comment_count 컬럼 추가 + 기존 데이터 계산
- 블로그 목록에 댓글 수 표시

### 아파트 단지별 상세 페이지
- /apt/complex/[name] — 면적별 비교 + 가격 추이 + 거래이력 + 리뷰 + 블로그 연결
- 실거래 검색 카드 → 단지 상세 링크

### 종목 비교 페이지
- /stock/compare — 두 종목 VS 테이블 (시총/등락률/거래량/섹터)
- 인기 비교 조합 5쌍 원클릭
- 종목 상세에 비교 버튼 + 주식 헤더에 비교 링크

## 다음 세션

- [x] 아파트 단지별 상세 페이지 (/apt/complex/[name]) ✅
- [x] 종목 비교 페이지 (/stock/compare) ✅
- [x] 관심종목 미니 스파크라인 ✅
- [ ] 블로그 태그 기반 관련글 추천 개선
