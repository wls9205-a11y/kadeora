...
- **⚠️ 마이그레이션 필요** `supabase/migrations/20260322_blog_antispam.sql`을 Supabase SQL Editor에서 실행해야 `published_at` 컬럼이 추가됨. 미실행 시 블로그 목록/상세 에러 발생 가능.
- **블로그 크론 전환 진행 중** — `blog-daily`, `blog-afternoon`, `blog-seed-guide`는 safeBlogInsert 적용 완료. 나머지 8개 크론은 import만 추가되어 있고 INSERT 교체 필요.
