...
### 세션 114 후반 — 크론 로깅 + IndexNow 확장 (3개 파일)

**크론 관측 공백 해결**
- `apt-image-crawl` + `blog-generate-images`에 `withCronLogging` 래퍼 적용 → 이제 `cron_logs` 테이블에 자동 기록 (이전엔 Vercel 로그만 있었고 실패/성공 추적 불가)
- Sentry 에러 자동 리포트 + admin_alerts 자동 생성
- 4~5개 리턴 포인트 전부 `{ processed, created, updated, failed, metadata }` 형식 통일

**IndexNow 신규 URL 확장**
- `indexnow-mass` 크론에 세션 114 신규 SEO URL 약 361개 추가
  - stock_glossary 41개 + `/glossary` 인덱스
  - 거래량 상위 100 종목 × `/chart` + `/financials` = 200 URL
  - 최근 7일 daily_reports (최대 119 URL)
  - 정적 페이지 3종
- 배포 후 다음 크론 실행(6h 주기) 시 Google/Bing/Naver에 자동 푸시

**현재 진행 실측**
- 블로그 발행: 7,777 (+6 since 세션 113)
- OG 커버 남은 수: 5,384 (세션 113 대비 -174건, 시간당 ~60~100건 처리 중)
- news-sitemap.xml 48h 윈도우: 125개 ✅
- 사이트맵 index 신규 3개 엔드포인트(`/sitemap.xml`, `/image-sitemap.xml`, `/news-sitemap.xml`) 전부 200 응답 확인

**검증**
- TypeScript 0 에러
- Next.js 빌드 ✓ Compiled successfully in 82s

**브랜치**: `fix/cron-logging-and-indexnow` (로컬만)
