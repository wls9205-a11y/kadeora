## 🔖 공리 — 「자가 맞아도 «대상» 이 죽어 있을 수 있다」 (2026-08-30 · U-1b)

> 「숫자를 고치기 전에 자를 고친다」의 다음 칸이다.
> 자를 고쳤어도, **재는 대상이 애초에 작동하지 않고 있으면** 그 측정은 여전히 거짓이다.

### 무슨 일이 있었나
U-1a 에서 나는 레일 정지선을 `118 → 103` 으로 «고쳤다» 고 보고했다. 근거는 토큰 값이었다:
헤더 45 + 점프바 42 + 여유 16 = 103. 계산은 맞았다.

U-1b 에서 점프바 높이를 재려고 «실제 좌표» 를 찍어 보니
스크롤 2,400px 지점에서 점프바의 `top` 이 **-1,920** 이었다. 화면 밖이다.
`position: sticky` 가 **처음부터 한 번도 붙은 적이 없었다** — 조상 넷이 스크롤 상자를 만들고 있었다
(`main` · `html/body` · 모바일 `article` 의 `overflow-x: hidden`, 그리고 레일 상자의 `align-items: start`).

**즉 나는 «아무도 안 쓰는 값» 을 정확하게 고쳤다.**

### 규칙
1. 「고쳤다」는 **그 대상이 살아 있다는 것을 확인한 뒤에만** 쓸 수 있는 말이다.
2. 토큰·계산값은 «의도» 를 증명할 뿐 «동작» 을 증명하지 않는다. 동작은 좌표로 잰다.
3. 방어 CSS 는 방어만 하지 않는다 — `overflow-x: hidden` 은 가로 넘침을 막으면서
   **그 안의 모든 sticky 를 조용히 죽인다**. `clip` 은 같은 방어를 하면서 죽이지 않는다.
   ⚠️ 「이 규칙이 무엇을 «더» 하는가」를 묻지 않으면, 방어가 기능을 먹는다.

### 실례 3 — «파생시킨 값» 도 대상이 죽어 있을 수 있다 (2026-08-30 · 결함 2호)

크롬 스택을 토큰으로 승격하면서 `/apt` 목록 레일의 정지선을 61 → **113** 으로 «파생» 시켰다.
그러고 재 보니 그 레일의 top 이 **−608** 이었다 — 정지선은 맞고 «붙지를 않는다».
부모 그리드가 `align-items: start` 라 상자가 763px 에서 끝나 있었고, 문서는 6,968px 이었다.
**E 층에서 형제(`.kd-detail-rail`)에 같은 수리를 했는데 이쪽은 안 봤다.**

> 파생은 「값이 따라오게」 할 뿐 「그 값이 쓰이게」 하지 않는다.
> 자를 고치고(실례 1·2) 값을 파생시켜도, **대상이 살아 있는지는 여전히 따로 재야 한다.**
> ⛔ 그래서 자에 계약 하나를 더 넣었다 — 「스크롤 뒤 rect.top < 자기 CSS top 이면 흘러간 것」.
>    그리고 sticky 를 **셀렉터로 적지 않고 찾아서** 잰다. 적어 둔 것만 재면 안 적은 것이 죽어도 초록이다.

---

### 실례 2 — 같은 병, 다른 속성 (2026-08-30 · 눈 검사 결함 1호)

헤더의 `backdrop-filter: blur(16px)`(유리 효과, «장식»)가 **자손 `position: fixed` 의
기준 상자**를 헤더로 만들었다. 헤더 안에서 렌더되던 검색 모달의 `inset: 0` 이
뷰포트가 아니라 **높이 45px 짜리 헤더**를 가리켰고, 세로 중앙 정렬이 패널을 그 45px
중앙에 맞추면서 **입력창이 화면 위 −54px 로 나갔다**(6폭 전부 동일).

| | 죽인 속성 | 죽은 기능 | 목적 |
|---|---|---|---|
| 실례 1 | `overflow-x: hidden` | `position: sticky` | 방어(가로 넘침) |
| 실례 2 | `backdrop-filter: blur()` | `position: fixed` | 장식(유리 효과) |

> **둘 다 「없앨 수 없는 속성」이다.** 그래서 답은 속성을 지우는 것이 아니라
> **영향권 밖으로 대상을 옮기는 것**이다 — 실례 1 은 `clip`, 실례 2 는 `createPortal(body)`.
> ⚠️ 그리고 둘 다 **상태는 정상이었다.** sticky 는 `position: sticky` 였고, 모달은 `open`
>    이었고 포커스까지 입력창에 갔다. **상태로 재는 자는 둘 다 통과시킨다 — 좌표로 재야 한다.**

---

## PL-B 종결 — PL-A 판정 8건 집행 완료 · 변경 전후 대비표

광고 계정 쓰기 **3건**(키워드 OFF 58 · 그룹 입찰 1). 삭제 0 · 그룹 구조 변경 0 · 타 캠페인 접근 0.

### 「카더라」 캠페인 12그룹 — 집행 전/후 (실측 재조회)

| 그룹 | 입찰 | 키워드 | OFF(전) | OFF(후) | 증가 |
|---|---:|---:|---:|---:|---:|
| 기존 | 90 | 117 | 1 | 2 | +1 |
| A_분양 | 90 | 380 | 0 | 20 | **+20** |
| B_입주예정 | 90 | 856 | 40 | 40 | – |
| C_정비사업 | 90 | 905 | 5 | 5 | – |
| D_기축 | 90 | 744 | 0 | 0 | – (9/11 동결 준수) |
| E_대표 | **300** | 158 | 0 | 0 | – |
| 부울경_A_분양중 | 90 | 24 | 0 | 0 | – |
| 부울경_B_분양예정 | 90 | 18 | 6 | 6 | – |
| 부울경_C_미분양 | 90 | 32 | 3 | 5 | +2 |
| 부울경_D_입주예정 | 90 | 608 | 75 | 110 | **+35** |
| 부울경_E_정비사업_1 | 90 | 894 | 39 | 39 | – |
| 부울경_E_정비사업_2 | 90 | 742 | 9 | 9 | – |
| **합계** | | **5,478** | **178** | **236** | **+58** |

**가동 중 키워드 5,300 → 5,242.** +58 = 접미어 단독 38 + 비활성 착지 20(나머지 5는 R3-1 이 이미 꺼 둠).

### 판정별 결말

| 판정 | 결말 |
|---|---|
| ① 별칭 규칙 | ✅ 생성기 수정 + 테스트 잠금(4,478현장·16,639키워드·재생성 0종) · **키워드 38 OFF** |
| ② 소재 16건 | ⛔ **API 로 불가 확정**(3방식 전부 400·3830) → E안 개편 재생성분에 흡수 |
| ③ 비활성 착지 | ✅ **25 OFF**(끌 것 20 집행) · 드리프트 86 은 «갱신할 URL 이 없어» 기록만 |
| ④ 중복 40 | ✅ **이미 해소돼 있었다**(R3-1 산물) — churn 만들지 않음 |
| ⑤ 계측 복구 | ✅ 착지 결측 4,081 → 205 · 동기화 코드의 틀린 전제 교체 · **「stats 누락」은 결함이 아니었다** |
| ⑥ E_대표 입찰 | ✅ **90 → 300원** · 하루예산 5,000 불변 확인 · 158/158 이 그룹 입찰을 따름 |
| ⑦ 부울경 등재 | ✅ `EXISTING_GROUPS` 등재 + 출처 규명(`sa.py apply --live` 8/25) + **rollback 무장 해제** |
| ⑧ 전국 확대 | ⏸ **보류 유지.** PL-B 재실측은 며칠 뒤 |

### ⚠️ 판정 전제 중 실측과 어긋나 «집행하지 않은» 것 셋
1. **③ URL 갱신** — 드리프트 86건 전부 자기 현장 상세로 정확히 간다(착지 불일치 0). 갱신 대상 없음.
2. **④ 반대쪽 OFF** — 이미 한쪽만 켜져 있고 착지 40/40 동일. 중복 노출이 일어나고 있지 않았다.
3. **⑤ 동기화 수리** — `ad_stats_daily` 카더라 0행은 API 가 노출 0 행을 안 주는 것. 고칠 결함이 없었다.

### 📌 중단점 PL-B′ — **9/3~9/6 사이 실행 예약** (Node 승인 2026-08-30)

> ⛔ **그때까지 광고 계정 쓰기 0건.** D_기축 9/11 동결도 그대로다.
> PL-B′ 는 «읽기만» 하고, 그 결과 위에서 전국 확대 가부를 다시 판정한다.

측정 3종 — 이것만 잰다:
1. **그룹별 노출 분포** — 접미어 38건 정리가 반영된 뒤의 모양
2. **스냅샷 완전성** — `ad_keywords` 착지·슬러그 결측이 205 선을 유지하는가
   (동기화가 매일 새 세대를 넣으므로, 고친 코드가 배포된 뒤에도 결측이 안 늘어야 한다)
3. **노출0 88% 의 변동** — 4,846 / 5,478 이 움직였는가

⚠️ 왜 9/3 이후인가: 접미어 38건을 **8/30 에** 껐다. 14일 창에는 「끄기 전 노출」이 남아 있어
   지금 재면 정리 효과가 «묻힌다». 최소 3~7일이 지나야 창의 대부분이 정리 후 구간이 된다.

> ⚠️ **③ 이 이번 판정의 진짜 질문이다.** 노출0 이 안 움직이면 문제는 별칭이 아니라
> «키워드 자체의 검색량» 이라는 뜻이고, 그러면 확대 논거가 근본적으로 달라진다 —
> 「정리하고 18배로 늘린다」가 아니라 「늘려도 안 뜬다」가 된다.

### 참고 — PL-B 재실측 설계 근거
접미어 38건을 오늘 껐으므로 **노출 분포가 바뀌는 데 최소 3~7일** 걸린다. 그 전에 재실측하면
「끄기 전 노출」이 14일 창에 그대로 남아 판정을 흐린다. 잴 것은 셋:
① 접미어 정리 반영 후 그룹별 노출 분포 ② 스냅샷 완전성 유지 여부 ③ **노출0 88% 가 움직였는가**.
⚠️ ③ 이 안 움직이면 문제는 별칭이 아니라 «키워드 자체의 검색량» 이라는 뜻이고, 확대 논거가 달라진다.

---
## PL-B (부분) — PL-A 판정 집행 · **광고 계정 쓰기는 0건 · 권한에 막혀 대기**

⚠️ `--live` 광고 쓰기가 auto mode 분류기에 «차단» 됐다. 우회하지 않았다.
   판정 ①의 OFF 집행 · ② 소재 · ③ URL 갱신 · ④ 중복 정리 · ⑥ 입찰은 **전부 대기**다.
   저장소·DB 몫만 끝냈다. 아래 「대기」 표가 남은 전부다.

### 끝난 것

| 판정 | 한 일 | 검증 |
|---|---|---|
| ① 생성기 | `sa.py` 에 `SUFFIX_ALONE` + `alias_is_fragment()` — 접미어 단독 별칭 차단. 이름 «자체» 가 브랜드뿐인 현장(『월드메르디앙』·『대광로제비앙』)도 같은 문으로 막는다 | `test_name_pool.py` — 현장 4,478 · 생성 16,639 · **금지어 재생성 0종** |
| ③ 규칙 | `construction` 오분류 정정 | 역전 **1,478 → 86키워드 / 12현장** |
| ⑤ 재수집 | `ad_keywords` 최신 스냅샷의 착지·슬러그를 **API `links` 로** 재수집 | 결측 **4,081 → 205** (205 = URL없음 47 + 허브착지 158, 설계상 null) |
| ⑤ 원인 | `ad-stats-sync` 가 「광고 API 는 키워드별 랜딩을 안 준다」는 **틀린 전제**로 직전 스냅샷만 잇고 있었다 → `links` 우선으로 교체 | `v_ad_coverage` 재판정: 활성 6,033현장 중 **광고 있음 575 · 미등록 5,458** |
| ⑦ 등재 | `EXISTING_GROUPS` 에 `부울경_*` 6그룹 추가 | 빠져 있어서 확대 시 **전량 중복 거부**가 날 자리였다(README 9번). 중복 40건의 출처이기도 하다 |

#### ⚠️ 판정 ③ 을 «그대로» 적용했으면 26현장이 반대로 틀렸다
`construction` 을 전부 입주예정으로 보내면 착공한 정비사업(『양정3 재개발』·『엄궁1 재개발』 등)
26현장 218키워드가 오분류된다. 판별자는 `lifecycle_stage` 가 아니라 **`site_type`** 이다
(실측: construction 757 = subscription 723 + redevelopment 34). 그 조건을 붙여 0건으로 만들었다.

#### ⚠️ 판정 ⑤ 의 전제 하나가 실측과 달랐다 — 「동기화 수리」할 것이 없다
`ad_stats_daily` 에 카더라(기존) 그룹 행이 0인 것은 **동기화 결함이 아니다**.
`/stats` 를 직접 쳐 보면 8/27·8/28·8/29 «전부 행 없음» 이다 — API 가 노출 0 인 키워드의 행을
아예 주지 않기 때문이고, 그 그룹은 그 전에 이미 멈춰 있었다.

> 그리고 이것이 **PL-0 의 숫자 하나를 설명한다.** 「기장원룸 14일 노출 492,633」이
> 8/14~8/27 창과 8/16~8/29 창에서 **완전히 같은 값**이었다. 겹치는 구간(8/16~8/26)에
> 전부 들어 있다는 뜻이고, 즉 **기장원룸은 8/28 OFF 이전에 이미 노출이 끊겨 있었다.**
> ⚠️ 「OFF 해서 멈췄다」가 아니다. OFF 는 재발을 막았을 뿐이고, 멈춘 원인은 따로 있다(미규명).

#### ✅ 「부울경_* 6그룹을 누가 만들었나」 — 기억에 기대지 않고 답이 나왔다

`tools/naver-sa/sa_state.json` 이 그 6그룹 ID 를 «정확히» 들고 있다. 이 파일은
`sa.py apply --live` 가 롤백용으로 쓰는 상태 파일이다. 등록 시각도 기계가 만든 모양이다 —
**2026-08-25 22:53:47~22:55:00 KST · 6그룹이 13~16초 간격.**
→ 출처는 **`sa.py apply --live` 부울경 zone 실행**이다. 사람이 화면에서 만든 것이 아니다.

> ⛔ **그래서 지금 `python sa.py rollback --live` 를 치면 안 된다.**
> 그 명령은 `sa_state.json` 에 적힌 그룹을 «삭제» 한다 — 지금 그 6그룹은 **가동 중**이고
> 키워드 2,318개(캠페인 노출의 최대치를 쓰는 `부울경_D_입주예정` 포함)가 그 안에 있다.
> 등록 직후를 되돌리려고 만든 장치가, 3일 지난 지금은 «라이브 삭제 버튼» 이 됐다.

### 대기 — 광고 계정 쓰기 권한이 필요하다

| 판정 | 대상 | 준비 상태 |
|---|---|---|
| ② 집행 | 「부울경」 표기 소재 16건 | `pl5_ad_copy.py` — 전후 표 확정. **`--probe` 1건 시험이 먼저다**(아래) |
| ⑥ | E_대표 기본입찰 → 300원 | **유효 확인** — 158건 전부 `useGroupBidAmt=True` 라 그룹 입찰이 실제로 먹는다 |

#### ✅ ③ 집행 완료 (2026-08-30)
```
대상 25 / 목록 25 · 이미 OFF 5 · 끌 것 20 → 재조회: 꺼짐 25 · 아직 ON 0 ✅
```

#### ✅ ④ 는 «이미 해소돼 있었다» — 집행할 것이 없다
중복 40건 전수 확인: **40건 전부 한쪽(`부울경_D_입주예정`)만 켜져 있고 `B_입주예정` 쪽은 꺼져 있다.**
게다가 **착지가 40/40 동일** 하다 — 같은 현장 상세를 가리킨다. 중복 노출이 실제로 일어나고 있지 않다.
→ 판정 ④ 의 「반대쪽 OFF」는 이미 적용된 상태다. 켜고 끄는 churn 을 만들지 않았다.

#### ✅ 「양산-물금 5건을 누가 껐나」 — 우리 산물이다. 미규명 안건 아님
상태 필드 실측: `userLock=True` · `statusReason=KEYWORD_PAUSED` · **`inspectStatus=APPROVED`**
→ 네이버 시스템 제한이 «아니다». 사람 또는 스크립트가 껐다.

대조 결과 **우리 것**이다 — `docs/m6/R3_광고_중단대상.csv` 에 그 5건이 사유 **「비활성현장」** 으로 있고,
`sa.py off --live` 가 그 CSV 를 읽어 OFF 하는 경로다. 계정에 미확인 쓰기 주체는 없다.
⛔ 네이버 콘솔 변경 이력 확인은 «필요 없다». 원인 사냥은 여기서 멈춘다(기장원룸과 같은 규율).

> 그리고 이 CSV 가 PL-5 의 «미해명 두 건» 을 한꺼번에 설명한다:
> `slug키워드 132` · **`중복40`** · `비활성현장 5` = 177행.
> 감사에서 「원인 미상」으로 적었던 **B_입주예정 40 PAUSED** 와 **C_정비사업 5 PAUSED** 가 바로 이것이다.
>
> ⚠️ 다만 R3 의 「비활성현장」 목록에는 **양산-물금 5건뿐** 이었다 — `괴정3구역-재건축`(10)·
> `초량-호반써밋-센트럴베이`(10)는 «없었다». 그 둘은 P0-2 병합으로 «나중에» 비활성이 됐기 때문이다.
> PL-5 가 그 20건을 새로 찾아 R3 의 커버리지를 **5 → 25** 로 넓혔다.
> → 교훈: 「비활성 현장 착지」는 한 번 훑고 끝낼 목록이 아니라 **DB 비활성화를 따라가는 상시 점검**이다.

#### ✅ ① 집행 완료 (2026-08-30)
```
대상 38건 / 승인목록 38단어 · 계정에 없는 단어 0
이미 OFF 0 · 끌 것 38 → 재조회: 꺼짐 38 · 아직 ON 0 ✅
```
집행 «전» 에 규칙으로 라이브 5,478건을 재훑어 **38이 전량임을 증명**했다(잔여 0 · 38단어 1:1).
오염은 `부울경_D_입주예정` 35 · `부울경_C_미분양` 2 · `기존` 1 «세 그룹뿐» 이고,
CSV 로 만든 6그룹은 **0건** 이다 — 별칭 폭발이 `sa.py apply` 경로에서만 났다는 뜻이고
`sa_state.json` 이 가리킨 출처와 정확히 맞물린다.

#### ⚠️ 판정 ③ 의 절반은 «집행할 것이 없다»
「드리프트 86키워드는 현행 stage 기준 연결 URL 갱신」 — 12현장 전수 확인 결과
**86건 전부 자기 현장 상세로 정확히 간다(착지 불일치 0).** 갱신할 URL 이 없다.
드리프트는 착지가 아니라 «그룹 배치» 의 문제인데, 그 교정(키워드 이동)은 판정이 금지했다.
→ **기록만 남긴다.** 판정 전제와 실측이 어긋난 세 번째 자리다.

#### ✅ `rollback` 무장 해제 (판정 ③ 부수)
`sa.py` 에 `rollback_gate()` — 「켜진 적 없는 그룹」(userLock=True **그리고** 14일 노출 0)만 지운다.
실행 확인: **부울경 6그룹 전부 거부**. 이제 `rollback --live` 로 라이브 2,318키워드가 지워지지 않는다.
⚠️ 부수 발견 — `STATE` 가 상대경로라 저장소 루트에서 이 명령을 치면 «조용히 아무것도 안 한다».
   위험한 방향은 아니라 그대로 뒀다.

#### ⛔ ② 결론 — 소재 문구는 API 로 «고칠 수 없다». 확정.
세 가지 본문 형태를 전부 시험했고 **셋 다 같은 400 `3830 Invalid ad type`** 이었다.

| 방식 | 결과 |
|---|---|
| A 단수 경로 + 전체 객체 `PUT /ncc/ads/{id}?fields=ad` | 400 · 3830 |
| B 복수 경로 + 배열 `PUT /ncc/ads?fields=ad` | 400 · 3830 |
| C 단수 + `type`·`nccAdgroupId` 보강 | 400 · 3830 |

본문 «형태와 무관하게» 같은 코드다 → 거부하는 것은 본문이 아니라 **소재 타입(TEXT_45) 자체**다.
키워드 `links` 는 전체 객체를 실으면 통과했지만(8/28), 소재는 그 경로가 아예 닫혀 있다.
⚠️ 실패한 시도는 아무것도 바꾸지 않았다 — 소재 16건은 «전부 원본 그대로» 다.

→ 아래 «미리 정해 둔» 판정대로 접는다. 잔여 16건은 E안 개편에 흡수한다.

#### ✅ ② 세 방식 모두 거부될 경우의 판정 (Node · 2026-08-30) — «미리» 정해 뒀다
소재 공백을 만드는 삭제·재생성은 **하지 않는다.** ② 잔여분은 **상세 E안 배포 후
소재 전면 개편 때 재생성분에 흡수**한다 — 어차피 E안 강조점(검증 뱃지·청약 D-day·타입표·톡방 CTA)으로
카피를 다시 쓰므로, 그때 **새 소재 등록 → 검토 통과 → 구 소재 삭제** 순서로 한 그룹씩 돌면
그룹당 단일형 한도 5에 걸리지 않고 «공백 없이» 금칙 표기까지 함께 사라진다.

#### ⚠️ ② 는 「고칠 수 있는지」부터 모른다
네이버는 소재에 대해 보통 on/off 만 열고 문구는 삭제·재생성을 요구한다.
그런데 **그룹당 단일형 한도 5가 이미 차 있어** 「새로 만들고 지우기」 순서를 쓸 수 없다.
→ 가장 한산한 `부울경_B_분양예정`(14일 노출 118 · 클릭 0) 1건으로 «먼저 찔러 본다».
   실패하면 그 자리에서 멈추고 Node 판단으로 넘긴다 — 소재를 지우면 그 사이 구멍이 난다.

⚠️ 치환은 «지역 단어를 빼는» 것이다(`display.ts` 규칙). 「부산·울산·경남」으로 풀지 않는다.
   ⛔ 다만 설명문에 이미 「부산 울산 경남」으로 «풀어 쓴» 소재가 따로 있다 — 같은 규칙 위반인데
      스모크 정규식이 `/부울경/` 리터럴만 봐서 게이트에 안 걸린다. 판정 ② 범위 밖이라 손대지 않았다.

#### ① 의 OFF 목록에서 «뺀» 것 — 판정의 「결합만 허용」에 맞춰 사람이 갈랐다
- 살림: `창원자이`·`경남아너스빌`·`대연디아이엘`·`신문그리니티`·`광안SK뷰` (지역+브랜드 결합형)
- 살림: `센텀파크`·`선암호수공원`·`장안지구`·`에코델타시티` (지명)
- 살림: `호수공원`(clk 4 · CTR 0.29% — 그 그룹 최고) · `에코팰리스`(clk 1) — **성과가 있는 것은 끄지 않는다**
- 살림: `주택청약`·`미분양`·`울산분양` 등 카테고리 수요어 — 브랜드 접미어가 아니고 E_대표의 존재 이유다

---
## PL-5 — 파워링크 「카더라」 캠페인 전수 감사 (2026-08-30) · **광고 설정 변경 0건**

조회 전용. 6축 × 캠페인 전 그룹. ⛔ 타 캠페인(일광더에스·트라비스·큐샵 등)은 API 로 조회하지 않았다 —
그 이름들은 `ad_keywords` 스냅샷(우리 DB)에 이미 적재돼 있어 «캠페인 경계를 가르는 데만» 썼다.

### ⚠️ 지시서·문서 전제 5건이 실측과 달랐다 — 실측이 진실

| # | 문서·지시서가 말한 것 | 실측 |
|---|---|---|
| ① | 「카더라」 캠페인 = 그룹 **6개** · 키워드 ~3,113 | **12개 · 5,478**. `부울경_*` 6그룹(2,318키워드)이 **같은 캠페인** 안에 있고 전부 ELIGIBLE 이다 |
| ② | E_대표 158 = 허브 125 + **현장 상세 33** (PL-0) | **158 전량이 지역 허브**. 8/28 에 옮긴 33개는 `/apt/unsold`·`pipeline` 등 «목록» 이었지 상세가 아니었다 |
| ③ | E_대표는 **300원대** 입찰 | **전량 70원**(그룹 기본 90원보다 낮다). 평균게재순위 5.82 · 노출0 75% |
| ④ | 소재 = 단일형 3 + 반응형 1 | 6그룹이 **TEXT_45 5 + RSA_AD 3 = 8개**, 나머지가 4개. 확장소재는 「미등록」이 아니라 **12그룹 × 6종 = 72건 등록 완료** |
| ⑤ | 오늘 01:31~ /apt RPC 결함(42703) **구간** | 01:31~01:32 는 `PGRST002`(스키마 캐시 재적재)였다. 42703 은 **구간이 아니라 연속 발생**이었고 `get_apt_subscription_hub` 건이다 — 10:48 `164d6e70` 로 수리, 이후 로그 0 |

### 축별 실측 (14일 · 2026-08-16~08-29)

| 그룹 | 키워드 | URL없음 | 노출0 | OFF | 노출 | 클릭 | 비용 | 무효슬러그 | 비활성착지 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 기존(카더라) | 117 | 47 | 79 | 1 | 507,044 | 246 | 19,003 | 0 | 0 |
| A_분양 | 380 | 0 | 344 | 0 | 584 | 9 | 720 | 0 | 20 |
| B_입주예정 | 856 | 0 | 731 | 40 | 3,291 | 35 | 2,789 | 0 | 0 |
| C_정비사업 | 905 | 0 | 848 | 5 | 386 | 7 | 557 | 0 | 5 |
| D_기축 | 744 | 0 | 632 | 0 | 3,474 | 9 | 719 | 0 | 0 |
| E_대표 | 158 | 0 | 119 | 0 | 51,929 | 8 | 1,036 | 0 | 0 |
| 부울경_A_분양중 | 24 | 0 | 18 | 0 | 168 | 0 | 0 | 0 | 0 |
| 부울경_B_분양예정 | 18 | 0 | 13 | 6 | 118 | 0 | 0 | 0 | 0 |
| 부울경_C_미분양 | 32 | 0 | 24 | 3 | 195 | 1 | 77 | 0 | 0 |
| 부울경_D_입주예정 | 608 | 0 | 516 | 75 | 119,838 | 16 | 1,447 | 0 | 0 |
| 부울경_E_정비사업_1 | 894 | 0 | 841 | 39 | 1,442 | 4 | 473 | 0 | 0 |
| 부울경_E_정비사업_2 | 742 | 0 | 681 | 9 | 967 | 7 | 790 | 0 | 0 |
| **합계** | **5,478** | **47** | **4,846** | **178** | **689,436** | **342** | **27,611** | **0** | **25** |

**노출 0 = 4,846 / 5,478 = 88%.** 등록의 대부분이 «있지만 뜨지 않는다».

#### 축1 착지 정합
- 무효 슬러그 **0** — 8/25 하이픈 깨짐 3건은 잔존 0. `sa.py` 가 DB `slug` 를 그대로 쓰는 경로가 지켜지고 있다.
- 비활성(`is_active=false`) 현장 착지 **25키워드 / 3현장** — `괴정3구역-재건축`·`초량-호반써밋-센트럴베이`(P0-2 병합 대상)·`양산-물금-재건축`.
  셋 다 200 이고 리드폼도 뜬다. `양산-물금-재건축` 은 `/apt/양산물금-브라운스톤` 으로 리디렉트된다.
  ⚠️ PV 지시서 P0-2 의 「병합 전 파워링크 대조 필수」가 **선행되지 않은 채로 남아 있는 상태**다.
- 그룹↔단계 역전 **1,478키워드** 중 **1,392 는 규칙 산물**이다 — `sa.py` 103~109행이 `construction` 을
  `E_정비사업` 으로 보내는데, 「공사중」은 상식적으로 입주예정이다. **규칙 쪽이 틀렸다고 본다.**
  진짜 드리프트는 **86키워드 / 12현장**뿐이다.

#### 축2 결정 이행 — 4건 중 3건 이행 확인, 1건은 «닫혔다»
| 8/28 결정 | 실측 |
|---|---|
| 기장원룸 OFF | ✅ `userLock=true` · `PAUSED` · 연결 URL 없음 그대로 |
| 리드폼 없는 착지 33 → 지역 허브 | ✅ 33/33 반영. `/apt/unsold`·`pipeline`·`busan`·`redev/*` 착지 **0건** |
| 자동 추적 URL ON | ✅ **추적 살아났다.** `leads` 9건 중 `n_keyword` **1건** — 8/29 12:00 KST `아크로라로체`·`n_rank=3`·`channel=naver_ad`. **PL-4 가 종결됐다** |
| D_기축 9/11까지 동결 | ✅ 입찰·상태 변경 흔적 없음. 14일 클릭 9 · 비용 719원 · **전환 0** |

#### 축3 효율 — 제2의 기장원룸이 있다
`부울경_D_입주예정` 의 노출 119,838 중 **116,000(97%)** 이 브랜드 접미어 «단독» 키워드다.

| 키워드 | 노출 | 클릭 | CTR | 비용 |
|---|---:|---:|---:|---:|
| 프리미엄 | 31,452 | 2 | 0.006% | 200 |
| 리미티드 | 29,734 | 2 | 0.007% | 192 |
| 시그니처 | 27,212 | 0 | 0% | 0 |
| 플래티넘 | 11,412 | 0 | 0% | 0 |
| 포레스트·그랑블루·프리미어·아이파크 외 | 16,000+ | 2 | — | 197 |

기장원룸과 같은 병이다 — 뜻이 없는 질의에 계속 뜬다. 비용은 아직 작지만(CTR 이 바닥이라)
**노출 점유와 품질지수를 갉는다.** 원인은 `name_variants` 별칭 확장이 «한 토큰» 별칭을 만든 것.
그룹 간 중복 55건 중 **40건이 `B_입주예정` ↔ `부울경_D_입주예정`** — 두 입주예정 그룹이 서로 경쟁한다.

#### 축4 소재
- ⚠️ **금칙 표기 「부울경」이 살아 있는 소재 16건** (헤드라인 대체문구 `{keyword:부울경 분양정보}` 포함).
  H7-1 이 화면에서 막은 표기가 **광고 문안에는 그대로**다. 대체문구는 검색어가 길 때 실제로 노출된다.
- 모든 소재의 연결 URL 이 `https://kadeora.app` 루트다. 키워드 URL 이 우선하므로 대부분 문제가 없지만,
  **URL 없는 47키워드(기존 그룹)의 실제 착지는 「그룹 기본」이 아니라 «홈»** 이다 — PL-0 의 표기가 부정확했다.

#### 축5 결함 노출 — 광고비가 결함 페이지에 닿지 않았다
깨진 RPC 는 `get_apt_subscription_hub` 이고 **`/apt` 허브 전용**이다(`src/lib/apt/hub.ts`).
착지 census 결과 **`/apt` 에 착지하는 키워드는 0개**다(E_대표는 전량 `/apt/region/*`, 이 라우트는 해당 RPC 를 쓰지 않는다).
클릭이 발생한 착지 URL 43종을 실제로 열어 본 결과 **전부 200 · 상세는 전부 리드폼 렌더**.
⚠️ 네이버 `/stats` 는 **시간대 분해를 지원하지 않는다**(`timeIncrement` → 11001). 시간 단위 광고비는 산출 불가이며,
   해당 일자(8/30) 캠페인 전체가 클릭 1 · 108원이라 노출 규모 자체가 상한이다.

#### 축6 확대 준비도
`sa.py plan` 전국 실측: **86그룹 · 4,425현장 · 58,318키워드**(지시서의 4,223현장과 다르다).
그룹당 최대 **900키워드**로 이미 분할돼 1,000 한도를 넘지 않는다. 슬러그는 DB 값 직행 — 사람 손 0.
**현행 정합 결함율 = 158 / 5,273 = 3.0%** (무효 슬러그 0 + 비활성 착지 25 + 진짜 드리프트 86 + URL없음 47).

### ⛔ 확대 착수 전에 막혀 있는 것 — 게이트는 슬러그가 아니라 «품질» 이다
정합률(3.0%)만 보면 통과다. 그러나 **노출0 88%** 와 **단독 토큰 별칭**이 현행 규모에서 이미 나왔고,
전국은 그 18배(58,318)다. 별칭 생성 규칙을 고치지 않고 확대하면 같은 쓰레기가 18배가 된다.

### 개선 후보 — **번호만. 집행하지 않았다.**
1. 단독 토큰 별칭 키워드 OFF (프리미엄·리미티드·시그니처·플래티넘 외) — `부울경_D_입주예정`
2. `sa.py` `name_pool()` 에 「한 토큰 별칭 금지」 규칙 추가 — 확대 전 선행
3. 소재 16건의 「부울경」 표기 교체 — U-1b 배포 후 강조점과 함께
4. `sa.py` 103~109행 `construction` → `D_입주예정` 재배치 (규칙 정정, 1,392키워드 영향)
5. 비활성 3현장 착지를 canonical 슬러그로 교체 (25키워드) — PV P0-2 와 함께
6. 진짜 드리프트 12현장 그룹 재배치 (86키워드) — D_기축분 5현장은 9/11 이후
7. `B_입주예정` ↔ `부울경_D_입주예정` 중복 40키워드 정리
8. 기존 그룹 URL 없는 47키워드 — 그룹 자체가 PAUSED 라 존폐부터 결정
9. E_대표 입찰 70원 재검토 (평균순위 5.82 = 사실상 안 보인다)
10. `ad_stats_daily` 가 「카더라(기존)」 그룹을 **한 행도 담지 않는다** — U3 동기화 커버리지 결함
11. `ad_keywords` 스냅샷의 `landing_pc`/`site_slug` 결측 4,081/5,478 — `v_ad_coverage` 판정이 이 위에서 틀린다

⚠️ 4·10·11 은 광고가 아니라 **저장소·동기화** 몫이라 PL 금지 항목이 아니다.

---
## 🔖 공리 — 「어휘가 두 벌이면 한쪽만 늘어난다」 (2026-08-30 · 두 트랙 동시 실측)

> 같은 날 **세션 A 와 세션 B 가 각자** 같은 병을 만났다. 우연이 아니라 «구조» 다 —
> 어휘를 두 곳에 적으면 «자라는 쪽» 과 «잠긴 쪽» 이 갈리고, 화면은 잠긴 쪽을 본다.

### 세션 B — 타임라인이 `verified`·`conflicting` 을 「미확인」으로 그렸다
`SiteHistoryTimeline` 이 **자체 CONFIDENCE 표**를 들고 있었고 **3값뿐**이었다
(`confirmed`·`estimated`·`rumor`). 폴백이 `unknown` 이라
**독립 출처 두 곳이 확인한 사건이 화면에서는 등급 없는 것으로** 보였다.
→ DS ⑥ `VerifiedBadge` 하나로 통합(5값 + null). 점 색만 타임라인 고유 표현으로 남겼다.

### 세션 A — `confidence` 제약이 「4값 / 무잠금」 두 벌이었다
`apt_permits.match_confidence` 는 4값 제약이 걸려 있었고
`apt_sites.confidence` 는 **제약이 아예 없었다**. PV-1 주석이 「같은 4단계를 쓴다」고
**선언했지만 지켜지지 않았다** — 잠긴 쪽은 그대로인데 **안 잠긴 쪽만 증식**해
`conflicting` 이 생겼다(`pv3b_confidence_vocab_5_2026-08-29.sql` 로 양쪽 5값 통일).

### 그래서 규칙
1. 어휘는 **한 곳**에만 적는다. 두 번째 사본은 «주석으로 선언» 해도 지켜지지 않는다 —
   PV-1 주석이 그 실례다(「두 벌을 만들지 않는다」고 적어 두고 두 벌이 됐다).
2. **잠금은 양쪽에** 건다. 한쪽만 제약이 있으면 그쪽이 «낡은 쪽» 이 된다.
3. 폴백은 «모름» 으로 떨어뜨리되, **아는 값이 모름으로 떨어지고 있지 않은지**를 테스트로 잡는다.
   (세션 B 의 `ds-confidence.test.ts` 가 지금 그것을 한다.)

---
## 🔖 중단점 B′ 종결 — 유실 6건 «전부» 회수 (2026-08-30 · CC 세션 A)

B′ 판정 3건 적용 후 스팟 재실행. **버림 10 → 1**(그 1건은 정당 차단 유지).

| 현장 | 필드 | 회수된 값 | 원인이었던 것 |
|---|---|---|---|
| 부산 범천동 주상복합 | total_units | **1000** | 「1,000**가구**」 |
| 하단2 재개발 | total_units | **1850** | 「1850**가구**」 |
| 충정로역자이르네 | total_units | **299** | 「299**가구**」 |
| 부산 한양프라자 | builder | **BS한양** | 「**시공사인** BS한양」 |
| 서면롯데캐슬 | builder | **롯데건설** | 「롯데건설(주)이 **시공한**」 |
| 그랑라크 에일린의 뜰 | total_units | **1521** | (버림 1건은 「건설사, 아이에스동서」 — 정당) |

### 덤으로 확인된 것 둘
1. **그랑라크 1,521 이 `announcement`(모집공고) 원출처로 잡혔다.** 어제 permit 축에서
   「이름으로는 못 닿고 세대수만이 연결고리」였던 그 값이 **V 축에서도 같은 값**으로 나왔다.
   두 축이 독립적으로 같은 곳을 가리킨다 — 매칭 설계(안건 ⑤)의 사후 증명이다.
2. 「자이S D」로 깨져 나왔던 표기가 이번엔 **「자이에스앤디」** 로 온전히 왔다.
   evidence 요구가 추출 품질 자체를 올렸다.

### 실패 3갈래 배선 «작동 확인»
`net {"ok":3, "no_result":0, "bad_json":0, "call_failed":0}` — 현장당 3콜(news·web·extract)이
전부 갈려서 찍힌다. 이전엔 `{0,0,0,0}` 이었다(유틸만 있고 배선이 없었다).

### B′ 최종 (첫 배치 → 재배치 → 보강 후)
| | 첫 배치 | 재배치 | 보강 후(스팟) |
|---|---|---|---|
| conflicting | 4 | **1** | — |
| 추출 실패 | 10/20 | **4/20** | 0/6 |
| 과잉 차단 | — | 10 | **1**(정당) |
| verified | 0 | 0 | 0 |
| D4 자동 반영 | 0 | 0 | 0 |

⚠️ `verified` 는 여전히 0 이다 — 독립 «원출처» 2곳이 일치한 필드가 아직 없다.
   D4 자동 반영도 0 이라 **아무것도 조용히 바뀌지 않는다**. 규칙이 그대로 서 있다.

**B′ 종결.** 이후는 크론 자연 가동(KST 04:50 · 20건/일)이고 정지점을 따로 두지 않는다 —
이슈 발생 시 안건으로 적립한다.

---

## 🔖 중단점 B — PV-5 첫 배치 20건 (2026-08-29 · CC 세션 A)

**verified 0건.** 그리고 그것이 «정상» 이다 — 독립 원출처 2곳이 일치한 필드가 하나도 없었다.

### 판정 분포 (기록 20행 · 현장 20곳)
| 필드 | verified | estimated | conflicting | rumor |
|---|---|---|---|---|
| display_name | 0 | 0 | 0 | 4 |
| builder | 0 | 3 | 3 | 3 |
| total_units | 0 | 3 | 1 | 3 |

`auto_applied 0` · `auto_apply_planned 0` — D4 자동 반영은 **한 건도 발생하지 않았다**.
독립 원출처가 전부 1곳이라 verified 가 서지 않았고, 규칙이 그대로 작동했다.

### ⚠️ conflicting 4건이 «세 가지 다른 것» 을 섞고 있다
| 현장 | 갈린 값 | 실체 |
|---|---|---|
| 대신 해모로 센트럴 2차 | 한진중공업 vs **HJ중공업** | **같은 회사다** — 2021년 사명 변경. 충돌이 아니라 «정규화 부족» |
| 부산 우동1 재건축 | DL이앤씨 vs 현대건설 vs 대우건설 vs 삼성물산 vs 대방건설 vs 동원개발 | 시공사 «입찰 경쟁» 기사에서 후보사를 전부 뽑았다 — «오추출» |
| 양정4 재개발 | 78 vs 849 세대 | 78 은 세대수가 아니다 — «오추출» |
| 양정4 재개발 | 계룡건설 vs 온라이프건설 | 진짜 충돌일 수 있음 — 검수 대상 |

⛔ **판정부는 맞게 동작했다.** 갈린 값을 고르지 않고 사람에게 넘겼다.
   문제는 «추출 정밀도» 다 — conflicting 4건 중 «진짜 충돌» 은 많아야 1건이다.

### 추출 실패 10/20 (50%)
그리고 실패 이유를 **구분하지 못한다** — 검색 결과 0건인지, Claude 응답이 JSON 이 아닌지,
호출 자체가 실패했는지가 전부 `null` 로 접힌다. 오늘 종일 잡은 그 형태를 «또» 썼다.

### 큐 실측 (배치 20 기준)
`ad_landing 575 → 29일` · `curated 0` · `pre_ann_urgent 5` · `pre_ann 9` · `lead 2` · `rest 5,442(302일)`
⚠️ 분양예정 급건 5곳에 **29일 뒤** 닿는다. §6 순서를 지킨 결과이고, 바꿀지는 중단점 B 판단이다.
⚠️ `curated 0` — `curated_copy` 5건이 전부 `ad_landing` 으로 먼저 분류됐다(상위 층이 먹는다).

### 중단점 B 에 올리는 판단 3건
1. **추출 정밀도** — 사명 변경 정규화(한진중공업=HJ중공업) · 「입찰 후보」와 「선정 시공사」 구분.
   지금 상태로 규모를 키우면 검수 큐가 «오추출» 로 찬다.
2. **추출 실패 원인 분리** — 실패 10건의 이유를 갈라 기록해야 무엇을 고칠지 안다.
3. **큐 순서** — 분양예정 급건 29일 대기를 받아들일지, 층을 섞을지(§6 은 현행 순서를 지시했다).

---

## 🔖 공리 보강 — 「조용한 실패는 «비교 대상» 이 있어야만 잡힌다」 (2026-08-29)

같은 날 «부재는 무행으로 온다» 공리에 한 줄이 붙었다. 부재를 갈라 기록해도,
**그 기록이 처음이면 이상한지 알 수 없다.**

### 실증 — 키워드 사전 동기화 (U-3층 ①)
`slug_carried: 1000` 이 찍혔다. 「1000」은 데이터의 수가 아니라 **PostgREST 서버 캡의 지문**이다.
`.limit(20000)` 은 그 캡을 넘지 못한다 — 5,273 중 1,000 만 이어받았고 4,273개 현장 연결이
**에러도 경고도 없이** 사라졌다.

⚠️ 오늘 세대만 놓고 보면 «정상» 이었다. 직전 세대(8/26)에 5,273 이 있었기에 잡혔다.

### 규칙
1. **둥근 수를 의심한다.** 1000 · 100 · 500 처럼 «시스템 기본값과 같은» 수치가 나오면
   데이터가 아니라 한도를 본 것이다.
2. **비교 대상을 응답에 같이 싣는다.** 「이번에 N 개」만 남기지 말고 「모집단은 M 개」를 함께
   남긴다(carry_pool · coverage_before · api_calls_expected 가 전부 같은 목적이다).
3. `.limit()` 은 «요청» 이고 서버 캡은 «강제» 다. 다 왔는지는 range 로 «돌아 봐야» 안다.

### 계보
| 날짜 | 조용한 실패 | 잡은 방법 |
|---|---|---|
| 08-29 | NO_CODE 42 = 「키 실패」로 오독 | 본문을 남겨 XML/JSON 혼합을 봄 |
| 08-29 | 빈 본문(HTTP 200) → 조용히 0건 | 재시도 후에도 빈 것을 «따로» 셈 |
| 08-29 | ids 무행 → 「수집 실패」로 오독 | empty_batches 를 실패와 분리 |
| 08-29 | 전수 패스가 2,486 호출 쓰고 산출물 0 | 시군구별 체크포인트 |
| 08-29 | slug 4,273개 증발 | **직전 세대와의 대조** |

---

## 🔖 공리 — 「이 시스템의 API 들은 부재를 0 이 아니라 «무행» 으로 말한다」 (2026-08-29)

하루에 «세 번» 같은 모양을 만났다. 전부 「데이터가 없다」로 오독될 뻔했다.

| # | 자리 | 부재의 표현 | 오독하면 |
|---|---|---|---|
| 1 | 건축HUB 봉투 | 데이터 없음 → **JSON**(있으면 XML) | `NO_CODE 42` = 「키가 안 통한다」 |
| 2 | 건축HUB 본문 | HTTP 200 + **빈 본문** | 조용히 0건 → 커버율이 거짓말 |
| 3 | 검색광고 `ids` | 노출 0 인 키워드는 **행이 아예 안 온다**(100개 요청 → 26행) | 「수집 실패」로 집계 |

**규칙**: 수집기는 「없다」를 만나면 «세 갈래» 로 갈라 기록한다 —
`ok`(있었다) · `empty`(물어봤고 없었다) · `error`(못 물어봤다).
⛔ 이 셋을 한 칸에 넣으면 나중에 «어느 것이었는지 복원할 수 없다».
실증: 1차 수집 커버율의 「0건 법정동 749」에 EMPTY_BODY 21 이 섞여, 울산 남구 달동이
진짜 0 인지 구멍인지 판정 «불가» 였다 — 하필 아실 결측 명단 2건이 거기 걸려 있었다.

**같은 결의 파생**: 403 도 한 종류가 아니다(`Invalid API-KEY` = 키를 못 찾음 /
`Invalid Signature` = 키는 찾았고 서명이 틀림). 오늘 이 구분이 자격 전사 오류를
«코드 버그로 착각하는 것»을 막았다.

---

## 🔖 U-3층 ⑤⑥ 가동 — 돈이 보인다 (2026-08-29 · CC 세션 A)

**첫 리드 단가가 숫자로 존재한다.**

```
아크로라로체 · /apt/아크로-라로체
  노출 53 · 클릭 3 · 지출 237원 · 리드 1
  → 리드단가 237원 · 실 CPC 79원   (표본부족 라벨 동반)
```

### 실적재 (프로덕션)
`1,098행 · 551키워드 × 3일 · 지출 19,611원 · 클릭 90 · 118호출 20.5초 · 실패 0 · 이상치 0`
2회차 재실행 후에도 **행 1,098 그대로 — 멱등 확인**(게이트 ③).

### ⚠️ ② 판정이 수치로 증명됐다
뷰 551행 중 **미동기 52건**. 그리고 **지출 1위(10,286원 · 클릭 21 · CPC 490원)가 그중 하나**다.
`from ad_keywords`(캠페인 1/11 부분 스냅샷)로 짰다면 **계정 최대 지출 키워드가 표에서
통째로 사라졌을 것이다.** 「수집한 것이 보이지 않는 구조를 만들지 않는다」가 이 한 줄로 값을 했다.

### 호출 설계 — 「기간을 하루로」
`id`(단수)는 일자별을 주지만 키워드 하나뿐(5,974 호출/일). `ids`(복수)는 기간 합계만 주고
timeIncrement 를 지원하지 않는다. **기간을 하루로 좁히면 합계가 곧 일별 행**이 된다 → 118호출.
배치는 개수가 아니라 **URI 길이**로 끊는다(전량은 414 URI Too Long).

### 자격 — 스크린샷 전사 금지
API_KEY 는 `00` 한 바이트가 더 있었고(76→74자), SECRET 은 `l`/`I` 두 글자가 뒤바뀌어 있었다.
두 값의 계정 접두부를 «바이트로 맞춰» API_KEY 오류를 먼저 잡았고, 그때 403 이
`Invalid API-KEY` → `Invalid Signature` 로 바뀐 것이 확증이었다.
**규칙: 자격 값은 스크린샷 전사 금지 — 복사 버튼 → 텍스트만.**

### 남은 것
①`ad_keywords` 주기 upsert(신규 insert + 이름·그룹·slug 갱신만, **bid 등 8/26 수동 필드 보존**) ·
크론 등록(vercel crons 개수 기록 갱신) · P4(9/11) 상신.

---

## 🔖 데이터 원칙 — 거짓 라벨을 만나면 «라벨» 을 고친다 (2026-08-29 · 전 트랙 공통)

> **합성값 컬럼으로 정렬을 갈아타면 거짓이 커진다.**
> 라벨이 데이터를 앞지르면 «데이터를 라벨에 맞추지» 말고 «라벨을 데이터에 맞춘다».

### 어디서 나왔나
`/blog` 의 「📚 인기 시리즈」 정렬 키가 `post_count` 였다 — «읽힌 횟수가 아니라 글 개수» 다.
「인기」는 데이터가 뒷받침하지 않는 주장이었다.

이때 «그럴듯한» 수리는 「정렬을 조회수로 바꾸기」다. 그런데 이 저장소에서 조회수 계열
컬럼은 **합성값** 이다(H4-1 이 「많이 보는 현장」을 뺀 이유 · 「인기순」을 뺀 이유).
그 컬럼으로 갈아탔다면 라벨은 맞아 보이지만 «근거는 여전히 없고» 거짓이 한 겹 깊어진다.
→ 라벨을 「글이 많은 시리즈」로 고쳤다. 화면이 말할 수 있는 참은 «글 개수» 뿐이다.

### 규칙
1. 라벨과 데이터가 어긋나면 **먼저 라벨을 의심한다**(데이터가 아니라).
2. 라벨을 맞추려고 «다른 컬럼으로 갈아타기» 전에 그 컬럼이 실측인지 합성인지 본다.
3. 화면이 말할 수 있는 참이 «작으면 작게» 말한다. 크게 말하려고 근거를 바꾸지 않는다.

⚠️ 같은 계보: 「N곳」이 캡 값이었던 것(H7-3·B7-0) — 그때도 숫자를 부풀리지 않고
   「21–40 / 전체 182곳」이라는 «말할 수 있는 참» 으로 바꿨다.

---
## 🔖 게이트 원칙 — 부대조건은 «검증 대상» 이다 (2026-08-29 · 전 트랙 공통)

> **부대조건은 이행 대상이 아니라 검증 대상이다 — 맹점이 보이면 조건을 고친다.**

### 어디서 나왔나
`--fs-2xs` 부대조건 ③ 은 「font-large 에서 표가 깨지면 클램프하라」였다.
그 조건을 «그대로 이행» 했다면 `ds-6x6` 이 default·font-small 만 보고 있었으므로
**정작 위험한 모드를 한 번도 재지 않은 채** 「깨지지 않았다」고 보고했을 것이다.

조밀 표는 글자가 «작아질 때» 가 아니라 «커질 때» 깨진다. 그래서 조건을 이행하기 전에
매트릭스를 고쳤다 — 모드 2종 → 3종, 검사 72 → 108.
그 결과 「클램프 불요」가 «판단» 이 아니라 «증명» 이 됐다.

### 규칙
1. 부대조건을 받으면 «그 조건이 실제로 그것을 검증하는지» 부터 본다.
2. 검증 도구가 그 조건을 못 재면 «도구를 먼저 고친다». 조건을 낮추지 않는다.
3. 조건을 고쳤으면 «무엇을 왜 고쳤는지» 를 보고에 남긴다 — 조건을 준 사람이
   맹점을 알아야 다음 조건이 나아진다.

⚠️ 같은 결의 선례: 「죽은 규칙이 걸리기를 기대하는 단언은 버그를 정상으로 고정시킨다」
   (DS-1b-2 · 설계서 §5). 둘 다 «게이트가 틀린 말을 하는» 경우를 막는 규칙이다.

---
## 🔖 DS-3-1 /stock 하드코딩 회수 (2026-08-29 · CC 세션 B)

중단점 C 통과 후 첫 화면. 색·라운드·간격 22건을 토큰으로 회수했다.

### ⚠️ 규명 — 인라인 fontSize 는 «글꼴모드를 아예 안 탄다»
`fontSize: 11.5` 같은 인라인 값은 `--fs-*` 를 참조하지 않으므로 `html.font-small`/
`font-large` 에서 «한 픽셀도 움직이지 않는다». 그래서 globals.css 에
`html.font-large [style*="font-size: 11px"] { font-size: 15px !important }` 류의
**전역 !important 해킹**이 존재한다 — 인라인이 토큰을 안 타서 생긴 우회로다.
→ **DS-3 가 화면별로 fontSize 를 회수할수록 그 해킹의 존재 이유가 소멸한다.**
   전 화면 회수 완료 시 **해킹 제거 후보**(장기).

### ⛔ 그런데 /stock 에서 fontSize 를 «회수하지 못했다» — 사다리에 단이 없다
주식 조밀 행은 10 · 10.5 · 11 · 11.5 · 12 · 12.5px 를 쓰는데
`--fs-*` 의 가장 작은 단이 **13px(모바일) / 14px(데스크탑)** 이다.
그대로 바꾸면 «글자가 커져» 조밀 표가 무너진다(11.5→13 = +13%).
→ 회수를 «중단하고» 색·라운드·간격만 넣었다. 커밋 순수성 유지.
→ 다음 판정 필요: `--fs-2xs` 신설(시스템 계층) 여부. 재사용 실측 3개 컴포넌트 15곳 이상.

### 회수 내역 (값 무변경 14 / 판단 8)
| 파일 | 처리 |
|---|---|
| `IssueGateCard` | 앰버 그라디언트·금테두리 → 중립 카드 + `--shadow-sm`. **미세 변화 아님** |
| `StockIssueCardV2` | hex 3건 정확 일치 · radius 6→`--radius-sm` · 3→`--radius-xs` |
| `StockHubRail` · `StockFilterBars` | 흰 «글자» → `--text-inverse` (정확 일치) |
| `StockTabCarousel` | `#E5E7EB`→`--border` · `#F3F4F6`→`--bg-elevated` · 999→`--radius-pill` |

⚠️ `StockTabCarousel` 선택 탭의 어두운 면에 쓸 semantic 토큰이 «없다».
   `--text-primary` 를 배경으로 쓴 것은 색은 정확하지만 «의미가 어긋난» 흡수다.
   `--surface-inverse` 신설은 DS-3 전 화면을 본 뒤 판단한다(한 화면 보고 만들면 어휘가 는다).
