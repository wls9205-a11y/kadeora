...
### 커밋: 9325dc40 → bef595fc

### 완료 항목

#### apt_sites (Supabase)
- **풀 등록**: slug `두산위브-트리니뷰-구명역`, 부산 북구 구포동 999번지
- key_features 7개, faq_items 6개 (구글 FAQ 리치스니펫)
- price_comparison 4개 단지 (반도유보라, 동원로얄듀크, 구포현대, 태성캐슬)
- analysis_text 투자분석 리포트 (입지/교육/시세/분양가전망/투자포인트/유의사항)
- nearby_facilities JSON (교통 4, 교육 3, 생활 3, 자연 2)
- transit_score 92, seo_title/description 최적화

#### 블로그 10편 시리즈 (id: 86616~86626)
1. 분양 총정리 (시한) — 두산위브 트리니뷰 구명역
2. 분양가 예측 (시한) — 구포동 시세, 실거래 기반
3. 구명역 역세권 분석 (에버그린) — 구명역 아파트, 역세권
4. 부산 북구 청약 전략 (시한) — 가점별 당첨, 추첨제
5. 지역주택조합 장단점 (에버그린) — 리스크 5가지, 체크리스트
6. 구포동 살기 총정리 (에버그린) — 교통, 학군, 단점까지
7. 주변 단지 시세 비교 (에버그린) — 4개 단지 실거래 비교
8. 북구 입주 물량 분석 (에버그린) — 공급 부족, 투자 기회
9. 두산건설 위브 브랜드 분석 (에버그린) — 시공 품질, 장단점
10. 구포동 시세 추이 (시한) — 연도별 데이터 분석
- 전편 품질게이트 통과 (내부링크 ✅ FAQ ✅ 카카오맵 ✅)
- 10편 간 상호 크로스링크 완비 (SEO 클러스터)

#### 커뮤니티 시드
- 토론방 (room_id:210, type:apt) + 시드 메시지 3개
- 피드 시드 게시글 5건 + 댓글 9개 + 좋아요 30개

#### 코드 수정
- seed-posts 크론 — 두산위브 트리니뷰 전용 엔티티 템플릿 6개 추가

### 전략 문서
- docs/DOOSAN_TRINIVUE_STRATEGY.md — 풀스택 최종 설계안

### PENDING
- [ ] 호스팅어 교차발행 10~15개 사이트
- [ ] Solapi D-24 알림톡 발송
- [ ] 5/8 모집공고 후: price_min/max 업데이트 + 속보 블로그 + 경쟁률 추적
