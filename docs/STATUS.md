...
- `5b885c1` — 프로필 사진 OAuth 연동 + CSS 간격 통일 + listing-grid 정의
- `8c27faa` — 주식 KOSPI/KOSDAQ 지수 표시 + 이슈종목 세분화 (3→15종목)
- `e8db59e` — 청약 카드 정보 확장 (5열 KPI + 시행사 + 일반/특별 비율바)
...
| 항목 | 수치 |
|------|------|
| 블로그 | **22,659편** (발행 22,659 / 미발행 0) |
| 커뮤니티 글 | 4,593편 · 댓글 2,761개 |
| 유저 | 127명 |
| 매매 실거래 | 496,987건 |
| 전월세 실거래 | 2,099,311건 |
| 단지백과 프로필 | 34,495개 |
| apt_sites | 5,525개 |
| 청약정보 | 2,694건 **(공고 100% 파싱 완료)** |
| 미분양 | 213건 |
| 재개발 | 206건 |
| 주식 종목 | 1,735개 활성 (KOSPI 279/KOSDAQ 983/NYSE 285/NASDAQ 186/지수 2) |
| 주식 무섹터 | **0건** |
| 투자 테마 | 23개 |
| 모집공고 파싱 | **2,694/2,694 (100%)** — PDF URL 2,485건, 입주예정월 2,662건, 규제지역 693건 |
| DB | ~1,409 MB |
| 크론 | 89개+ |
...
- [ ] KIS_APP_KEY 발급 (한국투자증권 API → 실시간 시세)
- [ ] FINNHUB_API_KEY 발급 (해외 실시간 시세)
- [ ] 통신판매업 신고 후 푸터 번호 추가
