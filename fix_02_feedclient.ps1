# ============================================================
# KADEORA 수정 스크립트 2/3
# FeedClient.tsx — 한글 깨짐 전면 수정 + kw.count → kw.heat_score
# ============================================================

Set-Location "C:\Users\82105\Documents\kadeora"
$enc = [System.Text.UTF8Encoding]::new($false)

$f = 'src\app\(main)\feed\FeedClient.tsx'
$c = @'
'use client';
      ...
      const io = new IntersectionObserver(
        (entries) => {
          if (entries[0].isIntersecting)
            setVisibleCount((c) => Math.min(c + 10, posts.length));
        },
        { threshold: 0.1 }
      );
      io.observe(node);
      ...
      <div>
        ...
        >
          {categories.map((cat) => (
            <button
              ...
              onClick={() =>
                router.push(
                  `/feed${cat.key !== 'all' ? `?category=${cat.key}` : ''}`
                )
              }
              style={{
                flex: 1,
                ...
                background:
                  activeCategory === cat.key
                    ...
                    : 'transparent',
                color:
                  activeCategory === cat.key ? 'white' : 'var(--kd-text-muted, #94A3B8)',
                border: 'none',
                cursor: 'pointer',
                fontWeight: 600,
                fontSize: 13,
        ...
        {isDemo && (
          ...
          >
            📢{' '}
            <span>
              현재 데모 데이터를 표시하고 있습니다. DB 연결 후 실제 데이터로 표시됩니다.
            </span>
          </div>
        )}
            ...
            {trending.map((kw, i) => (
              ...
              >
                ...
                <span style={{ fontSize: 11, color: '#64748B' }}>
                  {kw.heat_score >= 1000
                    ? (kw.heat_score / 1000).toFixed(1) + 'k'
                    : kw.heat_score}
                </span>
              </Link>
            ))}
...
Write-Host "✅ FeedClient.tsx 완료 — 한글 복원 + CSS변수 + heat_score 수정"
Write-Host ""
Write-Host "🎉 스크립트 2/3 완료! 다음 스크립트(middleware + 나머지)를 실행하세요."
