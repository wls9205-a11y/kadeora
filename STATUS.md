...
|---|---|---|
| 지번 | 부암동 705-1 | **690-8** (김해 외동 705 와 혼선으로 보인다) |
| 세대수 | 831 | `831세대 (공동주택 4개동 762세대, 오피스텔 1개동 69실)` — 831은 오피스텔 포함 합계 |
| DB | 없음 | **있다** — `서면-어반센트-데시앙` · 태영건설 · cs 100 · construction |
| 목록 | 분양예정 | **분양중·공사중** |
| 가칭 | 가칭 | 펫네임 확정 |

⚠️ 진짜 결함은 결측이 아니라 **세대수 불일치**다 — DB `total_units=211` vs 공식
아파트 762. 211 은 일반분양 회차 수치로 보인다. D4 밖(세대수)이라 자동 반영하지 않고
검수 안건으로 남긴다.
...
Node 확인이 필요한 것 둘 — 위 ①(부암동 항목 철회 + 세대수 211↔762 안건화),
그리고 AI 추출 모델을 `claude-opus-5` 로 둔 것(소스 20개 × 1콜/일 규모. 이 트랙의
«단일 경로» 라 싼 모델로 조용히 내리지 않았다).
