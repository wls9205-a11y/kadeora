...
- 보류 594 단위 처분: A 추정 분양가·분양가(추정)·분양가 시나리오 49 + B 예상 분양가+금액 단위 표 194 → 숨김 243 / C 예상 분양가(표 없음) 21 · D N가지 시나리오만 330 → 존치.
- 원장 blog_hallucination_disposition_20260915 누계 1,489(공개 잔존 16 = 1차 감사 hold).
- 유입 기준선: gsc_search_analytics 적재가 4월 이후 사실상 정지(9/1~9/7 사이트 노출 3) → DB 비교 불가. hallucination_traffic_baseline_20260915 에 기록.
- 신세대 글 112444 가 재스캔에 걸림(「2억원대 예상 분양가 기준」) — 정수 억 허용폭 ±5,000만원이 실거래 최저값과 우연 일치해 통과. 게이트 허용폭은 판정 대기.
- 110750 duplicate_same_site:112443 · 112444 비공개 → 연도 게이트 반영 후 재생성 큐.
- 26f6d860: 연도 결합 시기 게이트(esp·공고·상수만 대조) · 계약금 절차 표준 문장 · robots AI 13 UA 전면 Allow+파셋 차단 · llms.txt 인용 안내.
- ABG-A: BP·CV-N 라이브 15.

## 2026-09-15 (밤 2) — ABG 증분 2·3: 2회차 19 · 같은 현장 차단 · 공급 라벨 · lastmod · 구세대 시리즈 감사

- 2회차(BP-B-2) 19 적재 → 게시 7(112439~112445) · 차단 5 · 편집 대기 5 · 대기 2. X-2 회전당 생성 6·편집 3~5, 타임아웃 0. ABG-A 게시 13(≥10).
- ⑦ 재정의 집행: 구세대 시리즈 감사 11 → 숨김 10(추정 분양가·세대수 불일치) · 존치+hub 1(110750 더폴 금정→부곡동-223-1) · 111886 hub 해제.
- 시그니처 증보 재스캔(dry): 공개 1,675 적중 → 숨김 규칙 1,081 · 보류 594(원장 blog_hallucination_scan2_20260915). 역표본 10 오탐 0. 집행은 판정 대기(규모).
- 게이트 구멍 관측: 연도 단독(「2026년 일정」)은 수치 토큰이 아니라 대조 밖(112444 esp 없음).
...
- **E-2**(cf1c1096): cron_health_v2 2분→10분 · ci_alert 15분→1시간 · 두 함수 lock_timeout 3s. 491ms 는 꼬리(p50 0.13s·p95 3.4s·max 53s).
  기준선 24h 812s → 게이트 ≤406s(09-16 01:20Z 이후 재측정). og_batch 는 현 스케줄 없음(누적분). 실패 창은 무관 잡 동초수 정지 = DB 전역 정지 서명.
- **E-3 AB-1**(49bd69bb·bb899c1e): 「단지 개요」 정의문+불릿 · speakable 을 `#apt-overview-lead`(기존 `.site-description` 은 이 페이지에 없던 죽은 선택자).
  표본 5 라이브 확인. 준공 단지 일정 문장 제외·법인 표기 제거 수리.
- **E-4 AB-2**(ed73845d): issue-draft FAQ 규격 + 현장 문맥(합성가 배제·기준일·정비/비정비). 선점형 「예상 분양가 깊이 있게」 → 주변 시세 비교로만.
- **issue-draft 결함 2 수리**: ① 쿼터 합성 429 가 api_4xx 로 재시도 소각(48h 59건) → quota_deferred(47e3604d)
  ② 현장 지정 글감도 제목 재매칭 → 덮거나 no_entity(44d2426f). ②의 실례: CV-N 문서 카드 글감(거제·판문 P1)이 9/14 no_entity 로 소각돼 있었다.
- **E-5 BP-B**: 글감 20 적재(docs/bp70/BP-B_batch1_20260915.sql) → 발행 4편 판독에서 **LLM 추정 분양가·가짜 비교표·시기 정밀도 상향** 검출
  (「84㎡ 약 7,200~7,800만원대」「평당 12.5~14만 원」「A아파트」 표 · 「3분기(9월 중)」·없는 공고일). hub 연결·비정비 「구역」 0 은 통과.
  → **4편 is_published=false(본문 보존·가역) · 잔여 16 글감 held_quality**. 원인: 프롬프트가 «표 2개·반경 1km 시세표 필수» 를 요구하는데 실데이터를 안 준다 → 지어낸다.
- **E-6**: Naver IndexNow 실측 200(코드 주석 「422」는 낡음). apt 상세는 큐에 안 들어가고 있었다 → BP70 61 URL 적재(신규 12 urgent 제출 200).
- **E-7·E-11**(8211e215): 지역 허브가 원천 네 목록만 싣어 공고 전 현장 노출 0 → 「분양예정 단지」 섹션 + 질문형 FAQ(esp 파생, FAQPage 1블록 병합). 신규 12/12 허브 링크 라이브 확인.
- **E-9 TC**: 「미보유 10」 실측 → 8 기존재 · 리버루체는 반여4 별칭 기존재 · 실결측 1(부경경마공원역 디에트르 더 리버, 공공지원 민간임대 498) — 유사명 게이트가 EDC 디에트르 2곳과 병합 검토 큐(queued).

## 2026-09-14 — BPNR v2.1 Q·R 1차: 합성가 격리 · 폼 입력 소실 · 반응형 · 문구 분기 (중단점 NR-A 대기)

- **Q-1 1단**(723bc7df): `apt_sites.price_source` + `fn_mark_synthetic_prices(dry)` → **645** 마킹(top6 459 ∪ 지역 동일쌍≥10).
  ⚠️ 지시서 오탐 예외(공고 키 보유 제외)는 미적용 — 경기 클러스터 60행이 공고 키를 갖지만 평당가가 서로 달라 채움값이 확정적.
  상세는 «읽는 순간» 비움 → 범천1-1 og:price 0 · offers 0 · 「분양가 미공개」 렌더. 비교표 synthetic 이웃 제외 · 지역 평균·시공사·홈 동일.
  ⚠️ 2단(NULL화) 선행 조건: `sync_apt_prices` 가 NULL/0 행을 실거래 min/max 로 «되채운다» — 막지 않으면 합성가 대신 실거래가가 분양가 칸에 앉는다.
- **Q-2**(5f6828ae): synthetic 현장 analysis_text 렌더 차단 + 저장 FAQ 금액 문항 제외 — 범천1-1 「2.1억」 10 → 0.
  생성기 3본에 합성가 미탑재. 「180일 초과」 축은 생성 시각 열 부재로 측정 불가(미적용).
- **Q-7**(bd48e202): 재현(3G·CPU×4) — 하이드레이션 전 입력은 화면에 남고 state '' → 제출 시 빈칸+오류. 복원 effect 가 DOM 값 흡수 → 재검증: 값 유지·서식 적용·오류 0(전송 route abort, leads 0행 확인).
- **Q-3** SiteCard 「총」 · **Q-4** 시세표 단계 라벨 파생 · **Q-8** 비정비 공고 전 「이 현장 분양 소식 알림 받기」(SKY.V·더파크 실측, 초량3 정비판 유지).
- **R**(009ab177): 표 래퍼(상세 1·SpecTable·AptCompareTable) · repeat(7|12) minmax · BP 900/640 수렴 · nowrap 수리 7 + 게이트(기준선 65, CI 연결).
  390·768 실렌더 4표본(범천1-1·트라비스·그랑라크·가야역 롯데캐슬) 가로 넘침 0. ⚠️ 에이전트 산출의 CSS 주석 조기 종료(@layer 블록 소실 위험)를 커밋 전 수리.
- **F 진단**(읽기만): startup timeout 은 분 겹침과 무상관(최대 동시 11 인 18:00 실패 0) — 시간대 군집. 누적 최대 소비는 감시자 자신
  `cron_health_v2()` 58,777회×491ms ≈ 28,856s(2분 주기) · `cron_replace_blog_body_og_batch` 20,907s · `ci_alert_monitor_tick` 7,606s. 분산보다 이들 비용·주기가 먼저.

## 2026-09-14 — BPNR v2 P트랙: 촉진2-1 병합 · 원장 소급·촉진4 강등 · 리드 표본 3

- **P-1**(0a89ede1): `오티에르-시티즌파크` → `시민공원주변재정비촉진2-1구역-재개발`. 별칭 0 추가(기승계) · complex_units 9000 미승계 ·
  301(463, md5 e04b9381) 실측 → dead 비활성 → 정본 200 · 활성 중 「시티즌파크」 별칭 1곳. PL CSV 10키워드.
  ⚠️ 지시서의 「308」은 기존 middleware 301 맵으로 집행(리다이렉트 생산자 이중화 안 함).
- **P-2′**: site_name_applies 소급 9행(`bpnr-p2-retro-20260914`, op 단위로 분해 — 「3행」 대신 실제 쓰기 수) +
  「오티에르 시티즌파크」 후보 applied(야간 대사 보호권). 촉진4 display 제안명 → 구역명 강등. 라이브 「팔라티움」 0회 · 발행글 0편.
- **P-5 리드 표본 3**: SKY.V 센텀 · 창원 복합행정타운 · 울산 더파크 비스타동원 — `[TEST] 게이트확인` · Apps Script 302 → `leads` 3행
  (site_slug·interest_region 귀속 정확, inquiry_type 진행상황알림). ⏳ 시트·메일 확인과 테스트 행 삭제는 Node.
  ⚠️ 1회차 SKY.V 는 하이드레이션 전 입력이 지워져 검증 오류(행 0) → 1회 재시도 성공. 반복 실행 아님(dedupe 미발생 확인).
  관찰: subscription·pre_announcement 신규 현장에도 pre_notice CTA 「이 구역 진행 상황 알림 받기」 — 「구역」 문언 부적합(문구 안건).

## 2026-09-14 — BP70 1차: 가야1 병합 · 문서 카드 시드 12 · CV-N `?doc=` 모드 (중단점 BP-A 대기)

- **가야1 병합**(138cd5dc): `부산-가야-재개발` → `가야1-재개발`. 조각 3 컷·별칭 4 승계 · 301(462쌍, TSV md5 DB 대조) 실측 후 dead 비활성.
  PL 착지 대조 CSV `docs/bp70/BP70_PL_url_gaya_2026-09-14.csv`(12키워드). ⏳ relink 는 판정회신 C 로 «즉시 집행» 판정 — CC 권한 분류기가 sa.py 를 막아 Node 터미널 몫.
- **§4 폐기**: LEAD_ELIGIBLE 이 정비 6단계를 이미 포함(라이브 초량3·삼보·범천1-1 폼 실측). §1-10 단축 별칭 승계도 기해소.
- **§2 원문 재분류**(ece51a56): 명단 30 → 제외 9(기분양·준공·공공·DB 기존) · 시드 13 · 보류 9. 라이브 3회전 → **seeded 12** + 내덕 1은 유사명 게이트(`김해-내덕지구-중흥-클래스-더퍼스트`)로 병합 검토 큐.
  신규 12 전부 200·폼 렌더. 미상 3(덕하리·주촌·복합행정타운)은 ad_blocked — 복합행정타운은 보도상 민간(태영) → 공급유형 판정 안건.
- **CV-N 문서 카드**: `cvn-name-watch?doc=BP70_20260914` — applyCandidate 한 문. 거제시그니처·판문 레이크써밋 웰가 T-B applied(순위 표적 4+4 · P1 글감 각 1) ·
  드메인 데시앙 단건 pending · 팔라티움·엘리미엄 bid held. 골든셋 문서 카드 4케이스.
- ⚠️ **원장 밖 병행 쓰기 관측**: 거제(builder·별칭 4형·supply·ad_blocked)와 촉진4(builder 두산건설·display_name 「두산위브더제니스 팔라티움」·별칭 3형)가
  이 세션 실행 «전» 에 site_name_applies 없이 바뀌어 있었다. 촉진4 표시명은 조사상 «제안명 수준»(조합 의결 보도 없음) — 강등 여부 판정 안건.
- 촉진4 confidence=conflicting(세대수 840 vs 849) · 동문디이스트 P0-4 검수 종결(준공 실거주, stage 유지).
...
### ③ VIEW 사망 원인 — 「크론 수리」가 아니라 «스케줄 부재» 였다

RULES#145 의 4면 검증을 «역방향» 으로 돌렸다:
...
| `.github/workflows` | 없음 |
| `cron.job`(pg_cron) | **없음** |
| src 호출자 | `admin/god-mode/route.ts:50` — 「전체 실행」 팬아웃 목록에만 |

라우트는 살아 있었다(13,569바이트). `cron_logs` 12회 success 는 **사람이 god-mode 버튼을 누른 날들**이고
8/26 이 마지막이다. 즉 「8/26 이후 생성 정지」의 원인은 크론 사망이 아니라 **자동이었던 적이 없다**.

⛔ RULES#145 의 거울상이다 — 그때는 「스케줄을 끊어도 버튼이 남아 사람이 다시 부른다」였고,
   여기는 **「버튼만 있고 스케줄이 없어 아무도 안 부른다」** 다. 「크론이 죽었다」로 읽었으면
   엉뚱한 곳을 고쳤을 것이다.

⚠️ 후보 고갈도 아니었다 — RPC 는 지금도 100건을 돌려준다(8/25 실측과 동일).

수리: pg_cron `naver_blog_content`(jobid 178 · `30 22 * * *` = 07:30 KST) 등록 + 1회 발화.
**결과: success · 3건 생성 · 59.5초 · pending 16 → 19.** 13일 만의 소생을 실증했다.

⚠️ 소스 조건 교체도 방안서 전제가 달랐다. RPC 는 이미 «인기글 기준이 아니었다» —
부울경 + 분양/정비 단계로 tier 1~3 을 매기고 `view_count` 는 같은 tier·같은 날 안의 정렬일 뿐이다.
그래서 「교체」가 아니라 **tier 0(CV-N 선점 글감) 을 얹는 최소 개정**을 했다. 기존 티어 로직 무접촉.

### skipped 77 의 정체 — 사람이 누른 것

`admin/naver-syndication/route.ts:170` 의 「건너뛰기」다. 사유 컬럼이 없어 이유는 남지 않는다.
⚠️ 그리고 이 숫자가 방안서 §2-2 에 대한 경고다 — 4/12~8/24 실적이 **published 6 vs skipped 77** 이다.
대기함이 쌓이면 사람이 게시 대신 건너뛴 이력이고, 「하루 3건 5분」이 지속되지 않은 전례가 있다.

### ④⑤ — 「잔여 구멍」 둘은 구멍이 아니었다

**④ 블로그 FAQPage JSON-LD: 이미 있다.** `parseFaqFromContent` → `FAQBlock` 이 화면 문구와
FAQPage JSON-LD 를 «같은 배열에서» 낸다. 실측: 최근 30일 apt 발행 170편 중 FAQ 제목 155편,
그중 **파서가 잡을 형식 154편**(제목만 있고 파싱 불가는 1편).
⛔ 신설했으면 **해로웠다** — `FAQBlock` 주석이 「한 페이지에 FAQPage 는 하나여야 한다」고
   못박고 있어 중복 JSON-LD 가 됐다. LB-5 의 FAQ 오인에 이어 **두 번째**다.

**⑤ soft-404: 현재 없다.** 라이브 실측 —
`/apt/region/없는지역999` **404** · `/apt/region/부산` 200 · `/apt/sites/<없는slug>` 308→404 ·
`/blog/<없는post>` **404**. 메모리의 「위험군 7곳」은 그 사이 수리된 것으로 보인다.
⚠️ 동적 세그먼트 2곳 + blog 기준이고 전수는 아니다.

### ⑥ 스냅샷 — 「거짓 0」을 고쳤다

`naver_*_ranked` 가 `date = CURRENT_DATE` 라, 측정 크론이 아직 안 돈 시각에 보면 0 이 찍혔다.
실측 05:5x: 오늘 0행 · **마지막 측정 9/7 에 webkr 80 · blog 80**. 즉 뷰를 읽는 사람은 거짓 0 을 봤다.
→ 최근 «측정일» 기준으로 바꾸고 `naver_rank_last_date` 를 컬럼으로 노출했다.
**수리 후: 13 · 1 · 2026-09-07.** 0 을 건강으로도 병으로도 읽지 않게 하려면 기준일이 함께 나와야 한다.

### ④(표적) — 등재는 이벤트 시점, 수명은 밤

`registerRankTargets` — 확정된 이름(T-A·T-B)만 등재한다.
⛔ pending·merge_queue·held 는 넣지 않는다. 확정되지 않은 이름을 재기 시작하면 표적 풀이
   «답이 없는 질문» 으로 차고, 일 측정 80 안에서 새 예정명이 밀려난다.

`fn_nv5_rank_target_lifecycle()` + pg_cron `50 22 * * *`(측정 뒤).
⚠️ **강등까지만 한다.** 은퇴(tracked=false)는 측정을 끊는 것이라 되돌리기 어렵다 — 사람이 본다.
⚠️ **아직 한 번도 진입하지 못한 표적은 건드리지 않는다.** 그건 아직 답이 안 나온 질문이고,
   내리면 영영 답을 못 듣는다 — 선점 트랙이 재려는 구간이 정확히 거기다.
첫 실행: **P1 56 → 54**(진입 30일+ 순위 안정 2건 강등 · 상한 초과 0).

### ② 번들 — 기본 OFF 로 넣었다

⚠️ 방안서가 「미실측 가정」으로 표시한 자리에 **구체적 위험**이 있다: issue-draft 의
`title_similar`(pg_trgm 0.35) 중복 차단 때문에 「아크로 라로체」와 「아크로 라로체 분양가」가
**서로를 막을 수 있다**. 게이트를 완화하지 말라는 지시가 있으므로 스위치(`cvn.bundle_enabled`,
기본 false)를 두고 3단 퍼널을 실측한 뒤 켠다 — L2 게이트와 같은 정신이다.

### 판독 정정 1건 (내 것)

P1 표적이 「문서 47 vs 실제 56」이라 보고했으나, 이는 문서 오류가 아니라
**세션 A 가 05:48 에 `NV5-seed` 로 9건을 수동 등재한 결과**였다. 원인을 확인하지 않고
문서 부정확으로 귀속한 것을 정정한다.

### 미확인으로 남긴 것

- **리드타임 기준선 8일** — `keyword_rank_daily` 에 「복산」 rank 기록이 0건이라 내 쿼리로
  재현하지 못했다. A 측 관측으로 두고, §4 지표가 앞으로 스스로 측정한다.

---

## 2026-09-08 — CV-N 첫 회전 판독 후속(LB-6): 결함 4건 수리 · 오귀속 1건 적발

세션 A 가 낮에 수동 점화한 첫 회전이 결함 셋을 드러냈고, 그 원장을 교차판정하다 **넷째를 찾았다**.

### ① 후보 큐가 «조용히» 0행이었다 — 원인은 인덱스 모양

```
CREATE UNIQUE INDEX ... (source_url, proposed_name) WHERE source_url IS NOT NULL
```

Postgres 의 `ON CONFLICT (a,b)` 는 부분 인덱스를 추론하려면 «동일한 WHERE 술어» 를 함께 줘야 한다.
PostgREST 의 `onConflict` 는 그 술어를 실을 수 없어 **42P10** 으로 떨어졌고,
호출부가 `error` 를 읽지 않아 「분류 1건 · 큐 0행」이 됐다.

⛔ 「미매칭을 폐기하지 않는다」는 CV-N 의 상속 원칙인데, 그 원칙이 **코드가 아니라 «침묵» 때문에**
깨져 있었다. 인덱스에서 WHERE 를 떼고(중복 0건 사전 확인), 적용기가 `candErr` 를 읽어
`reason` 에 담아 돌아오게 했다. ⚠️ 던지지는 않는다 — 한 건이 회전 전체를 죽이면 더 나쁘다.

### ② 예산 분모가 «장애» 를 세고 있었다

실측: 24h 창 `error` **204** · success 13 → 「일 예산 소진 **211/200**」으로 회전이 잠겼다.
4xx/5xx 는 과금되지 않는데 세고 있었고, 창이 롤링 24h 라 **새벽 크레딧 사망기가 낮을 계속 잠갔다**.
그대로면 오늘 밤 21:10 예약 회전도 같은 문에서 침묵사할 운명이었다.

→ 분모를 `status='success'` 로, 창을 **KST 달력일**로 고쳤다. 폭주하는 실패는 예산이 아니라
파수꾼(`fn_llm_silence_watch`)의 몫이다 — 역할이 갈린다.
A 가 임시 상향한 450 은 **200 으로 복귀**시켰다.

### ③ 브랜드관 URL 위생 — 지어내지 않고 «비웠다»

프로덕션 회전 failures: 디에이치·트리니빌 «페이지 응답 없음», e편한세상 «본문 5자»(JS 렌더).
⚠️ 로컬에서는 외부 HTTP 가 막혀 있어 내 눈으로 재확인하지 못했다 — 근거는 프로덕션 원장이다.
⛔ 추측으로 새 URL 을 넣지 않았다(가칭 배제와 같은 정신). `registry_url` 만 비우고 사유를 남겼다 —
크론 퇴역 규약(RULES#145)의 방식이다. 브랜드 «행» 은 살려 뒀다: sa.py 의 길이필터 A 와
`alias_is_brand` 가 그 토큰을 쓰고 있고 브랜드관 접속과 무관하다.
⛔ e편한세상에 헤드리스 브라우저를 붙이지 않는다 — 어댑터가 「URL + 토큰」을 깨는 순간 파서가 된다.

### ④ 내가 찾은 넷째 — «오귀속» (가장 나쁜 결함)

`issue_alerts` 에 이런 글감이 앉아 있었다:

```
title: 「우동2재개발 — 아크로 광안」    apt_site_id: 596f39cc(우동2재개발 · 남구 · 삼한종합건설)
```

**「아크로 광안」(DL이앤씨·수영구)이 남구의 무관한 현장에 붙었다.** 원인은 내 코드의 fallback:

```ts
projectName: card.projectName ?? seed?.site.name ?? null
```

AI 가 사업명을 못 주면 «검색 씨앗 현장 이름» 으로 대체해 그 현장에 매칭했다.
검색 씨앗은 «어디를 찾아봤는가» 일 뿐 «기사가 무엇을 말하는가» 가 아니다.

⛔ 이것은 「미매칭 폐기」보다 나쁘다 — **틀린 매칭을 만든다.** DART 매칭 규칙(「애매하면 검수 큐」)과
정면으로 어긋나고, 「대연3↔디아이엘」이 막으려던 바로 그 형태다.
게다가 LB-4 가 이 글감을 **P1 으로 최우선 생성**하므로 그대로 뒀으면 틀린 기사가 나갔다.

수리: fallback 제거(사업명 없으면 미매칭 → 큐에 남는다) + 글감 제목도 매칭 확정 시에만 현장명을 쓴다.
오귀속 행은 `apt_site_id=null · blocked` 로 막았다. 사냥감은 `site_name_candidates` id 12 에 보존돼 있다.

### 세션 A 판독 중 정정 1건

A 는 워처의 pending 을 「충돌→병합 규칙이 실전 작동」으로 읽었으나, 원장의 outcomes 는
`tier:null · res:"pending"` 이다 — 이건 **「교차 출처 부족 — 1건 더 기다린다」** 다.
유일성 검사는 `apply=true` 일 때만 도는데 `decideTier` 가 이미 `apply=false` 를 줬으므로
**돌지도 않았다.** `merge_queue`(id 12)는 A 가 수동 등재한 것이고, 충돌 규칙은 아직 실전 검증 전이다.

### 광안A 이중 — 판정 재료만 올린다

| | 광안A 재개발 `5435f730` | 부산 망미 재건축 `62a9f1cd` |
|---|---|---|
| 동 | 광안동 | **망미동** |
| 시공사 | 대림산업(구명) | DL이앤씨 |
| 세대수 | 2,780 | 2,600 |
| 별칭 | 광안A구역 … | **광안A구역 · 망미2구역 · 아크로 광안** |
| 광고 착지 | **9건** | **12건** |

⚠️ **「광안A구역」이 양쪽에 있는 유일성 위반은 확실**하고, 광고 21건이 분산 가동 중이라 시급하다.
⛔ 그러나 «같은 사업인가» 는 내 손의 근거로 판정할 수 없다 — 동이 다르고 세대수가 다르다.
   라로체 때처럼 정본을 고르려면 그 판정이 먼저다. 오늘 오전에 고친 병을 추측으로 재생산하지 않는다.
   병합 방향과 착지 21건 교체는 **열거 승인 대상**으로 남긴다.

---

## 2026-09-08 — LB 집행: LLM 원장 관문화 · 1:9 쿼터 · 미래지향 큐 (지시서_LB FINAL)

CV-N 점화 직후 같은 세션에서 LB-1~5 를 집행했다. Node 몫 0(킬스위치·config 기본값만).

### 가장 큰 것 — 관측 구멍이 예상보다 훨씬 컸다

지시서는 「원장이 없어서 침묵을 못 봤다」고 적었는데, 실측은 그보다 나빴다:

| | 실측 |
|---|---|
| Anthropic 호출부 | **56파일** (grep 55 + SDK 1) |
| 원장(`llm_usage_logs`)을 거치던 곳 | **4개** |

⛔ **오늘 오전 제가 보고한 「48h LLM 성공 0건」은 그 4곳의 이야기였다.** 크레딧 소진이라는
결론은 맞았지만 실제 침묵 규모는 원장이 말한 것보다 훨씬 컸고, 51곳은 «애초에 관측 밖» 이었다.
**56번째는 grep 으로도 안 보였다** — `apt-summary-gen` 이 SDK(`@anthropic-ai/sdk`)를 써서
`api.anthropic.com` 문자열이 코드에 없었다. 원장에도 쿼터에도 안 잡히는 완전한 사각이었다.

### LB-1 — 새 표를 만들지 않았다

⛔ 지시서의 `llm_call_log` 신설을 따르지 않고 **기존 `llm_usage_logs` 에 컬럼 4개를 더했다**
(`category` · `api_kind` · `site_id` · `post_id`). 원장이 두 벌이면 반드시 갈라지고,
오늘 크레딧 사고를 판독한 근거가 바로 그 표였다 — 새 표로 옮기면 이력이 끊긴다.
CV-B 「이중 생산자 금지」와 같은 축이다.

관문 `src/lib/llm/gateway.ts` 에 문 넷: `anthropicFetch`(fetch 호환) · `anthropicJson`(Outcome) ·
`anthropicCreate`(SDK) · `anthropicPollFetch`(배치 조회 전용).
⚠️ **fetch 와 «같은 모양»** 으로 만든 것이 핵심이다 — 호출부 56곳의 응답 처리가 제각각이라
관문이 같은 시그니처여야 기계 치환이 안전해진다. 실제로 56곳 전부 함수명 교체만으로 끝났다.

⛔ **batch poll 은 원장에 넣지 않는다.** `GET /v1/messages/batches/{id}` 는 크레딧을 태우지 않아서,
넣으면 파수꾼이 poll 성공을 성공으로 세어 침묵을 놓친다 — 오늘 사고의 재발 경로다.
그런데도 `anthropicPollFetch` 라는 «문» 을 지나가게 했다: 규칙이 「관문 밖에서 직접 부르지 않는다」
한 줄이어야 CI 게이트가 성립하고, 예외를 grep 패턴으로 표현하면 그 패턴이 곧 구멍이 된다.

CI 게이트 `scripts/llm-gateway-gate.sh` — `quality` 잡에 등록. URL 직호출과 **SDK import 양쪽**을 본다.
⚠️ 게이트가 처음 잡은 것은 `usage-tracker.ts` 의 «주석 예제» 였다. 오탐이 아니라
문서가 낡은 방식을 가르치고 있던 것이라 주석을 고쳤다.

### LB-2 — 파수꾼을 «양방향» 으로 검증했다

`fn_llm_silence_watch()` + pg_cron 30분. 최근 6h 시도 ≥10 이고 성공 0 이면 `admin_alerts` 1행(12h 중복 억제).
⚠️ `skipped`(쿼터로 안 쏜 것)는 «시도가 아니다» — 시도로 세면 쿼터가 걸린 날마다 울어
알림이 무의미해진다(RULES#145 「빨강의 상시화」).

**긍정형 검증**(「실측했다」가 틀리는 자리):
...
→ 그날 파수꾼이 있었다면 30분 내에 울었다. 「안 운다」만 확인하고 넘어가지 않았다.

크레딧 회복도 확인: 20:00~02:00Z 전패(400) → **03:00Z 시간대부터 성공 복귀 · 04:00Z 는 1/1**.

### LB-3 — 쿼터는 «호출부를 안 고치고» 걸린다

초과 시 예외를 던지지 않고 **합성 429 Response** 를 돌려준다. 기존 `if (res.ok)` 분기가
그대로 skip 으로 동작해서 56곳 어디도 손대지 않았다. SDK 판은 `null` 을 준다.
config: `llm.daily_budget_calls=200` · `stock_share=0.10` · `quota_enabled=true`.

⚠️ 지시서가 정하지 않은 자리 하나를 판단했다 — **economy·tax·life·general 을 쿼터 «안»(finance)으로
넣었다.** 1:9 의 취지는 「크레딧의 9할을 부동산에」이므로 비부동산 콘텐츠를 예외로 빼면 취지가 샌다.
바꾸려면 `llmCategoryOfContent` 한 줄이다.

⛔ 계측이 실패하면 «막지 않는다». 셀 수 없다고 생성을 멈추면 그날 콘텐츠가 통째로 사라진다.

### LB-4 — P1 이 문턱에서 잘리고 있었다

`sortForGeneration` = (P순위 ASC, 지역 가중 DESC, **점수 DESC**, 신선도 DESC).
⚠️ 지시서 정렬 키에 점수가 없었지만 넣었다 — 빼면 같은 P 안에서 저품질 글감이 앞으로 온다.

⛔ **실제 결함을 잡았다.** issue-draft 선정이 `final_score >= 25` 인데 CV-N 이 넣는
`cvn_name_event` 는 점수를 안 줘서 기본값 0 이었다 — **P1(이름이 태어나는 순간)이 문턱에서
한 건도 안 뽑히고 있었다.** 양쪽을 고쳤다: 선정 쿼리에 `source_type` 예외 + N-2 가 45점 부여.

### LB-5 — 「FAQ 신설」은 이미 있는 것이었다

⚠️ 지시서가 종결 대상으로 적은 「issue-draft 생성글에 FAQ 없음」은 **이미 필수 구현**이다
(프롬프트가 5~8개 요구 · 검증기가 누락을 차단). 지시서는 3~5문 신설이라 했는데
그대로 했으면 **후퇴**였다. 손대지 않았다.

실제로 없던 것만 넣었다: 부동산 전용 규격 블록(제목 25~30자 검색어형 · 3줄 요약 ·
공급정보 표 · 하이픈 절단 금지 · 「부울경」 문면 금지) + **현장 컨텍스트 주입**
(`apt_site_id` → 예정명·구역명·시공사·세대수 + `/apt/{slug}` 내부링크).
⚠️ 제목 자수는 **부동산 카테고리에만** 25~30 을 건다. 주식 글까지 바꾸는 것은 범위 밖이고,
SEO 게이트는 15~80 이라 양쪽 다 통과한다.
⛔ 사실 규율을 프롬프트에 박았다 — 예정명은 확정 발표된 것만 단정하고, 선정 전이면
「제안 단지명」이라고 밝힌다. CV-N 의 T-C 차단과 문면이 일치해야 한다.

### 실측 — 1:9 현황

14일 발행: stock 61 + finance 17 = **78(60%)** vs apt 48 + unsold 3 = **51(39%)**. 약 6:4 역전.
⚠️ 발행 편수는 «대리 지표» 다. 크레딧은 생성 «시도» 에서 타므로, 이제 원장이 호출·토큰
양축으로 실측한다. 첫 수렴 수치는 3일 뒤 다이제스트에 자동으로 실린다.

### 남은 것

- LB-5 의 **FAQPage JSON-LD 동반**은 블로그 렌더 경로 작업이라 이 커밋에 넣지 않았다.
  현재 FAQ 는 본문 마크다운으로만 나가고 구조화 데이터가 없다(apt 페이지엔 있다).
- 쿼터 첫 발동·1:9 수렴은 3일 뒤 다이제스트가 수치로 보고한다.

---

## 2026-09-08 — CV-N 한방 배포 · 이층 게이트 L2 red(섀도) · **AI 크레딧 소진 발견**

설계서 CV-N FINAL v1.2 「고」(①~④ 포괄) 집행. 판정 2회 외 사람 개입 없이 체인 5단을 돌렸다.

### 가장 중요한 것 — 이 트랙이 «다른 사고» 를 발견했다

L2 셀프테스트가 red 인데 **원인이 코드가 아니다**:

```
AI 추출 실패: bad_json HTTP 400
"Your credit balance is too low to access the Anthropic API"
```

`llm_usage_logs` 48시간 실측 — **성공 0건**:

| cron | 호출 | 성공 | 실패 |
|---|---|---|---|
| `issue-draft` | 327 | **0** | 327 |
| `builder-presale-crawl` | 6 | **0** | 6 |
| `cvn-name-watch`(L2) | 1 | **0** | 1 |

⛔ **CV-N 이전부터 AI 파이프라인 전체가 죽어 있었다.** `issue-draft` 는 327회를 실패하며
재시도를 돌고 있었고 아무도 몰랐다 — RULES#145 가 적은 「빨강의 상시화」와 같은 병이다.
CV-N 은 그 위에 얹힌 것이 아니라 그것을 **드러낸** 셈이다.
⚠️ 크레딧 충전 전에는 N-1·N-2 가 카드를 한 장도 못 만든다. 충전 후 `cvn-selftest` 재호출이
남은 유일한 사람 행위다.

### 이층 게이트가 설계대로 동작했다 (RULES#147)

- **L1**(AI 무호출) — vitest 58파일 **880 통과**, `test_name_pool` ⑩ 신설 포함 전건 통과, `next build` 성공 → push
- **배포** — `dpl_D3Jb…` READY · production · `70f64bb7`
- **L2**(배포 환경 실추출) — red → `cvn.autoapply_enabled` **false 유지 = 섀도**.
  크론은 돌되 원장만 쌓이고 적용 0. `admin_alerts(type=cvn_selftest)` 에 보고 적재됨.

⛔ **배포는 죽지 않았다.** 위험할 때 스스로 안전 모드로 내려간다는 것이 v1.2-A 의 요구였고,
   그것이 «가정» 이 아니라 실제로 그렇게 일어났다. 로컬에 `ANTHROPIC_API_KEY` 가 없다는 것도
   이 세션에서 다시 확인했다 — L1 을 AI 무호출로 가른 판단의 근거다.

### N-0 — 실물이 설계서보다 정확했다

- **라로체 3각의 심장**은 `e2bb27ca` 였다. 이름은 촉진3(라로체)인데 **별칭이 촉진1을 가리켰고**
  `dong` 도 촉진1의 부암동이었다(촉진3은 범전동). 촉진1 별칭은 «진짜» 촉진1(`f411b641`)로 보내고
  「라로체」는 정본 촉진3(`69c148ec`)이 갖는다. ⛔ 오표기 별칭을 정본으로 승계하지 않았다.
- **「해운대 아크로」** — 우동1(1,476)·반여3(915) **둘 다 DL·해운대**라 어느 쪽도 가리키지 못한다.
  양쪽에서 제거. ⛔ 대체 별칭을 지어내지 않았다(가칭 배제).
- **sigungu 이형 3행** = 「창원시」(정본·50+행) vs 「창원시 성산구」1 vs 「창원시 의창구」1.
  ⚠️ `518a9490`(명서1)은 별칭이 «정확히 3» 이라 트리거 통째 교체를 **한 칸 차이로** 비켜갔다.

분모 대조 — 설계서 353/155 vs 실측 343/164. **builder 공란은 218/137 로 완전 일치**하므로
같은 집합이고, 차이는 정비형 판정 정규식의 미세차다.

### RULES#146 — 트리거 컬럼 먼저, 별칭 마지막
...
→ 별칭«만» 쓰는 UPDATE 는 트리거를 안 깨우고, `builder` 를 쓰는 UPDATE 는 깨운다.
사정권 실측: 활성 6,285 중 별칭 3 미만 **467건(7.4%)**.
순서로 못 막는 자리는 쓰기 대장 `site_name_applies` + 갭워치 야간 대사가 되살린다(`heal_realias`).

### 광고 착지 — 열거가 11이었고 실물이 17이었다

`A_분양` 라로체 10(일치) + `부울경_E_정비사업_1` 우동1 **7**(설계서 1). Node 가 17건 전부 승인.
**요청 17 · 실패 0 · 재조회 실제 반영 17/17** · `verify` 전수에서 라로체·우동1 깨짐 0건.

⛔ **API 가 200 을 주고 조용히 무시한다.** `fields=finalUrl` 도 `fields=zzz`(제어군)도 200 이었고
   재조회하니 하나도 안 바뀌어 있었다. 정답은 `fields=links` + **GET 으로 받은 전체 객체**다.
   `cmd_off` 가 세워 둔 「200 이 아니라 실제로 바뀌었나를 본다」가 없었으면
   **「17건 교체 완료」라고 틀리게 보고할 뻔했다.**
...
⚠️ `name_watch` 를 `gap_watch_daily`(`40 21`) **앞**에 뒀다 — 그날 예정명이 같은 날 지표에 실려야 한다.
갭워치 7 → **10지표**: `cvn_name_preempt` · `cvn_brand_alias_coverage` · `cvn_alias_heal`(0이 정상).

### 지표가 배포 직후 자기 결함을 드러냈다

갭워치 예행 실측 — `cvn_brand_alias_coverage` **27%** · `cvn_alias_heal` **0** · `cvn_name_preempt` **0**.

⚠️ 이 **0 은 현실이 아니라 정의의 결함**이었다. 「9/8 채팅 주입 별칭 8건」을 원장에 사후
등재하면서 쓰기 대장에도 `alias_add` 를 함께 남겼더니, 「대장에 alias_add 가 없으면 선점」
이라는 계산이 그 8건을 «새로 넣은 것» 으로 셌다. 그런데 그 8건은 «보도로 온 것이 아니라»
이미 갖고 있던 것이다 — 분모에 들어간 것 자체가 틀렸다.
→ 분모를 `source LIKE 'news:%'` 로 한정했다. 브랜드관·수동 주입은 선점률을 말하지 않는다.

⛔ 지표를 만들고 «한 번도 안 재보고» 넘어갔으면 첫 주간 다이제스트가 「선점률 0%」라는
   거짓을 들고 왔을 것이다. 배포 직후 예행 1회가 그걸 잡았다.

### 「9/8 채팅 주입 별칭 8건」 — 실측으로 특정해 등재 완료

설계서에 목록이 없어 DB 로 좁혔다. 오늘 **01:22:49 동일 타임스탬프**로 4현장이 갱신됐고
각각 예정명이 띄어쓰기 有/無 «쌍» 으로 들어 있었다 — 4×2 = **정확히 8건**:
촉진3(아크로 라로체) · 중동5(아크로 해운대) · 문현3(BIFC 시그니처) · 문현1(IFC 자이 더 스카이).

`site_name_candidates` 8행(`resolution=applied` · `source=manual:chat-2026-09-08`) +
`site_name_applies` 8행 등재. ⚠️ 등재 SQL 에 «실재 확인 가드» 를 걸었다 —
없는 별칭을 applied 로 적으면 원장이 거짓이 되고 야간 대사가 그 거짓을 근거로 별칭을 만들어 낸다.
등재의 실익은 이제 **이 8건이 야간 대사의 보호를 받는다**는 것이다.

### 범위 밖 발견 (손대지 않고 기록만)

- **창원 레코드 다수의 `dong` 이 「도계동」으로 일괄 오염** — 마산·진해 건까지 그렇다.
- `sa.py verify` 전수: DB에 없는 슬러그 **399**(`region/부산` 78·`region/경남` 42·`region/울산` 38 은
  지역 페이지 착지라 verify 의 slug 오탐).
- **문현1 세대수** — 설계서 3,813 vs DB 2,568. 어느 쪽이 참인지 근거가 없어 골든셋 기대값에서 뺐다.
  하드로 박으면 코드와 무관한 red 가 된다(v1.2-A 가 막으려던 형태).
...
AD-4 가 오전에 noindex·링크 회수까지 «가역» 으로 해 둔 것을 판정에 따라 접었다.
구매 성립 지점 `POST /api/payment` 는 **410 봉인**(삭제 아님) — 표면이 없어졌는데 성립 지점이
열려 있으면 남은 주소를 직접 두드리는 요청 하나로 되돌리기 어려운 결제가 성립한다.
`GET`(조회성)·points·등급 데이터·프로필 표시는 무접촉.

⚠️ **`permanent: true` 는 301 이 아니라 308 을 낸다**(Next 규약). 지시서 게이트 문언은 301 이었으나,
이 리포의 기존 영구 리다이렉트(`/feed`·`/rss`·`/sitemap`)가 «프로덕션에서 전부 308» 이라 선례를 따랐다.
검색엔진은 둘을 같게 다루고 308 은 메서드까지 보존한다 — 의도(영구·색인 통합)는 그대로다.

⚠️ AD-4 가 「/premium 은 남긴다 — redirect('/shop') 이라 지우면 유료 안내로 갈 길이 사라진다」로
남겨 둔 진입 링크 2곳(푸터·/more)과 sitemap 등재 1건이 **오늘 근거를 잃었다**(갈 곳이 홈이 됐다).
「프리미엄」이라 적고 홈으로 보내면 카피가 동작을 속인다(DS_RULES#5-3) — 같이 회수했다.

**전자상거래 표기 의무는 판매 표면 부재로 해소**된다. ⛔ 재개하면 상호·사업자번호·
통신판매업신고 표기가 «선행» 조건으로 부활한다 — 봉인 주석에도 같은 조건을 박았다.

외부 눈: `.github/workflows/uptime.yml` 신설(15분 간격, 정각 회피 7분 오프셋).
기존 health-check 는 Vercel 크론이라 사이트와 함께 죽는다 — 자기를 자기가 보는 관측은
전면 정지를 못 잡는다(8/21 결제 정지형). UptimeRobot 대신 둔 이유는 가입 0.
⛔ ci.yml 과 별파일이다 — 섞으면 「사이트가 죽음」과 「코드가 깨짐」이 같은 빨강이 된다.
...
걸쇠 3항 통과 — 30건 전량 ON · 오늘 집행 80건 및 ① 하향 15건과 교집합 0 · 그랑라크 양측 무접촉.
게이트 **30/30 `userLock=true`**. 집행 후 A_분양 생존측 `bid=100·ON`, 일광 15건 `{70}·{False}` 재확인.
**오늘 누적 OFF 110 고유 · 110/110 잠김**(② 1 + ③ 2 + ⑤ 47 + ⑥ 30 + Q2 30). 상세 `docs/pl/집행로그_Q_20260907.md`.

## 2026-09-07 — Q 후속 소커밋: CSP Report-Only 수집 개시 · /apt/ranking 깨진 링크 수리

**CSP** — 집행 정책 무변경, 옆에 Report-Only 초안을 세워 «unsafe-inline·unsafe-eval 을 빼고
nonce 로 가면 무엇이 깨지는가» 한 축만 잰다(호스트 허용목록은 그대로). 두 정책은 `CSP_PARTS`
배열에서 «파생» 시켜 갈라지지 않게 했다. ⚠️ 이제까지 nonce 는 매 요청 만들어 `x-nonce` 로
내보내면서 소비처가 **0건**인 죽은 값이었다 — 여기서 처음 쓴다.

수집은 살아 있다(프로덕션 로그 실측) — 네이버 광고 유입 페이지에서 `script-src-elem` /
`blocked: inline` 위반이 실제 사용자로부터 들어온다. 판독은 1주 뒤 세션 A.

그 과정에서 잡은 둘: ⓐ 브라우저 POST 가 204 를 받고도 로그를 안 남기는 구간이 있어
`[CSP_REPORT_UNPARSED]` 자가진단을 달았다 — 없으면 「빈손인데 왜 빈손인지 모르는」 채로
한 주가 간다. ⓑ **미들웨어 CSRF Origin 검사가 보고 POST 를 403 으로 떨궈** 수집이 새고
있었다(`Origin: null` 경로). 이 라우트만 예외로 뺐다 — 바꿀 상태가 없는 로그 싱크다.
대조군 `/api/polls` 는 여전히 403 인 것을 확인했다.

**/apt/ranking** — 지시서는 「프로덕션 정상이면 dev 한정 기록만」이었으나 **프로덕션도 404**였다.
맨 경로에 page.tsx 가 없고 라우트는 `[region]/[category]` 뿐이다(하위 76조합은 사이트맵에
실려 있고 200 — 깨진 건 입구 하나). 레일·archive 의 「청약 경쟁률 랭킹」을 실재 경로로 깊게 걸되,
「전국」에는 랭킹 허브가 없으므로(404 실측) 17 시도일 때만 그린다(DS_RULES#5-4).
⚠️ `/apt` 는 region 이 「전국」이라 이 입구가 사라진다 — 404 로 보내던 문이라 기능 손실은 0이지만
입구가 줄어든 것은 사실이다. 맨 경로 허브 신설은 새 색인 표면이라 별건.

커밋 `6b563c12` · `ab6760ff` · (CSRF 예외). CI 3241 초록.
## 2026-09-07 — 판정회신 Q(`!` 6항) 집행 · OFF 80 · 하향 11

회신 ①하향 ②OFF ③뒤집기 ④보류 ⑤승인 ⑥승인. 「카더라」 캠페인 한정 · 삭제 0 · 구조 변경 0.
게이트 전수 통과 — 집행 후 재조회 **OFF 80/80 `userLock=true`** · **일광 15/15 `bidAmt=70`**(링크·잠금 불변).
상세는 `docs/pl/집행로그_Q_20260907.md`.

갈림 셋: ⓐ 일광 15건의 계정 실값이 문서(300·100)가 아니라 120·100·70이었다 — 실물에 ×0.5 를 걸었고
네이버 최소 입찰가 70원(12캠페인 5,632 개별입찰 중 70원 미만 **0건** 실측)에 바닥을 쳐 전 건 70원, 실변경은 11건이다.
ⓑ **`PL_C1_BC_OFF_뒤집기` 32행 중 30행(9/6 「승인분」 법인명 15 + 그룹중복 15)이 아직 ON 이다** —
§3 게이트가 80(1+2+47+30)으로 못 박아 손대지 않았다. 판정거리로 남는다.
ⓒ 키워드 입찰 PUT 규약을 세 번 틀렸고(배열·nccAdgroupId·useGroupBidAmt) 세 번 다 400 이라 계정은 무변경이었다 —
규약은 `tools/naver-sa/pl_q_bid_ilgwang.py` 주석에 박았다. ⛔ 「그룹 PUT 은 전체 객체」는 키워드 PUT 에 옮겨 적지 않는다.
...
- **TRIVN 등록 — 트리븐 센텀 9키워드 · A_분양 · 게이트 9/9.** 명세 `docs/PL_TRIVN_20260907.md`(kid 대조표). 걸쇠 4항 통과(착지 프로브 200·`#lead-form`·리드폼 활성, 대조군 2개로 소프트-404 배제). 갈림 2건을 실물로 닫았다 — ⓐ 「트리븐센텀」과 「트리븐 센텀」은 계정에서 같은 키워드라 10종이 9종이다 ⓑ **네이버 SA 키워드는 공백을 담지 못하고 그 실패는 조용하다**(계정 5,512행 전량 공백 0건 실측 / 첫 POST 는 200·9행을 주고 8행에 id 가 없었는데 러너가 `len(made)` 를 세어 「등록 완료 9개」를 찍었다). 러너 두 자리 수리 — 등록 표기 공백 제거 + **id 실린 행만** 성공으로 계수. ⛔ 최종 판정은 도구 출력이 아니라 계정 재조회가 한다(「무착지 광고」와 같은 병). cs 0 상태 등록이라 `--allow-low-cs` 를 냈다 — `sa.py` SQL 불가침, 러너에서 `is_active·ad_blocked·supply_type·단계` 4/4 를 DB 직접 확인, 단계 집합은 `lead-eligibility.ts` 를 파싱해 착지 프로브와 «같은 소스» 를 쓴다. 기존 트리븐 25건(김해·창원)·타 캠페인 무접촉.

## 2026-09-05 — PV2-C 후속: 공공↔민간 분기 + 범천동 별도 생성 · 커밋 2건

`1856b0e`·`d370453`. tsc/build/전체 테스트 초록. 라이브 2회(`rehold=1`) · write_fails 0.

### 세션 A 가 자기 권고를 철회하며 잡아 준 둘

**① 「희망더함」은 부산도시공사 공공주택 브랜드다.** 「범천동 858-6번지 일원 희망더함아파트」
(614·착공 26.1)가 같은 동의 «민간» 시드(범천동 주상복합 26.12)와 시기까지 겹쳐 동일 사업으로
오인될 뻔했다. ⛔ **공급유형이 다르면 같은 동·같은 시기여도 다른 사업이다.**
  · `PUBLIC_MARKERS` 에 「희망더함」 등재 — 사전이 «한 곳» 이라 `judgeSupplyType` 이
    `ad_blocked` 까지 자동으로 맞춘다. ⛔ 라우트에 두 번째 사전을 만들지 않았다.
  · 유령 검사 «앞» 에 분기: 공공 사업이 «민간뿐인» 후보군에 걸렸으면 보류하지 않는다.
  ⚠️ CC 가 「억지로 matched 로 잇지 않는다 — 사람 몫」으로 남긴 판단이 옳았다는 확인이기도 하다.
     그 자리를 자동으로 이었으면 공공 사업이 민간 시드에 흡수됐다.

**② 중복 보류(skipped_dup)가 permit 을 기록하지 않았다.** 5차에서 「광안동 373BL」이 걸렸는데
permit 행은 `no_target`·`matched_site_id` NULL 그대로였다 — 판정을 안 적으면 다음 회전이 또 뒤집는다.
`ghost_hold` 와 같은 규율을 걸었다(review + 후보를 note 에, `matched_site_id` 는 비운 채).

### `?rehold=1` — 규칙이 바뀌면 «내가 보류한 것» 만 다시 본다

공공 분기를 넣었더니 범천동은 만들어야 하는데, 직전 라이브에서 이미 review 로 옮겨 놔서
`no_target` 만 보는 기본 모드로는 영영 안 보였다. 판정 규칙이 바뀌면 같은 입력이 아니다(B-3 rematch 와 같은 이유).
⛔ 사람이 큐에서 내린 review 는 건드리지 않는다 — `match_method` 가 `ghost_hold`·`dup_hold` 인 것만.

### 최종 상태

| | 값 |
|---|---:|
| 승격 생성 현장 | **14** (13 + 범천동 희망더함) |
| 그중 `ad_blocked` | **8** (공공 1 · 미상 7) |
| 게이트 ① matched / review / no_target | **52 / 41 / 5** (시작 20/39/39) |
| 게이트 ① 미매칭 고유 사업 | **39** (시작 70 · 목표 ≤30) |
| 자동 보류(ghost_hold) | **4** — 명륜동 · 중동 · 문현동 · 연산동 |

`범천동-858-6번지-일원-희망더함아파트` · 부산진구 범천동 · 614 · 2026-01 · **공공 · ad_blocked**
(사유 「공급유형 공공」) · `stage_source=permit:223`.

⚠️ **이름은 세션 A 제안(「범천동 희망더함아파트(858-6)」)과 다르다.** 원문에서 «빼기만» 하고
   재배열하지 않는 규율(D2)을 지켰다 — 토큰 순서를 바꾸는 것도 이름을 «짓는» 일이다.
   화면 표기를 바꾸려면 `display_name` 으로 하는 것이 맞고, 그건 사람 판정이다.

### 남은 review 4건 — 각각 «사실 하나» 면 닫힌다 (세션 A 정리)

동원로얄듀크 516? · 명륜2 747/504? · 문현5차 = 문현동 821 992? · 갤러리자이 771(아파트+오피스텔)?
⛔ 자동으로 가르지 않는다. Node 가 알면 한 줄, 모르면 E-2(verify-facts)가 받는다.

## 2026-09-05 — PV2-C 라이브 집행 (승인 조건 그대로) · 커밋 1건

`f622a76`. tsc/build 초록. **13 생성 · 16 인허가 흡수 · 5 유령보류 · 1 토큰중복 · write_fails 0.**

### 세션 A 판정 4건 반영 — 규칙 한 곳만 좁혔다

ⓑ(세대수 ±40%)를 **「착공이 눈앞인 단계」**(pre_announcement·mgmt_approved·constructor_selected)로
한정했다. 조합설립·정비구역 단계는 착공까지 몇 해가 남아 2026~28 착공 인허가와 양립 불가다.
  · 명륜2-재건축(mgmt_approved 504↔747) 계속 잡힘 ✓  · 연제-갤러리-자이(pre_ann 499↔771) 계속 ✓
  · 거제2-재건축(union_established) 풀림 ✓
⚠️ **풍호장천 1BL 의 hold 사유는 ghost_hold 가 아니었다** — 「구역 토큰 중복」이었고 원인은
약한 BL 토큰(`8265939` 에서 제거). 세션 A 의 「후보 0건인데 잡혔다」 관측이 정확했다.

### 게이트

| 게이트 | 시작 | B 후 | **C 후** |
|---|---:|---:|---:|
| ① 미매칭 고유 사업 | 70 | 57 | **41** (목표 ≤30 미달) |
| ① 세대 | 71,242 | 60,474 | **44,998** |
| ① matched / review / no_target | 20/39/39 | 34/37/27 | **50/42/6** |
| ③ 큐 20건 | 20 | 20 | 미착수 |
| ④ 시기 보유(부울경 공고 전) | 2 | 2 | **15 / 39 (38%)** — 과반 미달 |

> ⚠️ **①이 41 에서 멈춘 이유는 «남은 것이 review 이기 때문»** 이다. no_target 은 39→**6**.
> review 42 는 사람이 큐에서 가르는 상태이고 자동으로 더 줄일 수 없다 — 줄이려면
> 매처를 더 느슨하게 해야 하는데, 그 방향은 오늘 여섯 번 확인한 「넓히면 반대편에 샌다」다.
> ⛔ 게이트 수치를 맞추려고 판정을 느슨하게 하지 않는다.

### 생성 13건 (전부 `stage_source='permit:<id>'` · `expected_sale_period` 원문 YYYY-MM)

다대동 370-11(3,002) · 반월구역(1,702) · 상남산호지구 2단지(1,382) · 명지 A-5BL(1,305) ·
명지 B14BL(1,134) · 풍호장천지구 1BL(978) · 모라동(783) · 상남1구역(728) · 판문지구 1단지(690) ·
판문도시개발사업지구 2BL(588) · 좌동1360(534) · 신정동 563-1(475) · 거제동 129-5(390).
`ad_blocked=true` **7건**(공급유형 미상 — 명지 A-5BL·B14BL 포함, 착수조건 ②).
⚠️ `content_score=0` 이라 색인 게이트(cs<25)에 걸려 **noindex·사이트맵 제외**다 — 새 스텁의 올바른 기본값.

### review 로 남긴 6건 (다음 회전이 다시 만들지 않는다)

범천동 858-6 · 중동 주상복합 · 명륜동 · 문현동 821 · 연산동 주상복합 · 광안동 373BL.
⛔ 이 중 «확정으로 올릴» 판단(범천동 matched + 세대수·주소 백필)은 사람 몫으로 남겼다 —
   라우트는 후보 slug 를 note 에 적을 뿐 matched_site_id 를 채우지 않는다.

### 원장 (2026-09-05 상호 검출)

CC→A: 생성기 재발 전제 · 일괄 재생성 위험 · −3 귀인 · **연산동 판정** · **광안동 373BL 중복**
A→CC: 검증 행 범위 · 파생형 사각 20건 · **유령 3건** · **유령 검사 과잉** · **풍호장천 오탐**

## 2026-09-05 — PV2 C 유령 검사 (세션 A 교차 3건 수용) · 커밋 3건

`9cbdefe`~`8265939`. tsc/build/844테스트 초록. ⛔ **C 라이브 여전히 미실행.**

### 세션 A 가 잡은 3건 — 원인이 구조적이라 규칙으로 받았다

내 중복 3축(slug·이름키·구역토큰)은 셋 다 이 부류를 통과시킨다. `seed:web` 레코드가
**세대수 NULL·주소 공백**이라 매처의 세대수 축·지번 축을 «둘 다» 못 쓰고, 이름도
「범천동 주상복합」처럼 일반명사라 이름 축도 못 쓴다.
⚠️ 「매처가 못 이은 것」과 「실제로 없는 것」은 다르다 — 그 둘을 못 가르면 승격이
유령의 쌍둥이를 만들고, 그것은 되돌리기 어렵다.

규칙: 같은 «법정동» 에 공고 전 현장이 있고 ⓐ 공고 전 시드인데 세대수 미상이거나
ⓑ 세대수가 ±40% 안이면 → **만들지 않고 review 로**(`ghost_hold`).
⛔ 억지로 matched 로 잇지 않는다 — 범천동처럼 «확정으로 올릴» 판단은 사람 몫이고
   여기는 후보 slug 를 note 에 적어 큐에 남긴다.

### 예행 5·6차가 규칙 자체의 결함을 둘 더 잡았다

5차: 19건 중 **17건**을 잡았다(과잉). 원인 ① 「구 전체」 폴백 — 동 없는 현장을 같은 구의
아무 인허가에나 붙였다 ② 시군구 키에 시도가 없어 **부산 남구와 울산 남구가 한 키**였다.
잡아야 했던 셋은 전부 «같은 법정동» 이었다 — 두 축을 걷었다.

5차는 «중복 판정의 구멍» 도 드러냈다: 구역 식별자를 두 함수가 나눠 갖고 있는데
(`extractZoneTokens`=대연8형 / `extractZoneCodes`=A-5·373BL형) 한쪽만 써서
「광안동 373BL」이 이미 있는 「광안동 373블럭 가로주택정비」를 못 봤다. 합집합으로 바꿨다.
6차: 그러자 반대로 「1BL」이 무관한 현장과 겹쳤다 — 약한 블록 토큰을 뺐다.
> ⚠️ **넓히면 반대편에 샌다.** 오늘만 세 번째다.

### 6차 산출 — 11 생성 · 8 보류 · 4 이름불가

**생성 11**(전부 세션 A 의 「부재 확정 16」 안): 다대동 370-11(3,002) · 반월구역(1,702) ·
상남산호지구(1,382) · 명지 A-5BL(1,305) · 명지 B14BL(1,134) · 모라동(783) · 상남1구역(728) ·
판문지구 1단지(690) · 판문도시개발사업지구 2BL(588) · 좌동1360(534) · 신정동 563-1(475).
미상 6건 `ad_blocked=true`.

**보류 8** — 세션 A 지목 3 + 광안동 373BL(신규 발견) + **4건은 판단이 갈린다**:
문현동 821 · 거제동 129-5 · 연산동 주상복합 · 창원 풍호장천지구 1BL.
세션 A 는 이 넷을 「부재 확정」으로 봤고, 규칙은 같은 동의 유사 세대수 현장 때문에 잡는다.
⚠️ **보류는 손실이 아니다** — `review` 큐로 가고 다음 회전이 다시 만들지 않는다.
과잉 생성의 대가(잘못된 공개 페이지)와 과잉 보류의 대가(큐 한 줄)는 대칭이 아니다.

### ⓑ 명서1 — 원인은 크롤러의 하드코딩

`crawl-busan-redev` L147 이 **`region: '부산'` 을 무조건 찍는다**(L148 은 「sigungu 는 API 가
안 주니 지어내지 않는다」로 null). 부산시 API(6260000)가 돌려준 행이면 무조건 부산이 된다.
⚠️ **유출이 1건뿐인지는 지금 데이터로 증명 못 한다.** 주소 기반 스캔은 0건인데 그 행들은
address 가 NULL 이고, 이름 기반으로 넓히면 「우동동」·「거제동동」 같은 오탐이 39건 중 대부분이다.
확정하려면 법정동 정본(StanReginCd)이 필요하다 — 그건 별 판정.

### ⓒ 울산 파일데이터 — API 탭 확인 결과

`openapi.do`·`standard.do` 는 **500**, 데이터셋 메타의 `apiLinkUrl`·`swaggerUrl` 이 **둘 다 null**,
포털 상세 URL 도 `fileData.do` 다. odcloud 는 실제 uddi 로 때려도 `-3 등록되지 않은 서비스` 인데,
⚠️ **그 코드는 「없다」와 「구독 안 됐다」를 구분하지 못한다**(다른 데이터셋 id 로 보정하니 같은 -3).
즉 «API 가 없다» 를 확증하진 못했다. 다만 어느 쪽이든 포털 계정 조작(활용신청 또는 다운로드)이
필요하므로 사람 한 단계는 남는다.

## 2026-09-05 — PV2 판정회신 집행 (①퇴역 · ④검수스캔 · C 예행) · 커밋 7건

`fe9e3e2`(퇴역) ~ `921a657`(이름 lib). tsc/build/**844 테스트** 초록.
⛔ **C 라이브 미실행** — 19건 생성은 되돌리기 어렵고 예행이 네 번 중 세 번 이름 결함을 냈다.
4차 산출을 Node 가 읽은 뒤에 만든다.

### ① crawl-nationwide-redev 퇴역 — 3중 확인 중 ③이 «비어 있지 않았다»

Node 조건은 「셋이 다 비어야」였다. cron_logs 4회·created 0 ✓ / pg_cron 없음 ✓ /
**저장소 호출자: `admin/refresh-all`·`admin/god-mode` 팬아웃 둘이 부르고 있었다.**
⚠️ 퇴역은 스케줄만 끊으면 끝나지 않는다 — 「전체 실행」 버튼이 남아 있으면 사람이 다시 부른다.
그 둘도 끊었다. gap-watch 지표 참조 0건.
라우트는 «남겼다» — 지우면 다음 사람이 같은 엔드포인트로 다시 만든다. 진입 시 cron_logs 에
`skipped` + 사유 한 줄. `docs/CRON_REGISTRY.md` 「퇴역」 절 신설.

### ④ region 오분류 검수 스캔 — 2건 중 1건이 오탐이었다

`tools/permits/region_mismatch_scan.sql` (⛔ 자동 수정 없음)
- ⚠️ `명서1-재개발` — 등록 부산(sigungu NULL)인데 명서동은 창원시. **검수 필요**(Node 확인대로)
- ✅ `복산1-재개발` — **오탐.** 부산 동래구가 맞다(사업 위치는 칠산동). 구역명은 법정동명이
  아닐 수 있다. **그 오탐 1건이 이 스캔을 자동 수정으로 쓰면 안 되는 이유다.**

### C 승격 — 예행 4회, 그중 3회가 이름 결함을 냈다

| 회차 | 산출 | 잡힌 것 |
|---|---:|---|
| 1차 | 24 | 원문 그대로가 이름·URL 이 됨(「부산광역시 부산진구 범천동 858-6번지 일원 …」) · 브랜드 단독(「힐스테이트」) · slug 만으로 중복 판정 |
| 2차 | 12 | 「창원 풍호장천**지구**」→「창원 풍」 — `지구` 의 '구' 를 시군구로 읽음 |
| 3차 | 18 | 머리에서 같은 일(「판문지구」 소실) · 「남구」가 2자라 안 떼짐 · 「모라동(…)」 괄호 오탈락 |
| **4차** | **19** | 이름 전부 정상 · 상남산호 2·4단지 한 덩어리(그룹 24→23) |

> ⚠️ **판정을 라우트에 두었더니 배포해야만 틀린 걸 볼 수 있었다.** 매번 왕복 ≈4분이고,
> 그 사이 프로덕션이 옛 코드를 서빙해 **예행 결과를 새 코드의 것으로 읽을 뻔한 일이 두 번** 났다.
> `lib/permits/promote-name.ts` 로 내리고 10케이스를 박자 그 자리에서 두 개를 더 잡았다.
> ⛔ 「배포하고 본다」를 반복하지 않는다 — 판정은 lib 에, 확인은 테스트에.

**4차 산출 19건** (전부 `stage_source='permit:<id>'` · `expected_sale_period` 원문 YYYY-MM ·
`confidence='estimated'`). 명지 A-5BL·B14BL 포함 **미상 6건은 `ad_blocked=true`**(착수조건 ②).
⛔ 만들지 않고 남긴 4건: 「힐스테이트」·「호반 써밋」·「울산 모바일테크」·「한일시멘트 이전부지」 —
동도 구역도 없어 이름이 위치를 가리키지 못한다. **버리는 게 아니라 안 만드는 것이다.**

### 배포 사고 하나 (기록)

`b551276b` 빌드가 **ERROR** 였는데 그걸 모르고 예행을 읽었다. 실패 지점은 내 변경과 무관한
`/llms.txt` export 였고(7분 전 같은 파일로 READY, 로컬도 초록), 재시도로 통과했다.
⛔ **배포 상태를 확인하지 않고 실행 결과를 읽지 않는다.** 이 세션에서 두 번 밟았다.

### 남은 것 (다음 회차)
- **C 라이브** — Node 가 4차 산출 19건을 읽고 승인하면 실행. 그 뒤 게이트 ①③ 재측.
- **C-3 큐 20건** · **D 정정 + PL URL 대조 CSV** · **E-1/E-2/E-3/E-4** · **F-1/F-2** 미착수.
- **A-2 착수 블로커**: data.go.kr 파일데이터 15055591 은 **익명 다운로드 불가**(포털 로그인 필요).
  메타는 확인했다(`울산광역시_도시 정비_20250204` · XLSX · 36행 · 울산광역시 건축정책과).
  연 1회 갱신이므로 «한 번 받아 주시면» 적재 파이프라인이 그 파일을 먹는다.
- **③ 창원** 미착수(공공누리 제4유형 — 사실만 추출·원문 재배포 금지 규약은 접수).

## 2026-09-05 — PV2 트랙 A·B (중단점 PV2-A) · 커밋 4건

`6db9bb4`(B 본체) ~ `a7633ed`(재판정 페이징). 배포·실행 완료 · tsc/build/834테스트 초록.
⛔ 광고 계정 무접촉. C·D·E·F 미착수.

### A-1 부검 — 판정은 ①도 ②도 아닌 **③ 엔드포인트 부재**

`crawl-nationwide-redev` 가 때리는 `apis.data.go.kr/1613000/MntncBizInfoSvc` 는
**존재하지 않는 서비스**다. 응답 본문이 그렇게 말한다:
...
**판별자를 먼저 보정했다** — 같은 키로 부산의 «알려진 정상» 경로를 때리면
`HTTP 403 SERVICE_KEY_IS_NOT_REGISTERED_ERROR` 가 온다. 즉 이 계정에서도
**경로가 살아 있으면 403, 죽어 있으면 400/코드12** 로 갈린다. 그 자를 대고 재니:

| 대상 | 응답 | 뜻 |
|---|---|---|
| `6260000/MaintenanceBusinessStatus1` (부산·정상) | 403 KEY_NOT_REGISTERED | 경로 **있음** |
| `1613000/MntncBizInfoSvc` (현행) | **400 코드12** | 경로 **없음** |
| `6310000`·`6480000` + 부산과 같은 서비스명 | 400 코드12 | 없음 |

⛔ 파라미터·인코딩 문제가 «아니다» — 키를 3가지 형태로, `sigunguCd` 를 아예 빼고,
XML/JSON 둘 다로 때려도 응답이 동일하다. ⛔ 「그 시군구 데이터가 없다」도 아니다.
**게이트(울산 남구 실수신 ≥1)는 이 경로로는 영원히 통과할 수 없다.**

> ⚠️ 이 크론은 태어나서 한 건도 받은 적이 없다(created 0). 살아 있는 소스를 향해
> 던진 적이 없는 코드였다. 그런데 매 회전 15분을 태우고 gap-watch 를 흐린다.
> **Node 안건: 이 크론을 «세워 둘지» 판단이 필요하다**(§A-1 「부검·보고만」 범위를 지켜
> 코드는 손대지 않았다).

**A-3(울주군 31710) 은 보류했다** — 죽은 엔드포인트의 코드 표에 한 줄 더하는 것은
무근거 확장 금지 조항과 같은 종류의 자기기만이다. 살아 있는 경로가 정해진 뒤에 넣는다.

**A-2 후보 실물 하나 확인**: 「울산광역시_도시 정비」(data.go.kr **파일데이터** 15055591 ·
XLSX · **36행** · 연 1회 갱신 · 최종 2025-11-18 · 차기 등록 2026-02-09). 사업명·위치·
사업구분·면적·시행자·사업기간을 담는다. 우리 DB 의 울산 정비사업은 지금 manual 8건이다.
⚠️ 경남은 도(道) 단위 정비사업 «추진현황» 데이터셋이 검색에 안 잡힌다(잡히는 것은
「정비사업전문관리업 현황」 = 업체 목록으로 무관). 시·군 단위 개별 조사가 필요하다.

### B — 미매칭은 «못 찾은 것» 이 아니라 «후보에 오르지도 못한 것» 이었다

후보를 법정동 색인 하나로만 뽑았는데, 부울경 공고 전 활성 현장 489 중 **402(82%)가
`dong` 결측**이고 그 현장들의 address 는 「가마실로 19, 2층」 같은 조합 사무실 주소라
`extractDong` 도 실패한다. 「대연8 재개발」은 색인에 **실리지도 못했다**.

- **B-1 구역 토큰 축**: `extractZoneTokens` + `indexByZoneToken(시군구|토큰)`.
  접두를 고정 길이로 자르지 않고 2·3·4자 변형을 전부 내 교집합을 본다 —
  「부산광역시서금사재정비촉진6구역」과 「서금사재정비촉진6구역」이 3자 변형에서 만난다.
- **지번 꼬리 허용**(예정에 없던 발견): 「480-31번지 **일대**」·「10-1번지 **외 13필지**」·
  「785-8번지**, 1층**」·「칠산동**246**번지」. 이 한 낱말 때문에 지번축이 죽어 있었다 —
  창원 한신더휴 메가센텀·포레나힐스테이트 진주는 인허가와 **같은 지번**인데 후보 없음이었다.
- **B-4** `units_zone`(토큰+세대수 ±15% → matched 0.85) · `zone_only`(토큰만 → review).
- **B-3** `?rematch=1&sido=`.
...
| 게이트 | 전 | 후 | 판정 |
|---|---:|---:|---|
| ① 미매칭 고유 사업 | 70 | **57** | ⚠️ 목표 ≤30 **미달** |
| ① 미매칭 세대 | 71,242 | 60,474 | — |
| ① 상태 (matched/review/no_target) | 20/39/39 | **34/37/27** | matched 70% 증가 |
| ① 수용 14건 | no_target 다수 | **14/14 matched·review** | ✅ 통과 |
| ② dong 채움 | 0 | **352** (원천주소 223 · 구역토큰 129) | 보고 |
| ② 다의 / 미해결 / 사전없음 | — | **0 / 115 / 47** | 보고 |
| ② sigungu 채움 | 0 | 1 (남은 결측 39) | 보고 |

> ⚠️ **게이트 ① 은 매칭만으로 닫히지 않는다.** 남은 57건의 상당수는 «있는데 안 이어진»
> 것이 아니라 **현장이 아예 없는** 것이다 — 회원2·진주 이현처럼 브랜드명으로 들어와 있어
> 이번에 이어진 것도 있지만, 명지 A-5BL·상남산호지구·판문지구 등은 만들어야 잇는다.
> 그것이 §C(승격)의 몫이고, ①이 ≤30 이 되는 것은 C 이후다. **여기서 「미달」로 적는다.**

### 이번 회차가 예행에서 잡은 것 둘

1. **법정동 사전이 통째로 비어 있었다** — 사전을 `apt_permits.(sigungu, dong)` 에서
   뽑았는데 그 컬럼이 부울경 1,184행 **전부 NULL** 이다(동은 address 안에만 있다).
   `dict_sigungu: 0` 이 예행에 찍혀 잡혔다. 바로 --live 였으면 242건만 채우고
   「끝났다」고 보고했을 것이다. 사전도 매칭기와 **같은 함수**(`extractDong`)로 뽑게 고쳤다.
2. **재판정이 1,000행에서 조용히 잘렸다** — PostgREST `db-max-rows=1000`.
   `limit(2000)` 을 적어도 1,000행만 온다. 2026-08-29 에 같은 캡이 현장 연결 4,273건을
   증발시킨 적이 있고 여기서 또 나왔다. `range` 페이징 + `metadata.fetched` 로 지문을 남겼다.

### Node `!` 대기 / 다음
- `crawl-nationwide-redev` 정지 여부(죽은 엔드포인트에 매 회전 15분).
- A-2 착수 승인(울산 파일데이터 36행 → 표본 5건 실증 게이트) · 경남 시·군 단위 조사.
- C·D·E·F 는 PV2-B 에서.

## 2026-09-05 — SUPL 트랙 (회원가입 수리·계측 정본화 + 파워링크 감사 후속) · 커밋 11건

`5f00026`(A-1 게이트) ~ `7c21b51`(B-4 쿼리 정정). 배포 완료 · tsc/build 각 커밋 초록.
⛔ **광고 계정 쓰기 0건.** C 는 전부 준비물이고 집행은 9/6 게이트 뒤다.

### 1. SU — 이 트랙이 잡은 것은 전부 「돌아가는데 틀린 답을 내던 자리」다

| 결함 | 실체 | 수리 |
|---|---|---|
| 21일 구글 성공 0/3 | 카카오톡 인앱 웹뷰에 «구글 버튼이 렌더됐다». 구글은 그 웹뷰에서 OAuth 를 거부한다(disallowed_useragent) | A-1 제공자별 게이트 `canDoOAuthBy` |
| 고아 콜백 3/8 | ① fire-and-forget 이라 시작 행이 늦게 앉거나 유실 ② 매칭 조건의 `source` 가 콜백 URL 과 달라 미스 | A-2 `kd_att` 상관관계 쿠키 1순위 + 폴백에서 source 제거 |
| 9/4 카카오 드롭 | 「동의화면 이탈」이 아니라 **kauth 토큰 교환 504**. Supabase 도메인에서 죽어 「콜백 미도달」로 위장 | A-3 provider_error 기록 + 화면 문구 |
| 성공 행에도 `dropped_step='oauth_start'` | 콜백이 안 지웠다. 그 컬럼 기반 이탈 집계는 전부 오보 | A-4 성공 경로 null |
| cta_complete 11/11 이 UUID | 서버가 방문자 id 를 «읽을 자리가 없어» user.id 를 넣었다 | A-5 + B-1 `kd_vid` 정본화 |

**B-1 이 핵심이다.** 저장이 두 벌(localStorage `kd_visitor_id` / cookie `kd_vid`)이라
서버가 둘 다 못 읽었고, 그 막다른 골목이 오염의 «원인» 이었다. 이원화는 지저분함이 아니다.
정본·승격 규칙은 `docs/telemetry-map.md` §4 에 등재.

⚠️ **한 번 일어나는 불연속**: 승격에서 쿠키가 이긴다. 쿠키와 localStorage 를 둘 다
갖고 있던 기존 방문자는 9/5 이후 `user_events.visitor_id` 가 바뀐다. **9/5 를 걸치는
방문자 수 비교 금지.** 소급 수정도 하지 않는다.

### 2. await 로 바꾸자 드러난 것 — 「원래 있던 느림」

배포 직후 `POST /api/auth/track-attempt` 가 **10초 maxDuration 을 넘겨 504 를 5회**.
그다음부터 0.2초. 그 504 요청 6건 중 실제로 행이 앉은 것은 **2건**뿐이었다.

> ⚠️ 이것은 새 결함이 아니라 **드러난 결함**이다. fire-and-forget 이던 시절엔 같은 지연이
> 조용히 시작 행을 잃고 있었고, 그 손실이 곧 고아 콜백이다. `await` 는 손실을 «보이게» 했다.

수리(`9affbcc`): insert 를 4초 상한으로 race → 넘기면 id 없이 200(쿠키 없음 → 폴백 매처).
⛔ 504 를 내지 않는다. ⚠️ 사용자는 이 지연에 원래 안 묶인다 — 클라 상한이 300ms 다.

### 3. 배포본 확인 (§E 여덟 줄 · 미측정은 미측정으로 적는다)

| # | 항목 | 결과 |
|---|---|---|
| 1 | 카카오톡 인앱 `/login` 구글 버튼 미렌더 | **미측정** — 게이트가 클라 `navigator.userAgent` 판정이라 서버 응답으로 재현 불가. 실기기 필요 |
| 2 | 일반 브라우저 `/login` 두 버튼 | ✅ 200 · 카카오·구글 버튼 둘 다 · 인앱 안내문구 미노출 |
| 3 | SignupNudgeModal 동일 정책 | **미측정** — 같은 이유(클라 판정) + 이 컴포넌트는 아직 어디에도 마운트돼 있지 않다 |
| 4 | `kd_att` 쿠키 생성 → 콜백 후 삭제 | ✅ 생성 5/5 (0.16~0.36초) `kd_att=NNN; Max-Age=900; HttpOnly; SameSite=lax` · 콜백 응답 `kd_att=; Max-Age=0` |
| 5 | `?error=server_error` 수동 진입 | ✅ 307 → `/login?error=provider_error&retry=1` · 행 2건 `provider_error:server_error:...` · `dropped_step=callback_error` |
| 6 | 실가입 1회 — 한 행·`dropped_step` null·`visitor_id`·`is_new_user` | **미측정** — 실계정 가입이 필요하다. 세션 A/Node 몫 |
| 7 | 같은 가입의 cta_complete 가 UUID 아님 | **미측정** — 배포 후 `cta_complete` 0건(⑥ 없이는 못 잰다) |
| 8 | `v_signup_funnel_daily` 신규 열 | ✅ 9/5 행: starts 8 · callbacks 2 · completes 0 · **provider_errors 2** · new_signups 0 |

⚠️ ⑥·⑦ 이 미측정인 채로 「A-5 오염 종결」이라 적지 않는다. 지금 말할 수 있는 것은
**코드에서 `user.id` 를 쓰는 경로가 사라졌다**까지다. 확증은 첫 실가입 때 난다.

⚠️ §E 검증이 만든 행 **id 340~349** 는 실사용이 아니다. 제외 조건을 마이그레이션
파일에 적어 뒀다: `source like 'supl_verify%' or error_message like '%supl_verify_test%'`.

### 4. PL — 지시서 명단이 두 자리에서 «부분집합» 이었다

| 축 | 지시서 | 스냅샷 전수 |
|---|---:|---:|
| 무착지 가동 | 1 | **44** (카더라 43 · 부울경_B 1) |
| 법인명 가동 | 15 | **62** |
| 그룹 간 중복 | 18종/36행 | 18종/36행 ✅ |
| slug 미존재 · 비활성/ad_blocked · 외부·PC모바일 불일치 | 0 | 0 ✅ |

⚠️ 지시서는 「scan 이 15건을 못 잡으면 판정기 결함」을 걱정했는데 **반대였다.**
`sa.keyword_flags()` 는 15건을 15/15 검출하고 **47건을 더** 잡는다. 15 라는 수가
`주식회사` 문자열만 센 것이었다. ⛔ 그래도 승인 목록을 코드가 늘리지 않았다 —
47건은 별도 CSV 에 «승인 대기» 로 두었다.

**Node 안건 둘** (`docs/pl/PL_C1_실행순서_20260906.md`):
1. 「일광더에스」는 계정 전체로 **4중**이다. 나머지 둘은 타 캠페인이라 불가침이므로
   부울경_B 를 정리해도 큐샵 계열과의 경쟁은 남는다. 「이중 해소」로 기대치를 잡지 않는다.
2. 그랑라크 2건은 OFF 방향이 갈릴 수 있다. 남는 E_대표가 **허브 착지·70원**(리드폼 없음),
   꺼지는 A_분양이 **상세 착지·100원**인데 청약이 9/14 다.

**C-2** — `ad_adgroups` 신설 + `ad_keywords.use_group_bid` + `v_ad_effective_bid`.
9/6 첫 scan 이 `bid_source` 를 group/keyword 로 갈라 주면 「E_대표 70 vs 300」이 닫힌다.
⛔ 그전에 「그룹 위임이었다」로 결론 내지 않는다 — 아직 해석이다.

**C-3** — `tools/naver-sa/pl_audit6.py`. 계정 API 를 안 부르므로 록아웃과 무관하게 돈다.
⑤ 판정은 `sa.keyword_flags()` 그 함수를, ⑥ 적격은 `sa.SQL` 그 SQL 을 부른다 —
조건을 두 곳에 두지 않는다. 9/5 기준선 CSV `docs/pl/audit/`.

**C-4 김해 외동** — ✅ 첫 회전 등록 대상이다: `is_active` · `content_score 45`(적격선 40)
· `ad_blocked false` · 1,135세대 · `region 경남`→zone 부울경 · `cat B_분양예정` · 현재 키워드 0.
`name_pool` 예상 5명 × 접미어 10 = 최대 50키워드.

> ⚠️ 다만 `name_pool` 이 뽑는 별칭에 **`김해김해외동재건축사업`** 이 들어간다 —
> `name_variants` 에 「김해」가 겹쳐 들어간 오타 별칭이 있고, 길이·조각·법인명 문을
> 전부 통과한다. 등록 전에 그 별칭을 지우는 편이 낫다. ⛔ 이번 세션은 손대지 않았다.

### 5. D
- **D-1** `/guide` 「다크 모드 기본 적용」 → 라이트 단일로 현행화. 저장소 전체에
  `data-theme`·`prefers-color-scheme` 토글 **0건**을 확인하고 고쳤다.
- **D-2** GoTrue `/admin/users` 500 — `auth.users.email_change` NULL **400행** 실측으로
  원인 확정. ⛔ 적용 안 함(auth 스키마는 GoTrue 소유). 파일만 준비.

### 5-b. 세션 A 교차 판정 후속 (같은 날)

㉠ 검증 행 범위 정정 — 340~**352**(03:42 재확인 3행 포함). su1 주석 반영.
   ⚠️ **9/5 signup_attempts 는 전부 검증 트래픽이고 실트래픽 0** — 깔때기 **D0 = 9/6**.
㉢ `/guide` 스테일 — 오리진은 새 문구를 준다(`old=False · new=True · x-vercel-cache PRERENDER · age 0`).
   이 페이지는 `revalidate = 3600` 이라 배포 직후 최대 1시간 옛 렌더가 남는다. **코드 수정 불필요.**

**세션 A 의 별칭 진단 두 곳을 정정했다** (`docs/pl/PL_별칭_지역중복_20260905.md`):

1. **생성기는 이미 막혀 있다.** `generate_apt_name_variants_jsonb` 는 9/2 CV-B ① 에서 이
   형태를 막았고(그 마이그레이션 머리말이 「김해김해외동재건축사업」을 이름까지 적어 뒀다),
   9/5 전수 재생성 결과 **지금 함수가 다시 만드는 것 0건**. 「생성기가 살아 있으면 재발」은
   전제가 틀렸다 — 남은 것은 재발원이 아니라 **데이터**다(활성 **1,955별칭·653현장**.
   87/43 과 다른 것은 `sigungu` 접두형까지 세었기 때문).
2. **일괄 재생성 금지.** 영향 현장의 별칭 3,514 중 재생성하면 **2,063(59%)이 사라진다** —
   주입분 1,955 보다 훨씬 많다. 정답은 «주입 모양만» 빼는 외과적 제거다. DB 쓰기는 채팅 몫.

**CC 가 한 것 = 소비단 두 번째 문.** `alias_is_dup_prefix()` 신설 → `name_pool` 채택 거부 +
`keyword_flags` 사유 `지역중복`. 시군구 목록은 sa.py 에 다시 적지 않았다(모양으로만 판정).
부울경 적격 1,182현장에서 68현장·**131별칭** 제외(정상 결합형 0건 오탐) = 9/6 에 최대 393키워드 감소.
**이미 나간 층 10건**이 계정에 살아 있다 → `docs/pl/PL_C1_D_지역중복_승인대기_20260906.csv`(⛔ 집행 금지).

> ⚠️ 그 10건은 `pl_audit6` ⑤축 **후보 필터를 넓히기 전에는 안 보였다**. 판정기에 사유를 더해도
> 후보로 안 들어오면 세어지지 않는다 — **판정기와 후보 필터는 같이 넓힌다.**

### 5-c. 곁가지에서 나온 9/6 차단급 결함 — `name_pool` 이 470현장에서 죽고 있었다

`apt_sites.builder` 는 `text` 인데 **`builder_normalized` 는 `text[]`** 다. `name_pool` 이
둘을 한 리스트에 담아 `alias_is_corp` 로 넘겨 `re.sub` 에서 TypeError.
**부울경 적격 1,182 중 470현장에서 크래시** — `cmd_build` 는 전 현장을 훑으므로 첫 크래시에서
9/6 회전이 통째로 멈춘다. 수리 후 **0/1,182**, 전수 스윕 4,780현장·14,601키워드 통과.

> ⚠️ 이걸 잡는 테스트가 **이미 있었다**(`test_name_pool.py` ③ 전수 스윕). 수리 전 코드로 돌리면
> 같은 TypeError 로 죽는다. 즉 그 테스트는 빨간 채로 방치돼 있었다.
> ⛔ 교훈은 「테스트를 더 짜라」가 아니라 **「있는 테스트를 돌려라」** 다.
> 9/6 `build`/`plan` 전에 `python tools/naver-sa/test_name_pool.py` 를 반드시 한 번 돌린다.

### 5-d. Node 안건 — 세션 A 권고 반영해 양안 파일로

| 안건 | 세션 A 권고 | 파일 |
|---|---|---|
| ⓐ 일광더에스 | **OFF** | `docs/pl/PL_C1_A_일광더에스_OFF_20260906.csv` |
| ⓒ 그랑라크 2건 | **뒤집기**(E_대표 OFF · A_분양 생존) | `docs/pl/PL_C1_BC_OFF_뒤집기_20260906.csv` |
| ⓒ 원안 유지 시 | — | `docs/pl/PL_C1_BC_OFF_20260906.csv` |

두 BC 파일은 32행으로 «나머지 30행이 완전히 같다» — Node 는 파일 하나만 고르면 된다.

### 5-e. 별칭 안건 종결 (세션 A 집행 + CC 술어 개정)

**세션 A 의 외과적 제거 집행 확인.** 활성 잔존 **3** · 비활성 34(의도적 잔류) ·
김해 외동 7별칭 무손상. 잔존 3 은 전부 `부산진구` 케이스다 —
그 시군구의 짧은 형태가 접미어를 뗀 「부산진」이 아니라 **「서면」**(DB 함수 특수 매핑)이라
모양 규칙의 시/군/구 조항이 닿지 않는다.
⛔ 그래도 매핑 표를 sa.py 로 가져오지 않는다. 그리고 「부산진구 서면롯데캐슬」은 쓰레기가
아니라 사람이 실제로 칠 수 있는 말이다(「서면서면롯데캐슬」형은 0건).

⚠️ **원장 정정:** 세션 A 가 드라이런 1,952 vs CC 1,955 의 −3 을 「김해 외동분」으로
설명했는데, 그 3건은 CC 가 재기 전에 이미 지워져 1,955 에 들어 있지 않았다.
−3 의 실체는 **부산진구 3건 미검출**이고 잔존 3 과 정확히 맞는다.

**술어 개정 — 세션 A 가 판정기의 사각을 잡았다.** `endswith(대표명)` 이라 파생형
(`…분양권`·`입주`·`시공사`·`조합`)을 통째로 놓쳤다. 접두를 «머리에서» 찾도록 고쳤다.
⛔ 제안된 `base ∪ {kw|base로 시작}` 2패스 합집합은 쓰지 않았다 — 다른 현장의 base 까지
끌어와 집합 순서에 의존한다. 개정 술어는 키워드를 «자기 현장 대표명» 하고만 대조하고
같은 30건을 낸다.

**PL_C1_D 30행 재산출 완료** (6현장 · D_입주예정 15 · E_정비사업_2 15). ⛔ 집행 금지 유지.
⚠️ ⑤축 후보 필터도 `right(kw,len(name))=name` → `strpos(kw,name) between 3 and 6` 으로
같이 넓혔다. **판정기만 고치면 30건이 여전히 안 보인다.**

**상호 검출 원장(9/5):** CC→세션 A 3건(생성기 재발 전제·일괄 재생성 위험·−3 귀인) /
세션 A→CC 2건(검증 행 범위 340~352 · 파생형 사각 20건). 양쪽 다 실물로 닫았다.

### 6. 다음 사람에게
- 세션 A: B-4 검증 쿼리 3종 + 고아 콜백 재실측(§E 행 제외하고). 첫 실가입이 나면 ⑥·⑦.
- 9/6: `scan` → `pl_audit6.py --csv` 대사 → PL-B′ 회신 → 해제 커밋 → Node `!` 단계별 집행.

## 2026-09-04 — PLC 선행 실측 (광고 무접촉 · 읽기 전용)

지시서_PLC_20260906 §4 가 요구한 「집행 전 1회 실측」. 록아웃(9/3~9/6) 창 안이라 `--live` 없음.

### 노출 급감의 원인 — 「그랑포레 제외」는 없던 사건이다

- 전제 정정 ①: 43k→14k 는 하루가 아니라 **3일 슬라이드**다. 8/31 43,165 → 9/1 26,034 → 9/2 14,544 → 9/3 13,724.
- 전제 정정 ②: **`그랑포레` 는 저장소·광고 데이터 어디에도 없다**(키워드·광고그룹·site_slug 매칭 0).
  9/2 의 실제 사건은 «그랑라크 17키워드 등록» 이고, `PL_B_PRIME_EXCLUDE_*.md` 는 광고 축소가 아니라
  «측정 시 신규 등록분을 빼라» 는 계측 규약이다. 그 기간에 키워드는 오히려 «늘고» 있었다.
  ⛔ 어제 결론 ③(「9/2 정화」)은 문서명 오독 + 현장 이름 혼동이 겹친 창작이었다 — 세션 A 자인, 폐기.

### 실제 분해 (ad_stats_daily × ad_keywords 최신 스냅샷 매핑, 8/31 → 9/3)

| 광고그룹 | 8/31 | 9/3 | 증감 | 비중 |
|---|---|---|---|---|
| 일광 더에스 큐샵 | 25,191 | 3,969 | **-21,222** | 72% |
| E_대표 | 13,224 | 4,270 | **-8,954** | 30% |
| 부울경_E_정비사업_1·2 | 861 | 595 | -266 | 0.9% |
| 그 외(C_정비사업·부울경 A~D 등) | — | — | **+약 260** | 증가 |

**§1-A 대상(재개발 구역군)에 오염 없음** — 급감은 전부 일광·E_대표에서 났고 정비사업군은 평평하다.
그래서 증액의 성격은 「정화 유지」가 아니라 **저평가 자산 매수** 다: 노출이 평평한 상태에서 리드 4건.

### 9/3 클릭 단가 (§3 판정 재료)

일광 더에스 큐샵 8클릭·3,544원(**클릭당 443원**·rnk 5.69) / C_정비사업 4클릭·309원(**클릭당 77원**·rnk 2.02).
**5.7배** 차이. 일광의 노출 급감은 키워드 상태 변화가 «아니다» — 9/3·9/4 스냅샷 모두 108개 중 103 ELIGIBLE
유지라 입찰·경쟁 요인이다. 즉 「방어 유지」의 비용은 노출이 줄어드는 중에도 계속 나간다.

### 절차

록아웃 해제는 **달력이 아니라 판독 종결**이다(`tools/naver-sa/sa.py` 명문): 9/6 `scan`(읽기) → PL-B′ 회신 →
그 판정 커밋에 해제 한 줄 동봉. 그 전에는 `--live` 를 붙이지 않는다. 9/6 실행 순서 A → B → C, 단계마다 보고.

## 2026-09-03 — M4NV 트랙 (메인 4화면 리뉴얼 + 네이버 노출 + 원장 개혁) · 커밋 27건

`68b7120`(M4-C0 시안 커밋) ~ `af826cb`(피처드 컬럼 정정). 롤백 태그 `pre-m4-20260903`.
아침의 설계안 한 장에서 출발해 4화면 배포·게이트 초록까지 하루에 닫았다. 아래는 «다음 사람이
다시 재지 않아도 되는» 값들이다.
...
| 화면 | HTML 바이트 | 한 일 |
|---|---|---|
| /apt | 460,032 → **234,112 (-49.1%)** | 섹션 10+→4 · 청약 상태 그룹 단일화 · 단계 스트립 4분할 스탯 · 레일-본문 상호배타 · 캐러셀 5장 |
| /blog | 191,251 → **162,589 (-15.0%)** | 현장별 필터·다음 페이지 미리보기 제거 · 피처드 부동산 최신 우선 · 이모지 0 |
| /stock | 325,912 → **286,684 (-12.0%)** | 게이트 카드 제거(Q2ⓐ) · 행 정규화(사유 1개·댓글 ≥1) · `*_IDX` 제외 · 탭 30→10행 + 전체 보기 |
| 홈 | 72,407 → **87,314 (+20.6%)** | 지표 이중 렌더 해소 · 「공고 전·카더라 단독」 5행 + 「분석 최신·부동산」 3행 신설(Q3) · 링크 그리드 제거 |

공통: 섹션 8px 밴드(`.kd-band`, 색은 `--bg-base` 실토큰) · 골드 룰 섹션 기본 · 위계 제목 600/수치 700
(`--fw-*` 4종 토큰 **신설**, 800+ 없음) · 이모지 0(전역 셸 `GlobalMissionBar` 포함).
DS_RULES **§5 6조항** 등재(레일-본문 상호배타 · 밴드/골드 · 카피=동작 · 조건부 · 굵기 위계 · 상태코드).

### 2. soft-404 (G-2) — 위험군 7곳 전부 종결

기전 3판: **자기 «또는 조상» 세그먼트에 `loading.tsx` 가 있으면** 셸이 먼저 흘러 `notFound()` 가 404 를,
`redirect()` 가 308 을 만들지 못한다. 교차 대조 9/9 정합(예외 0).
수리: `apt/region/[region]` 은 가드를 `layout` 으로 상향(스켈레톤 보존), 나머지는 경계 제거 —
`apt/complex/[name]` · `apt/unsold/[id]` · `discuss` · `feed` · `stock/sector` · `blog`(4중 경계).
`loading.tsx` 26 → 22. `/apt/unsold/{숫자}` 308 · 없는 대상 404 복구 확인.

### 3. NV (네이버 노출)

`/apt/unsold/{시도}` 17경로 신설(기존 `[id]` 안에서 REGIONS 분기 — 같은 깊이 슬러그 2개 금지 회피),
지역·구군 허브 title/description/h1 키워드 공식, `SiteThumb` 정보 alt(빈 alt 26→0), 사이트맵 17회.
title 꼬리 이중(`… | 카더라 | 카더라`) 15자리 수리 — layout 템플릿이 정본.

**NV-4 계측 D0 = 2026-09-03 13:48** (세션 A 수동 트리거 3회 중 3차).
표적 34키워드 68행 · `err_429 = 0` · 전부 권외 · 양성 대조 11건 진입.
⛔ **8/31~9/2 데이터는 비교 금지** — 429 침묵으로 매일 160행 중 130~140행이 미측정이었다(`d00d3f4` 수리:
429 한정 재시도 3회·백오프·Retry-After, 청크 간 600ms, 예산 초과분 `skipped_time_budget` 기록).

### 4. 규칙 원장 단일화

`docs/RULES.md` **단일 원장**(최대 #143). `docs/ARCHITECTURE_RULES.md` **퇴역** — 실규칙 0, 스텁 28.
매핑: ARCH#21~41·107·115·116 → RULES#123~143 · #17→117 · #42→122 · #43→118 · 전승 #66·#119·#120 · #65 결번.
대사표 `docs/RULES_LEDGER_대사_20260903.md`. 인용은 `RULES#N`·`DS_RULES#N` 접두 필수(맨 번호 금지).

### 5. 이 세션이 잡은 결함형 — 「돌아가는데 틀린 답을 조용히 내는 것」

상호 검출 4:4. CC 결함 4: 308 원인 오진(정적 굽기 → 실제는 스트리밍 경계) · `realestate` 를 DB 값으로
착각(실제는 탭 키) · 「목록 안에서 고르기」라 규칙 무발동(최신 30편에 부동산 0편) · 없는 컬럼
`read_minutes` 로 조회 전체 에러. 세션 A 결함 4: unsold `region` 축 지시 오류 · 게이트 문구가 스텁을
못 가름 · blog 경계 3중 오기(실제 4중) · 발행 필터 누락 판별.

**넷 다 tsc·빌드·리뷰가 통과시킨 것들이고, 배포본 grep 과 런타임 로그가 잡았다.**
문법은 옳고 조건은 성립하고 화면도 안 무너진 채 틀린 답이 조용히 나가는 자리라, 「돌아가는가」가 아니라
«그래서 화면에 무엇이 섰는가» 를 배포본에서 다시 재는 것 말고는 그물이 없다.

### 6. 열린 것

중단점 **G-1′**(4화면 실기기 여덟 줄 — stock 4항 + 홈·blog·apt 각 1 + 뷰 필터 1) · **FN 세션 미가동**
(P0-A 리드폼 무반응 = 오늘 유일하게 손대지 않은 영업 직결 결함, 걸쇠 B 는 13:5x 직권 해제) ·
**H**(9/17, D0 대조) · **C 큐**(9/6 록아웃 해제 후). 별건 적립: `v_blog_posts_listing` 이 미발행을 담는다
(소비자 3곳은 전부 `is_published` 를 걸어 새는 자리는 없음) · 자동발행 믹스 주식 편중(최근 30편 부동산 1).
문서 앵커: 최종지시서_M4NV + 판정증분 1~11 + FN 가동브리핑(모두 9/3자).

> **M4-1 /stock (2026-09-03 · d4e74a8)** — 게이트 카드 제거(Q2ⓐ) · 행 정규화(최상위 사유 1개 인라인 · 댓글 0 미표기 · 빈 둘째 줄 미렌더) · 시총 뷰 `*_IDX` 제외 · 8px 밴드 · 위계 정정 · `(main)/stock/loading.tsx` 제거(G-2 동승).
>
> **⚠️ 토큰 신설 보고(TY2 흡수분)** — `src/app/styles/tokens.css` 에 굵기 토큰 4종을 신설했다:
> `--fw-body: 500`(본문 400→500 상향의 정본) · `--fw-title: 600` · `--fw-num: 700` · `--fw-quiet: 400`.
> ⛔ 800 이상은 만들지 않는다(상한 700). 밴드 색은 **신설하지 않고** 페이지 배경 실토큰 `--bg-base`(#F5F7FA)로 매핑했다 — 시안의 #EAEEF4 는 근사치였다.
> 행 위계 정정: `.kd-lrow-t` 가 700 이라 제목과 수치가 같은 굵기였다 → 제목 `--fw-title`(600), 수치 `--fw-num`(700).
>
> 커밋 메시지 주의: d4e74a8 의 본문 일부가 셸 백틱 치환으로 잘려 나갔다(강제 푸시로 역사를 고치지 않고 이 항목을 정본으로 둔다).

> **순위 계측 침묵 수리 (2026-09-03 · NV-4)** — `naver-sc-sync` 가 429 재시도 0 · 청크 간 딜레이 0 이라
> 8/31~9/3 매 배치 160행 중 130~140행이 `naver_openapi_http_429` 였다(초반 버스트만 통과). 「권외」로 보이던 값은
> 순위가 없던 게 아니라 «재지 못한» 것이다. 429 한정 재시도(3회·700→1400→2800ms·Retry-After 우선) + 청크 간 600ms
> + 시간 예산 초과분 `skipped_time_budget` 기록으로 수리. ⚠️ 8/31~9/2 소실분은 소급 불가 — 기준선은 오늘부터다.

> **규칙 번호 공지 (2026-09-03 · G-4)** — `ARCHITECTURE_RULES.md` 는 퇴역했고 전 규칙이 `docs/RULES.md` 로 옮겨졌다.
> 구↔신 매핑(ARCH#21~#41·#107·#115·#116 → RULES#123~#143, #17→117 · #42→122 · #43→118)은 `docs/RULES_LEDGER_대사_20260903.md`.
> 아래 «과거 기록의 옛 번호는 그대로 둔다» — 역사는 고치지 않는다.
...
### 5. [CV-A] 본실행 실측 (2026-09-02) — 통과. 예상 대비 어긋남 3건 · CV-B 이관 4건

실행·검증은 **DB 쪽 세션이 `_call_vercel_cron` 으로 직접 트리거**해 읽었다(CC 터미널에
CRON_SECRET 을 붙여넣지 않았다). ⚠️ **앞으로 수동 트리거는 curl+시크릿이 아니라
`_call_vercel_cron` 으로 통일한다** — 경로에 쿼리스트링을 넣을 수 있고 타임아웃 290초라
AI 추출 왕복이 그 안에 끝난다.

| 체크 | 결과 |
|---|---|
| 시드 | 2건 — `김해-외동-재건축사업`(경남·태영·1,135·pre_announcement·`crawl:desian:presale`) · `대전-유천1구역-지역주택조합`(930) |
| 매칭 | 11건 · 오생성 0 (주소축 4건 작동) |
| 게이트 | 고창 공공 큐 격리 ✓ · 청주 region 화이트리스트 격리 ✓ · 3분류 실패 0 · health 3소스 ok |
| 비용 | 태영 1회전 = 입력 ~7.3k · 출력 ~1.1k 토큰 · `truncated` 0. `htmlToText` 상한이 물었다 — 20소스로 늘려도 규모 문제 없다. 비용 논쟁 종료, 섀도 평가는 CV-B 에서 «정확도» 판정용으로만 쓴다 |

**예상표(위 §3)가 어긋난 자리 3건 — 전부 «안전측» 이지만 원인이 다르다**

1. **dry 10 → live 11 matched.** presale 에서 시드된 유천1을 **같은 실행의 building 목록이
   matched 로 회수**했다. 루프가 실행 «안에서» 자가 수렴한다 — dry 수치를 live 예상치로
   그대로 쓰면 안 된다는 뜻이다.
2. **고창 덕산지구: seeded 예상 → 실제 queued.** 유사 현장 후보로 «창원 2건»이 걸렸다.
   전북 카드인데 경남 현장이 후보로 나왔다 → 아래 안건 ②.
3. **유천1 세대수 930**(실측). 설계서 §0-2 의 994 는 틀렸다 — 다만 **스니펫이 틀린 것이
   아니다**. 위 §2 표의 원문은 `세대 : 994세대(공동주택 930세대, 오피스텔 64실…)` 로
   둘 다 적고 있었고, 994(오피스텔 포함 총계)를 «세대수» 로 옮겨 적은 쪽이 설계서다.
   **AI 추출은 930 을 골라 맞았다.** 규칙: 세대수는 «공동주택» 기준이고, 총계·오피스텔
   실수는 세대수가 아니다. 설계서 수치를 930 으로 갱신한다.

**DB 쪽에서 이미 처리한 것 2건** (시드 name 이 «사업명» 이라 파워링크 키워드에 '데시앙'이
빠지는 갭): ① `name_variants` 에 「김해 외동 데시앙」·「외동 데시앙」 추가(태영 브랜드
페이지 게재가 근거, D4 범위). ② 자동 별칭 조각 「외 데시앙」 제거 — `sa.py` 가 별칭을
**짧은 순으로** 채택하므로(`name_pool()` 의 `cands.sort(key=len)`) 이 조각이 키워드
1순위로 들어가는 «실제» 오염 경로였다.

#### CV-B 이관 안건 4건 (코드 위치는 CC 가 리포에서 확인함)

| # | 안건 | 코드 위치 · 확인된 원인 |
|---|---|---|
| ① | 변형 생성기가 **조각·중복 별칭**을 산출 — 「외 데시앙」·「김해김해외동재건축사업」 | `src/lib/apt-name-variants.ts`. 「외 데시앙」은 규칙 6 의 `${dongClean} ${brand}` 에서 나온다 — `dong='외동'` → `replace(/동$/,'')` → **1글자 「외」**. 「김해김해…」는 규칙 3 의 `${short}${name공백제거}` 가 **name 이 이미 시군구로 시작하는지 안 본다**(규칙 4 에만 `name.includes` 가드가 있다). `sa.py:285 alias_is_fragment()` 는 «공백 있으면 무조건 통과 + 목록 대조» 라 2토큰 조각을 못 거른다 — 그 구조 규칙은 「창원자이」·「경남아너스빌」을 죽여서 되돌린 것이므로 **필터를 조이지 말고 생성기에서 원천 차단**한다 |
| ② | 유사 현장 검색에 **region 필터 부재** — 고창(전북) 카드에 창원 2건 (「공공분양」 토큰 오탐) | `src/app/api/cron/builder-presale-crawl/route.ts:186-193`. `near` 가 `pool` 전체를 훑는데 `loadPool()` 은 **그 소스의 모든 카드 시군구·시도를 합친 풀**이다(`:249-269`). 카드별 `region`/`sigungu` 일치 조건이 `near` 에 없다. 안전측 오류지만 큐 노이즈 |
| ③ | 동탄 A78BL 사업명 카드가 region 오염으로 **주소축까지 못 타고 큐 잔류** | 같은 소스 안의 「자연&데시앙」과 동일 현장. 현재 카드는 **DB 풀하고만** 대조하고 카드끼리는 안 맞춘다 — 소스 내 상호 매칭(세대수+지역) 후보 |
| ④ | 유천1 세대수 994 → **930(실측)** | 위 3번. 스니펫은 옳았고 설계서가 총계를 세대수로 옮겼다. 설계서 수치 갱신 + 「세대수=공동주택 기준」 명문화 |

**다음:** 시드 2건이 `cs=0` 이므로 **인리치 SLA 48h 가 첫 시험대**다(48시간 내 `cs≥40`).
그 다음 CV-B(백필 32건) · CV-3 · CV-4. 김해 외동은 9/6 이후 첫 `sa-sync` 에서
「김해 외동 데시앙」 별칭을 달고 등록될 준비가 끝났다.

### 6. CV-B ① — 별칭 조각 차단 (2026-09-02, 코드·DB 반영 완료 / 정리는 게이트 대기)

**생산자가 리포에 없었다.** `src/lib/apt-name-variants.ts` 는 «호출자 0» 이다. 실제로
`name_variants` 를 만드는 것은 DB 트리거다 — `trg_apt_sites_auto_variants`(BEFORE
INSERT/UPDATE OF name·sigungu·dong·builder) → `apt_sites_auto_variants()` →
`generate_apt_name_variants_jsonb()`. 어제 시드된 2건에 별칭이 붙은 것도 이 경로다.
⚠️ 그래서 «정리(DELETE)를 먼저 하면 안 된다» — 그 행의 이름·시군구·동·시공사가
한 번이라도 갱신되면 트리거가 조각을 다시 채운다. 순서는 **생산자 → 정리** 다.

**실측이 잡아낸 조각의 최다 원인은 동(洞)이 아니라 «구(區)» 였다.** 활성 6,258현장 ·
별칭 38,378개에서 한 글자 토큰 685개를 전수로 갈랐다:

| 원인 | 예 | 건수 |
|---|---|---|
| 시군구 접미어 제거가 한 글자를 남김 | `중구`→「중 유천1구역…」 | 중 134 · 동 43 · 남 42 · 북 37 · 서 36 … |
| 동 접미어 제거가 한 글자를 남김 | `외동`→「외 데시앙」 | (김해 외동 등) |
| 시군구 중복 부착 | 「김해김해외동재건축사업」 | — |
| **살려야 할 것** | 「더」 189(『DS 더 웰가』) · 「린」 27(『우미 린』) · 「후」 33 · 「뜰」 | 322 |

⚠️ 그래서 **「한 글자 토큰이면 조각」으로 걸면 안 된다** — 322건이 정상이다. 대표명이
붙여쓰기인 현장이 많아 «토큰» 비교도 안 된다(『가평센트럴파크더스카이』 ↔ 「가평 센트럴파크
더 스카이」). 판정은 «공백 지운 대표명이 그 글자를 품고 있느냐» 로 한다.
⚠️ 소비자(`sa.py alias_is_fragment`)는 «건드리지 않았다» — 그 구조 규칙은 「창원자이」·
「경남아너스빌」을 죽여서 되돌린 이력이 있다. 생산자에서 막는다.

**반영**: `supabase/migrations/cvb1_name_variants_fragment_2026-09-02.sql` 프로덕션 적용 완료
(시군구 짧은형 한 글자면 자르지 않음 · 동 결합은 자르지 않은 형태 우선 · 시군구 중복 가드 ·
마지막 문). TS 사본과 단위 11케이스 동시 반영. **전수 시뮬레이션: 6,258현장 재생성
...
### 9. CV-B ② · ③ — 유사명 울타리 · 소스 내 상호 대조 (2026-09-02, 코드 반영)

**② 유사명 검색에 지역 울타리.** `isSameArea(card, site)` 를 `near` 앞에 세웠다
(`src/lib/presale/candidate.ts`). CV-A 본실행에서 고창(전북) 카드에 창원(경남) 2건이
「유사 현장」으로 걸려 시드 대신 큐로 갔던 그 자리다. 원인은 `loadPool()` 이 그 소스의
**모든 카드** 시군구·시도를 합친 풀을 주기 때문이고, 이름축만으로 훑으면 다른 카드의
지역이 섞인다. ⚠️ 한쪽 값이 «없으면» 막지 않는다 — region 을 못 읽은 행까지 쳐내면
「이미 있는 현장 옆에 새 페이지를 또 만드는」 더 비싼 실패로 되돌아간다.

**③ 소스 내 상호 대조 — 붙이지 않고 «적는다».** 같은 목록에 사업명과 브랜드명으로 두 번
올라온 현장(동탄 A78BL ↔ 자연&데시앙)을 카드끼리 대조한다(`isSameSiteHint`: 세대수 일치
+ 시·도 불일치 아님). ⛔ 자동 병합은 하지 않는다 — 같은 택지지구의 다른 블록이 같은
세대수를 쓰는 일이 있고, 잘못 붙인 쪽이 되돌리기 어렵다. queued 카드의 note 에
「같은 소스 안의 동일 현장 후보(세대수 N): …」을 붙여 검수 큐에서 사람이 본다.
...
### 10. 9/6 준비 — 록아웃 캘린더 · 계정 실존 스캔 (2026-09-02)

9/6 을 «빌드하는 날» 이 아니라 «버튼만 누르는 날» 로 만들기 위한 두 가지. 둘 다 광고 계정
무접촉이다(하나는 읽기 전용, 하나는 오히려 쓰기를 막는 문).

**① 록아웃 캘린더 — `sa.py LOCKOUTS`.** `2026-09-03 ~ 09-06` = PL-B′ 측정 창.
계정에 «쓰는» 실행(`apply/off/rollback --live`)은 창 안에서 `sys.exit` 한다.
⚠️ 읽기(`plan·verify·scan`)는 막지 않는다 — 록아웃이 관측까지 막으면 목적과 반대가 된다.
⚠️ 날짜를 코드가 아니라 «캘린더 표» 에 둔 이유: 창을 여는 것도 의도한 행위로 남기기
위해서다. 급하면 `--ignore-lockout` 이 있고, 그 실행은 로그에 크게 남는다.
**첫 회전을 9/6 «당일» 에 돌리려면 끝 날짜를 `09-05` 로 줄이는 커밋이 필요하다** —
지금 표는 STATUS 머리의 「PL-B′(9/3~9/6)」를 그대로 옮긴 것이다.

**② 계정 실존 스캔 — `sa.py scan` (읽기 전용).** 9/6 산출물 ①이다. 계정에 **이미 나가 있는**
키워드를 세 사유로 훑어 `off` 가 그대로 먹는 CSV(`docs/m6/CVB_광고_조각법인명_스캔.csv`)를
낸다 — 사유·현재상태(ON/OFF) 포함.

| 사유 | 판정 |
|---|---|
| `조각` | ⓐ「<한 글자> + 대표명」 ⓑ「<한 글자> + 브랜드」 — CV-B ① 에서 **실제로 지운 두 형태 그대로** |
| `조각(대표명 미상)` | 키워드 URL 로 현장을 못 찾은 경우. 면제 여부를 모르므로 OFF 로 바로 넘기지 않는다 |
| `법인명` | `alias_is_corp` 재사용 + 공백이 지워진 형태(「사하구두산건설」)까지 |
| `slug형` | `SLUG_KEYWORD` — R3 가 132건 지운 그 형태 |

⚠️ 「한 글자 토큰이면 조각」으로 짜다가 테스트가 **두 번** 잡았다: 처음엔 「가평 센트럴파크
더 스카이」를 죽였고(면제 과잉), 면제를 「대표명에 그 글자가 있으면 통과」로 바꾸니 이번엔
「외 데시앙」이 살아났다(외동→외 라 대표명에 「외」가 있다). **위치·모양으로 가르는 것만이
두 방향 모두를 만족한다** — 정리 때 쓴 ⓐ/ⓑ 형태 판정이 곧 정답이었다.
⚠️ OFF 이고 삭제가 아니다. 노출이 붙은 뒤 다시 판단할 수 있어야 한다.

`test_name_pool.py` ⑥(록아웃 4케이스)·⑦(스캔 6케이스) 추가 — 전부 통과.

### 11. CV-B 백필 1회전 — 문서 근거를 «크롤과 같은 문» 으로 (2026-09-02, 배포 대기)

Node 판정: 기본안 GO · 손 INSERT 금지 · 전부 `presale_candidates` 경유.
`src/lib/presale/backfill.ts` 에 카드 목록을 두고, `builder-presale-crawl?source=doc:PV_20260829`
로 부르면 fetch·AI 추출만 건너뛰고 **matchSite·seedGate·seedSite·upsertCandidate 는 그대로**
지난다. 문이 하나여야 규칙이 하나다.

| 등급 | 수 | 내용 |
|---|---|---|
| 시드 후보 | **9** | permits 미매칭 7 + 거제 옥포(1,963) + 명지A5(876) |
| 보류(`holdReason`) | **22** | EDC 12BL · 전국 4건 · A-2c 미검출 17 |

**실측이 문서를 세 군데 고쳤다.**
1. **permits 근거는 8이 아니라 7이다.** 밀양 나노융합 유승한내들(745)은 `apt_permits` 에
   **없다**(밀양 행 0 · 경남 수집분 197행). 이름이 걸리는 두 행은 「울산다운2지구 B-1BL
   유승한내들 507」로 **다른 현장**이다. 밀양은 보류로 내렸다.
2. **「리치벨트」 철회** — 현장이 아니라 「광안리 리치벨트」라는 **지역 용어**(광안대교 조망
   부촌)다. 목록에서 뺐고 테스트가 재유입을 막는다. `seedGate`(정확히는 근거 확인 절차)가
   가짜 현장을 걸러 낸 **두 번째 사례**다 — 첫 번째는 부암동 831세대였다.
   ⇒ 「광안리 리치벨트」는 E_대표 계열 **지역 키워드 후보**로 9/6 이후 안건에 올린다.
3. **「잔여 29」는 산술이었다.** 실측 기준으로 다시 세면 A-2b 결측 31 중 사천 엘크루·
   부암2차 비스타동원 등은 이미 있고, 이들은 카드로 넣어 **matched 로 기록**되게 했다 —
   「없다고 적힌 문서」가 실행으로 스스로 정정된다.

**⚠️ 백필이 드러낸 결함 하나 — `apt_permits.source_url` 이 1,465행 «전부 NULL» 이었다.**
컬럼은 있는데 채우는 코드가 없었다. 「원본으로 돌아갈 수 없는 값은 앉히지 않는다」(seedGate)에
백필이 걸리면서 드러났다 — 게이트가 데이터 결함을 잡은 사례다. 조치 둘:
`hub.ts` 에 `permitRecordUrl()` 을 넣어 **앞으로 들어오는 행이 채워지게** 했고,
기존 1,465행은 같은 규칙으로 **소급 채움**(전량 `#pk` 포함). ⚠️ serviceKey 는 넣지 않는다 —
주소는 기록이지 호출이 아니다. 택지 블록(`platGbCd=2`)은 지번이 `0000` 이라 좁혀지지 않으므로
관리대장 PK 를 **프래그먼트**로 붙여 행을 특정한다.

**집행(Node)**: `_call_vercel_cron` 으로 두 번.
`builder-presale-crawl?source=doc:PV_20260829&dry=1` → 판정 표 확인 →
같은 경로에서 `dry=1` 빼고 실행. ⚠️ dry 수치를 live 예상으로 그대로 쓰지 말 것 —
CV-A 에서 실행 «안에서» 자가 수렴이 일어났다(§5).

### 12. 길이 필터 개정 A∪B (2026-09-02 `!` 판정 · 9/6 첫 회전부터)

`sa.py SQL` 의 `length(name) >= 6` 을
`length(name) >= 6 OR total_units >= 300 OR name ~ <명시 브랜드 토큰>` 으로 바꿨다.
A(브랜드 토큰)가 광안자이·남천자이·엘시티더샵·래미안장전·거제더샵·물금자이·삼산자이·
중산자이·명륜자이·두산위브를, B(300세대)가 레이카운티(2,759)·라엘에스(1,073)·
정촌올리움(794)·더폴 우정(316)을 살린다.
⚠️ 토큰은 **명시 목록만**이다 — 1차 정규식의 「리버」가 윤성리버티류를 오탐했다.
「개발·공사」 꼬리 사고와 같은 함정이고, 이번 트랙에서 **세 번째** 같은 결론이다.
목록은 생성기·「<한 글자>+브랜드」 판정과 **같은 `BRANDS`** 를 쓴다.

### 13. CV-4 갭워치 — 결측을 «표가» 들고 있게 (2026-09-02, 배포 대기)

`/api/cron/gap-watch` (pg_cron `gap_watch_daily` · 06:40 KST). 하루 한 번 7지표를 재서
`gap_watch_snapshots` 에 적고, **월요일이거나 임계를 넘으면** `admin_alerts` 로 다이제스트를
보낸다. ⚠️ 조용한 주에도 주 1회는 보낸다 — 침묵이 「문제 없음」인지 「죽었음」인지 구분되지
않는 것이 이 트랙이 고치려는 병이다.

⛔ 알림 생산자를 새로 만들지 않았다(`admin_alerts` 재사용). ⛔ Vercel cron 에 넣지 않았다 —
한도 100 도달분이라 추가하면 배포가 죽는다(레지스트리 문서의 규칙 그대로).
⛔ **이 크론은 아무것도 고치지 않는다.** 자동 수정을 붙이면 지표가 스스로를 낮추는 길이 생긴다.

| 지표 | 오늘 값 | 판정 |
|---|---|---|
| 분양예정 현장 수 | 31 | ok (내려가면 발견 루프가 죽은 것) |
| **미매칭 인허가** | **1,465** | **critical — 전량이다.** 인허가 근거가 있는데 한 건도 안 붙어 있다 |
| 충돌 판정 | 1 | ok |
| 같은 법정동 유사쌍 | 129 | ok (임계 150) — 늘면 유사명 검색이 새는 것 |
| 정비축 180일 정체 | 0 | **측정 불가** — `stage_updated_at` 이 8/23 에 처음 채워졌다(773 중 374행). 창이 열리는 것은 2027-02 다 |
| 검수 큐 | 27 | ok |
| 연속 0카드 소스 | 0 | ok |

⚠️ **첫 관측이 바로 하나를 잡았다 — 미매칭 인허가 1,465 = 전량.** `permits/match.ts` 는
있는데 매칭이 프로덕션에서 한 번도 «앉지» 않았다는 뜻이다. 백필이 `source_url` NULL 을
드러낸 것과 같은 종류의 발견이고, 다음 안건 후보다.
⚠️ 「정비축 0」을 건강으로 읽지 않도록 `blindNote` 를 지표 정의에 넣었다. 못 재는 지표는
심각도를 올리지도 내리지도 않고 「측정 불가」로 적힌다 — 오늘 하루 반복된 교훈(0 ≠ 건강)을
코드에 심은 것이다. 단위 11케이스가 이 동작을 잠근다.

### 14. PV-3b 배선 — 「소비자 없는 판정기」를 잇는다 (2026-09-02, 1회전 대기)

CV-4 첫 관측의 critical(미매칭 인허가 1,465 = 전량)의 원인 계층이 갈렸다:
**로직 결함이 아니라 배선 부재다.** `src/lib/permits/match.ts` 를 임포트하는 곳이
리포 전체에서 **테스트 파일 하나뿐**이었다. 파일 머리가 「이 파일은 판정만 한다,
DB 접근·upsert 는 PV-3b 라우트가 한다」고 적어 둔 그 라우트가 만들어진 적이 없다.
⇒ 오늘 잡은 **이중 생산자의 쌍둥이 = 소비자 없는 판정기**. 같은 병의 반대편이다.

**`/api/cron/permits-match` 신설.** 판정 규칙은 한 줄도 다시 쓰지 않았다 — 라우트는
후보를 좁혀 넣고, 판정기가 내린 답을 그대로 적는다.

| 배선 결정 | 이유 |
|---|---|
| 후보는 **법정동 색인**으로만 좁힌다 | 전수 비교는 오매칭의 온상이다(CV-B ②와 같은 원칙) |
| 세대수는 `complex_units` → `total_units` 순 | 인허가 `totHhldCnt` 는 «단지 전체» 다. `total_units` 에는 공급분이 섞여 있어 뒤집으면 같은 현장이 세대수 불일치로 갈린다 |
| 주소 없는 행은 `region·sigungu·dong` 으로 **합성** | 활성 6,261 중 주소가 3,579뿐이다. 합성해도 지번은 생기지 않으니 지번축은 안전하고 법정동축만 살아난다 |
| `review` 의 `matched_site_id` 는 **비운다** | 「id 가 있다 = 확정이다」 불변식을 지킨다. 후보 id 는 note 에만 |
| 재판정 대상은 `pending` 뿐 | 사람이 큐에서 내린 판단을 배치가 덮어쓰지 않는다 |
| 현장을 **만들지 않는다** | 인허가 → 시드 경로는 백필과 같다(candidates 경유). 여기서 만들면 문이 둘이 된다 |

**⚠️ 1,465가 다 붙지는 않는다 — 미리 실측했다.** 법정동 후보가 있는 것 950,
지번 정확일치 후보 23, 세대수 정확일치 후보 41(중복 있음). 즉 1회전의 기대값은
**matched 수십 건**이고 나머지는 review·unmatched 다.
⇒ 그래서 갭워치 지표에 `detail.by_status` 를 붙였다. 같은 숫자라도 **pending 이면
「PV-3b 가 안 돈 것」, unmatched 면 「붙을 현장이 없는 것」** 이라 할 일이 완전히 다르다.
지표 문구도 그렇게 고쳤다 — 숫자만 두면 다음 사람이 또 「매칭이 죽었나」부터 본다.

**집행(Node)**: 백필과 같은 3단.
`permits-match?dry=1&limit=500` → 표 확인 → `permits-match?limit=500` ×3.
⛔ 증분 훅(`permits_match_hourly`, permits-sync 25분 뒤인 45분)은 **1회전을 끝낸 뒤** 켠다 —
지금 켜면 사람이 dry 를 보기 전에 배치가 먼저 써 버린다. SQL 은 마이그레이션 파일에 주석으로 있다.

단위 10케이스(배선) + 그랑라크·문수로대공원 실측 케이스.

#### 1회전 실측과 «침묵한 실패» (2026-09-02)

3회전 결과: **matched 89**(현장 61개에 연결 · `jibun_exact` 포함) · **review 221** ·
나머지 1,155. 첫 matched 가 **오늘 오후 백필로 시드한 화정동 638-3 에 지번 정확일치로 붙은 것**
이다 — 시드가 인허가를 부르고 인허가가 시드를 찾는 폐루프의 첫 완주다.

⛔ **그런데 「잔여 1,155 = 후보 없음, 설계대로」가 아니다.** 응답은 세 실행에서
`unmatched` 306·412·472(합 1,190)를 «판정했다» 고 적었는데, DB 의 `unmatched` 는 **0**이고
그 행들은 `pending` 그대로다. 원인:

| 층 | 사실 |
|---|---|
| 어휘 불일치 | 판정기는 `unmatched` 를 쓰고, 컬럼 CHECK 는 `pending·matched·review·rejected·**no_target**` 만 받는다 |
| 침묵 | UPDATE 의 `error` 를 **안 보고 있었다**. 1,190건이 거부됐는데 응답은 성공이었다 |
| 결과 | 매시 훅(`permits_match_hourly`)이 같은 1,155건을 **영원히 다시 판정**한다 — 수렴하지 않는다 |

⚠️ 「수확 체감 51→24→14」는 풀 소진의 증거로 읽혔지만, 그 해석은 «쓰기가 됐다» 는 전제 위에
있었다. matched·review 는 실제로 앉았으므로 체감 자체는 사실이다 — 다만 **1,155는 아직
판정 결과가 기록되지 않은 상태**이고, 다시 돌면 대부분 `no_target` 으로 «내려앉아야» 한다.

**수리 3종**: ① `toColumnStatus()` — 판정기 어휘를 컬럼 어휘로 번역(`unmatched`→`no_target`)
② UPDATE 의 error 를 세고 `write_fails`·`first_write_error` 를 응답에 싣는다. 실패한 건은
`processed` 에 «세지 않는다» ③ `confidenceOf` 가 CHECK 밖의 `low` 를 쓰고 있었다 —
`verified·confirmed·estimated` 로 고쳤다(matched score 가 늘 0.8↑ 이라 아직 안 터졌을 뿐이다).
단위 테스트가 세 가지를 잠근다.

⚠️ 이 트랙의 반복 결함형이 또 나왔다: **「침묵 성공」**. R1 이 「0카드를 성공으로 적지 않는다」로
잡은 것과 같은 병이고, 이번엔 «쓰기» 쪽에서 났다. 규칙: **DB 쓰기의 error 를 버리지 않는다.**

#### ⚠️ 배선 직전에 잡은 것 — `.limit(20000)` 이 «거짓말» 이었다

갭워치 첫 라이브에서 「같은 법정동 유사쌍」이 **8** 로 찍혔다. 같은 정의로 DB 에서 직접 세면
**129** 다. 정의 차이가 아니라 **PostgREST `db-max-rows`=1,000** — `.limit(20000)` 을 써도
첫 장만 온다. 리포는 이미 이 함정을 알고 있었고(`/blog` 주석 · `fetchAll` 헬퍼) 새 코드가
그걸 몰랐다.

⛔ **PV-3b 가 이 상태로 1회전을 돌았으면** 후보 색인이 6,261 중 1,000 으로 잘려
대부분이 「후보 없음」으로 떨어졌을 것이다 — 배선을 고쳐 놓고 결과는 여전히 0에 가깝고,
「역시 붙을 게 없다」는 «틀린 결론» 을 근거까지 갖춰 내렸을 것이다.

세 곳을 `fetchAll` 로 고쳤다: `gap-watch`(유사쌍) · `permits-match`(후보 색인) ·
**`builder-presale-crawl`(매칭 풀 `.limit(2000)`/`.limit(4000)`)**. 마지막 것은 «이미 돌고
있던» 크론이다 — 풀이 조용히 잘리면 이미 있는 현장을 못 찾아 새 페이지를 또 만든다.
9/2 백필·본실행이 오생성 0 이었던 것은 그 소스의 시군구 행이 1,000 미만이었던 덕이다.

규칙: **행 수가 1,000 을 넘을 수 있는 조회에 `.limit()` 을 쓰지 않는다 — `fetchAll` 을 쓴다.**
지표가 스스로를 낮추는 길이 여기에도 있었다.

### 15. 그랑라크 17키워드 즉시 등록 (2026-09-02 `!` 집행)

견본주택 개관일에 맞춘 예외 집행. 8/31 일광 절차를 그대로 따랐고, 그 수작업을
`tools/naver-sa/pl_add_site_keywords.py` 로 **굳혔다**(다음 현장은 `--slug` 만 바꾼다).
목록은 `tools/naver-sa/urgent_keywords.json`. 상세·제외 id 는
`docs/PL_B_PRIME_EXCLUDE_20260902.md`.

- A_분양 그룹 395 → **412(+17)** · 브랜드 9(입찰 300) · 부지·구역 4 / 지역 4(입찰 100)
- 착지 `https://kadeora.app/apt/그랑라크-에일린의-뜰` · 전 건 userLock False
- ⛔ **손으로 적은 키워드도 sa.py 의 문을 그대로 지난다** — 조각·법인명·slug형 판정을
  스크립트가 재사용한다. 규칙을 두 곳에 두지 않는다(오늘 하루의 교훈).
- ⚠️ 등록 직후 17건 전부 `PAUSED`. 8/31 일광 15건이 «지금은 전부 ELIGIBLE» 임을 재확인해
  경로가 열린다는 것은 확인했다. **「등록했다」와 「노출된다」를 같은 말로 쓰지 않는다** —
  9/3 오전 판독의 첫 항목이 이 17건의 ELIGIBLE 전환 여부다.
- ⚠️ 오늘 집행이라 록아웃(9/3~) 창 밖이다. PL-B′ 측정에서 8/31 일광 15 + 오늘 17 =
  **32건이 「신규분」** 이다. 이 32건을 빼지 않으면 「노출0 이 안 줄었다」는 틀린 결론이 나온다.
...
> ⛔ **AD-1(2026-09-07) — 아래는 2026-03-29 스냅샷이다. 「현황」으로 읽지 말 것.**
>
> 이 꼬리가 5개월 넘게 그대로 남아 세션마다 오독을 만들어 왔다. 이번에 실측으로
> 확인된 어긋남만 적어도 이 정도다:
>
> | 아래 표의 값 | 2026-09-07 실측 | 측정 방법 |
> |---|---|---|
> | 크론 79개 | **81개** | `vercel.json` 의 `crons` 길이 |
> | API 라우트 180개 | **447개** | `src/app/api/**/route.ts` 개수 |
> | 파일 수 544개 | **1,355개** | `git ls-files src` |
>
> 그리고 `/feed` 는 그 뒤 **영구 폐쇄**됐다(현재 308 → `/apt`). 아래의 게시글·댓글
> 수치와 "Google URL 검사 → `/feed` 수동 인덱싱" 항목은 이미 대상이 없다.
>
> ⚠️ **API 키·결제 상태는 `env` 가 정본이다.** 아래 "[중요] API 키" · "Toss 키 미설정" ·
> "Anthropic 크레딧 충전" 은 3월 값이므로 **참고 금지** — 실제 상태는 Vercel 환경변수에서
> 확인할 것. DB 유래 수치(유저·게시글·블로그·청약·재개발 등)는 이번 세션에서 측정하지
> 않았으므로 **갱신하지 않고 무효로만 표시**한다. 없는 숫자를 지어내는 쪽이 낡은 숫자보다
> 나쁘다 — 실측 규칙.
...
| 지표 | 2026-03-29 | 2026-09-07 실측 |
|------|------|------|
| 파일 수 | 544개 | **1,355개** (`git ls-files src`) |
| API 라우트 | 180개 | **447개** (`src/app/api/**/route.ts`) |
| 크론 | 79개 | **81개** (`vercel.json`) |
| DB 테이블 | 127개 | 미측정 — DB 조회는 이 세션 범위 밖 |
| 최신 커밋 | `3350f00` | 이 표로 커밋을 추적하지 않는다 (`git log` 가 정본) |
