...
### 지시서 안에서 충돌한 것 — 더 강한 제약을 따랐다

**D-1 '각 블록 3~4건' vs '텍스트 총량을 줄이지 말 것'.**
현재 5/5/3/3 을 3/3/4/3 으로 줄이면 카드 3장 분량이 사라진다. 같은 문단의 ⚠️(SEO)가
더 강한 제약이라 **건수는 그대로 두고 순서만 바꾸고 라운지를 더했다.** 총량은 늘었다.
건수를 실제로 줄이려면 별도 지시가 필요하다.
