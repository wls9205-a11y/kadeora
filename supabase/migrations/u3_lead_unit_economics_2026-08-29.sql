-- U-3층 ⑥ — 리드 단가 뷰 (2026-08-29 · Node 판정 ② 채택)
--
-- ⛔ `from ad_keywords` 로 시작하지 «않는다». 그 표는 캠페인 1/11 · 그룹 12/22 의
--    부분 스냅샷(8/26)이라, 그것을 기준으로 삼으면 나머지 10개 캠페인의 지출이
--    «수집됐는데 표에서 사라진다». 수집한 것이 보이지 않는 구조를 만들지 않는다.
-- → ad_stats_daily 를 기준으로 두고 ad_keywords 는 «이름 보강용» left join 이다.
--   이름이 없는 행은 unsynced=true 로 뜬다(화면 라벨 「미동기 키워드」).
--
-- ⚠️ 리드 집계는 «키워드 ID 축» 으로만 한다. 문자열(n_keyword) 로 붙이면 동명 키워드가
--    캠페인마다 다른 값을 갖는 탓에 남의 지출이 섞인다(지시서_U3 §6).
create or replace view v_lead_unit_economics as
with spend as (
  select keyword_id,
...
from spend s
left join ad_keywords k on k.keyword_id = s.keyword_id
...
comment on view v_lead_unit_economics is
  'U-3층 리드 단가. ad_stats_daily 기준(수집한 것이 반드시 보이도록) + ad_keywords 는 이름 보강용 left join. unsynced=true 는 미동기 키워드, low_sample=true 는 리드 n<5 로 단가에 통계적 의미가 없다.';
